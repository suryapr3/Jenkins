...
/**
 * @file    ax_v2d_flow3_pdpd_ingress_capture.c
...
int main (int argc, char **argv)
{
    ...
    /* if we want to stop the capture with BFN event in a single shot mode then sample size should be higher value
     * capture will be stopped based on whichever condition occurs first
     * i.e either BFN time or the number of samples*/
    sample_size = 0x7FFF;
#else
    ...
    /*BFNE_INCR register will have impact only if event_mode is other than SINGLE*/
    configure_v2d_bfn_incr(v2d_num, 3/*reg_instance*/, 512/*value in BFN clocks*/);
#endif
