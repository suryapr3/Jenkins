...
/**
 * @file    rx_lock.c
 * @brief   lock test file on RX core
...
int main(int argc, char **argv)
{
    ...
    write_rsm[4] = 0x44;
	// update the written val to the flag which triggers cal/tx core for locked region write
	rsm_rx_shared_flag = p_system_data->test;
	write_rsm[4] = 0x55;

	// wait to check if tx or cal core could modify the locked var
	while(p_system_data->test == val)
	{	write_rsm[4] = 0x65;
		write_rsm[1] = waitCnt++;
		if(waitCnt > 0x5000) // give some time for rx/cal core to try
		{
			// set pass flag in RSM
			rsm_com_shared_flag = 0xBA55C0C1;
			break;
		}
	...
	// release lock, either cal or tx may get lock now
