...
/**
 * @file    ax_v2d_gubuffer_mode1_update.c
        ...
        tr_width_value_t dst_tr_width)
{
    ...
    pdpd_ctrl_reg_t pdpd_control_reg = {.value = PDPD_CTRL_UPDATE_PASSTHRU_BF_MSK | PDPD_CTRL_UPDATE_C_LUT_BF_MSK};
    ...
    gpdma_set_descr (0, (uint32_t)(v2d_base + AV2D_AXI4_AV2D_AXI4_PC_M1_OFFSET)/*dest address*/, ((uint32_t)src_buffer), block_size);
    ...
    *(PDPD_CTRL_REG(pdpd_base)) = pdpd_control_reg;
...
{
    int num_vectors_to_write = 0, num_128_bit_writes = 0, num_transactions = 0;
    ...
    num_vectors_to_write = rsm_buf[2];
    ...
    src_buffer[num_vectors_to_write * WORDS_PER_VEC] = rsm_buf[3];  // PASSTHRU COEFF

    gpdma_test_LLI(num_128_bit_writes - 1/*(block_size + 1) * src_tr width*/, DATA_ITEM_1/*number of transactions*/, DATA_ITEM_1/*dst_msize*/,
