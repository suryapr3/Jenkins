...
volatile uint64_t __attribute__ ((section (".sram.tx.rw"))) crux_source_buffer[512];
volatile uint32_t __attribute__ ((section (".sram.tx.rw"))) x_source_buffer[1024]; // 2k samples.
volatile uint32_t __attribute__ ((section (".sram.tx.rw"))) x_dest_buffer[1024];   // 2k samples.
volatile  gpdma_desc_t __attribute__ ((section (".sram.tx.rw"))) descr_buffer[100];
...
int gpdma_test_shadow_mode(int dmac_channel,int transfer_type,int multiblock_type,int block_size,int src_msize,int dst_msize,int src_tr_width,int dst_tr_width)
{
	...
	{
		x_source_buffer[dataCnt] = 0xC0C00000|dataCnt; 
	}
	...
	{ 
		...
		*RCS_DMAC_CHENREG_REG = chen_reg; 
		...
		/*rcs_dmac_ch1_intstatus_reg_t rcs_dmac_ch1_intstatus_reg = *(RCS_DMAC_CH1_INTSTATUS_REG);	       
		  ...
		  rsm_tx_shared_flag=rcs_dmac_ch1_intstatus_reg.value;
		  }
		//clear INTSTATUS REG   
	...
	*RCS_DMAC_CH2_CTL_REG = dmac_ch2_ctl_reg;
	...
	rsm_tx_shared_flag=0x8877;
	while(rsm_com_shared_flag!=0x8877); //host will update flag to 0x8877 after read
...
uint32_t gpdma_test_rcs2vex_crux(uint32_t vex_id)
{
	...
	*CRUX_INTF_CRUX_WDATA3_R_REG = crux_wdata3_reg;
	...
	rsm_tx_shared_flag=crux_wrouting_reg.value;
