...
/**
 * @file    ax_v2d_quad_flow3_pdpd_ingress_capture_repeat_3_ch.c
 * @brief   Program the BFN for capturing the data for three channels in a QUAD in REPEAT mode
 *          along with starting the Rx and Tx Vex cores.
...
int main (int argc, char **argv)
{
    ...
    uint32_t v2d_base = 0, vex_base = 0, quad_instance = 0, v2d_num = 0, pdpd_rate = 0, match_value = 0,
             ...
             capture_start_value[4] = {0}, num_antennas = 0;
    int lc = 0, vex_num = 0, v2d_index = 0, timer_value = 0, bfn_incr_value = 0, apb_num = 0;
    ...
    num_antennas = ((v2d_rcs_host_param_t *) rsm_buf) -> num_antennas;
    ...
    bfn_incr_value = (num_antennas * capture_blocks_per_antenna * clk_per_blk) + skip_cycles + \
                     ((v2d_rcs_host_param_t *) rsm_buf) -> ant1_cycles + ((v2d_rcs_host_param_t *) rsm_buf) -> ant2_cycles;
    ...
    /*BFN configuration*/
    for (lc = 0; lc < num_antennas; lc++)
    {
        v2d_num = g_v2d_instance_in_quad[quad_instance][(((v2d_rcs_host_param_t *) rsm_buf) -> configured_channel[lc]) / 2];
        apb_num = (((v2d_rcs_host_param_t *) rsm_buf) -> configured_channel[lc]) % 2;
        configure_v2d_bfn(v2d_num, apb_num, 1/*capture_enable*/, sample_size, match_value);
    ...
    *(BFN_CSR_BFNE_START0_REG(v2d_base)) = bfne_start_reg;
    ...
    /*First VEX cores should be started and later the capture memory BFN should be programmed*/
    ...
    for (lc = 0; lc < num_antennas; lc++)
    {
            ...
            /*RX VEX CORE*/
            vex_num = g_vex_instance_in_v2d[v2d_num][(2 * (((v2d_rcs_host_param_t *) rsm_buf) -> configured_channel[lc] % 2)) + 1];
            vex_base = VEX_BASE(vex_num);
            *(VE32_CELL_CTRL_REG(vex_base)) = ve32_cell_mem_control_reg;

            /*Tx VEX Core*/
            vex_num = g_vex_instance_in_v2d[v2d_num][(2 * (((v2d_rcs_host_param_t *) rsm_buf) -> configured_channel[lc] % 2))];
            vex_base = VEX_BASE(vex_num);
            *(VE32_CELL_CTRL_REG(vex_base)) = ve32_cell_mem_control_reg;
    }
    ...
    for (lc = 0; lc < num_antennas; lc++)
    {
        ...
        apb_num = (((v2d_rcs_host_param_t *) rsm_buf) -> configured_channel[lc]) % 2;
        v2d_base = V2D_BFN_BASE(v2d_num);
        capture_bfn_start_value += ant_delay_values[lc];

        ((bfn_csr_t*) (v2d_base)) -> BFNE_START[apb_num + 1].value = capture_bfn_start_value;
        capture_bfn_start_value += capture_duration;

        /*Get the HALT value in frame and offset format*/
        capture_halt_bfn.OFFSET = capture_halt_value % (((bfn_csr_t*) (v2d_base)) -> AW_ISYNC_TEN_MS_CNT.value);
        capture_halt_bfn.FRAME = capture_halt_value / (((bfn_csr_t*) (v2d_base)) -> AW_ISYNC_TEN_MS_CNT.value);

        /*Trigger for capture halt*/
        if ((((v2d_rcs_host_param_t *) rsm_buf) -> configured_channel[0]) % 2)
        {
            ((bfn_csr_t*) (v2d_base)) -> BFNE_START[3].value = ((bfn_csr_t*) (v2d_base)) -> BFNE_START[2].value + capture_halt_bfn.value;
        }
        else
        {
            ((bfn_csr_t*) (v2d_base)) -> BFNE_START[3].value = ((bfn_csr_t*) (v2d_base)) -> BFNE_START[1].value + capture_halt_bfn.value;
        }
    }

    /*Compare the MEM_FB samples for the QUAD*/
    dram_buff[4] = quad_mem_fb_compare_3_ch_repeat(ref_pattern, (v2d_rcs_host_param_t *)rsm_buf);
