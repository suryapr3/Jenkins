...
uint32_t gpdma_test_rcs2vex_crux(uint32_t vex_id)
{
	...
	{
	    crux_source_buffer[dataCnt]=0xD0D00000|dataCnt;
	}
	dmac_ch1_ctl_reg.SRC_TR_WIDTH=2;
	dmac_ch1_ctl_reg.DST_TR_WIDTH=2;
	dmac_ch1_ctl_reg.SRC_MSIZE=1;
	dmac_ch1_ctl_reg.DST_MSIZE=1;						
	gpdma_set_descr (0,(uint64_t)CRUX_INTF_CRUX_WDATA0_R_REG,(uint64_t)crux_source_buffer,15);
	...
	*CRUX_INTF_CRUX_TX_PUSH_REG = crux_tx_push;
    ...
    //wait till host read
    while(rsm_com_shared_flag!=0x7788); 
}
