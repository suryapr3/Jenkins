...
#define RSM_MEM_SIZE 0x100
uint32_t write_rsm[RSM_MEM_SIZE]  __attribute__((section(".sram.rx.rw")));
...
int main(int argc, char **argv)
{
	...
	rsm_rx_shared_flag = 0x3366;
	//while(rsm_rx_shared_flag!=0x5678);
    return (0);
}
int8_t rsm_tests()
{

	rsm_wr_core_rd_pcie_tests(pattern);

    return 0;
}


int8_t rsm_wr_core_rd_pcie_tests(int32_t pattern)
{
	int32_t dataCnt = 0;

	for (dataCnt = 0; dataCnt < RSM_MEM_SIZE; dataCnt++)
	{
		write_rsm[dataCnt] = pattern;
	}
