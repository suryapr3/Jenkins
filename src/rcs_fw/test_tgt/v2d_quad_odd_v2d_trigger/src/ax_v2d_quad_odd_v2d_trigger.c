...
/**
 * @file    ax_v2d_quad_odd_v2d_trigger.c
 * @brief   Trigger the delayed start of for the given DRF channel in a ODD V2D for the given quad
 *          along with start the corresponding vex cores.
...
int main (int argc, char **argv)
{
    ...
    uint32_t v2d_base = 0, vex_base = 0, quad_instance = 0, v2d_num = 0, apb_num = 0;
    int lc = 0, vex_num = 0;
    ...
    /*Take only odd V2D*/
    v2d_num = g_v2d_instance_in_quad[quad_instance][1];
...
void configure_v2d_force_bfn(int quad_instance)
{
    ...
    /*Enabling the BFN start Enable only for even V2D's*/
    ...
    *(BFN_CSR_GENERAL_CSR_REG(v2d_base)) = bfn_csr_general_csr_reg;
    ...
    /*Enabling the BFN EVENT in the BFN config only for even V2D's*/
    *(BFN_CSR_BFNE_CONFIG0_REG(v2d_base)) = bfne_config_reg;

    /*Writing the LM_START values with the required match values*/
    bfn_csr_start_lm_reg.MATCH_VALUE = rsm_buf[2];

    /*need to configure only for odd V2D*/
    v2d_base = v2d_base + (V2D1_BFN_CSR_BASE - V2D0_BFN_CSR_BASE);

    /*Program the LM register based on the provided apb_num*/
    if (0 == apb_num)
