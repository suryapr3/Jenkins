...
/**
 * @file    ax_v2d_quad_post_processing_2channel_trigger.c
...
int main (int argc, char **argv)
{
    ...
    uint32_t v2d_base = 0, vex_base = 0, quad_instance = 0, v2d_num = 0, v2d_index = 0;
    int lc = 0, vex_num = 0;
...
void quad_bfn_sync_ver()
{
    uint32_t rv = 0, v2d_num = 0, buff_index = 0, v2d_base = 0, v2d_index = 0, rcs_timestamp_value = 0, v2d_timestamp_value = 0,
             adjustment_value[V2D_PER_QUAD] = {0}, quad_instance = 0;
...
{
    int v2d_num = 0, v2d_index = 0, v2d_base = 0, quad_instance = 0;
...
void configure_v2d_force_bfn(int quad_instance)
{
    ...
    *(BFN_CSR_BFNE_CONFIG0_REG(v2d_base)) = bfne_config_reg;
    ...
    for (lc = 0; lc < V2D_PER_QUAD; lc++)
    {
        ...
        /*Writing the LM_START values with the required match values for BUF A*/
        bfn_csr_start_lm_reg.MATCH_VALUE = rsm_buf[2] + (2 * CLK_PER_BLK) + (CLK_PER_BLK / 2);
