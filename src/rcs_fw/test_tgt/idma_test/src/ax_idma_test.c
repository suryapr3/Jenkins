...
#define IDMA_TEST_BUFF_SIZE 0x300
...
volatile uint32_t  buff_rsm[IDMA_TEST_BUFF_SIZE] __attribute__ ((section (".sram.ax.rw")));
...
volatile uint32_t  buff_r[IDMA_TEST_BUFF_SIZE*8] __attribute__ ((section (".sram.ax.rw")));
volatile uint32_t  buff_iram[16] __attribute__ ((section (".iram0.text")));
...
volatile uint32_t num_bytes_val=0,ctrl_reg[];
uint32_t dataCnt,errCnt = 0,count = 0,cnt = 0,value,buff_dst1[1],block_size=0;
uint32_t test_var1 =0x55556666;
volatile unsigned int g_test_val = 0;
/***********************************************
...
void idma_test_1d_poll_dcode_to_dcode ()
{
    ...
    rsm_rx_shared_flag=0x5555555;
    memset (buff_dst,0, IDMA_TEST_BUFF_SIZE);
    for (dataCnt = 0; dataCnt < IDMA_TEST_BUFF_SIZE; dataCnt++)
    {
	buff_src[dataCnt] = 0x88885555;
    }
    com_idma_trans_1d (buff_dst, buff_src,4, NULL);
...
void idma_test_1d_poll_dcode_to_rsm ()
{
    ...
    do
    {   
	if(rsm_com_shared_flag==0x8844)
	    break;
    }while(rsm_com_shared_flag!=0x8844);
...
void idma_test_desc_2d_dcode_to_rsm(uint32_t num_bytes,uint32_t blk_size)
{
    ...
    uint32_t xferd_bytes =0;
    rsm_ax_shared_flag=0;
    memset (buff_r,0, IDMA_TEST_BUFF_SIZE);
    idma_task_info_t task_test = {.pfunc_idma_callback = idma_cb_test, .p_arg = &test_var};
    g_test_val = 0;
    uint32_t num_blks;
    num_blks=num_bytes/blk_size; 
...
void idma_test_desc_2d_dcode_to_dcode(uint32_t num_bytes,uint32_t blk_size)
{
    ...
    idma_task_info_t task_test = {.pfunc_idma_callback = idma_cb_test, .p_arg = &test_var1};
    g_test_val = 0;
    uint32_t num_blks;
    num_blks=num_bytes/blk_size; 
    if(num_blks>254)
...
void idma_test_broadcast_rsm_to_dcode ()
{
    ...
    uint32_t dataCnt, errCnt = -1;
    memset (buff_b, 0, IDMA_TEST_BUFF_SIZE);
    for (int idx = 0; idx < IDMA_TEST_BUFF_SIZE; idx++) buff_rsm[idx] = idx;
...
{
    //idma_test_desc_2d_dcode_to_dcode(16,16);
#if 1
    uint32_t test_var = 0x456,num_blks=1,blk_size=64;
    uint32_t xferd_bytes =0;
    rsm_ax_shared_flag=0x0;
    com_idma_init ();
    idma_task_info_t task_test = {.pfunc_idma_callback = idma_cb_test, .p_arg = &test_var};
    idma_desc1d_t *p_desc_1d = (idma_desc1d_t *)&g_idma_desc1d_test;
    for (int dataCnt = 0; dataCnt < 64; dataCnt++)
    {
	buff_src[dataCnt]=0xB5B5B5B5;
    }
    for (int i = 0; i < num_blks; i++)
    {
	g_idma_desc1d_test[i].src = (uint32_t)&(buff_src[xferd_bytes]);
	g_idma_desc1d_test[i].dst = (uint32_t)0x0023fff0;
	g_idma_desc1d_test[i].row_bytes = blk_size;
	xferd_bytes = xferd_bytes + blk_size;
    }
    com_idma_fill_ctrl (g_idma_desc1d_test, num_blks, e_IDMA_DESC_TYPE_1D, NULL);
    com_idma_trans_desc (g_idma_desc1d_test, num_blks, &task_test);
    //status_reg=idma_reg_read (IDMA_STATUS_REG);
#endif
}
...
uint8_t  idma_tests ()
{
    ...
    for (int i = 0; i < 1; i++)
    { 
	//idma_test_1d_poll_dcode_to_dcode();
	idma_test_broadcast();
#if 1
	//idma_test_broadcast_dcode_to_dcode();
	...
	//wait for brodcast tests to be completed on host 
	do
	{ 
	    if(rsm_com_shared_flag==0x8844)
		break;
	...
	rsm_com_shared_flag=0;
	rsm_ax_shared_flag=0;
	//IDMA tests with number of decriptors varying from min to max(0-244)
	...
	//wait till host complete read operation
