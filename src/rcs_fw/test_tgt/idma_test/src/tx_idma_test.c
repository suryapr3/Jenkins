...
volatile uint32_t  buff_rsm[IDMA_TEST_BUFF_SIZE] __attribute__ ((section (".sram.tx.rw")));
...
volatile uint32_t  buff_r[IDMA_TEST_BUFF_SIZE*8] __attribute__ ((section (".sram.tx.rw")));
...
uint32_t dataCnt,errCnt = 0,count = 0,cnt = 0,value,buff_dst1[1];
...
int main(int argc, char **argv)
{
    ...
    do
    {	
	if(rsm_tx_shared_flag==0x1234)
	    break;
	com_timing_wait_ns(10000);   

    }while(rsm_tx_shared_flag!=0x1234);
    idma_tests();
    rsm_cal_shared_flag=0x1234;
    //while(1);
    //idma_negative_test();
    return (0);
}
...
void idma_test_1d_poll_dcode_to_rsm ()
{
    ...
    do
    {   
	if(rsm_com_shared_flag==0x6644)
	    break;
    }while(rsm_com_shared_flag!=0x6644);
...
void idma_test_desc_2d_dcode_to_dcode(uint32_t num_bytes,uint32_t blk_size)
{
    ...
    idma_task_info_t task_test = {.pfunc_idma_callback = idma_cb_test, .p_arg = &task_test};
    g_test_val = 0;
    uint32_t num_blks;
    num_blks=num_bytes/blk_size; 
    if(num_blks>254)
...
uint8_t  idma_tests ()
{
	...
	while(rsm_tx_shared_flag!=0x6644);
	rsm_tx_shared_flag=0;  
	rsm_com_shared_flag=0;
