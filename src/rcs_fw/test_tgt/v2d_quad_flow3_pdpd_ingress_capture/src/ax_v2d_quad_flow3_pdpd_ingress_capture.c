...
/**
 * @file    ax_v2d_quad_flow3_pdpd_ingress_capture.c
...
int main (int argc, char **argv)
{
    ...
    uint32_t v2d_base = 0, vex_base = 0, quad_instance = 0, v2d_num = 0, capture_delay_value = 0, pdpd_rate = 0, match_value = 0,
             ...
             dram_buff_index = 0, samples_per_antenna = 0;
    int lc = 0, vex_num = 0, v2d_index = 0, timer_value = 0;
    ...
    pdpd_rate = rsm_buf[4];
    drf_clock = rsm_buf[5];
    samples_per_antenna = rsm_buf[6];

    clk_per_blk = (drf_clock) ? (CYCLES_PER_BLOCK_1P5G) : (CYCLES_PER_BLOCK_983M);
    delay_value_per_antenna = ((samples_per_antenna / g_samples_per_blk[drf_clock][pdpd_rate]) * clk_per_blk) + rsm_buf[3];

    /*BFN configuration*/
    quad_configure_v2d_bfn(quad_instance, 1/*capture_enable*/, (SAMPLES_TO_FB_MEM_SIZE(samples_per_antenna) - 1), match_value);
    ...
    *(BFN_CSR_BFNE_START0_REG(v2d_base)) = bfne_start_reg;
    ...
    for (v2d_index = 0; v2d_index < V2D_PER_QUAD; v2d_index++)
    {
        v2d_num = g_v2d_instance_in_quad[quad_instance][v2d_index];
        v2d_base = V2D_BFN_BASE(v2d_num);
		((bfn_csr_t*) (v2d_base)) -> BFNE_START[0].value = 0xFFFFFFFF;
    }
