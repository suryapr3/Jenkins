...
def main():
    ...
    if args.msg != None and (args.c2h == None and args.x2h == None):
        p_red('can only use with --c2h or --x2h option for message header generating')
        sys.exit()
    if args.ctypes_dir != None and (args.c2h == None and args.x2h == None):
        p_red('can only use with --c2h or --x2h option for ctypes compatible struct generating')
        sys.exit()
    ...
    if (args.xml or args.html_dir or args.no_test or args.table_init != None or args.test_file != None) \
        ...
        if args.x2c != None:
            ...
            f = Path(f_name)
            if f.is_file():
                x2h_convert(f_name, args.sheets, args.dir, args.no_test, args.use_xtensa, args.address + args.section, args.length, args.offset, args.xml, args.html_dir, args.table_init, args.test_file, args.macros, args.msg, args.hdr, args.ctypes_dir)
            ...
            for f in args.x2h:
                if Path(f).is_file():
                    x2h_convert(f, args.sheets, args.dir, args.no_test, args.use_xtensa, args.address + args.section, args.length, args.offset, args.xml, args.html_dir, args.table_init, args.test_file, args.macros, args.msg, args.hdr, args.ctypes_dir)
                else:
                    p_purple3("given file", f, "doesn't exist!!!")
        elif args.csv2api != None:
            filename = (args.csv2api[0]).split(":")[0]
            mode = (args.csv2api[0]).split(":")[1]
            automate_msg.csv2api(filename, r"ipcommon/fw/srp/", mode)
        else:
            parser.print_help()
            sys.exit()
