...
def x2h_convert(xlsx_file, sheets, dest_dir, no_test, use_xtensa, addr_sec, length, ofs, xml, html_dir, table_init, test_file, tb_access_macros, do_msg, hdr_list, ctypes_dir):
    ...
    if xlsx_file.endswith(".xlsx"):
        ...
        # tbl_dic[tbl_name] = (tbl_des, bf_blks, sub_tbl_dic, tbl_bitwidth_tup, align)
        # bf_blks = [[bf_blk], [], ...]
        #   bf_blk = [[bf_info], [], ...]
        ...
        tbl_dic, enum_dic = process_workbook(wb, valid_sheets, table_dim_dic)
        ...
        with open(dest_dir + '/' + super_table_name.lower() + ext + '.h', "w") as out_f:
            build_copyright_info(out_f)
            build_includ_head_info(out_f, super_table_name, ext)
            build_header_bitfield(out_f,super_table_name, tbl_dic, enum_dic, do_msg, hdr_list, ctypes_dir)
            build_includ_tail_info(out_f, super_table_name, ext)
            p_green2("Generated header file", super_table_name + ext + '.h')
        ...
        if do_msg == None:
            ...
            if no_test == False:
                ...
                if not use_xtensa:
                    import subprocess
                    try:
                        subprocess.check_call("{0} {1} {2} {1}.c {3}".format('gcc -std=c99 -Wall -o', test_file, " -I" + dest_dir, (table_init, '')[table_init == None]), shell = True)
