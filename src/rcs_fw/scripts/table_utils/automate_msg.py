...
def write_copyright(filename):
    ...
    with open(filename, "w") as f:
        ...
        f.write("** Copyright 2023 Intel Corporation All Rights Reserved.\n")
        f.write("**\n")
...
def write_headers(filename):
    ...
    if (len(filename.split(".c")) > 1):
        with open(filename, "a") as f:
            ...
            f.write("extern e_queue_ret_status_t srp_rcs_queue_insert (void *srp, msg_queue_t *p_msg_queue, msg_t *const p_msg, const uint32_t msg_len_word);\n\n")
        ...
        with open(filename, "a") as f:
            ...
            f.write(" *  @{\n")
            f.write(" */\n\n")
            f.write("#ifndef __SRP_RCS_MSG_API_H__\n")
            f.write("#define __SRP_RCS_MSG_API_H__\n\n")
...
def generate_func_body(msg_title, field_dic): 
    ...
    func_body += "    srp_rcs_queue_insert (srp, p_msg_queue, &msg, msg_len);\n"
...
def write_doxygen_functions(directory_name, output_path):
    ...
    for csv_filename in files:
        ...
        #splitting the .csv off of the file names
        msg_title = csv_filename.split(".")[0]
...
def csv2api(source_dir, dest_dir, command):
    """writes the header and c files for a specified source directory with message csv files

    Parameters
    ----------
    source_dir: the name of the directory following the path /abc_fw/arborcrest/rcs/msgs/ with the csv files
    ...
    os.makedirs("ipcommon/fw/srp/", exist_ok=True)
    os.makedirs("ipcommon/fw/srp/inc", exist_ok=True)
    os.makedirs("ipcommon/fw/srp/src", exist_ok=True)

    # if received the write append 'w', then we are writing for the first time so headers are needed
    if command == "w":
        write_headers(prepath + "/src/" + outputfile)
        write_headers(prepath + "/inc/" +  (outputfile).split(".")[0] + r".h")

    # always write the functions
    write_functions(source_dir, prepath + "/src/" + outputfile, rc_dic)
    write_doxygen_functions(source_dir, prepath + "/inc/" + outputfile.split(".")[0] + ".h")

    # if this function is the tail, needs to end with #endif at the end of the file
    if command == 't': 
        with open(prepath+"/inc/" + outputfile.split(".")[0] + ".h", "a") as f:
            f.write("#endif\n")
        os.remove("scripts/table_utils/pm/msg_dic.dat")
