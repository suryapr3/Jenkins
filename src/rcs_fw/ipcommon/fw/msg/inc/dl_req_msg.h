...
#ifndef __DL_REQ_MSG__
#define __DL_REQ_MSG__

#include <stdint.h>
#include "com_msgs_enum.h"

/*
 * message for DL configuration
 */
typedef union {
    struct
    {
        ...
        uint32_t ant_group_dl_ant_mask : 16;   // the mask reflects the antennas enabled
        uint32_t hw_tdd : 1;                   // Select HW/SW TDD
        uint32_t pdpd_rate : 3;                // index to table tx_pdpd_pc_sel_lut for PDPD rate selection in specific operating mod
        uint32_t pdpd_mode : 3;                // index to table tx_pdpd_mod_lut, specifying PDPD mod
        uint32_t pdpd_delay : 4;               // index to table tx_pdpd_dly_lut, for PDPD related delay info
        uint32_t fifo_delay : 4;               // index to table tx_v2d_start_dly_lut, for FIFO related delay settings
        uint32_t drf_filter_mode : 1;          // specifying DRF filter mod: 0 auto, 1:manual
        uint32_t jesd_dl_idx : 8;              // index into JESD config table
        uint32_t q0_j : 2;                     // indicates which JESD link is used to source data for quad 0
        uint32_t q1_j : 2;                     // indicates which JESD link is used to source data for quad 1
        uint32_t q2_j : 2;                     // indicates which JESD link is used to source data for quad 2
        uint32_t q3_j : 2;                     // indicates which JESD link is used to source data for quad 3
        uint32_t jesd_latency : 16;            // jesd latency in ns from JESD to antenna
        uint32_t fbr_mask : 4;                 // indicates the FBRs used for the band config
        uint32_t fbr_idx : 8;                  // Index into the FBR configuration table
        uint32_t res_191_172 : 20;             // reserved
        uint32_t vha_dl_latency : 16;          // Latency in ns from output of VHA to antenna
        uint32_t res_223_208 : 16;             // reserved
        uint32_t ant_group_dl_band_freq0 : 24; // Center channel frequency for band 0 during band combining(VHA)
        uint32_t vha_rate : 4;                 // vha rate selection to table com_vha_rate_conf_lut
        uint32_t strm_cfg : 4;                 // index to table com_v2d_strm_cfg_lut for STREAM related info
        uint32_t ant_group_dl_band_freq1 : 24; // Center channel frequency for band 1 during band combining(VHA)
        uint32_t vha_filter_mode : 1;          // specifies the filter mode enabled in HW accelerator, 0:auto, 1:manual
        uint32_t vha_dl_xn_en : 1;             // Enable X/duplicate data path for DPD learning
        uint32_t pdpd_op_mode : 2;             // index to table tx_pdpd_pc_sel_lut, specifying PDPD operation mod
        uint32_t topology : 3;                 // index to tx_topology_lut, for topology related info
        uint32_t res_287 : 1;                  // reserved
    };
    uint32_t value[9];
} dl_config_ant_grp_bf_t;
...
typedef union {
    struct
    {
        ...
        uint32_t quad_mask2 : 4;     // Mask to indicate quads that are activated with delayed timing relative to timestamp
        uint32_t quad_mask3 : 4;     // Mask to indicate quads that are activated with delayed timing relative to timestamp
        int32_t power_ant0 : 8;      // power for antenna0
        int32_t power_ant1 : 8;      // power for antenna1
        int32_t power_ant2 : 8;      // power for antenna2
        int32_t power_ant3 : 8;      // power for antenna 3
        int32_t power_ant4 : 8;      //  power for antenna 4
        int32_t power_ant5 : 8;      // power for antenna5
        int32_t power_ant6 : 8;      // power for antenna 6
        int32_t power_ant7 : 8;      // power for antenna7
        int32_t power_ant8 : 8;      // power for antenna8
        int32_t power_ant9 : 8;      // power for antenna9
        int32_t power_ant10 : 8;     // power for antenna10
        int32_t power_ant11 : 8;     // power for antenna 11
        int32_t power_ant12 : 8;     //  power for antenna 12
        int32_t power_ant13 : 8;     // power for antenna13
        int32_t power_ant14 : 8;     // power for antenna 14
        int32_t power_ant15 : 8;     // power for antenna15
        uint32_t quad_delay1 : 12;   // Time delay from timestamp to activation of quads specified in quad_mask1
        uint32_t res2 : 4;           // reserved
        uint32_t quad_delay2 : 12;   // Time delay from timestamp to activation of quads specified in quad_mask2
        uint32_t res3 : 4;           // reserved
...
typedef union {
    struct
    {
        ...
        e_dl_cal_update_type_t update : 4; // allow or disallow calibration updates
    ...
    uint32_t value[9];
} dl_update_control_bf_t;

#endif // __DL_REQ_MSG__
