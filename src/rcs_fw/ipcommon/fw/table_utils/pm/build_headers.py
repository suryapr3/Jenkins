...
def build_header_bitfield(out_f, super_table_name, tbl_dic, enum_dic, do_msg, hdr_list, ctypes_dir, no_msg_dic_gen):
    ...
    for tbl_name in tbl_sorted_keys: #tbl_dic.keys():
                ...
                if field_type[:2] == 'e_' and field_type[-2:] == '_t': # enum
                    ...
                    if bit_width != type_bitwidth:
                        ...
                        out_f.write("{5}{0} {1}{2:>{3}}{4}\n".format(field_type, e_name,
                                "; // ", set_pos(e_name + field_type, (bf_max_len + 4 + 7, 0)[tbl_struct_blk_num == 1]),
                    ...
                    out_f.write("{5}{0} {1}{2:>{3}}{4}\n".format(field_type, name,
                        "; // ", set_pos(name + field_type, (bf_max_len + 4 + 7, 0)[tbl_struct_blk_num == 1]),
                        wrap(bit_des, 128 - bf_max_len - 4 - 7 - 8, 2, '// ', bf_max_len + 4 + 7 - 2), pre_tab*2))
                    # for python ctypes struct
                    if ctypes_dir:
                    ...
                    # all reserved/unused fields treated as anonimouse field in generated struct:
                    # see module table_info line 289
        ...
        if not no_msg_dic_gen:
            # write dictionaries into the data file
            # FIXME: hardcoded path?
            MSG_DIC_FILE="fw/table_utils/pm/msg_dic.dat"
            if not os.path.exists(MSG_DIC_FILE):
                with open(MSG_DIC_FILE, "a+") as f:
                    f.write("")
            table_dict = {}
            with open(MSG_DIC_FILE, "r+") as f:
                if (os.stat(MSG_DIC_FILE).st_size != 0):
                    table_dict = json.load(f)
            table_dict[tbl_name] = field_dict
            with open(MSG_DIC_FILE, "w+") as f:
