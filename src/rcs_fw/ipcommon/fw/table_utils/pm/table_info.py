...
def float_to_hex(f):
    """float to hexadecimal convertion
    turn float (32-bit) to hex format
    ...
    Returns
    -------
    hex string (32-bit format) for converted float value
    """
    return hex(unpack('<I', pack('<f', f))[0])

def double_to_hex(f):
    """double to hexadecimal convertion
    turn double (64-bit) to hex format
    ...
    Returns
    -------
    hex string (64-bit format) for converted double value
    """
...
def get_table_pos_info():
    ...
    table_pos_info += ('C1',) # tbl_category_pos = 'C1' # cell holding table Category info
    table_pos_info += ('D1',) # tbl_subcategory_pos = 'D1' # cell holding table Subcategory info
...
def build_bf_struct(sheet, tbl_bitwidth_pos, tbl_struct_pos, bf_info_depth, enum_dic):
    ...
    xlsx_updt = False # track if any RES field name updates
    ...
    for blk_num in range(tbl_struct_blk_num):
        ...
        while bitwidth_check <= type_bitwidth and bitwidth_check > 0:
            ...
            # update empty field or reserved field to use uniq name in tbl_dic
            # see module build_headers line 277
            if val == None or val.lower() == 'reserved':
                if bitwidth == 1:
                    val = 'reserved_' + str(blk_num * type_bitwidth + pos)
                else:
                    val = 'reserved_' + str(blk_num * type_bitwidth + pos) + '_' + str(blk_num * type_bitwidth + pos - bitwidth + 1)
                sheet.cell(row = itr_row, column = itr_col).value = 'reserved' # val update bitfield name in sheet
                xlsx_updt = True
            ...
            if val == None:
                ...
                val = int(val, 16)
            bf.append(val)
            val = sheet.cell(row = itr_row + 5, column = itr_col).value # max
            if val == None:
                val = 0
            elif isinstance(val, str) and val.lower().startswith('0x'):
                val = int(val, 16)
    ...
    return (xlsx_updt, tbl_bitwidth, type_bitwidth, tbl_struct_blk_num, bf_blks, bf_pos_info, bf_max_len)
        ...
        for bf_val in sub_tbl_bf_info:
            ...
            for v, (p, w) in zip(bf_val, pos_info): # v: bit_field_value, w: bit_field_width, p: bit_field_pos
                if isinstance(v, float):
                    if w == 32:
                        v = int(float_to_hex(v),16)
                    else:
                        v = int(double_to_hex(v),16)
                try:
...
def process_workbook(workbook, worksheets, table_dim_dic): # {tbl_name:(A, B), ...}
    ...
    tbl_category_pos = table_pos_info[7]
    tbl_subcategory_pos = table_pos_info[8]
    ...
    for ws in worksheets:
        ...
        if tbl_name not in tbl_names.keys():
            ...
            sys.exit()
        xlsx_updt, tbl_bitwidth, type_bitwidth, tbl_struct_blk_num, bf_blks, bf_pos_info, bf_max_len = build_bf_struct(sheet, tbl_bitwidth_pos, tbl_struct_pos, bf_info_depth, enum_dic)
        tbl_bitwidth_tup = (tbl_bitwidth, type_bitwidth, tbl_struct_blk_num, bf_max_len)
        if table_dim_dic != {}:
            ...
            sub_tbl_dic = {}
        # sub_tbl_dic = sub_tbl_name:{L, R, sub_tbl_hex, sub_tbl_bf_info}, ...}}
        tbl_dic[tbl_name] = (tbl_des, bf_blks, sub_tbl_dic, tbl_bitwidth_tup, sheet[tbl_alignment_pos].value, sheet[tbl_category_pos].value, sheet[tbl_subcategory_pos].value)
        #pp.pprint(tbl_dic)
    return (xlsx_updt, tbl_dic, enum_dic)
