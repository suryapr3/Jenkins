...
def build_html_output(file_name, tbl_dic, table_dim_dic, html_dir, do_msg):
    ...
    with open(html_dir + '/' + file_name + '.html', "w") as of:
        ...
        if do_msg == None:
            ...
            of.write('<tr><th bgcolor=\"E0C0C0\">Name and layout</th><th bgcolor=\"E0C0C0\">Size (num of entries)</th>\n')
            for tbl_name in sorted(table_dim_dic.keys()):
                dim_info = table_dim_dic[tbl_name]
                if dim_info[0] == 1:
                    dim_info = str(dim_info[1])
                elif dim_info[1] == 1:
                    dim_info =str(dim_info[0])
                else:
                    dim_info = str(dim_info[0]) + 'x' + str(dim_info[1])

                of.write('<tr><td align=\"center\"><a href=\"#REF_' + tbl_name + '\">' + '<span style=\"color:blue;font-weight:bold\">' + tbl_name + '</span></a></td>\n')
                of.write('<td align=\"center\"><span style=\"color:green;font-weight:bold\">' + dim_info + '</span></td></tr>\n')
            of.write('</table><br>\n')
            ...
            for tbl_name in sorted(table_dim_dic.keys()):
                ...
                if dim_info[0] == 1:
                    ...
                    dim_info = ' [' + str(dim_info[0]) + '][' + str(dim_info[1]) + ']'
                tbl_des = tbl_dic[tbl_name][0]
                tbl_category = tbl_dic[tbl_name][5]
                tbl_subcategory = tbl_dic[tbl_name][6]
                ...
                of.write('<h3>Table: ' + tbl_name + '<span style=\"color:green;font-weight:bold\">' + dim_info + '</span></h3>\n')
                of.write('<p><span style=\"font-weight:bold\">Description:</span> ' + tbl_des + '</p>\n')
                if tbl_category != None:
                    of.write('<p><span style=\"font-weight:bold\">Category:</span> ' + str(tbl_category) + '</p>\n')
                    of.write('<p><span style=\"font-weight:bold\">Subcategory:</span> ' + str(tbl_subcategory) + '</p>\n')
                of.write('<p><span style=\"font-weight:bold\">Alignment:</span> ' + tbl_alignment + '</p>\n')
                of.write('<p><span style=\"font-weight:bold\">Single entry size:</span> ' + str(tbl_bitwidth//type_bitwidth) + 'x' + str(type_bitwidth) + '-bits' + '</p>\n')
            ...
            for tbl_name in sorted(tbl_dic.keys()):
                of.write('<tr><td align=\"center\"><a href=\"#REF_' + tbl_name + '\">' + '<span style=\"color:blue;font-weight:bold\">' + tbl_name + '</span></a></td>\n')
            of.write('</table><br>\n')
            of.write('<h2>Tables details:</h2>\n')
            for tbl_name in sorted(tbl_dic.keys()):
