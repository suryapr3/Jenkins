function [eqSlotGrid,refSlotGrid,emissions,eqSym,refSym] = hDecodeSlots(carrier,rxGrids,hest,...
    ...
    %   
    %   [...] = hDecodeSlots(...,SKIPCONTROLSYMBOLS,PDSCHENCODINGON)
    %   Create a grid of equalized symbols and reference symbols by decoding
    %   an array of demodulated PDSCH IQs .
    ...
    refSlotGrid = zeros(size(HestLow,1),nSlots*L,nLayers,nEVMWindowLocations);
    ...
    for slotIdx=slotRange
        ...
        if decodePdsch
            [channelIndices,channelSymbols,ptrsIndices,ptrsSymbols] = hSlotResources(cfgArray,slotIdx);
        elseif decodePusch
            ...
            % If tbs is empty, get the Transport block size from the
            % resource coressponding to RV 0
            if rv == 0
                tbs = cfgArray.Resources(resourceIdx).TransportBlockSize;
            elseif cfgArray.PUSCH.Coding && isempty(tbs)
        ...
        if decodePdsch
            for idx = 1:length(pxschObj)
                ...
                if any(mod(slotIdx,period) == cfgArray(idx).PDSCH.SlotAllocation) && cfgArray(idx).PDSCH.EnablePTRS
                    eqGridLow = [];
                    eqGridHigh = [];
        ...
        if decodePdsch && pdschEncodingOn
            ...
            for pIdx = 1:cfgLen
                ...
                if any(mod(slotIdx,period) == cfgArray(pIdx).PDSCH.SlotAllocation) && cfgArray(pIdx).PDSCH.Enable
                    currentSlotRange = [];
                    currentSlotRange = [currentSlotRange cfgArray(pIdx).Resources.NSlot];
                    resourceIdx = find(slotIdx == currentSlotRange);
                    trBlkSizes = [trBlkSizes;cfgArray(pIdx).Resources(resourceIdx).TransportBlockSize];
                    G = [G;cfgArray(pIdx).Resources(resourceIdx).G];
                    rv = [rv;cfgArray(pIdx).Resources(resourceIdx).RV];
                end
            end
        end
        ...
        for e = 1:nEVMWindowLocations
            ...
            if (decodePdsch && (~pdschEncodingOn || ~any(trBlkSizes))) || decodePdcch || (decodePusch && ~cfgArray.PUSCH.Coding)
                rxSymbols = eqGrid;
                if decodePusch
                ...
                if decodePdsch
                    ...
                    for pIdx = 1:cfgLen
                        ...
                        if ~cfgArray(pIdx).PDSCH.Coding
                            rx{1} = [rx{1} ; eq];
                            [~,ref,~,~] = hSlotResources(cfgArray(pIdx),slotIdx);
                        ...
                        [demodBits,blkerr] = decodeDLSCH(dlschLLRs,pxschObj{pIdx}.Modulation,nLayers,rv(tbIdx));
                        if any(blkerr)
                            ...
                            recodedBits = dlsch(pxschObj{pIdx}.Modulation,nLayers,G(tbIdx),cfgArray(pIdx).PDSCH.RVSequence(1));
                        end
            ...
            if decodePdsch && pdschEncodingOn && any(trBlkSizes)
            ...
            if decodePusch && ~isempty(emissions.DeltaRB)
                ...
                Y = rxGridLow(:,symIdx,:);
                Y([puschIndCurrentSlot; dmrsIndices]) = NaN;
                ...
                scPower = sum(abs(Y(:,1:L)).^2,2,'omitnan');
                scPower(scInd) = [];
                rbPower = sum(reshape(scPower, 12, nDeltaRB).', 2);
                emissions.Absolute(:, slotIdx+1) = rbPower/(L);
                emissions.Relative(:, slotIdx+1) = emissions.Absolute(:, slotIdx+1)/ ...
                    mean(emissions.Absolute(:, slotIdx+1));
            end
        end
    end
    if decodePusch && transformPrecoding
...
function puschEq = puschCPE(carrier,pusch,rxGrid,estChannelGrid)
    ...
    % tempGrid.
    cpe = nrChannelEstimate(tempGrid,ptrsAllIndices,betaPTRS*ptrsSymbols,'CDMLengths',pusch.DMRS.CDMLengths,'CyclicPrefix',carrier.CyclicPrefix);
