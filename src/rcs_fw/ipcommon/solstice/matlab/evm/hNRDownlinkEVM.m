function [evmInfo,eqSym,refSym,flatness] = hNRDownlinkEVM(waveConfig,rxWaveform,cfg)
...
% Copyright 2019-2022 The MathWorks, Inc.
    ...
    for bwpIdx = 1:numBWPs
        ...
        [refGrid,idealGrid] = hReferenceGrid(carrier,bwpCfg,pdschArray,nSlots,'PDSCH');
        ...
        if correctFineFO && pdschEnable && pdschCfgLen
            [rxWaveform,averageFineFO] = hNRFrequencyOffset('fineFO',carrier,bwpCfg,rxWaveform,refGrid,idealGrid,...
                                                                     cdmLengths,sampleRate,activeSlots);
        ...
        if nEVMWindowLocations == 2
         ...
         %return the flatness from the channel estimation
         hl=mean(HestLow.'); %average over all symbols
         hh=mean(HestHigh.');
         FlatLow=20*log10(max(abs(hl))/min(abs(hl)));
         FlatHigh=20*log10(max(abs(hh))/min(abs(hh)));
         flatness=max(FlatLow,FlatHigh);
        ...
        if pdschCfgLen
            skipCtrlSymbols = ~isempty(label) && ~extrapolateHest;
            [eqSlotGrid,refSlotGrid,~,eqSym(:,bwpIdx),refSym(:,bwpIdx)] = hDecodeSlots(carrier,rxGrids,hest,...
                pdschArray,pdschObj,skipCtrlSymbols,pdschEncodingOn);
        end
        ...
        for pdcchCfgIdx = 1:size(pdcchWaveCfg,2)
            ...
            if currentPdcchCfg.PDCCH.Enable
                ...
                % the length of the input waveform. This grid contains only the
                % DM-RS and is primarily used for channel estimation.
                refGrid = hReferenceGrid(carrier,bwpCfg,currentPdcchCfg,nSlots,'PDCCH');
                ...
                % Demodulate the waveform
                pdcchRxGrid = nrOFDMDemodulate(carrier,rxWaveform,'SampleRate',sampleRate);
...
%   EVMCFG used for EVM measurement.
    ...
    if isfield(evmCfg,'Label')
        validateattributes(evmCfg.Label,{'string','char'},{'nonempty'},fcnName,'Label');
    end
