 ...
 * STATIC VARIABLES                            *
 ***********************************************/
static int32_t __attribute__ ((section (".sram.reserved"))) rf_calib_kp1_test_signal_A_int[] = {
    1351727940, 1742863099,  -1602079425, 1775435783,  568478634,  -1728550798, -951342906,  201354592,  1964976896,
    1996681055, -1470540617, 2021180607,  1963517092,  -62811110,  1289694794,  -1538086464, -336032732, 1785570484,
    1255020924, 1973504944,  668901210,   -1994103156, 1499498951, 1863986806,  767661645,
};

static int32_t __attribute__ ((section (".sram.reserved"))) rf_calib_kp1_test_signal_B_int[] = {
    1106985432,  1044245999,  -462881426, 667772454,  -1412242422, 884961210,   -2010762614, -958108484, -1949179035,
    -1730305824, 1389240794,  836782564,  -785551752, 1933688976,  -1999538859, -263090972,  -508702553, 1140385922,
    1267873921,  -1344871923, -43961585,  -233705488, 628409593,   899215101,   1093870970,
};

static int32_t __attribute__ ((section (".sram.reserved"))) rf_calib_kp1_result_output_int[] = {
    -186234901,  301758380,  -395508448, 262172202,  273701258, 922180687, 654740371,  -1138414389, -930304886,
    -1155124218, -317235782, 392896048,  -366149101, 332926129, 434065594, -466008112, 533893604,   326600217,
    -285675518,  206662842,  -143299471, -124286504, 524233977, 301602630, 341464564};
...
int main (int argc, char *argv[])
{
    ...
    {
        FW_PRINT (FW_ERR, "Error in creating heap");
        return (-1);
    }
    ...
    for (int i = 0; i < CALTEST_MATRIX_SIZE; ++i) { dma_in[i] = rf_calib_kp1_test_signal_A_int[i]; }
    ...
    for (int i = 0; i < CALTEST_MATRIX_SIZE; ++i) { dma_in[i] = rf_calib_kp1_test_signal_B_int[i]; }
    // Convert the fixed point input matrix A to float and assign it to a matrix
    ...
    caltest_matlab_output_float =
        (caltest_matlab_output_int.template cast<float> () * output_factor).reshaped (CALTEST_MATRIX_ROW_SIZE, CALTEST_MATRIX_COLUMN_SIZE);
