...
#endif
#define TASK_HEAP_SIZE 153600 // Needs to be multiple of header size (32 bytes)
...
void rf_calib_nleq_driver (int argc, char *p_argv[])
{

    if (!rf_calib_heap_create (heap_storage, TASK_HEAP_SIZE))
    {
        rf_calib_nleq_log_message_test ("Error in creating heap");
        return;
    }
#if DO_PRINTF
    ...
    { // Begin a scope so that all temporary matrixes can be removed before starting the final iteration
        ...
        for (uint32_t k = 0; k < RF_CALIB_NLEQ_NUM_REF_SIGNALS; ++k)
        {
            ...
            int16_t *dma_in = (int16_t *)rf_calib_calloc (RF_CALIB_NLEQ_CAPTURE_SIZE, sizeof (int16_t));
            rf_calib_int16_t_map_dy_1 dma_in_int (dma_in, RF_CALIB_NLEQ_CAPTURE_SIZE);
#if DO_INJECT
...
#endif
            rf_calib_com_convert_dma_output_to_float (dma_in, RF_CALIB_NLEQ_CAPTURE_SIZE, 3,
                                                      rf_calib_dma_in_float.data () + RF_CALIB_NLEQ_INTERPOLATION_FILTER_INPUT_PRE_CURSOR);
...
#endif
            rf_calib_com_convert_dma_output_to_float (dma_in, RF_CALIB_NLEQ_CAPTURE_SIZE, 0,
                                                      rf_calib_dma_in_target_float.data () + RF_CALIB_NLEQ_LINEAR_FILTER_PRECURSOR);
            end_time = rf_calib_nleq_profile_read_counter ();
...
#if DO_PRINTF
std::ofstream &rf_calib_nleq_obtain_ofstream_ref (void)
{
    return file_output_log;
}
...
{
    return file_input_observed_signal;
}
...
{
    return file_input_target_signal;
}
/* ---------------------------------------------------------------------------------------------------------------------
...
{
    return file_input_matlab_nleq_coeff;
}
#endif
