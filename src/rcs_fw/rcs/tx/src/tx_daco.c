...
#define DRF_ANT(quad_idx)                                                                                               \
    .strm = {{DACO_MC_5_PARAM (.p_ddac, ANT, DDAC, quad_idx, 0)}, {DACO_MC_5_PARAM (.p_ddac, ANT, DDAC, quad_idx, 1)}}, \
    DACO_MC_4_PARAM (.p_csr, ANT, CSR, quad_idx)

#define TRX_ANT_CHNL(trx_ant_idx, chnl_idx)                                                                                             \
    {                                                                                                                                   \
        DACO_MC_5_PARAM (.p_cdpd, TRX_ANT, CDPD, trx_ant_idx, chnl_idx), DACO_MC_5_PARAM (.p_smc, TRX_ANT, SMC, trx_ant_idx, chnl_idx), \
            DACO_MC_5_PARAM (.p_txm, TRX_ANT, TXM, trx_ant_idx, chnl_idx)                                                               \
    }
...
#define TRX_SYS_CHNL(trx_sys_idx, chnl_idx)                                                                                                \
    {                                                                                                                                      \
        DACO_MC_5_PARAM (.p_txg, TRX_SYS, TXG, trx_sys_idx, chnl_idx), DACO_MC_5_PARAM (.p_txcpm, TRX_SYS, TXCPM, trx_sys_idx, chnl_idx),  \
            DACO_MC_5_PARAM (.p_txsr, TRX_SYS, TXSR, trx_sys_idx, chnl_idx), DACO_MC_5_PARAM (.p_duc, TRX_SYS, DUC, trx_sys_idx, chnl_idx) \
    }
#define TRX_SYS(trx_sys_idx)                                                                                                     \
    .chnl = {TRX_SYS_CHNL (trx_sys_idx, 0), TRX_SYS_CHNL (trx_sys_idx, 1)}, DACO_MC_4_PARAM (.p_mod, TRX_SYS, MOD, trx_sys_idx), \
...
#define MEM_FB_CHAIN(fb_idx, chain_idx)                                                                                                  \
    {                                                                                                                                    \
        DACO_MC_5_PARAM (.p_nsip_hbi, MEM_FB, NSIP_HBI_, fb_idx, chain_idx), DACO_MC_5_PARAM (.p_dmac, MEM_FB, DMAC_, fb_idx, chain_idx) \
    }
#define MEM_FB(fb_idx)                                                                                                            \
    .fbr_chain = {MEM_FB_CHAIN (fb_idx, 0), MEM_FB_CHAIN (fb_idx, 1)}, DACO_MC_4_PARAM (.mem_fb_sram, MEM_FB, SRAM_BASE, fb_idx), \
    DACO_MC_4_PARAM (.p_mem_fb, MEM_FB, CFG, fb_idx), DACO_MC_4_PARAM (.p_desc, MEM_FB, DESC, fb_idx)

#define V2D_CHNL(v2d_idx, chnl_idx)                                                                                                   \
    {                                                                                                                                 \
        DACO_MC_5_PARAM (.p_pdpd, V2D, PDPD_, v2d_idx, chnl_idx), DACO_MC_5_PARAM (.p_av2d_axi4, V2D, AV2D_AXI4_, v2d_idx, chnl_idx), \
            DACO_MC_6_PARAM (.av2d_apb.p_nsip_hbi, V2D, AV2D_APB_, NSIP_HBI_1, v2d_idx, chnl_idx),                                    \
...
#define VHA_HW_CHNL(vha_idx, chnl_idx)                                                                                                  \
    {                                                                                                                                   \
        DACO_MC_6_PARAM (.p_duc[0], VHA, ANT, DUC0, vha_idx, chnl_idx), DACO_MC_6_PARAM (.p_duc[1], VHA, ANT, DUC1, vha_idx, chnl_idx), \
            DACO_MC_6_PARAM (.p_mod, VHA, ANT, MOD, vha_idx, chnl_idx), DACO_MC_5_PARAM (.p_vha_ant, VHA, ANT, vha_idx, chnl_idx),      \
...
#define VHA_HW(vha_idx)                                                                                   \
    ...
    DACO_MC_4_PARAM (.p_swg, VHA, DL_SWG, vha_idx)
 ...
 ***********************************************/
daco_t tx_daco = {.is_first_cfg = true,
                  .hw = {.drf_ant[0] = {DRF_ANT (0)},
                         ...
                         .mem_fb_cap[0] = {MEM_FB (0)},
                         .mem_fb_cap[1] = {MEM_FB (1)}},
...
void tx_daco_clear ()
{
    ...
    memset (&tx_daco.sys_params, 0, sizeof (sys_params_t));
    memset (&tx_daco.quad_params, 0, sizeof (quad_params_t) * NUM_QUAD);
    memset (&tx_daco.drf_params, 0, sizeof (drf_params_t) * NUM_DRF); // to clear this?
}
