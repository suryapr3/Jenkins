...
function get_block(label)
{
  blocks=document.getElementsByName(label);
...
function clear_deps()
{
  ...
  for (var i = 0; i < tds.length; i++) {
    ...
    tds[i].setAttribute("masked_pred", "");
    tds[i].setAttribute("masked_succ", "");
  ...
  for (var i = 0; i < tds.length; i++) {
    if (tds[i].getAttribute("class")!="basic_block") continue;
    tds[i].setAttribute("target_bb", "");
  }
  ...
  for (var i = 0; i < pres.length; i++) {
    ...
    pres[i].setAttribute("masked_pred", "");
    pres[i].setAttribute("masked_succ", "");
  }
}
...
function add_masked_successors(operation)
{
  var masked_succs=eval(operation.id + "_masked_succs")
  for (var i=0; i<masked_succs.length; i++) {
    var operation = get_operation(masked_succs[i])
    if (!operation) continue;
    operation.setAttribute("masked_succ", "on");
  }
}

function add_masked_predecessors(operation)
{
  var masked_preds=eval(operation.id + "_masked_preds")
  for (var i=0; i<masked_preds.length; i++) {
    var operation = get_operation(masked_preds[i])
    if (!operation) continue;
    operation.setAttribute("masked_pred", "on");
  }
}


function add_target_bbs(operation,mode)
{
  var attrs = operation.getAttribute("hive:target_bbs");
  if(!attrs) {
    return;
  }
  var arr = attrs.split(';');
  for (var i=0; i<arr.length; i++) {
    ...
    var block = get_block(arr[i])
    if (!block) {
        continue;
    }
    block.setAttribute("target_bb", mode);
  }
}
...
function show_deps(e)
{
  ...
  if (get_operation_dep(operation) != "source") {
    ...
    add_masked_predecessors(operation);
    add_masked_successors(operation);
    add_reguses (operation,"on");
    add_regdefs (operation,"on");
    add_target_bbs (operation,"on");
