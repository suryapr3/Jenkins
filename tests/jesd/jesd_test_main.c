    ...
    const char *argv[])
{
    ...
    {
        if ( argc > 5) {
            // error case - handler does not exist.
            printf("ERROR:  param exceeded, handler dont exist....!\n");
            rv = -1;
        }else
        {
            // collect params 
            usecase_id  = (usecase_id_t) atoi(argv[0]);
            flow_id     = (flow_id_t) atoi(argv[1]);
            vex_id      = (vex_id_t) atoi(argv[2]);
            jesd_id     = (jesd_id_t)atoi(argv[3]);
            // generic function to provide option for any usecase/any flow.
            configure_jesd_generic (usecase_id, flow_id, vex_id, jesd_id);
        }
    }
    else //  
    {
