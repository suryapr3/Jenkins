 ...
 *********************************************************************************************/
/*
 * @file    nsip_b2h_nomap_drop.c
 * @brief   this test will enable a VEX core to transmit samples to the VHA without enabling
 *          correct B2H address map table which will help simulating H2B NOLKUP DROP interrupt.
 */
    ...
    const char* argv[])
{
    ...
    printf("\n");
    printf("Test: NSIP B2H NOMAP DROP test\n");
    printf("%s\n", g_compile_time);
    printf("BUILD_DIR: %s\n\n", BUILD_DIR);
    ...
    // Check if there is a B2H_NOMAP interrupt getting set or not
...
int test_irq_handler(uint32_t vha_num, uint32_t ant_num, int band)
{
    ...
    if(nsip_int_status != (1 << 1))           // B2H_NOMAP_DROP
    {
    ...
    uint32_t band)
{
    ...
    cm->B2H_CSR_ADDR_TBL[0].value = 0;                     // to simulate B2H_NOMAP interrupt
    cm->B2H_CSR_ADDR_MASK_TBL[0].value = 0x1ffff;          // mask
    cm->B2H_CSR_ADDR_TBL[1].value = 0;                     // to simulate B2H_NOMAP interrupt
