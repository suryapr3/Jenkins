...
// Macros
#define RCS_CORE_READY            ( 0xc001babe )
#define RCS_CORE_FINISHED         ( 0xdeaddead )
#define RCS_ISR_SERVICED          ( 0x0000dead )
...
#define RSM_NUM_IRQ_ADDR          ( 0x0023fff0 )
...
int test_irq_post_handler(uint32_t vha_num, int ant, uint32_t force, int module);
int test_check_status(uint32_t vha_num);
void test_show_regs(uint32_t vha_num);
int wait_for_rcs_handshake(uint32_t rcs_core, uint32_t addr, uint32_t handshake_msg, char* msg);

int g_num_interrupt_forced = 0;

int main(int argc, const char * argv[])
{
    int dev_id = 0, rv = 0;
    int rcs_core, vha_num, irq_type;
    const char* irq_type_str[2] = {"LOW", "HIGH"};
    srp_st_t srp_st = SRP_ST_SUCCESS;

    init();
    jump_to_ram();
    LOG_PRINT("Test: VHA interrupt force test with RCS\n");
    LOG_PRINT("%s\n", g_compile_time);
    ...
    printf("INFO: File    : %s\n", __FILE__);
    printf("INFO: argc    : %d\n", argc);
    printf("INFO: argv[0] : %d = RCS_CORE = (%d)\n", rcs_core, rcs_core);
    printf("INFO: argv[1] : %d = vha_num  = (VHA%d)\n", vha_num, vha_num);
    printf("INFO: argv[2] : %d = irq_type = (%s)\n", irq_type, irq_type_str[irq_type]);
    ...
    write_reg_18a(rsm_buf_addr, vha_num, 0xffffffff, "RSM_BUF_ADDR");
    write_reg_18a((rsm_buf_addr + 4), irq_type, 0xffffffff, "RSM_BUF_ADDR + 4");
    ...
    if(rcs_core == RCS_CORE_AX)
    {
        ...
        rcs_params.rcs_cfg[rcs_core_id].img_file = "./rcs_fw/ax_hw.elf";
    } else if(rcs_core == RCS_CORE_RX)
    {
        rcs_core_id = SRP_RCS_RCP_CTRL_RX;
        rcs_params.rcs_cfg[rcs_core_id].img_file = "./rcs_fw/rx_hw.elf";
    } else if(rcs_core == RCS_CORE_TX)
    {
        rcs_core_id = SRP_RCS_RCP_CTRL_TX;
        rcs_params.rcs_cfg[rcs_core_id].img_file = "./rcs_fw/tx_hw.elf";
    }
    ...
    // waiting for the RCS CORE to update the RSM_HANDSHAKE_ADDR as a means for handshaking
    if(wait_for_rcs_handshake(rcs_core, RSM_HANDSHAKE_ADDR, RCS_CORE_READY, "RCS_CORE_READY") < 0)
        return FAILURE;

    LOG_PRINT("INFO: Going to force the interrupts\n");
    ...
    if(wait_for_rcs_handshake(rcs_core, RSM_HANDSHAKE_ADDR, RCS_CORE_FINISHED, "RCS_CORE_FINISHED") < 0)
        return FAILURE;

    int num_isr_called;
    num_isr_called = read_reg_18a(RSM_NUM_IRQ_ADDR, "RSM_NUM_IRQ_ADDR");
    if(num_isr_called != g_num_interrupt_forced)
    {
        printf("ERROR: Number of interrupts forced is NOT same as number of times the ISR is called\n");
        rv = FAILURE;
    }
    printf("INFO: Number of interrupts forced = %d, Number of times the RCS ISR is called = %d\n", g_num_interrupt_forced, num_isr_called);
...
int test_run(uint32_t vha_num)
{
    ...
    for(ant = 0; ant < 2; ant++)
    {
        ...
        for(i = 0; i < VHA_ANT_NUM_INTERRUPTS; i++)
        {
             ...
             g_num_interrupt_forced++;
             if(wait_for_rcs_handshake(0, RSM_HANDSHAKE_ADDR, RCS_ISR_SERVICED, "RCS_ISR_SERVICED") < 0)
                 return FAILURE;

             if(rv = test_irq_post_handler(vha_num, ant, force, module))
             {
                  // Here means, something went wrong
                  printf("ERROR: test_irq_post_handler() returned rv = %d\n", rv);
                  return rv;
             } else
             {
                  printf("INFO: Interrupt test passed for VHA = %d, ANT = %d, force = %x\n\n", vha_num, ant, force);
             }
             write_reg_18a(RSM_HANDSHAKE_ADDR, ISR_ACKNOWLEDGED, 0xffffffff, "RSM_HANDSHAKE_ADDR");
        }
    }
    ...
    for(cm = 0; cm < 2; cm++)
    {
        ...
        for(i = 0; i < NSIP_HBI_NUM_INTERRUPTS; i++)
        {
             ...
             if(rv = test_irq_post_handler(vha_num, cm, force, module))
             {
                  // Here means, something went wrong
                  printf("ERROR: test_irq_post_handler() returned rv = %d\n", rv);
                  return rv;
    ...
    for(swg = 0; swg < 2; swg++)
    {
        ...
        for(i = 0; i < SWG_NUM_INTERRUPTS; i++)
        {
             ...
             if(rv = test_irq_post_handler(vha_num, swg, force, module))
             {
                  // Here means, something went wrong
                  printf("ERROR: test_irq_post_handler() returned rv = %d\n", rv);
                  return rv;
    ...
    write_reg_18a(RSM_HANDSHAKE_ADDR, TEST_DONE, 0xffffffff, "RSM_HANDSHAKE_ADDR");
...
/**
 * function         test_irq_post_handler()
...
int test_irq_post_handler(uint32_t vha_num, int ant, uint32_t force, int module)
{
    ...
    if((vha_int_high_stat != vha_int_mask) || (vha_int_low_stat != vha_int_mask))
    {
        ...
        printf("VHA_INT_HIGH_STAT = %x\nVHA_INT_LOW_STAT  = %x\nEXPECTED_VAL = %x\n", vha_int_high_stat, vha_int_low_stat, vha_int_mask);
        return FAILURE;
    }
...
/*
 * function    : wait_for_rcs_handshake()()
 * @brief      : this function is a helper routine that wait for a handshake from the selected rcs core
 *
 */
int wait_for_rcs_handshake(uint32_t rcs_core, uint32_t addr, uint32_t handshake_msg, char* msg)
{
    ...
    LOG_PRINT("HOST: Waiting for the RCS CORE-%d to update the RSM_HANDSHAKE_ADDR (0x%x) as %s\n", rcs_core, addr, msg);
    cnt = 0;
    while((handshake != handshake_msg) && (cnt++ < 65536))
    {
        handshake = read_reg_18a(addr, "RSM_HANDSHAKE_ADDR");
    }
    if(cnt > 65536)
    {
