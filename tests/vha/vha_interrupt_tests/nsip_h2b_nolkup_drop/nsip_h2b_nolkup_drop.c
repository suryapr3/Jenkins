 ...
 *********************************************************************************************/
/*
 * @file    nsip_h2b_nolkup_drop.c
 * @brief   this test will enable a VEX core to transmit samples to the VHA without enabling
 *          correct H2B address map table which will help simulating H2B NOLKUP DROP interrupt.
 */
    ...
    const char* argv[])
{
    ...
    printf("\n");
    printf("Test: NSIP H2B NOLKUP DROP test\n");
    printf("%s\n", g_compile_time);
    printf("BUILD_DIR: %s\n\n", BUILD_DIR);
    ...
    // Check if there is an H2B_NOLKUP interrupt getting set or not
...
int test_irq_handler(uint32_t vha_num, uint32_t ant_num, int band)
{
    ...
    uint32_t vha_int_high_stat, vha_int_low_stat, nsip_int_status;
    int rv;
    ...
    addr1 = VHA_BASE(vha_num) + VHA_APB_OFFSET(ant_num) + VHA_NSIP_OFFSET;
    nsip_int_status = read_reg_18a(NSIP_HBI_INTR_INT_STAT_ADR(addr1), "VHA_NSIP_HBI_INT_STAT");
    printf("INFO: NSIP_HBI_INT_STAT = %x\n", nsip_int_status);
    ...
    if((vha_int_high_stat != (1 << ant_num)) || (vha_int_low_stat != (1 << ant_num)))
    {
    ...
    if(nsip_int_status != (1 << 6))           // H2B_NOLKUP_DROP
    {
    ...
    uint32_t band)
{
    ...
    cm->H2B_ADDR_LKUP_TBL[0].value = (0 << 31) | HIVE_ADDR_vha2vex_p5_8iq_vbuffer_out;
                                                           // address not valid to simulate error
    ...
    cm->H2B_TK_ADDR_LKUP_TBL[2].value = STRM2CIO_2_VEX_DMEM_OFFSET + HIVE_ADDR_vha2vex_p5_8iq_consumer_sm_gate;
                                                           // token will be written to the address 0x80 in data memory of VEX
    cm->H2B_ADDR_LKUP_TBL[2].value = (0 << 31) | HIVE_ADDR_vha2vex_p5_8iq_vbuffer_out;
                                                           // address not valid to simulate error
