 ...
 *********************************************************************************************/
/*
 * @file    dl_input_fifo_underflow.c
    ...
    const char* argv[])
{
    ...
    printf("\n");
    printf("Test: DL Input fifo underflow test\n");
    printf("%s\n", g_compile_time);
    printf("BUILD_DIR: %s\n\n", BUILD_DIR);
    ...
    printf("INFO: argv[7] : %d = dl_clk   (%s)\n", dl_clk, vha_dl_clk[dl_clk]);
    printf("INFO: argv[8] : %d = band%d\n", band, band);
    ...
    if((band < 0) || (band > 1))
    {
        printf("ERROR: Only 2 input fifos per VHA antenna in DL path, so the fifo_index can only take\n"
               "       values 0 or 1. Exiting test..!\n");
        return FAILURE;
    }
    ...
    if(rv = test_configure(tx_vex, rx_vex, vha_num, ant_num, in_rate, out_rate, mode, dl_clk, band))
    {
    ...
    // Tx VEX is programmed to stream ONLY a finite number of samples. Stoping streaming should
    //  cause an UNDERFLOW interrupt. Check the status of that interrupt now in a handler.
    rv = test_irq_handler(vha_num, ant_num, band);
    if(SUCCESS == rv)
    {
        printf("\nTest Result:------ PASS ------!\n\n");
    } else
 ...
 * @return          status
 */
int test_irq_handler(uint32_t vha_num, uint32_t ant_num, int band)
{
    uint64_t addr0, addr1;
    uint32_t vha_int_high_stat, vha_int_low_stat, vha_ant_int_status, vha_ant_dl_cc_if_stat;
    int rv;
    ...
    vha_ant_int_status = read_reg_18a(VHA_ANT_INT_STAT_ADR(addr1), "VHA_ANT_INT_STAT");
    printf("INFO: ANT_INT_STAT = %x\n", vha_ant_int_status);
    vha_ant_dl_cc_if_stat = read_reg_18a(VHA_ANT_DL_CC_IF_STAT0_ADR(addr1), "VHA_ANT_DL_CC_IF_STAT0");
    if(band) vha_ant_dl_cc_if_stat = read_reg_18a(VHA_ANT_DL_CC_IF_STAT1_ADR(addr1), "VHA_ANT_DL_CC_IF_STAT1");
    printf("INFO: ANT_DL_CC_IF_STAT = %x\n", vha_ant_dl_cc_if_stat);
    ...
    if(vha_ant_int_status != (1 << ((band + 1) % 3)))      // DL_IF_ERROR
    {
    ...
    if((vha_ant_dl_cc_if_stat & 1) != FIFO_UNDERFLOW)
    {
        // VHA_ANT_DL_CC_IF_STAT register is NOT getting set correctly
        printf("ERROR: VHA_ANT_DL_CC_IF_STAT register is NOT getting set correctly\n");
        rv = FAILURE;
    }
    return rv;
}
 ...
 * @param [in]      tx_vex, rx_vex, vha_num, ant_num, in_rate, out_rate, mode, dl_clk, band
 * @return          status
 */
    ...
    uint32_t band)
{
    ...
    if(rv = test_configure_vha(vha_num, ant_num, in_rate, out_rate, mode, dl_clk, dest_id, band))
    {
 ...
 * @param [in]      vha_num, ant_num, in_rate, out_rate, mode, dl_clk, band
 * @return          status
 */
    ...
    uint32_t band)
{
    ...
    uint64_t duc_base_addr, mod_base_addr, ant_reg_base_addr, cm_base_addr;
    uint32_t value;
    int i, rv;
    ...
    duc = &(vha_ant->duc[0]);
    duc_base_addr = vha_apb_base_addr + VHA_DUC_OFFSET(0);
    if(band)
    {
        duc = &(vha_ant->duc[1]);
        duc_base_addr = vha_apb_base_addr + VHA_DUC_OFFSET(1);
    }
    ...
    duc->CFG.value = value;
    if(rv = write_reg_18a(VHA_DUC_CFG_ADR(duc_base_addr),
                          duc->CFG.value,
                          VHA_DUC_CFG_WR_MASK,
                          "VHA_DUC_CFG"))
        return rv;
    ...
    ant_reg->VHA_ANT_DL_CFG.value = (1 << (14 + band)) | (out_rate << 8) | (3 << 6) | in_rate;
                                                           // bit15:14: dl_of_enable,
                                                           // bit[10:8]: out_rate0,
                                                           // bit[7:6]: mod_bypass,
                                                           // bit[2:0]: in_rate0
    ...
    cm->B2H_CSR_ADDR_TBL[1].value = 0x80000000;            // bit31: valid
                                                           // bit[27:0]: map_byte_addr
    ...
    cm->H2B_MD_CTRL_TBL[2].value = g_md_control_tbl[dl_clk][out_rate];
                                                           // bit[9:0]: da_block_size
    cm->H2B_TK_ROUTE_LKUP_TBL[2].value = dest_id;          // route the tokens to rx_vex
    cm->H2B_MD_ROUTE_LKUP_TBL[2].value = dest_id;          // route the metadata to rx_vex
    cm->H2B_ROUTE_LKUP_TBL[2].value = dest_id;             // route the data to rx_vex
    ...
    vha_reg->VHA_DL_IF_CFG.value = (1 << ((ant_num * 10) + (band * 5)));
                                                           // af_gate
    vha_reg->VHA_DL_CTL.value = (1 << ((2 * ant_num) + band));
                                                           // dl_ififo_enable ant0/1_band0
