 ...
 *********************************************************************************************/
/*
 * @file    ul_output_fifo_overflow.c
 * @brief   this test will enable a VEX core to transmit samples to the VHA without enabling
 *          correct H2B credits which will help simulating OFIFO OVERFLOW interrupt.
 */
    ...
    const char* argv[])
{
    ...
    printf("\n");
    printf("Test: UL Output fifo overflow test\n");
    printf("%s\n", g_compile_time);
    printf("BUILD_DIR: %s\n\n", BUILD_DIR);
    ...
    val_kargs[3] = 2;                                      // only 2 blocks of input samples
    ...
    printf("HOST: Doing VHA RESET cycle\n");
    ...
    // Check if there is an OVERFLOW interrupt getting set or not
...
int test_irq_handler(uint32_t vha_num, uint32_t ant_num)
{
    ...
    uint32_t vha_int_high_stat, vha_int_low_stat, vha_ant_int_status, vha_ant_ul_of_stat;
    int rv;
    ...
    vha_ant_ul_of_stat = read_reg_18a(VHA_ANT_UL_OF_STAT0_ADR(addr1), "VHA_ANT_UL_OF_STAT0");
    printf("INFO: ANT_UL_OF_STAT = %x\n", vha_ant_ul_of_stat);
    ...
    if(((vha_int_high_stat & (4 << ant_num)) != (4 << ant_num)) ||
       ((vha_int_low_stat & (4 << ant_num)) != (4 << ant_num)))
    {
    ...
    if(vha_ant_int_status != (1 << 12))                     // UL_OF_ERROR
    {
    ...
    if((vha_ant_ul_of_stat & 2) != FIFO_OVERFLOW)
    {
        // VHA_ANT_UL_OF_STAT register is NOT getting set correctly
        printf("ERROR: VHA_ANT_DL_OF_STAT register is NOT getting set correctly\n");
        rv = FAILURE;
    }
    ...
    uint32_t dest_id)
{
    ...
    cm->H2B_CREDIT_READY_CS.value = 0x0;                   // no credits to simulate overflow
