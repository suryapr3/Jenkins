function [r] = vha_decimate(s,config)
%VHA_DECIMATE simulates decimation stages W4 through W1 in the VEX HW
%accelerator
%INPUTS
%s=I+jQ complex input vector sc16.15
%config.vha_hbf_stages=boolean vector indicating which stages are active [w1,w2,w3,w4]
%OUTPUTS
...
scale_hbf=2^-18; %bit-shift only

%simulation each stage of decimation
q=fi(s,1,16,15); %load input values and quantize
for i=4:-1:1
    if(config.vha_hbf_stages(i))
        switch(i)
            ...
            case 1
                b=HB1;
            otherwise
                break;%bypass mode with no stages selected
        end
        coeffw=fi(b*scale_hbf,1,19,18);
        q=downsample(filter(coeffw,1,q),2,1);
