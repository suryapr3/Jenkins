...
stem(n1, input_iq); title("Input IQ samples @p187");

% write this into a file which can be sourced while running on FRIO
fileId = fopen("input_samples/upconversion/single_band_upconversion_p187.txt", "w");
fprintf(fileId, "%08x\n", input_iq);
fclose(fileId);
...
output_iq_from_model_p187 = vha_interpolate(input_iq_for_model, config);
...
stem(n2, output_iq_p187);  title ("Output - No filters enabled");

% write this output to another file
fileId = fopen("output_samples/upconversion/p187/single_band_upconversion_p187.txt", "w");
fprintf(fileId, "%08x\n", output_iq_p187);
fclose(fileId);
...
% now do an interpolation
output_iq_from_model_p375 = vha_interpolate(input_iq_for_model, config);
...
stem(n2, output_iq_p375); title ("Output - W1 enabled");
fileId = fopen("output_samples/upconversion/p187/single_band_upconversion_p375.txt", "w");
fprintf(fileId, "%08x\n", output_iq_p375);
fclose(fileId);
...
% now do an interpolation
output_iq_from_model_p75 = vha_interpolate(input_iq_for_model, config);
...
fileId = fopen("output_samples/upconversion/p187/single_band_upconversion_p75.txt", "w");
fprintf(fileId, "%08x\n", output_iq_p75);
fclose(fileId);
...
stem(n2, output_iq_1p5); title ("Output - W1, W2, W3 enabled");
fileId = fopen("output_samples/upconversion/p187/single_band_upconversion_1p5.txt", "w");
fprintf(fileId, "%08x\n", output_iq_1p5);
fclose(fileId);
...
stem(n2, output_iq_3);title ("Output - W1, W2, W3, W4 enabled");
fileId = fopen("output_samples/upconversion/p187/single_band_upconversion_3.txt", "w");
fprintf(fileId, "%08x\n", output_iq_3);
fclose(fileId);
