...
stem(n1, input_iq); title("Input IQ samples @p75");

% write this into a file which can be sourced while running on FRIO
fileId = fopen("input_samples/upconversion/single_band_upconversion_p75.txt", "w");
fprintf(fileId, "%08x\n", input_iq);
fclose(fileId);
...
stem(n2, output_iq_p75);  title ("Output - No filters enabled");

% write this output to another file
fileId = fopen("output_samples/upconversion/p75/single_band_upconversion_p75.txt", "w");
fprintf(fileId, "%08x\n", output_iq_p75);
fclose(fileId);
...
fileId = fopen("output_samples/upconversion/p75/single_band_upconversion_1p5.txt", "w");
fprintf(fileId, "%08x\n", output_iq_1p5);
fclose(fileId);
...
stem(n2, output_iq_3);title ("Output - W1, W2 enabled");
fileId = fopen("output_samples/upconversion/p75/single_band_upconversion_3.txt", "w");
fprintf(fileId, "%08x\n", output_iq_3);
fclose(fileId);
...
stem(n2, output_iq_6); title ("Output - W1, W2, W3 enabled");
fileId = fopen("output_samples/upconversion/p75/single_band_upconversion_6.txt", "w");
fprintf(fileId, "%08x\n", output_iq_6);
fclose(fileId);
