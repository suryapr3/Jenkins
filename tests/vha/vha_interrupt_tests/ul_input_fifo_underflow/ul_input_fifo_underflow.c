 ...
 *********************************************************************************************/
/*
 * @file    ul_input_fifo_underflow.c
 * @brief   this test will enable a VEX core to transmit a finite number of samples to a VHA
 *          input fifo and then stop to simulate an input fifo underflow condition.
 */
...
const char* input_path_prefix = "./vha_matlab_wrappers/input_samples/downconversion/";
const char* g_input_file[2][3] = {
    {
    "single_band_downconversion_1.txt",
    "single_band_downconversion_2.txt",
    "single_band_downconversion_4.txt"
    },
    {
    "single_band_downconversion_1p5.txt",
    "single_band_downconversion_3.txt",
    "single_band_downconversion_6.txt"
    }
   };

// Pointers to the Matlab output reference IQ samples
const char* output_path_prefix = "./vha_matlab_wrappers/output_samples/downconversion/";
const char* g_output_ref_file[2][15] = {
    {
    // For the inputs at 1Gsps
    "1g/single_band_downconversion_1.txt",                 // without any downconversion
    "1g/single_band_downconversion_p5.txt",                // W1 only
    "1g/single_band_downconversion_p25.txt",               // W1+W2
    "1g/single_band_downconversion_p125.txt",              // W1+W2+W3
    "NULL",

    // For the inputs at 2Gsps
    "2g/single_band_downconversion_2.txt",                 // without any downconversion
    "2g/single_band_downconversion_1.txt",                 // W1 only
    "2g/single_band_downconversion_p5.txt",                // W1+W2
    "2g/single_band_downconversion_p25.txt",               // W1+W2+W3
    "2g/single_band_downconversion_p125.txt",              // W1+W2+W3+W4

    // For the inputs at 4Gsps
    "4g/single_band_downconversion_4.txt",                 // without any downconversion
    NULL,                                                  // /2 is not legal
    "4g/single_band_downconversion_1.txt",                 // W1+W2
    "4g/single_band_downconversion_p5.txt",                // W1+W2+W3
    "4g/single_band_downconversion_p25.txt",               // W1+W2+W3+W4
    },
    {
    // For the inputs at 1.5Gsps
    "1p5/single_band_downconversion_1p5.txt",              // without any downconversion
    "1p5/single_band_downconversion_p75.txt",              // W1 only
    "1p5/single_band_downconversion_p375.txt",             // W1+W2
    "1p5/single_band_downconversion_p187.txt",             // W1+W2+W3
    "NULL",

    // For the inputs at 3Gsps
    "3g/single_band_downconversion_3.txt",                 // without any downconversion
    "3g/single_band_downconversion_1p5.txt",               // W1 only
    "3g/single_band_downconversion_p75.txt",               // W1+W2
    "3g/single_band_downconversion_p375.txt",              // W1+W2+W3
    "3g/single_band_downconversion_p187.txt",              // W1+W2+W3+W4

    // For the inputs at 6Gsps
    "6g/single_band_downconversion_6.txt",                 // without any downconversion
    "6g/single_band_downconversion_3.txt",                 // W1 only
    "6g/single_band_downconversion_1p5.txt",               // W1+W2
    "6g/single_band_downconversion_p75.txt",               // W1+W2+W3
    "6g/single_band_downconversion_p375.txt",              // W1+W2+W3+W4
    },
};
    ...
    const char* argv[])
{
    ...
    uint64_t base_addr;
    uint32_t tx_vex = 0, rx_vex =0, vha_num, ant_num;
    ...
    printf("\n");
    printf("Test: UL input fifo underflow test\n");
    printf("%s\n", g_compile_time);
    printf("BUILD_DIR: %s\n\n", BUILD_DIR);
    ...
    printf("INFO: argv[1] : %d = rx_vex   (VEX%d)\n", rx_vex, rx_vex);
    printf("INFO: argv[2] : %d = vha_num  (VHA%d)\n", vha_num, vha_num);
    printf("INFO: argv[3] : %d = ant_num  (ANTENNA%d)\n", ant_num, ant_num);
    printf("INFO: argv[4] : %d = in_rate  (%s)\n", in_rate, sys_rate_str[ul_clk][in_rate]);
    printf("INFO: argv[5] : %d = out_rate (%s)\n", out_rate, sys_rate_str[ul_clk][out_rate]);
    printf("INFO: argv[6] : %d = mode     (%s)\n", mode, ddc_mode_str[mode]);
    printf("INFO: argv[7] : %d = ul_clk   (%s)\n", ul_clk, vha_ul_clk[ul_clk]);
    ...
    if(out_rate > in_rate)
    {
        printf("ERROR: DDC output rate cannot be lesser than the input rate. Exiting test..!\n");
        return FAILURE;
    }
       ...
       ((in_rate == SYS_RATE_IQ_6) && (out_rate < SYS_RATE_IQ_P375)))
    {
        ...
        printf("       Least out_rate for %s in_rate is %s. Exiting test..!\n", sys_rate_str[ul_clk][in_rate],
                sys_rate_str[ul_clk][2]);
        return FAILURE;
    }
    if(((in_rate == SYS_RATE_IQ_4) && (out_rate > SYS_RATE_IQ_1)) ||
       ((in_rate == SYS_RATE_IQ_6) && (out_rate > SYS_RATE_IQ_1P5)))
    {
        printf("ERROR: Maximum output rate supported for %s input rate is %s\n", sys_rate_str[ul_clk][in_rate],
                       sys_rate_str[ul_clk][3]);
        printf("       Exiting test..!\n");
        return FAILURE;
    }
    ...
    val_kargs[3] = 2;
    val_kargs[1] = g_vectors_per_block[ul_clk][in_rate];
    ...
    if(rv = test_configure(tx_vex, rx_vex, vha_num, ant_num, in_rate, out_rate, mode, ul_clk))
    {
    ...
    write_mem_18a((VEX_BASE(tx_vex) + VEX_VMEM_OFFSET + HIVE_ADDR_vex2vha_vbuffer),
                  (g_vectors_per_block[ul_clk][in_rate] * NUM_OF_ELEMENTS_PER_VEC * sizeof(int)),
    ...
    strcat(file_name, g_output_ref_file[ul_clk][((in_rate - 3) * 5) + (in_rate - out_rate)]);
    ...
    write_mem_18a((VEX_BASE(rx_vex) + VEX_VMEM_OFFSET + HIVE_ADDR_vha2vex_p5_8iq_vbuffer),
                  (g_vectors_per_block[ul_clk][out_rate] * NUM_OF_ELEMENTS_PER_VEC * sizeof(int)),
    ...
    print_registers(VEX_BASE(rx_vex) + VEX_DMEM_OFFSET, 24, "Rx VEX DMEM");
    ...
    while(1)
    {
        ...
        if(h2b_flit_cnt > (g_max_blocks * g_vectors_per_block[ul_clk][out_rate] * 2))
        {
            break;
        }
        cnt++;
    ...
    // Tx VEX is programmed to stream ONLY a finite number of samples. Stop streaming should
    //  cause an UNDERFLOW interrupt. Check the status of that interrupt now in a handler.
    rv = test_irq_handler(vha_num, ant_num);
    if(SUCCESS == rv)
    {
        printf("\nTest Result:------ PASS ------!\n\n");
    } else
 ...
 * @param [in]      vha_num, ant_num, band
 * @return          status
 */
int test_irq_handler(uint32_t vha_num, uint32_t ant_num)
{
    uint64_t addr0, addr1;
    uint32_t vha_int_high_stat, vha_int_low_stat, vha_ant_int_status, vha_ant_ul_cc_if_stat;
    int rv;
    ...
    addr1 = VHA_BASE(vha_num) + VHA_APB_OFFSET(ant_num) + VHA_ANT_REG_OFFSET;
    vha_ant_int_status = read_reg_18a(VHA_ANT_INT_STAT_ADR(addr1), "VHA_ANT_INT_STATUS");
    printf("INFO: ANT_INT_STATUS = %x\n", vha_ant_int_status);
    vha_ant_ul_cc_if_stat = read_reg_18a(VHA_ANT_UL_CC_IF_STAT_ADR(addr1), "VHA_ANT_UL_CC_IF_STAT");
    printf("INFO: ANT_UL_CC_IF_STAT = %x\n", vha_ant_ul_cc_if_stat);

    // Clear the interrupt at the earliest
    if(rv = write_reg_18a(VHA_ANT_INT_CLR_ADR(addr1),
                          vha_ant_int_status, /*WTC*/
                          VHA_ANT_INT_CLR_WR_MASK,
                          "VHA_ANT_INT_CLR"))
        return rv;

    // Now check whether the status register values are as expected or not
    if((vha_int_high_stat != (1 << (2 + ant_num))) || (vha_int_low_stat != (1 << (2 + ant_num))))
    {
        // VHA interrupt high/low status registers are NOT getting set correctly
        printf("ERROR: VHA_INT_HIGH/LOW_STATUS registers are NOT getting set correctly\n");
        return FAILURE;
    }

    if(vha_ant_int_status != 1)                            // UL_IF_ERROR
    {
        // VHA_ANT interrupt status register is NOT getting set correctly
        printf("ERROR: VHA_ANT_INT_STAT register is NOT getting set correctly\n");
        return FAILURE;
    }

    if((vha_ant_ul_cc_if_stat & 1) != FIFO_UNDERFLOW)
    {
        // VHA_ANT_UL_CC_IF_STAT register is NOT getting set correctly
        printf("ERROR: VHA_ANT_UL_CC_IF_STAT register is NOT getting set correctly\n");
        return FAILURE;
    }
    return rv;
}
 ...
 * @param [in]      tx_vex, rx_vex, vha_num, ant_num, in_rate, out_rate, mode, ul_clk
 * @return          status
 */
    ...
    uint32_t ul_clk)
{
    ...
    if(rv = test_configure_vha(vha_num, ant_num, in_rate, out_rate, mode, ul_clk, dest_id))
    {
 ...
 * @param [in]      vha_num, ant_num, in_rate, out_rate, mode, ul_clk
 * @return          status
 */
    ...
    uint32_t dest_id)
{
    ...
    uint64_t ddc0_base_addr, dmod_base_addr, ant_reg_base_addr, cm_base_addr;
    uint32_t value;
    int i, rv;
    ...
    ant_reg->VHA_ANT_UL_CFG.value = (1 << 8) | (out_rate << 4) | (1 << 3) | in_rate;
    ...
    cm->H2B_MD_CTRL_TBL[4].value = g_md_control_tbl[ul_clk][out_rate];
                                                           // bit[9:0]: da_block_size
    cm->H2B_TK_ROUTE_LKUP_TBL[4].value = dest_id;          // route the tokens to rx_vex
    cm->H2B_MD_ROUTE_LKUP_TBL[4].value = dest_id;          // route the metadata to rx_vex
    cm->H2B_ROUTE_LKUP_TBL[4].value = dest_id;             // route the data to rx_vex
    ...
    vha_reg->VHA_CFG.value = 0x3fc | (ul_clk << 1);
