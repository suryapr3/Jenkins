    ...
    const char* argv[])
{
    ...
    uint64_t base_addr;
    uint32_t tx_vex = 0, rx_vex0 = 0, rx_vex1 = 0, vha_num, ant_num;
    ...
    printf("\n");
    printf("Test: Dual band demodulation\n");
    printf("%s\n", g_compile_time);
    printf("BUILD_DIR: %s\n\n", BUILD_DIR);
    ...
    if(rv = test_configure(tx_vex, rx_vex0, rx_vex1, vha_num, ant_num, in_rate, out_rate, mode, ul_clk))
    {
    ...
    print_registers(VEX_BASE(rx_vex0) + VEX_VMEM_OFFSET, 16, "Rx VEX Output ref samples");
    print_registers(VEX_BASE(rx_vex1) + VEX_VMEM_OFFSET, 16, "Rx VEX Output ref samples");
    ...
    while(1)
    {
       ...
       if(host_vec_processed0 >= (g_max_blocks * g_vectors_per_block[ul_clk][out_rate]))
       {
          break;
       }
       cnt++;
    ...
    while(1)
    {
       host_vec_processed1 = read_reg_18a((VEX_BASE(rx_vex1) + VEX_DMEM_OFFSET + HIVE_ADDR_vha2vex_p5_8iq_g_count1),
                                         "RX_VEX_HOST_VEC_PROCESSED");
       if(host_vec_processed1 >= (g_max_blocks * g_vectors_per_block[ul_clk][out_rate]))
       {
          break;
       }
       cnt++;
       if(128 == cnt)
       {
           printf("ERROR: Waited for %d cycles, but host_vec_processed is NOT reaching %d\n", cnt, (g_max_blocks * g_vectors_per_block[ul_clk][out_rate]));
           break;
       }
    }
    ...
    {
       printf("     Rx VEX %d did not process any samples (Band0)\n", rx_vex0);
       rv = FAILURE;
    }
    ...
    {
       printf("     Rx VEX %d did not process any samples (Band1)\n", rx_vex1);
       rv = FAILURE;
    }
 ...
 * @param [in]      tx_vex, rx_vex0, rx_vex1, vha_num, ant_num, in_rate, out_rate, mode, ul_clk
 * @return          status
 */
    ...
    uint32_t ul_clk)
{
    ...
    dest_id1 = rx_vex1;
    if(rv = test_configure_vha(vha_num, ant_num, in_rate, out_rate, mode, ul_clk, dest_id0, dest_id1))
    {
    ...
    uint32_t dest_id1)
{
    ...
    ant_reg->VHA_ANT_UL_OF_CFG[1].value = 0;               // no split
    ...
    cm->H2B_ADDR_LKUP_TBL[6].value = (1 << 31) | HIVE_ADDR_vha2vex_p5_8iq_vbuffer_out;
                                                           // data will be written to the address 0x4000 in vector memory
    cm->H2B_MD_CTRL_TBL[6].value = g_md_control_tbl[ul_clk][out_rate];
                                                           // bit[9:0]: da_block_size
    cm->H2B_TK_ROUTE_LKUP_TBL[6].value = dest_id1;         // route the tokens to rx_vex0
    cm->H2B_MD_ROUTE_LKUP_TBL[6].value = dest_id1;         // route the metadata to rx_vex0
    cm->H2B_ROUTE_LKUP_TBL[6].value = dest_id1;            // route the data to rx_vex0
