/*************************************************
*                                                *
*     Dual band demodulation with SPLIT          *
*                                                *
* Author : Haridas P                             *
...
This test case aims to validate the data path of the VHA involving the DEMODULATOR in the uplink data path.
The IQ samples will be fed to the VHA from any of the VEX cores. The VHA will be configured to demodulate these
IQ samples to produce band0 and band1 samples, which will then get down converted to the configured rate by the
digital down converter chain (ddc). The demodulator of the uplink path will also be enabled with the sine wave
generators needed for demodulating the band0 and band1 samples. The cordics per SWG will be enabled depending upon
the rate selected. For example only 1 for 1Gsps, 2 for 2Gsps and so on. The test will cover validation of all the
input rates (1G/2G/4G) of demodulation.


How to run the test ?
	From the FRIO bash terminal
	python dual_band_demodulation_with_split_processing.py <tx_vex> <rx_vex0> <rx_vex1> <rx_vex2> <rx_vex3> <vha_num> <port_num> <in_rate> <out_rate> <ddc mode> <ul_clk>

	@input arguments
		tx_vex   : This is the VEX core who'll stream samples to the VHA instance.
		rx_vex0  : This is the VEX core who'll recieve splitted VHA processed band0 samples.
		rx_vex1  : This is the VEX core who'll recieve splitted VHA processed band0 samples.
		rx_vex2  : This is the VEX core who'll recieve splitted VHA processed band1 samples.
		rx_vex3  : This is the VEX core who'll recieve splitted VHA processed band1 samples.
		...
		ddc_mode : This is the mode of operation of the DDC (0: AUTO or 1: MANUAL)
		ul_clk   : This is the downlink clock selection (0: 983.04MHz, 1: 1.47GHz)
