...
%               dual band demodulation testing at 1Gsps
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% generate cw0 for the modulation Fs = 1Gsps, Fclk = 1Ghz
Fclk = 983040;
Fs   = 983040;
fout1 = (Fclk / 2);
...
n3 = 1:1:(length(cw0));
stem(n3, real(cw0), 'red'); title("CW0 @1Gsps");
stem(n3, imag(cw0), 'blue');
subplot(4, 2, 6); grid on; hold on;
stem(n3, real(cw1), 'blue'); title("CW1 @1Gsps");
...
stem(n1, real(input_iq), 'green'); title("Input IQ samples @1Gsps");
...
config.vha_conj = [0 0];
cw0 = transpose(cw0);
demod_output_1g_band0 = vha_split(input_iq_for_model, cw0, config);
n3 = 1:1:length(demod_output_1g_band0);
subplot(6, 2, 5); grid on; hold on;
stem(n3, real(demod_output_1g_band0), 'red'); title("Band0 Output - 1Gsps Demodulation");
stem(n3, imag(demod_output_1g_band0), 'blue');

% Band1 demodulation
cw1 = transpose(cw1);
demod_output_1g_band1 = vha_split(input_iq_for_model, cw1, config);
n3 = 1:1:length(demod_output_1g_band1);
subplot(6, 2, 6); grid on; hold on;
stem(n3, real(demod_output_1g_band1), 'red'); title("Band1 Output - 1Gsps Demodulation");
stem(n3, imag(demod_output_1g_band1), 'blue');
...
ddc_output_p125_band0 = vha_decimate(demod_output_1g_band0, config);
ddc_output_p125_band1 = vha_decimate(demod_output_1g_band1, config);
...
fileId = fopen("output_samples/demodulation/1g/band0_dual_band_demodulation_p125.txt", "w");
...
fileId = fopen("output_samples/demodulation/1g/band1_dual_band_demodulation_p125.txt", "w");
...
config.fine_delay = 0;
config.coarse_delay = 0;
ddc_output_p25_band0 = vha_decimate(demod_output_1g_band0, config);
ddc_output_p25_band1 = vha_decimate(demod_output_1g_band1, config);
...
fileId = fopen("output_samples/demodulation/1g/band0_dual_band_demodulation_p25.txt", "w");
...
fileId = fopen("output_samples/demodulation/1g/band1_dual_band_demodulation_p25.txt", "w");
...
config.fine_delay = 0;
config.coarse_delay = 0;
ddc_output_p5_band0 = vha_decimate(demod_output_1g_band0, config);
ddc_output_p5_band1 = vha_decimate(demod_output_1g_band1, config);
...
fileId = fopen("output_samples/demodulation/1g/band0_dual_band_demodulation_p5.txt", "w");
...
fileId = fopen("output_samples/demodulation/1g/band1_dual_band_demodulation_p5.txt", "w");
