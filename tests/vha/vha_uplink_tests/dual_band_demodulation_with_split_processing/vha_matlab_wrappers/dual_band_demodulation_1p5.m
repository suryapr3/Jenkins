...
%               dual band demodulation testing at 1.5Gsps
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% First generate an upconverted modulated output (1.5Gsps), which can later be
...
% generate cw0 for the modulation Fs = 1.5Gsps, Fclk = 1.5Ghz
Fclk = 1474000;
Fs   = 1474000;
fout1 = (Fclk / 2);
fprintf("fout1 = %d KHz, Fs = %d KHz, Fclk = %d KHz\n", fout1, Fs, Fclk);
cw0 = nco_pp(fout1, Fs, Fclk, length(band0_output_iq_from_duc), 0);   % 0 clk cycles delay
fout2 = (Fclk / 4);
fprintf("fout2 = %d KHz, Fs = %d KHz, Fclk = %d KHz\n", fout2, Fs, Fclk);
cw1 = nco_pp(fout2, Fs, Fclk, length(band1_output_iq_from_duc), 2);   % 2 clk cycles delay
subplot(4, 2, 5); grid on; hold on;
n3 = 1:1:(length(cw0));
stem(n3, real(cw0), 'red'); title("CW0 @1.5Gsps");
stem(n3, imag(cw0), 'blue');
subplot(4, 2, 6); grid on; hold on;
stem(n3, real(cw1), 'blue'); title("CW1 @1.5Gsps");
...
cw0 = transpose(cw0);
cw1 = transpose(cw1);
[mod_output_1p5, dummy] = vha_combine(band0_output_iq_from_duc, band1_output_iq_from_duc, cw0, cw1, config);
mod_output_1p5 = mod_output_1p5 * scale_factor;    % scale up the final output
n2 = 1:1:length(mod_output_1p5);
subplot(4, 2, 7); grid on; hold on;
stem(n2, real(mod_output_1p5), 'red'); title ("Output - 1.5Gsps Modulation");
stem(n2, imag(mod_output_1p5), 'blue');
...
subplot(6, 2, 1); grid on; hold on;
stem(n1, real(input_iq), 'green'); title("Input IQ samples @1.5G");
...
input_1p5 = strcat(input_real, input_imag);
fileId = fopen("input_samples/demodulation/dual_band_demodulation_1.txt", "w");
for i = 1:length(input_1p5)
...
% Fclk = 1.5GHz, Fs = 1.5Gsps
fprintf("fout1 = %d KHz, Fs = %d KHz, Fclk = %d KHz\n", fout1, Fs, Fclk);
cw0 = nco_pp(fout1, Fs, Fclk, length(input_iq), 0);
subplot(6, 2, 3); grid on; hold on;
n2 = 1:1:length(cw0);
stem(n2, real(cw0), 'red'); title("CW0 @1.5Gsps");
stem(n2, imag(cw0), 'blue');
fprintf("fout2 = %d KHz, Fs = %d KHz, Fclk = %d KHz\n", fout2, Fs, Fclk);
cw1 = nco_pp(fout2, Fs, Fclk, length(input_iq), 0);
subplot(6, 2, 4); grid on; hold on;
n2 = 1:1:length(cw1);
stem(n2, real(cw1), 'red'); title("CW1 @1.5Gsps");
...
config.vha_conj = [0 0];
cw0 = transpose(cw0);
demod_output_1p5_band0 = vha_split(input_iq_for_model, cw0, config);
n3 = 1:1:length(demod_output_1p5_band0);
subplot(6, 2, 5); grid on; hold on;
stem(n3, real(demod_output_1p5_band0), 'red'); title("Band0 Output - 1.5Gsps Demodulation");
stem(n3, imag(demod_output_1p5_band0), 'blue');

% Band1 demodulation
cw1 = transpose(cw1);
demod_output_1p5_band1 = vha_split(input_iq_for_model, cw1, config);
n3 = 1:1:length(demod_output_1p5_band1);
subplot(6, 2, 6); grid on; hold on;
stem(n3, real(demod_output_1p5_band1), 'red'); title("Band1 Output - 1.5Gsps Demodulation");
stem(n3, imag(demod_output_1p5_band1), 'blue');
...
ddc_output_p187_band0 = vha_decimate(demod_output_1p5_band0, config);
ddc_output_p187_band1 = vha_decimate(demod_output_1p5_band1, config);
...
fileId = fopen("output_samples/demodulation/1p5/band0_dual_band_demodulation_p187.txt", "w");
...
fileId = fopen("output_samples/demodulation/1p5/band1_dual_band_demodulation_p187.txt", "w");
...
config.fine_delay = 0;
config.coarse_delay = 0;
ddc_output_p375_band0 = vha_decimate(demod_output_1p5_band0, config);
ddc_output_p375_band1 = vha_decimate(demod_output_1p5_band1, config);
...
fileId = fopen("output_samples/demodulation/1p5/band0_dual_band_demodulation_p375.txt", "w");
...
fileId = fopen("output_samples/demodulation/1p5/band1_dual_band_demodulation_p375.txt", "w");
...
config.fine_delay = 0;
config.coarse_delay = 0;
ddc_output_p75_band0 = vha_decimate(demod_output_1p5_band0, config);
ddc_output_p75_band1 = vha_decimate(demod_output_1p5_band1, config);
...
fileId = fopen("output_samples/demodulation/1p5/band0_dual_band_demodulation_p75.txt", "w");
...
fileId = fopen("output_samples/demodulation/1p5/band1_dual_band_demodulation_p75.txt", "w");
