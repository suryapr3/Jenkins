 ...
 *********************************************************************************************/
/*
 * @file    single_band_downconversion.c
 ...
 *             band UL path.
 *          2. DDC filters are enabled as per the input and output rates passed down.
    ...
    const char* argv[])
{
    ...
    printf("\n");
    printf("Test: Single band downconversion\n");
    printf("%s\n", g_compile_time);
    printf("BUILD_DIR: %s\n\n", BUILD_DIR);
    ...
    printf("HOST: Waiting for the Rx VEX to receive and compare %d vectors\n",
           (g_max_blocks * g_vectors_per_block[ul_clk][out_rate]));
    cnt = 0;
    while(1)
    {
       ...
       if(host_vec_processed >= (g_max_blocks * g_vectors_per_block[ul_clk][out_rate]))
       {
          break;
       }
       cnt++;
    ...
    } else
    {
        ...
        printf("DEBUG INFO: Test failed for the input combination TXVEX=%d, RXVEX=%d, VHA=%d, ANT=%d, INRATE=%d, OUTRATE=%d, MODE=%d, "
               "ULCLK=%d\n\n", tx_vex, rx_vex, vha_num, ant_num, in_rate, out_rate, mode, ul_clk);
    }
    ...
    num_samples =(g_max_blocks * g_vectors_per_block[ul_clk][out_rate]) * NUM_OF_ELEMENTS_PER_VEC;
