...
be configured to work ONLY as a digital down-converter by bypassing the DEMODULATOR module. The test will cover
validation of the entire filter chain in all possible combinations (4 cascaded decimation filters).


How to run the test ?
	From the FRIO bash terminal
	python single_band_downconversion.py <tx_vex> <rx_vex> <vha_num> <port_num> <in_rate> <out_rate> <DDC mode> <ul_clk>
