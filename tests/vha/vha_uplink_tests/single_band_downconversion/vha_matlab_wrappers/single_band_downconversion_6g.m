...
stem(n1, input_iq); title("Input IQ samples @6G");

% write this into a file which can be sourced while running on FRIO
fileId = fopen("input_samples/downconversion/single_band_downconversion_6.txt", "w");
fprintf(fileId, "%08x\n", input_iq);
fclose(fileId);
...
output_iq_from_model_6 = vha_decimate(input_iq_for_model, config);
...
stem(n2, output_iq_6);  title ("Output - No filters enabled");

% write this output to another file
fileId = fopen("output_samples/downconversion/6g/single_band_downconversion_6.txt", "w");
fprintf(fileId, "%08x\n", output_iq_6);
fclose(fileId);
...
fileId = fopen("output_samples/downconversion/6g/single_band_downconversion_3.txt", "w");
fprintf(fileId, "%08x\n", output_iq_3);
fclose(fileId);
...
fileId = fopen("output_samples/downconversion/6g/single_band_downconversion_1p5.txt", "w");
fprintf(fileId, "%08x\n", output_iq_1p5);
fclose(fileId);
...
stem(n2, output_iq_p75);title ("Output - W1, W2, W3 enabled");
fileId = fopen("output_samples/downconversion/6g/single_band_downconversion_p75.txt", "w");
fprintf(fileId, "%08x\n", output_iq_p75);
fclose(fileId);
...
stem(n2, output_iq_p375);title ("Output - W1, W2, W3, W4 enabled");
fileId = fopen("output_samples/downconversion/6g/single_band_downconversion_p375.txt", "w");
for i = 1 : length(output_iq_p375)
    output_iq_p375_hex = dec2hex(output_iq_p375(i), 4);
