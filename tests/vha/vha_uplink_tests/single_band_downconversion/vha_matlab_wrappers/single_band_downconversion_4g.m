...
stem(n1, input_iq); title("Input IQ samples @4G");

% write this into a file which can be sourced while running on FRIO
fileId = fopen("input_samples/downconversion/single_band_downconversion_4.txt", "w");
fprintf(fileId, "%08x\n", input_iq);
fclose(fileId);
...
output_iq_from_model_4 = vha_decimate(input_iq_for_model, config);
...
stem(n2, output_iq_4);  title ("Output - No filters enabled");

% write this output to another file
fileId = fopen("output_samples/downconversion/4g/single_band_downconversion_4.txt", "w");
fprintf(fileId, "%08x\n", output_iq_4);
fclose(fileId);
...
stem(n2, output_iq_2); title ("Output - W1 enabled");
fileId = fopen("output_samples/downconversion/4g/single_band_downconversion_2.txt", "w");
fprintf(fileId, "%08x\n", output_iq_2);
fclose(fileId);
...
fileId = fopen("output_samples/downconversion/4g/single_band_downconversion_1.txt", "w");
fprintf(fileId, "%08x\n", output_iq_1);
fclose(fileId);
...
stem(n2, output_iq_p5);title ("Output - W1, W2, W3 enabled");
fileId = fopen("output_samples/downconversion/4g/single_band_downconversion_p5.txt", "w");
fprintf(fileId, "%08x\n", output_iq_p5);
fclose(fileId);
...
stem(n2, output_iq_p25);title ("Output - W1, W2, W3, W4 enabled");
fileId = fopen("output_samples/downconversion/4g/single_band_downconversion_p25.txt", "w");
for i = 1 : length(output_iq_p25)
    output_iq_p25_hex = dec2hex(output_iq_p25(i), 4);
