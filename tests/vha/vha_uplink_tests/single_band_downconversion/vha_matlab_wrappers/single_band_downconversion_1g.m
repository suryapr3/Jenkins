...
stem(n1, input_iq); title("Input IQ samples @1G");

% write this into a file which can be sourced while running on FRIO
fileId = fopen("input_samples/downconversion/single_band_downconversion_1.txt", "w");
fprintf(fileId, "%08x\n", input_iq);
fclose(fileId);
...
stem(n2, output_iq_1);  title ("Output - No filters enabled");

% write this output to another file
fileId = fopen("output_samples/downconversion/1g/single_band_downconversion_1.txt", "w");
fprintf(fileId, "%08x\n", output_iq_1);
fclose(fileId);
...
fileId = fopen("output_samples/downconversion/1g/single_band_downconversion_p5.txt", "w");
fprintf(fileId, "%08x\n", output_iq_p5);
fclose(fileId);
...
stem(n2, output_iq_p25);title ("Output - W1, W2 enabled");
fileId = fopen("output_samples/downconversion/1g/single_band_downconversion_p25.txt", "w");
fprintf(fileId, "%08x\n", output_iq_p25);
fclose(fileId);
...
stem(n2, output_iq_p125);title ("Output - W1, W2, W3 enabled");
fileId = fopen("output_samples/downconversion/1g/single_band_downconversion_p125.txt", "w");
fprintf(fileId, "%08x\n", output_iq_p125);
fclose(fileId);
