...
% single band demodulation testing at 1.5Gsps
clc;
clear;
close all;
...
cw0 = transpose(cw0);
[mod_output_1p5, dummy] = vha_combine(output_iq_from_duc, 0, cw0, 0, config);
scale_factor = 2^15;
...
fileId = fopen("input_samples/demodulation/single_band_demodulation_1p5.txt", "w");
...
config.vha_conj = [0 0];
cw2 = transpose(cw2);
demod_output_1p5 = vha_split(input_iq_for_model, cw2, config);
n3 = 1:1:length(demod_output_1p5);
subplot(4, 3, 8); grid on; hold on;
stem(n3, real(demod_output_1p5), 'red'); title("Output - 1.5Gsps Demodulation");
stem(n3, imag(demod_output_1p5), 'blue');
...
ddc_output_p187 = vha_decimate(demod_output_1p5, config);
...
fileId = fopen("output_samples/demodulation/1p5/single_band_demodulation_p187.txt", "w");
...
config.fine_delay = 0;
config.coarse_delay = 0;
ddc_output_p375 = vha_decimate(demod_output_1p5, config);
...
fileId = fopen("output_samples/demodulation/1p5/single_band_demodulation_p375.txt", "w");
...
config.fine_delay = 0;
config.coarse_delay = 0;
ddc_output_p75 = vha_decimate(demod_output_1p5, config);
...
fileId = fopen("output_samples/demodulation/1p5/single_band_demodulation_p75.txt", "w");
