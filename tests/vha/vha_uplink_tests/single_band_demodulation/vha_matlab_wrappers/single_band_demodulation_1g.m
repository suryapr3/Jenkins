...
% single band demodulation testing at 1Gsps
clc;
clear;
close all;

% First generate an upconverted modulated output (1Gsps), which can later be
...
stem(n1, real(input_iq), 'green'); title("Input IQ samples @1G");
...
fileId = fopen("input_samples/demodulation/single_band_demodulation_1.txt", "w");
...
% Fclk = 1GHz, Fs = 1Gsps, freq = 500MHz
Fclk = 983040;
Fs   = 983040;
freq = (Fs / 2);
fprintf("freq = %d KHz, Fs = %d KHz, Fclk = %d KHz\n", freq, Fs, Fclk);
...
config.vha_conj = [0 0];
cw2 = transpose(cw2);
demod_output_1g = vha_split(input_iq_for_model, cw2, config);
n3 = 1:1:length(demod_output_1g);
subplot(4, 3, 8); grid on; hold on;
stem(n3, real(demod_output_1g), 'red'); title("Output - 1gsps Demodulation");
stem(n3, imag(demod_output_1g), 'blue');
...
ddc_output_p125 = vha_decimate(demod_output_1g, config);
...
fileId = fopen("output_samples/demodulation/1g/single_band_demodulation_p125.txt", "w");
...
config.fine_delay = 0;
config.coarse_delay = 0;
ddc_output_p25 = vha_decimate(demod_output_1g, config);
...
fileId = fopen("output_samples/demodulation/1g/single_band_demodulation_p25.txt", "w");
...
config.fine_delay = 0;
config.coarse_delay = 0;
ddc_output_p5 = vha_decimate(demod_output_1g, config);
...
fileId = fopen("output_samples/demodulation/1g/single_band_demodulation_p5.txt", "w");
