...
% single band demodulation testing at 6Gsps
clc;
clear;
close all;

% First generate an upconverted modulated output (6gsps), which can later be
% given as the input to the uplink path
num_vectors = 6;                       % say p375
...
% generate cw0 for the modulation freq = (Fs / 2) Fs = 6Ghz, Fclk = 1Ghz
Fclk = 1474000;
Fs   = 1474000 * 4;
freq = (Fs / 2);
...
input_iq = mod_output_6g;
num_vectors = 96;                      % 6Gsps
num_samples_per_vector = 32;
num_samples = (num_vectors * num_samples_per_vector);

n1 = 1:1:num_samples;
subplot(4, 3, 2); grid on; hold on;
stem(n1, real(input_iq), 'green'); title("Input IQ samples @6g");
...
input_6g = strcat(input_real, input_imag);
fileId = fopen("input_samples/demodulation/single_band_demodulation_6.txt", "w");
for i = 1:length(input_6g)
...
config.vha_conj = [0 0];
cw2 = transpose(cw2);
demod_output_6g = vha_split(input_iq_for_model, cw2, config);
n3 = 1:1:length(demod_output_6g);
subplot(4, 3, 8); grid on; hold on;
stem(n3, real(demod_output_6g), 'red'); title("Output - 6gsps Demodulation");
stem(n3, imag(demod_output_6g), 'blue');
...
ddc_output_p375 = vha_decimate(demod_output_6g, config);
...
fileId = fopen("output_samples/demodulation/6g/single_band_demodulation_p375.txt", "w");
...
config.fine_delay = 0;
config.coarse_delay = 0;
ddc_output_p75 = vha_decimate(demod_output_6g, config);
...
fileId = fopen("output_samples/demodulation/6g/single_band_demodulation_p75.txt", "w");
...
config.fine_delay = 0;
config.coarse_delay = 0;
ddc_output_1p5 = vha_decimate(demod_output_6g, config);
...
title("Output - demodulated, down converted samples to 1.5-gsps");
...
fileId = fopen("output_samples/demodulation/6g/single_band_demodulation_1p5.txt", "w");
