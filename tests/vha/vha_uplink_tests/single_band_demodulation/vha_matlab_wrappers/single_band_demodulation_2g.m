...
% single band demodulation testing at 2gsps
clc;
clear;
close all;

% First generate an upconverted modulated output (2Gsps), which can later be
% given as the input to the uplink path
num_vectors = 2;                       % say p125
...
num_vectors = 32;                      % 2Gsps
num_samples_per_vector = 32;
num_samples = (num_vectors * num_samples_per_vector);

n1 = 1:1:num_samples;
subplot(4, 2, 1); grid on; hold on;
stem(n1, real(input_iq), 'green'); title("Input IQ samples @2g");
...
fileId = fopen("input_samples/demodulation/single_band_demodulation_2.txt", "w");
...
config.vha_conj = [0 0];
cw2 = transpose(cw2);
demod_output_2g = vha_split(input_iq_for_model, cw2, config);
n3 = 1:1:length(demod_output_2g);
subplot(4, 2, 5); grid on; hold on;
stem(n3, real(demod_output_2g), 'red'); title("Output - 2Gsps Demodulation");
stem(n3, imag(demod_output_2g), 'blue');
...
ddc_output_p125 = vha_decimate(demod_output_2g, config);

% now scale the samples up
scale_factor = 2^15;
ddc_output_p125 = (ddc_output_p125 * scale_factor);
n4 = 1:1:length(ddc_output_p125);
subplot(4, 2, 2); grid on; hold on;
title("Output - demodulated, down converted samples to p125-gsps");
stem(n4, real(ddc_output_p125), 'red');
stem(n4, imag(ddc_output_p125), 'blue');

% now write this to a file
ddc_output_p125_real = dec2hex(real(ddc_output_p125), 4);
ddc_output_p125_imag = dec2hex(imag(ddc_output_p125), 4);
output_p125 = strcat(ddc_output_p125_real, ddc_output_p125_imag);
fileId = fopen("output_samples/demodulation/2g/single_band_demodulation_p125.txt", "w");
for i = 1:length(output_p125)
    nbytes = fprintf(fileId, '%8s\n', output_p125(i,:));
end
fclose(fileId);
...
config.fine_delay = 0;
config.coarse_delay = 0;
ddc_output_p25 = vha_decimate(demod_output_2g, config);
...
fileId = fopen("output_samples/demodulation/2g/single_band_demodulation_p25.txt", "w");
...
config.fine_delay = 0;
config.coarse_delay = 0;
ddc_output_p5 = vha_decimate(demod_output_2g, config);
...
fileId = fopen("output_samples/demodulation/2g/single_band_demodulation_p5.txt", "w");
...
config.fine_delay = 0;
config.coarse_delay = 0;
ddc_output_1 = vha_decimate(demod_output_2g, config);
...
fileId = fopen("output_samples/demodulation/2g/single_band_demodulation_1.txt", "w");
