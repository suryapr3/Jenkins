...
IQ samples to produce a band0/band1 samples, which will then get down converted to the configured rate by the
digita down converter chain (ddc). The demodulator of the uplink path will also be enabled with the sine wave
generator needed for demodulating the band0 samples (SWG2). The cordics per SWG will be enabled depending upon
the rate selected. For example only 1 for 1Gsps, 2 for 2Gsps and so on.
The test will cover validation of all the input rates (1G/2G/4G) of demodulation.


How to run the test ?
	From the FRIO bash terminal
	python single_band_demodulation.py <tx_vex> <rx_vex> <vha_num> <port_num> <in_rate> <out_rate> <ddc mode> <ul_clk>
