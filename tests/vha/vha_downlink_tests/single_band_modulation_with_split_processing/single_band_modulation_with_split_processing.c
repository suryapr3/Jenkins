 ...
 *********************************************************************************************/
/*
 * @file    single_band_modulation_with_split_processing.c
 ...
 *          3. Modulator will be enabled for a single band.
 *          4. VHA processed samples will be streamed back to another two VEX cores enabling SPLI feature of the VHA.
...
const char* g_input_file[2][4] = {
    {
    "single_band_modulation_p125.txt",
    "single_band_modulation_p25.txt",
    "single_band_modulation_p5.txt",
    "single_band_modulation_1.txt"
    },
    {
    "single_band_modulation_p187.txt",
    "single_band_modulation_p375.txt",
    "single_band_modulation_p75.txt",
    "single_band_modulation_1p5.txt"
    }
   };
...
const char* g_output_ref_file[2][12] = {
    ...
    "p125/single_band_modulation_1.txt",                   // W1+W2+W3
    "p125/single_band_modulation_2.txt",                   // W1+W2+W3+W4
    NULL,

    // For the inputs at p25 Gsps
    "p25/single_band_modulation_1.txt",                    // W1+W2
    "p25/single_band_modulation_2.txt",                    // W1+W2+W3
    "p25/single_band_modulation_4.txt",                    // W1+W2+W3+W4

    // For the inputs at p5 Gsps
    "p5/single_band_modulation_1.txt",                     // W1 only
    "p5/single_band_modulation_2.txt",                     // W1+W2
    "p5/single_band_modulation_4.txt",                     // W1+W2+W3

    // For the inputs at 1 Gsps
    NULL,
    "1g/single_band_modulation_2.txt",                     // W1 only
    "1g/single_band_modulation_4.txt",                     // W1+W2
    },
    {
    // For the inputs at p187 Gsps
    "p187/single_band_modulation_1p5.txt",                 // W1+W2+W3
    "p187/single_band_modulation_3.txt",                   // W1+W2+W3+W4
    NULL,

    // For the inputs at p375 Gsps
    "p375/single_band_modulation_1p5.txt",                 // W1+W2
    "p375/single_band_modulation_3.txt",                   // W1+W2+W3
    "p375/single_band_modulation_6.txt",                   // W1+W2+W3+W4

    // For the inputs at p75 Gsps
    "p75/single_band_modulation_1p5.txt",                  // W1 only
    "p75/single_band_modulation_3.txt",                    // W1+W2
    "p75/single_band_modulation_6.txt",                    // W1+W2+W3

    // For the inputs at 1.5 Gsps
    NULL,
    "1p5/single_band_modulation_3.txt",                    // W1 only
    "1p5/single_band_modulation_6.txt",                    // W1+W2
    }
};
    ...
    const char* argv[])
{
    ...
    printf("\n");
    printf("Test: Single band modulation with split processing\n");
    printf("%s\n", g_compile_time);
    printf("BUILD_DIR: %s\n\n", BUILD_DIR);
    ...
    vex_rx_val_kargs[1] = g_vectors_per_block[dl_clk][out_rate] / 2;
    ...
    // We've a pointer to the input IQ samples, let's write that to the vector memory of the tx_vex
    ...
    // We've a pointer to the output IQ samples, let's write that to the vector memory of the rx_vex
    ...
    write_mem_18a((VEX_BASE(rx_vex1) + VEX_VMEM_OFFSET + HIVE_ADDR_vha2vex_p5_8iq_vbuffer),
                  ((g_vectors_per_block[dl_clk][out_rate] / 2) * NUM_OF_ELEMENTS_PER_VEC),
                  val_out[(g_vectors_per_block[dl_clk][out_rate] / 2)], "RX_VEX_VMEM");
    printf("HOST: Initialized pattern into the vector memory of vex core %d\n", rx_vex1);
    ...
    print_registers(VEX_BASE(tx_vex) + VEX_VMEM_OFFSET, (g_vectors_per_block[dl_clk][in_rate] * NUM_OF_ELEMENTS_PER_VEC),
                    "Tx VEX Input samples");
    print_registers(VEX_BASE(rx_vex0) + VEX_VMEM_OFFSET, (g_vectors_per_block[dl_clk][out_rate] / 2) * NUM_OF_ELEMENTS_PER_VEC,
                    "Rx VEX0 Output ref samples");
    print_registers(VEX_BASE(rx_vex1) + VEX_VMEM_OFFSET, (g_vectors_per_block[dl_clk][out_rate] / 2) * NUM_OF_ELEMENTS_PER_VEC,
                     "Rx VEX1 Output ref samples");
    ...
    while(1)
    {
       ...
       if(host_vec_processed0 >= (g_max_blocks * g_vectors_per_block[dl_clk][out_rate]) / 2)
       {
          break;
       }
       cnt++;
    ...
    while(1)
    {
       host_vec_processed1 = read_reg_18a((VEX_BASE(rx_vex1) + VEX_DMEM_OFFSET + HIVE_ADDR_vha2vex_p5_8iq_g_count1),
                                         "RX_VEX_HOST_VEC_PROCESSED");
       if(host_vec_processed1 >= (g_max_blocks * g_vectors_per_block[dl_clk][out_rate]) / 2)
       {
          break;
       }
       cnt++;
    ...
    num_samples =(g_max_blocks * g_vectors_per_block[dl_clk][out_rate] / 2) * NUM_OF_ELEMENTS_PER_VEC;
    ...
    uint32_t dest_id1)
{
    ...
    ant_reg->VHA_ANT_DL_CFG.value = (1 << 14) | (out_rate << 8) | (2 << 6) | in_rate;
                                                           // bit14: dl_of_enable band0,
                                                           // bit[10:8]: out_rate0,
                                                           // bit[7:6]: mod_bypass,
                                                           // bit[2:0]: in_rate0
    ant_reg->VHA_ANT_CFG.value = 0x3;                      // bit[1:0]: burst_size = 4
    ds_burst_count = ((((g_vectors_per_block[dl_clk][out_rate] * 2/*flits*/) / 2/*dual split*/) /
                      4/*burst size*/) - 1);
    ant_reg->VHA_ANT_DL_OF_CFG[0].value = (1 << 16) | (ds_burst_count << 8) | ds_burst_count;
                                                           // enable split
    ...
    cm->H2B_MD_CTRL_TBL[0].value = g_md_control_tbl[dl_clk][out_rate] / 2;
    ...
    cm->H2B_BUF_SZ_HW_TBL[1].value = 1;                    // dest_buf_size = (1 + 1) x da_block_size
    cm->H2B_MD_ADDR_LKUP_TBL[1].value = STRM2CIO_2_VEX_DMEM_OFFSET;
                                                           // metadata if enabled will get written to address 0x00 data memory
    cm->H2B_TK_ADDR_LKUP_TBL[1].value = STRM2CIO_2_VEX_DMEM_OFFSET + HIVE_ADDR_vha2vex_p5_8iq_consumer_sm_gate;
                                                           // token will be written to the address 0x80 in data memory of VEX
    cm->H2B_ADDR_LKUP_TBL[1].value = (1 << 31) | HIVE_ADDR_vha2vex_p5_8iq_vbuffer_out;
                                                           // data will be written to the address 0x4000 in vector memory
    cm->H2B_MD_CTRL_TBL[1].value = g_md_control_tbl[dl_clk][out_rate] / 2;
                                                           // bit[9:0]: da_block_size
    cm->H2B_TK_ROUTE_LKUP_TBL[1].value = dest_id1;         // route the tokens to rx_vex1
    cm->H2B_MD_ROUTE_LKUP_TBL[1].value = dest_id1;         // route the metadata to rx_vex1
    cm->H2B_ROUTE_LKUP_TBL[1].value = dest_id1;            // route the data to rx_vex1
    ...
    if(rv = write_reg_18a(NSIP_HBI_H2B_ROUTE_LKUP_TBL0_ADR(cm_base_addr),
        ...
        return rv;
    if(rv = write_reg_18a(NSIP_HBI_H2B_BUF_SZ_HW_TBL1_ADR(cm_base_addr),
                          cm->H2B_BUF_SZ_HW_TBL[1].value,
                          NSIP_HBI_H2B_BUF_SZ_HW_TBL_WR_MASK,
                          "NSIP_HBI_H2B_BUF_SZ_HW_TBL1"))
        return rv;
    if(rv = write_reg_18a(NSIP_HBI_H2B_TK_ADDR_LKUP_TBL1_ADR(cm_base_addr),
                          cm->H2B_TK_ADDR_LKUP_TBL[1].value,
                          NSIP_HBI_H2B_TK_ADDR_LKUP_TBL_WR_MASK,
                          "NSIP_HBI_H2B_TK_ADDR_LKUP_TBL1"))
        return rv;
    if(rv = write_reg_18a(NSIP_HBI_H2B_MD_ADDR_LKUP_TBL1_ADR(cm_base_addr),
                          cm->H2B_MD_ADDR_LKUP_TBL[1].value,
                          NSIP_HBI_H2B_MD_ADDR_LKUP_TBL_WR_MASK,
                          "NSIP_HBI_H2B_MD_ADDR_LKUP_TBL1"))
        return rv;
    if(rv = write_reg_18a(NSIP_HBI_H2B_ADDR_LKUP_TBL1_ADR(cm_base_addr),
                          cm->H2B_ADDR_LKUP_TBL[1].value,
                          NSIP_HBI_H2B_ADDR_LKUP_TBL_WR_MASK,
                          "NSIP_HBI_H2B_ADDR_LKUP_TBL1"))
        return rv;
    if(rv = write_reg_18a(NSIP_HBI_H2B_MD_CTRL_TBL1_ADR(cm_base_addr),
                          cm->H2B_MD_CTRL_TBL[1].value,
                          NSIP_HBI_H2B_MD_CTRL_TBL_WR_MASK,
                          "NSIP_HBI_H2B_MD_CTRL_TBL1"))
        return rv;
    if(rv = write_reg_18a(NSIP_HBI_H2B_TK_ROUTE_LKUP_TBL1_ADR(cm_base_addr),
                          cm->H2B_TK_ROUTE_LKUP_TBL[1].value,
                          NSIP_HBI_H2B_TK_ROUTE_LKUP_TBL_WR_MASK,
                          "NSIP_HBI_H2B_TK_ROUTE_LKUP_TBL1"))
        return rv;
    if(rv = write_reg_18a(NSIP_HBI_H2B_MD_ROUTE_LKUP_TBL1_ADR(cm_base_addr),
                          cm->H2B_MD_ROUTE_LKUP_TBL[1].value,
                          NSIP_HBI_H2B_MD_ROUTE_LKUP_TBL_WR_MASK,
                          "NSIP_HBI_H2B_MD_ROUTE_LKUP_TBL1"))
        return rv;
    if(rv = write_reg_18a(NSIP_HBI_H2B_ROUTE_LKUP_TBL1_ADR(cm_base_addr),
                          cm->H2B_ROUTE_LKUP_TBL[1].value,
                          NSIP_HBI_H2B_ROUTE_LKUP_TBL_WR_MASK,
                          "NSIP_HBI_H2B_ROUTE_LKUP_TBL1"))
        return rv;
