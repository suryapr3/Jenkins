...
%               dual band modulation testing at p187 Gsps
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
stem(n1, band1_input_iq, 'green'); title("Band1 Input IQ samples @p187");

% write this into a file which can be sourced while running on FRIO
fileId = fopen("input_samples/modulation/band0_dual_band_modulation_p187.txt", "w");
fprintf(fileId, "%08x\n", bitshift(band0_input_iq, 16));
fclose(fileId);
fileId = fopen("input_samples/modulation/band1_dual_band_modulation_p187.txt", "w");
...
stem(n2, (real(band1_output_iq_from_duc * scale_factor)), 'red'); title("DUC band1_ Output 1.5Gsps");
...
fileId = fopen("output_samples/modulation/p187/dual_band_modulation_1p5.txt", "w");
...
n3 = 1:1:(length(cw0));
stem(n3, real(cw0), 'red'); title("CW0 + 1 @3Gsps");
stem(n3, imag(cw0), 'blue');
stem(n3, real(cw1), 'black');
stem(n3, imag(cw1), 'green');
...
fileId = fopen("output_samples/modulation/p187/dual_band_modulation_3.txt", "w");
