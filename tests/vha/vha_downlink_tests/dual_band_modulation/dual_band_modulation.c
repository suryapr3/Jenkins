 ...
 *          3. VHA processed samples will be streamed back to a VEX core for data comparison.
    ...
    const char* argv[])
{
    ...
    int tx_in_list, rx_in_list, vex_combo_good, same_cores;
    ...
    printf("\n");
    printf("Test: Dual band modulation\n");
    printf("%s\n", g_compile_time);
    printf("BUILD_DIR: %s\n\n", BUILD_DIR);
    ...
    for(i = 0; i < NUM_VE_PER_VHA; i++)
    {
        ...
        if((rx_vex == *((*ve_list) + i)) && (!rx_in_list))
        {
            rx_in_list = 1;
        }
    ...
    if((tx_in_list && rx_in_list) && (!same_cores))
    {
        vex_combo_good = 1;
    }
    if(!vex_combo_good)
    ...
    print_registers(VEX_BASE(rx_vex) + VEX_VMEM_OFFSET,  16, "Rx VEX Output ref samples");    // Band combined IQ samples
    ...
    printf("HOST: Comparison result (VEX%d) : 0x%x --> %s\n      Processed vectors : %d\n", rx_vex, host_result, test_status, host_vec_processed);
    ...
    print_registers(VEX_BASE(tx_vex) + VEX_VMEM_OFFSET + HIVE_ADDR_vex2vha_vbuffer0, num_samples, "Tx VEX (Band0) input samples");
    print_registers(VEX_BASE(tx_vex) + VEX_VMEM_OFFSET + HIVE_ADDR_vex2vha_vbuffer1, num_samples, "Tx VEX (Band1) input samples");
    ...
    uint32_t dl_clk)
{
    ...
    dest_id = rx_vex;                                      // VHA will stream to rx_vex, the band combined samples
