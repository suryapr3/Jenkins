...
(4 cascaded interpolation filters). The VHA processed samples will be streamed to a VEX core where they will get
compared with a known reference pattern.

How to run the test ?
	From the FRIO bash terminal
	python dual_band_modulation.py <tx_vex> <rx_vex> <vha_num> <port_num> <in_rate> <out_rate> <duc mode> <dl_clk>

	@input arguments
		tx_vex   : This is the VEX core who'll stream band0 and band1 IQ samples to the VHA instance.
		rx_vex   : This is the VEX core who'll recieve VHA processed band combined samples.
