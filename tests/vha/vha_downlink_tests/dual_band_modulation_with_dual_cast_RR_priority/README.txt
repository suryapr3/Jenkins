/*************************************************
*                                                *
*     Dual band modulation with DUAL CAST, RR    *
*                                                *
* Author : Haridas P                             *
* Date   : 15 Aug 2024                           *
*                                                *
...
This test case aims to validate the data path of the VHA involving the MODULATOR in the downlink direction.
The IQ samples for two different bands will be fed to the VHA from a single VEX core as two different streams.
The VHA will be configured to work as a digital up-converter and the MODULATOR module will be enabled for both
the bands. The test will cover validation of the entire filter chain in all possible combinations (4 cascaded
interpolation filters). The same VHA processed samples will be streamed to two different VEX cores by enabling
both of the output FIFOs. By default the antenna priority scheme is STRICT priority where in the order of the
priority is UL, DL0, DL1. In this test, the priority will be made to Round Robin so that the highest priority
rotates.

How to run the test ?
	From the FRIO bash terminal
	python dual_band_modulation_with_dual_casting.py <tx_vex> <rx_vex0> <rx_vex1> <vha_num> <port_num> <in_rate> <out_rate> <duc mode> <dl_clk>

	@input arguments
		tx_vex   : This is the VEX core who'll stream band0 and band1 IQ samples to the VHA instance.
		rx_vex0  : This is the VEX core who'll recieve first half of the VHA processed band combined samples.
		rx_vex1  : This is the VEX core who'll recieve second half of the VHA processed band combined samples.
...
Notes:-
	...
	3. Only VHA0-3 are available on FRIO build.
