 ...
 *********************************************************************************************/
/*
 * @file    dual_band_modulation_with_split_processing.c
 ...
 *          3. VHA processed samples will be streamed back to two different VEXes by enabling SPLIT.
    ...
    const char* argv[])
{
    ...
    printf("\n");
    printf("Test: Dual band modulation with split processing\n");
    printf("%s\n", g_compile_time);
    printf("BUILD_DIR: %s\n\n", BUILD_DIR);
    ...
    print_registers(VEX_BASE(tx_vex) + VEX_VMEM_OFFSET + HIVE_ADDR_vex2vha_vbuffer1, 16, "Tx VEX Input samples"); // Band1 IQ samples
    ...
    num_samples =(g_max_blocks * g_vectors_per_block[dl_clk][out_rate]) * NUM_OF_ELEMENTS_PER_VEC / 2;
    ...
    uint32_t dest_id1)
{
    ...
    ant_reg->VHA_ANT_DL_CFG.value = (1 << 14) | (out_rate << 11) | (out_rate << 8) | (in_rate << 3) | in_rate;
