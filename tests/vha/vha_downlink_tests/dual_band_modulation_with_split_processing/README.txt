/*************************************************
*                                                *
*           Dual band modulation with SPLIT      *
*                                                *
* Author : Haridas P                             *
* Date   : 20 Mar 2024                           *
*                                                *
...
The IQ samples for two different bands will be fed to the VHA from a single VEX core as two different
streams. The VHA will be configured to work as a digital up-converter and the MODULATOR module will be enabled
for both the bands. The test will cover validation of the entire filter chain in all possible combinations
(4 cascaded interpolation filters). The VHA processed samples will be streamed to two different VEX cores by
enabling SPLIT feature of the output FIFOs.

How to run the test ?
	From the FRIO bash terminal
	python dual_band_modulation_with_split_processing.py <tx_vex> <rx_vex0> <rx_vex1> <vha_num> <port_num> <in_rate> <out_rate> <duc mode> <dl_clk>
