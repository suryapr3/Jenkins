...
%               dual band modulation testing at p375 Gsps
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
stem(n1, band0_input_iq, 'green'); title("Band0 Input IQ samples @p375");
subplot(7, 2, 2); grid on; hold on;
stem(n1, band1_input_iq, 'green'); title("Band1 Input IQ samples @p375");

% write this into a file which can be sourced while running on FRIO
fileId = fopen("input_samples/modulation/band0_dual_band_modulation_p375.txt", "w");
fprintf(fileId, "%08x\n", bitshift(band0_input_iq, 16));
fclose(fileId);
fileId = fopen("input_samples/modulation/band1_dual_band_modulation_p375.txt", "w");
...
fileId = fopen("output_samples/modulation/p375/dual_band_modulation_1p5.txt", "w");
...
% generate cw0 for the modulation freq = 500Mhz, Fs = 3Ghz, Fclk = 1.5Ghz
Fs   = 1474000 * 2;
fprintf("3GSPS Modulation\n");
fprintf("fout0 = %d KHz, Fs = %d KHz, Fclk = %d KHz\n", fout0, Fs, Fclk);
cw0 = nco_pp(fout0, Fs, Fclk, length(band0_output_iq_from_duc), 0);   % 0 clk cycle delay
fprintf("fout1 = %d KHz, Fs = %d KHz, Fclk = %d KHz\n", fout1, Fs, Fclk);
cw1 = nco_pp(fout1, Fs, Fclk, length(band1_output_iq_from_duc), 2);   % 2 clk cycles delay
...
fileId = fopen("output_samples/modulation/p375/dual_band_modulation_3.txt", "w");
...
% generate cw0 for the modulation freq = 500Mhz, Fs = 4Ghz, Fclk = 1.5Ghz
Fs   = 1474000 * 4;
...
cw0 = transpose(cw0);
cw1 = transpose(cw1);
[mod_output_6g, dummy] = vha_combine(band0_output_iq_from_duc, band1_output_iq_from_duc, cw0, cw0, config);
...
stem(n2, real(mod_output_6g), 'red'); title ("Output - 4Gsps Modulation");
...
fileId = fopen("output_samples/modulation/p375/dual_band_modulation_6.txt", "w");
