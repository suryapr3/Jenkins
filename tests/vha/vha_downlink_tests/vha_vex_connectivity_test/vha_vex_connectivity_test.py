...
# =============================================================================
# test description : this test checks the RCS register access
# =============================================================================
# Import statements
# =============================================================================
import sys
...
if dut_preconfig == 1:
    ...
    vha_0_valid_vex_list = [0, 1, 8, 9, 32]
    vha_1_valid_vex_list = [2, 3, 10, 11, 34]
    vha_2_valid_vex_list = [4, 5, 12, 13, 36]
    vha_3_valid_vex_list = [6, 7, 14, 15, 38]

    # executing test specific stimulus

    for tx_id in vha_0_valid_vex_list:
        for rx_id in vha_0_valid_vex_list:
            for ant_num in range(2):
                if rx_id != tx_id:
                    logger.info(my_functions.main(tx_id, rx_id, 0, ant_num))

    for tx_id in vha_1_valid_vex_list:
        for rx_id in vha_1_valid_vex_list:
            for ant_num in range(2):
                if rx_id != tx_id:
                    logger.info(my_functions.main(tx_id, rx_id, 1, ant_num))

    for tx_id in vha_2_valid_vex_list:
        for rx_id in vha_2_valid_vex_list:
            for ant_num in range(2):
                if rx_id != tx_id:
                    logger.info(my_functions.main(tx_id, rx_id, 2, ant_num))

    for tx_id in vha_3_valid_vex_list:
        for rx_id in vha_3_valid_vex_list:
            for ant_num in range(2):
                if rx_id != tx_id:
                    logger.info(my_functions.main(tx_id, rx_id, 3, ant_num))
