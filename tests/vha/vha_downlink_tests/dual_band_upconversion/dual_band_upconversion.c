 ...
 * @TC Description:
 *          1. A certain number of IQ samples will be streamed from two VEX cores to the VHA dual
 *             band DL path. One VEX core for each band.
 *          2. DUC filters are enabled as per the input and output rates passed down.
 *          3. VHA processed samples will be streamed back to another two VEX cores for data comparison.
    ...
    const char* argv[])
{
    ...
    int tx0_in_list, rx0_in_list, tx1_in_list, rx1_in_list, vex_combo_good, same_cores;
    int (*ve_list)[NUM_VE_PER_VHA];
    uint64_t base_addr;
    uint32_t tx_vex0 = 0, rx_vex0 = 0, tx_vex1 = 0, rx_vex1 = 0,  vha_num, ant_num;
    ...
    printf("\n");
    printf("Test: Dual band upconversion\n");
    printf("%s\n", g_compile_time);
    printf("BUILD_DIR: %s\n\n", BUILD_DIR);
    ...
    printf("INFO: File    : %s\n", __FILE__);
    printf("INFO: argc    : %d\n", argc);
    printf("INFO: argv[0] : %d = tx_vex0  (VEX%d)\n", tx_vex0, tx_vex0);
    printf("INFO: argv[1] : %d = rx_vex0  (VEX%d)\n", rx_vex0, rx_vex0);
    printf("INFO: argv[2] : %d = tx_vex1  (VEX%d)\n", tx_vex1, tx_vex1);
    printf("INFO: argv[3] : %d = rx_vex1  (VEX%d)\n", rx_vex1, rx_vex1);
    printf("INFO: argv[4] : %d = vha_num  (VHA%d)\n", vha_num, vha_num);
    printf("INFO: argv[5] : %d = ant_num  (ANTENNA%d)\n", ant_num, ant_num);
    printf("INFO: argv[6] : %d = in_rate  (%s)\n", in_rate, sys_rate_str[dl_clk][in_rate]);
    printf("INFO: argv[7] : %d = out_rate (%s)\n", out_rate, sys_rate_str[dl_clk][out_rate]);
    printf("INFO: argv[8] : %d = mode     (%s)\n", mode, duc_mode_str[mode]);
    printf("INFO: argv[9] : %d = dl_clk   (%s)\n", dl_clk, vha_dl_clk[dl_clk]);
    ...
    if((tx_vex0 > (NUM_VE_CORES - 1)) || (rx_vex0 > (NUM_VE_CORES - 1)) ||
       (tx_vex1 > (NUM_VE_CORES - 1)) || (rx_vex1 > (NUM_VE_CORES - 1)))
    {
    ...
    for(i = 0; i < 4; i++)
    {
        ...
        if(2 == i) vex_num = tx_vex1;
        if(3 == i) vex_num = rx_vex1;
    ...
    for(i = 0; i < NUM_VE_PER_VHA; i++)
    {
        ...
        if((tx_vex1 == *((*ve_list) + i)) && (!tx1_in_list))
        {
            tx1_in_list = 1;
        }
        ...
        if(tx0_in_list && rx0_in_list && tx1_in_list && rx1_in_list)
        {
            break;
        }
    }
    ...
    same_cores = 0;
    if((tx_vex0 == rx_vex0) || (tx_vex0 == tx_vex1) || (tx_vex0 == rx_vex1) || (rx_vex0 == tx_vex1) ||
              (rx_vex0 == rx_vex1) || (tx_vex1 == rx_vex1))
        same_cores = 1;
    if(same_cores)
        printf("ERROR: Cannot run this test with the same Tx and Rx cores\n");
    if(!tx0_in_list)
        printf("ERROR: Tx VEX (core %d) is NOT connected to VHA%d\n", tx_vex0, vha_num);
    if(!rx0_in_list)
        printf("ERROR: Rx VEX (core %d) is NOT connected to VHA%d\n", rx_vex0, vha_num);
    if(!tx1_in_list)
        printf("ERROR: Tx VEX (core %d) is NOT connected to VHA%d\n", tx_vex1, vha_num);
    if(!rx1_in_list)
        printf("ERROR: Rx VEX (core %d) is NOT connected to VHA%d\n", rx_vex1, vha_num);
    ...
    if((tx0_in_list && rx0_in_list && tx1_in_list && rx1_in_list) && (!same_cores))
    {
        vex_combo_good = 1;
    }
    if(!vex_combo_good)
    {
        ...
        printf("Please make sure,\n"
               "   i. Tx VEX's and Rx VEX's are connected to VHA%d\n"
    ...
    if(rv = test_check_status(tx_vex0, rx_vex0, tx_vex1, rx_vex1, vha_num, ant_num))
    {
        printf("ERROR: One or many register statuses are NOT clean. (rv = %d) Exiting test..!\n", rv);
        return FAILURE;
    }

    if(rv = test_configure(tx_vex0, rx_vex0, tx_vex1, rx_vex1, vha_num, ant_num, in_rate, out_rate, mode, dl_clk))
    {
    ...
    {
        if(1 == i) vex_num = rx_vex0;
        else if(2 == i) vex_num = tx_vex1;
        else if(3 == i) vex_num = rx_vex1;
    ...
    if((rv = vex_loader(tx_vex0, "vex_fw/sim_m64/vex2vha.fw")))
    {
        LOG_ERROR("Failed to download progarm: %s to VEX%d\n", "vex_fw/sim_m64/vex2vha.fw", tx_vex0);
        return rv;
    }
    printf("HOST: Loaded %s to vex core %d\n", "vex_fw/sim_m64/vex2vha.fw", tx_vex0);

    if((rv = vex_loader(tx_vex1, "vex_fw/sim_m64/vex2vha.fw")))
    {
        LOG_ERROR("Failed to download progarm: %s to VEX%d\n", "vex_fw/sim_m64/vex2vha.fw", tx_vex1);
        return rv;
    }
    printf("HOST: Loaded %s to vex core %d\n", "vex_fw/sim_m64/vex2vha.fw", tx_vex1);
    ...
    {
        if(rv = write_reg_18a((VEX_BASE(tx_vex0) + VEX_DMEM_OFFSET + offset_val[lc]), val_kargs[lc],
                              0xffffffff, "TX_VEX_PARAMS"))
            return rv;
    }
    printf("HOST: Loaded host params to vex core %d\n", tx_vex0);
    val_kargs[2] = 0x10000;                                // tx_vex1 should stream with word address 0x10000
    for(lc = 0; lc < ARRAY_SIZE(offset_val); lc++)
    {
        if(rv = write_reg_18a((VEX_BASE(tx_vex1) + VEX_DMEM_OFFSET + offset_val[lc]), val_kargs[lc],
                              0xffffffff, "TX_VEX_PARAMS"))
            return rv;
    }
    printf("HOST: Loaded host params to vex core %d\n", tx_vex1);
    ...
    fclose(fptr);
    write_mem_18a((VEX_BASE(tx_vex0) + VEX_VMEM_OFFSET + HIVE_ADDR_vex2vha_vbuffer),
                  (g_vectors_per_block[dl_clk][in_rate] * NUM_OF_ELEMENTS_PER_VEC),
                  val_inp[0], "TX_VEX_VMEM");
    printf("HOST: Initialized pattern into the vector memory of vex core %d\n", tx_vex0);
    write_mem_18a((VEX_BASE(tx_vex1) + VEX_VMEM_OFFSET + HIVE_ADDR_vex2vha_vbuffer),
                  (g_vectors_per_block[dl_clk][in_rate] * NUM_OF_ELEMENTS_PER_VEC),
                  val_inp[0], "TX_VEX_VMEM");
    printf("HOST: Initialized pattern into the vector memory of vex core %d\n", tx_vex1);

    // We've a pointer to the output IQ samples, let's write that to the vector memory of the vex core
    ...
    print_registers(VEX_BASE(tx_vex0) + VEX_DMEM_OFFSET, 24, "Tx VEX0 DMEM");
    print_registers(VEX_BASE(tx_vex1) + VEX_DMEM_OFFSET, 24, "Tx VEX1 DMEM");
    print_registers(VEX_BASE(rx_vex0) + VEX_DMEM_OFFSET, 24, "Rx VEX0 DMEM");
    print_registers(VEX_BASE(rx_vex1) + VEX_DMEM_OFFSET, 24, "Rx VEX1 DMEM");

    // Try reading some input and output samples so programmed from the VEX vector memories
    print_registers(VEX_BASE(tx_vex0) + VEX_VMEM_OFFSET, 16, "Tx VEX0 Input samples");
    print_registers(VEX_BASE(rx_vex0) + VEX_VMEM_OFFSET, 16, "Rx VEX0 Output ref samples");
    print_registers(VEX_BASE(tx_vex1) + VEX_VMEM_OFFSET, 16, "Tx VEX1 Input samples");
    ...
    vex_start(rx_vex0, 0/*start_addr*/);
    printf("HOST: Started vex core %d (Rx VEX)\n", rx_vex0);
    vex_start(rx_vex1, 0/*start_addr*/);
    printf("HOST: Started vex core %d (Rx VEX)\n", rx_vex1);

    vex_start(tx_vex0, 0/*start_addr*/);
    printf("HOST: Started vex core %d (Tx VEX)\n", tx_vex0);
    vex_start(tx_vex1, 0/*start_addr*/);
    printf("HOST: Started vex core %d (Tx VEX)\n", tx_vex1);
    ...
    printf("HOST: Waiting for the VHA H2B flit count to be atleast %d\n",
           (2 * g_max_blocks * g_vectors_per_block[dl_clk][out_rate] * 2));
    cnt = 0;
    while(1)
    {
        h2b_flit_cnt += read_reg_18a(NSIP_HBI_H2B_FLIT_CNT_ADR(base_addr), "H2B_FLIT_COUNT");
        if(h2b_flit_cnt > (2 * g_max_blocks * g_vectors_per_block[dl_clk][out_rate] * 2))
        {
            break;
        }
        cnt++;
    ...
    printf("HOST: Waiting for the Rx VEXes to receive and compare %d vectors\n",
    ...
    {
        printf("ERROR: Output samples did not match with the Matlab reference output samples for BAND0\n");
        rv = FAILURE;
    }
    ...
    {
        printf("ERROR: Output samples did not match with the Matlab reference output samples for BAND1\n");
        rv = FAILURE;
    }
    ...
    if(test_check_status(tx_vex0, rx_vex0, tx_vex1, rx_vex1, vha_num, ant_num))
    {
        printf("ERROR: One or many register statuses are NOT clean. (rv = %d) Exiting test..!\n", rv);
        rv = FAILURE;
    }
    ...
    } else
    {
        ...
        printf("DEBUG INFO: Test failed for the input combination TXVEX0=%d, RXVEX0=%d, TXVEX1=%d, RXVEX1=%d, VHA=%d, ANT=%d, INRATE=%d,"
               "OUTRATE=%d, MODE=%d, DLCLK=%d\n\n", tx_vex0, rx_vex0, tx_vex1, rx_vex1, vha_num, ant_num, in_rate, out_rate, mode, dl_clk);
    }
    ...
    print_registers(VEX_BASE(tx_vex0) + VEX_VMEM_OFFSET + 0x0000, num_samples, "Tx VEX0 (Band0) input samples");
    print_registers(VEX_BASE(tx_vex1) + VEX_VMEM_OFFSET + 0x0000, num_samples, "Tx VEX1 (Band1) input samples");
    ...
    print_registers(VEX_BASE(rx_vex0) + VEX_VMEM_OFFSET + 0x0000, num_samples, "Rx VEX0 (Band0) input samples");
    print_registers(VEX_BASE(rx_vex0) + VEX_VMEM_OFFSET + 0xc000, num_samples, "Rx VEX0 (Band0) Output ref samples copy");
    print_registers(VEX_BASE(rx_vex0) + VEX_VMEM_OFFSET + 0xe000, num_samples, "Rx VEX0 (Band0) Output samples");
    print_registers(VEX_BASE(rx_vex1) + VEX_VMEM_OFFSET + 0x0000, num_samples, "Rx VEX1 (Band0) input samples");
    print_registers(VEX_BASE(rx_vex1) + VEX_VMEM_OFFSET + 0xc000, num_samples, "Rx VEX1 (Band0) Output ref samples copy");
    print_registers(VEX_BASE(rx_vex1) + VEX_VMEM_OFFSET + 0xe000, num_samples, "Rx VEX1 (Band0) Output samples");
#endif
    ...
    uint32_t dl_clk)
{
    ...
    dest_id0 = rx_vex0;                                    // VHA will stream to rx_vex0, the band0 samples
    dest_id1 = rx_vex1;                                    // VHA will stream to rx_vex1, the band1 samples
    if(rv = test_configure_vha(vha_num, ant_num, in_rate, out_rate, mode, dl_clk, dest_id0, dest_id1))
    {
        printf("ERROR: Failed to configure VHA%d (rv = %x) Exiting test..!\n", vha_num, rv);
        return rv;
    }

    // Configure the CIO2STRM registers of tx_vex0
    dest_id = vha_host_id[vha_num][ant_num];               // tx_vex will stream to VHA
    if(rv = vex_cio2strm_configure(tx_vex0, dest_id))
    {
        printf("ERROR: Failed to configure CIO2STRM of VEX%d (rv = %x) Exiting test..!\n", tx_vex0, rv);
        return rv;
    }
    // Configure the CIO2STRM registers of tx_vex1
    if(rv = vex_cio2strm_configure(tx_vex1, dest_id))
    {
        printf("ERROR: Failed to configure CIO2STRM of VEX%d (rv = %d) Exiting test..!\n", tx_vex1, rv);
        return rv;
    }

    // Configure STRM2CIO registers of rx_vex0
    if(rv = vex_strm2cio_configure(rx_vex0))
    {
        printf("ERROR: Failed to configure STRM2CIO of VEX%d (rv = %d) Exiting test..!\n", rx_vex0, rv);
        return rv;
    }
    // Configure STRM2CIO registers of rx_vex1
    ...
    uint32_t dest_id1)
{
    ...
    vha_t* vha_reg;
    nsip_hbi_t* cm;
    uint64_t vha_base_addr, vha_apb_base_addr, cm_base_addr, vha_reg_base_addr;
    ...
    ant_reg->VHA_ANT_DL_CFG.value = (3 << 14) | (out_rate << 11) | (out_rate << 8) | (3 << 6) | (in_rate << 3) | in_rate;
    ...
    ant_reg->VHA_ANT_DL_OF_CFG[1].value = 0;               // no split
    ...
    //    band0 processed data will exit with tbl[0] parameters
    ...
    cm->H2B_TK_ROUTE_LKUP_TBL[0].value = dest_id0;         // route the tokens to rx_vex
    cm->H2B_MD_ROUTE_LKUP_TBL[0].value = dest_id0;         // route the metadata to rx_vex
    cm->H2B_ROUTE_LKUP_TBL[0].value = dest_id0;            // route the data to rx_vex

    //    band1 processed data will exit with tbl[2] parameters
    ...
    cm->H2B_MD_CTRL_TBL[2].value = g_md_control_tbl[dl_clk][out_rate];
                                                           // bit[9:0]: da_block_size
    cm->H2B_TK_ROUTE_LKUP_TBL[2].value = dest_id1;         // route the tokens to rx_vex
    cm->H2B_MD_ROUTE_LKUP_TBL[2].value = dest_id1;         // route the metadata to rx_vex
    cm->H2B_ROUTE_LKUP_TBL[2].value = dest_id1;            // route the data to rx_vex
    ...
    vha_reg->VHA_DL_IF_CFG.value = ((1 << (ant_num * 10)) | (1 << ((ant_num * 10) + 5)));
                                                           // bit[11:8]: dl_ant1_band0_af_gate,
                                                           // bit[3:0]: dl_ant0_band0_af_gate
                                                           // bit[15:12]: dl_ant1_band1_af_gate,
                                                           // bit[7:4]: dl_ant0_band1_af_gate
                                                           // Independent triggers for band0 and band1
    vha_reg->VHA_DL_CTL.value = (3 << (2 * ant_num));      // dl_ififo_enable ant0/1_band0 & band1
    ...
    uint32_t ant_num)
{
    ...
    if(rv = vex_cio2strm_check_status(tx_vex0))
    ...
    if(rv = vex_cio2strm_check_status(tx_vex1))
        e_code |= 8;                                       // FAILURE code for CIO2STRM status wrong
    if(rv = vex_strm2cio_check_status(rx_vex1))
        e_code |= 16;                                      // FAILURE code for STRM2CIO status wrong

    test_show_regs(tx_vex0, rx_vex0, tx_vex1, rx_vex1, vha_num, ant_num);
    ...
    uint32_t ant_num)
{
    ...
    vex_cio2strm_show_regs(tx_vex0);                       // CIO2STRM registers of tx_vex0
    vex_strm2cio_show_regs(rx_vex0);                       // STRM2CIO registers of rx_vex0
    vex_cio2strm_show_regs(tx_vex1);                       // CIO2STRM registers of tx_vex1
    vex_strm2cio_show_regs(rx_vex1);                       // STRM2CIO registers of rx_vex1
}
