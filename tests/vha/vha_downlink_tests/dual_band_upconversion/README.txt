...
direction. The IQ samples for two different bands will be fed to the VHA from any of the two VEX cores.
The VHA will be configured to work ONLY as a digital up-converter by bypassing the MODULATOR module.
The test will cover validation of the entire filter chain in all possible combinations (4 cascaded
interpolation filters). The VHA processed (up-converted) samples will be streamed to two different VEX
cores where they will get compared with a known reference pattern.

How to run the test ?
	From the FRIO bash terminal
	python dual_band_upconversion.py <tx_vex0> <rx_vex0> <tx_vex1> <rx_vex1> <vha_num> <port_num> <in_rate> <out_rate> <duc mode> <dl_clk>

	@input arguments
		tx_vex0  : This is the VEX core who'll stream band0 samples to the VHA instance.
		rx_vex0  : This is the VEX core who'll recieve VHA processed band0 samples.
		tx_vex1  : This is the VEX core who'll stream band1 samples to the VHA instance.
