...
#define DCODE_MEM_COMPARE_SIZE  0x300
#define DCODE_MEM_COMPARE_SIZE_CAL 0x2000
#define DCODE_OFFSET (0x24030)
#define  DCODE_OFFSET_CAL (0x44030)
#define MEM_COMPARE_SIZE  8
#define REG_SIZE 4


int test_pattern[] ={0xA5A5A5A5,0xAAAAAAAA,0x55555555,0X5A5A5A5A};
...
int main(int argc, const char * argv[])
{
	...
	uint32_t dataCnt=0;
	uint32_t rcs_ctrl = 0; 	
	volatile uint32_t Val_out[MEM_COMPARE_SIZE] =  {0},buffer[DCODE_MEM_COMPARE_SIZE]={0};
	uint32_t data_tx=0;	
    ...
    rcs_params.rcs_cfg[SRP_RCS_RCP_CTRL_AX].img_file = "../../../src/rcs_fw/out/bin/dcode_core_wr_pcie_rd//ax_hw.elf"; // core0
	rcs_params.rcs_cfg[SRP_RCS_RCP_CTRL_RX].img_file = "../../../src/rcs_fw/out/bin/dcode_core_wr_pcie_rd//rx_hw.elf"; // core1
	rcs_params.rcs_cfg[SRP_RCS_RCP_CTRL_TX].img_file = "../../../src/rcs_fw/out/bin/dcode_core_wr_pcie_rd//tx_hw.elf";
    rcs_params.rcs_cfg[SRP_RCS_RCP_CALIB].img_file = "../../../src/rcs_fw/out/bin/dcode_core_wr_pcie_rd//cal_hw.elf";
    ...
    for (rcsCoreId = SRP_RCS_RCP_CTRL_AX; rcsCoreId <4; rcsCoreId++)
    {
		...
		read_mem_18a(dcode_offset[rcsCoreId],DCODE_MEM_COMPARE_SIZE,buffer,"DCODE_READ");
		for (dataCnt = 0; dataCnt < DCODE_MEM_COMPARE_SIZE/MEM_ACCESS_SIZE; dataCnt++)  
		{		
			//errCnt=read_mem_18a(dcode_offset[rcsCoreId]+ (dataCnt*4) ,MEM_ACCESS_SIZE/4,buffer,"DCODE_OFFESET");	
			if(buffer[dataCnt]!=test_pattern[rcsCoreId])
			{
				printf("ERROR:test_pattern  %x  , data %x address %x \n",test_pattern[rcsCoreId],buffer[dataCnt], dcode_offset[rcsCoreId] + (dataCnt*4) );
			    errCnt++;
			}
			
		}
		
		//errCnt=read_mem_expect_18a(dcode_offset[rcsCoreId],DCODE_MEM_COMPARE_SIZE,test_pattern[rcsCoreId],"DCODE_OFFESET");	
    ...
    if(errCnt==0)
    {
        print("Host:------ DCODE write from core Read from PCIe PASS ------\n");
    }
    else
    {
    	print("Host:------ DCODE write from core Read from PCIe FAILED!!! ------\n");
    }
