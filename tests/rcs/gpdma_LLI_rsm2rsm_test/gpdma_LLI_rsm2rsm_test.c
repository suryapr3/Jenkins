...
int rsm_offset[] = {0x12900,0x1a900,0x22900,0x2a900};
...
int main(int argc, const char * argv[])
{
    ...
    rcs_params.rcs_cfg[SRP_RCS_RCP_CTRL_AX].img_file = "../../../src/rcs_fw/out/bin/gpdma_LLI_rsm2rsm_test/ax_hw.elf"; // core0
    rcs_params.rcs_cfg[SRP_RCS_RCP_CTRL_RX].img_file = "../../../src/rcs_fw/out/bin/gpdma_LLI_rsm2rsm_test/rx_hw.elf"; // core1
    rcs_params.rcs_cfg[SRP_RCS_RCP_CTRL_TX].img_file = "../../../src/rcs_fw/out/bin/gpdma_LLI_rsm2rsm_test/tx_hw.elf";
    rcs_params.rcs_cfg[SRP_RCS_RCP_CALIB].img_file = "../../../src/rcs_fw/out/bin/gpdma_LLI_rsm2rsm_test/cal_hw.elf";
	...
	for ( i = 0; i < 1; i++)
	{
	    ...
	    //gpdma_test_rcs_to_vex(srp,rcsCoreId);
	    //gpdma_test_rsm_to_DIGRF_capture_mem(srp,rcsCoreId);
...
int gpdma_test_rsm_to_DIGRF_capture_mem(void *srp,uint8_t core_id)
{
	...
	for (dataCnt = 0; dataCnt < RSM_MEM_COMPARE_SIZE; dataCnt++)
	{
		...
		print("ERROR:reg_ctrl :%x ,test_pattern tx %x  , data %x , address %x \n",Val_out,test_pattern[core_id]|dataCnt, data_tx, 0x4000000 + (dataCnt*4) );
		errCnt++;
    ...
    if(errCnt==0)
    {
	print("Host:------ CORE %x: GPDMA TEST RSM TO DIGRF capture mem PASSED ,test_pattern: %x------\n",core_id,test_pattern[core_id]);
    }
    else
    {
	print("Host:------ CORE %x: GPDMA TEST RSM TO  DIGRF capture mem  FAILED ------\n",core_id);
    }
}
...
int gpdma_test_rcs_to_vex(void *srp,uint8_t core_id)
{
	...
	//print("count %d,reg :%x,%x ,block:%x , testpattern %x \n",i,Val_out ,Dal_out,Eal_out,test_pattern[core_id]);
	...
	for (destVexId = 0; destVexId< 16; destVexId++)
	{
		...
		if ((rv = srp_dev_block_read32(srp,
		    ...
		    return rv;
		print("vmem %x,dmem :%x,pmem:%x , testpattern %x vex_id%d \n",Val_out ,Dal_out,Eal_out,test_pattern[core_id],destVexId);

		if(((Val_out) != (test_pattern[core_id]|dataCnt))&&((Eal_out) != (test_pattern[core_id]|dataCnt))&&((Dal_out) != (test_pattern[core_id]|dataCnt)))
		    //if(((Val_out) != (test_pattern[core_id]|dataCnt)))
		{
		    ...
		    if ((rv = srp_dev_block_read32(srp,
				    ...
				    flags)))
			return rv;
		    //print("ERROR:reg_ctrl: %xtest_pattern[%d]: 0x%08x [0x%08x] vexid:%x address :%x \n",reg_val,dataCnt, Val_out,(test_pattern[core_id]|dataCnt) ,destVexId,0x30000  + (dataCnt*4));
		    errCnt++;
		}
