...
int main(int argc, const char* argv[])
{
    ...
    LOG_PRINT("Test: Negative test: Access check modifying the default addr mask register\n");
    LOG_PRINT("      %s\n", g_compile_time);
    ...
    LOG_PRINT("INFO: Waiting for the RCS to write to the RBM registers of M_RCS_1\n");
    if(poll_rsm_flag(srp, RSM_HANDSHAKE_ADDR, HANDSHAKE_MODIFIED_RBM_REG) < 0)
    {
        LOG_PRINT("Exiting test..!\n");
        return FAILURE;
    }
    LOG_PRINT("INFO: RCS core finished updating the RBM registers of M_RCS_1\n");
    write_reg_18a(RSM_HANDSHAKE_ADDR, HANDSHAKE_MOVEON, 0xffffffff, "RSM_HANDSHAKE_ADDR");
