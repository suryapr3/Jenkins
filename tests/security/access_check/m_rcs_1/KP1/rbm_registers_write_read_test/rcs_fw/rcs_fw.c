...
/**
 * @file    rcs_fw.c
 * @brief   This test is to validate write/read access to all RBM registers.
 * @details This test is trying to write and then read all the available RBM registers
...
int main()
{
    ...
    uint32_t reg_num, index, wr_pattern, num_slaves_connected;
    ...
    if(!(skip_master_bridge & (1 << M_RCS_0)))
    {
        ...
        for(reg_num = 0; reg_num < ARRAY_SIZE(m_rcs_0->regs); reg_num++)
        {
            ...
            *wr_rd_addr_ptr = wr_pattern;                  // Core writing to the address
            g_rsm_buf[index++] = *wr_rd_addr_ptr;          // Core reading from register address
        }
    }
    ...
    if(!(skip_master_bridge & (1 << M_RCS_1)))
    {
        ...
        for(reg_num = 0; reg_num < ARRAY_SIZE(m_rcs_1->regs); reg_num++)
        {
            ...
            if((uint32_t)wr_rd_addr_ptr == 0x00d386c0)     // VEX_SFC_S_RBM
            {
                // Writing any value other than the default value to this register will disable access to the rbm register space.
                //  So, skip it.
                reg_num = reg_num + 2;
                continue;
            }
    ...
    if(!(skip_master_bridge & (1 << M_SYSCON_0)))
    {
        ...
        for(reg_num = 0; reg_num < ARRAY_SIZE(m_syscon_0->regs); reg_num++)
        {
            ...
            if((((uint32_t)wr_rd_addr_ptr & 0xfff) == 0xe20) ||
               (((uint32_t)wr_rd_addr_ptr & 0xfff) == 0xf20) ||
               (((uint32_t)wr_rd_addr_ptr & 0xfff) == 0xf40))
                continue;
    ...
    for(bridge_num = S_JESD0; bridge_num < ARRAY_SIZE(g_noc_bridges.s_bridges); bridge_num++)
    {
        ...
        if(!(skip_slave_bridge & (1 << bridge_num)))
        {
            ...
            for(reg_num = 0; reg_num < ARRAY_SIZE(s_bridge->regs); reg_num++)
            {
                ...
                if((((uint32_t)wr_rd_addr_ptr & 0xfff) == 0xe20) ||
                   (((uint32_t)wr_rd_addr_ptr & 0xfff) == 0xf10))
                    continue;                              // (offset 0xe20) -> error inject register is 'V', so skip it
                                                           // (offset 0xf20, 0xf40) -> event counter registers, so skip it
