...
int main(int argc, const char * argv[])
{
    ...
    if(!(skip_master_bridge & (1 << M_RCS_1)))
    {
        ...
        for(reg_num = 0; reg_num < ARRAY_SIZE(m_rcs_1->regs); reg_num++)
        {
            ...
            {
                // Witing any value except the default value to this register disable access to the rbm register space.
                //  So, skip it.
                reg_num = reg_num + 2;
                continue;
            }
    ...
    if(!(skip_master_bridge & (1 << M_PCIE)))
    {
        ...
        for(reg_num = 0; reg_num < ARRAY_SIZE(m_pcie->regs); reg_num++)
        {
            ...
            {
                // Writing any value other than the default value to this register causes access denial to RCS memory space.
                //  So, skip it.
                reg_num = reg_num + 2;
                continue;
            }
