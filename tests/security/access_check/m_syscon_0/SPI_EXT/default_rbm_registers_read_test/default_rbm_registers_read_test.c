...
int main(int argc, const char * argv[])
{
    ...
    LOG_PRINT("Test: Default RBM register read test\n");
    LOG_PRINT("      %s\n", g_compile_time);
    ...
    int reg_num;
    uint32_t rd_addr, act_val, exp_val, rd_mask, result;
    int i;
    ...
    if(!(skip_master_bridge & (1 << M_RCS_0)))
    {
        ...
        {
            rd_addr = m_rcs_0->base_addr + m_rcs_0->regs[reg_num].offset;
            exp_val = m_rcs_0->regs[reg_num].def_val;
            rd_mask = m_rcs_0->regs[reg_num].rd_mask;
            act_val = read_reg_18a(rd_addr, m_rcs_0->regs[reg_num].reg_name);
            result = read_reg_expect_18a(rd_addr, exp_val, rd_mask, m_rcs_0->regs[reg_num].reg_name);
            msg = "INFO";
            if(result) msg = "ERROR";
            LOG_PRINT("%s: offset = %x, act_val = %x, exp_val = %x, rd_mask = %x, reg_name = %s\n", msg,
                      m_rcs_0->regs[reg_num].offset, act_val, exp_val, rd_mask, m_rcs_0->regs[reg_num].reg_name);
        }
    }
    ...
    if(!(skip_master_bridge & (1 << M_RCS_1)))
    {
        ...
        {
            rd_addr = m_rcs_1->base_addr + m_rcs_1->regs[reg_num].offset;
            exp_val = m_rcs_1->regs[reg_num].def_val;
            rd_mask = m_rcs_1->regs[reg_num].rd_mask;
            act_val = read_reg_18a(rd_addr, m_rcs_1->regs[reg_num].reg_name);
            result = read_reg_expect_18a(rd_addr, exp_val, rd_mask, m_rcs_1->regs[reg_num].reg_name);
            msg = "INFO";
            if(result) msg = "ERROR";
            LOG_PRINT("%s: offset = %x, act_val = %x, exp_val = %x, rd_mask = %x, reg_name = %s\n", msg,
                      m_rcs_1->regs[reg_num].offset, act_val, exp_val, rd_mask, m_rcs_1->regs[reg_num].reg_name);
        }
    }
    ...
    if(!(skip_master_bridge & (1 << M_PCIE)))
    {
        ...
        {
            rd_addr = m_pcie->base_addr + m_pcie->regs[reg_num].offset;
            exp_val = m_pcie->regs[reg_num].def_val;
            rd_mask = m_pcie->regs[reg_num].rd_mask;
            act_val = read_reg_18a(rd_addr, m_pcie->regs[reg_num].reg_name);
            result = read_reg_expect_18a(rd_addr, exp_val, rd_mask, m_pcie->regs[reg_num].reg_name);
            msg = "INFO";
            if(result) msg = "ERROR";
            LOG_PRINT("%s: offset = %x, act_val = %x, exp_val = %x, rd_mask = %x, reg_name = %s\n", msg,
                      m_pcie->regs[reg_num].offset, act_val, exp_val, rd_mask, m_pcie->regs[reg_num].reg_name);
        }
    }
    ...
    if(!(skip_master_bridge & (1 << M_CS_DBG)))
    {
        ...
        {
            rd_addr = m_cs_dbg->base_addr + m_cs_dbg->regs[reg_num].offset;
            exp_val = m_cs_dbg->regs[reg_num].def_val;
            rd_mask = m_cs_dbg->regs[reg_num].rd_mask;
            act_val = read_reg_18a(rd_addr, m_cs_dbg->regs[reg_num].reg_name);
            result = read_reg_expect_18a(rd_addr, exp_val, rd_mask, m_cs_dbg->regs[reg_num].reg_name);
            msg = "INFO";
            if(result) msg = "ERROR";
            LOG_PRINT("%s: offset = %x, act_val = %x, exp_val = %x, rd_mask = %x, reg_name = %s\n", msg,
                      m_cs_dbg->regs[reg_num].offset, act_val, exp_val, rd_mask, m_cs_dbg->regs[reg_num].reg_name);
        }
    }
    ...
    if(!(skip_master_bridge & (1 << M_SYSCON_1)))
    {
        ...
        {
            rd_addr = m_syscon_1->base_addr + m_syscon_1->regs[reg_num].offset;
            exp_val = m_syscon_1->regs[reg_num].def_val;
            rd_mask = m_syscon_1->regs[reg_num].rd_mask;
            act_val = read_reg_18a(rd_addr, m_syscon_1->regs[reg_num].reg_name);
            result = read_reg_expect_18a(rd_addr, exp_val, rd_mask, m_syscon_1->regs[reg_num].reg_name);
            msg = "INFO";
            if(result) msg = "ERROR";
            LOG_PRINT("%s: offset = %x, act_val = %x, exp_val = %x, rd_mask = %x, reg_name = %s\n", msg,
                      m_syscon_1->regs[reg_num].offset, act_val, exp_val, rd_mask, m_syscon_1->regs[reg_num].reg_name);
        }
    }
    ...
    if(!(skip_master_bridge & (1 << M_SYSCON_0)))
    {
        ...
        {
            rd_addr = m_syscon_0->base_addr + m_syscon_0->regs[reg_num].offset;
            exp_val = m_syscon_0->regs[reg_num].def_val;
            rd_mask = m_syscon_0->regs[reg_num].rd_mask;
            // ROE bit might get cleared because we are initiating a transaction from m_sycon_0.
            if(0x00d5bd00 == rd_addr)
                rd_mask &= (~(1 << 2));
            act_val = read_reg_18a(rd_addr, m_syscon_0->regs[reg_num].reg_name);
            result = read_reg_expect_18a(rd_addr, exp_val, rd_mask, m_syscon_0->regs[reg_num].reg_name);
            msg = "INFO";
            if(result) msg = "ERROR";
            LOG_PRINT("%s: offset = %x, act_val = %x, exp_val = %x, rd_mask = %x, reg_name = %s\n", msg,
                      m_syscon_0->regs[reg_num].offset, act_val, exp_val, rd_mask, m_syscon_0->regs[reg_num].reg_name);
        }
    }
    ...
    if(!(skip_master_bridge & (1 << M_DRF0_MEM_FB)))
    {
        ...
        {
            rd_addr = m_drf0_mem_fb->base_addr + m_drf0_mem_fb->regs[reg_num].offset;
            exp_val = m_drf0_mem_fb->regs[reg_num].def_val;
            rd_mask = m_drf0_mem_fb->regs[reg_num].rd_mask;
            act_val = read_reg_18a(rd_addr, m_drf0_mem_fb->regs[reg_num].reg_name);
            result = read_reg_expect_18a(rd_addr, exp_val, rd_mask, m_drf0_mem_fb->regs[reg_num].reg_name);
            msg = "INFO";
            if(result) msg = "ERROR";
            LOG_PRINT("%s: offset = %x, act_val = %x, exp_val = %x, rd_mask = %x, reg_name = %s\n", msg,
                      m_drf0_mem_fb->regs[reg_num].offset, act_val, exp_val, rd_mask, m_drf0_mem_fb->regs[reg_num].reg_name);
        }
    }
    ...
    if(!(skip_master_bridge & (1 << M_DRF1_MEM_FB)))
    {
        ...
        {
            rd_addr = m_drf1_mem_fb->base_addr + m_drf1_mem_fb->regs[reg_num].offset;
            exp_val = m_drf1_mem_fb->regs[reg_num].def_val;
            rd_mask = m_drf1_mem_fb->regs[reg_num].rd_mask;
            act_val = read_reg_18a(rd_addr, m_drf1_mem_fb->regs[reg_num].reg_name);
            result = read_reg_expect_18a(rd_addr, exp_val, rd_mask, m_drf1_mem_fb->regs[reg_num].reg_name);
            msg = "INFO";
            if(result) msg = "ERROR";
            LOG_PRINT("%s: offset = %x, act_val = %x, exp_val = %x, rd_mask = %x, reg_name = %s\n", msg,
                      m_drf1_mem_fb->regs[reg_num].offset, act_val, exp_val, rd_mask, m_drf1_mem_fb->regs[reg_num].reg_name);
        }
    }
    uint32_t skip_slave_bridge1, skip_slave_bridge2, skip_slave_bridge;
    ...
    for(bridge_num = S_JESD0; bridge_num < ARRAY_SIZE(g_noc_bridges.s_bridges); bridge_num++)
    {
        if(!(skip_slave_bridge & (1ULL << bridge_num)))
        {
            ...
            {
                rd_addr = s_bridge->base_addr + s_bridge->regs[reg_num].offset;
                exp_val = s_bridge->regs[reg_num].def_val;
                rd_mask = s_bridge->regs[reg_num].rd_mask;
                act_val = read_reg_18a(rd_addr, s_bridge->regs[reg_num].reg_name);
                result = read_reg_expect_18a(rd_addr, exp_val, rd_mask, s_bridge->regs[reg_num].reg_name);
                msg = "INFO";
                if(result) msg = "ERROR";
                LOG_PRINT("%s: offset = %x, act_val = %x, exp_val = %x, rd_mask = %x, reg_name = %s\n", msg,
                          (s_bridge->regs[reg_num].offset), act_val, exp_val, rd_mask, s_bridge->regs[reg_num].reg_name);
            }
        }
    }
    ...
    if(!(skip_slave_bridge & (1ULL << bridge_num)))
    {
        ...
        for(reg_num = 0; reg_num < max_regs; reg_num++)
        {
            rd_addr = s_bridge_rbm->base_addr + s_bridge_rbm->regs[reg_num].offset;
            exp_val = s_bridge_rbm->regs[reg_num].def_val;
            rd_mask = s_bridge_rbm->regs[reg_num].rd_mask;
            act_val = read_reg_18a(rd_addr, s_bridge_rbm->regs[reg_num].reg_name);
            result = read_reg_expect_18a(rd_addr, exp_val, rd_mask, s_bridge_rbm->regs[reg_num].reg_name);
            msg = "INFO";
            if(result) msg = "ERROR";
            LOG_PRINT("%s: offset = %x, act_val = %x, exp_val = %x, rd_mask = %x, reg_name = %s\n", msg,
                      (s_bridge_rbm->regs[reg_num].offset), act_val, exp_val, rd_mask, s_bridge_rbm->regs[reg_num].reg_name);
        }
    }
srp_return:
