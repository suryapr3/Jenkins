...
int main(int argc, const char* argv[])
{
    ...
    init();
    system ("srpBusSelect -s spi");                        // Enable SPI_EXT
    ...
    // Negative test, the access should fail and result in 0x00000000
    uint32_t err_cnt = 0;
    exp_val = 0x00000000;
    for(target_num = HUB; target_num < ARRAY_SIZE(g_noc_targets); target_num++)
    {
        if(!(skip_targets & (1ULL << target_num)))
        {
            ...
            for(reg_num = 0; reg_num < ARRAY_SIZE(target->regs); reg_num++)
            {
                ...
                wr_mask = target->regs[reg_num].wr_mask;
                rd_mask = target->regs[reg_num].rd_mask;
                result = read_reg_expect_18a(rd_addr, exp_val, wr_mask, target->name);
                ...
                LOG_PRINT("%s: addr = %x, act_val = %x, exp_val = %x, rd_mask = %x\n", msg,
                          rd_addr, act_val, exp_val, rd_mask);
