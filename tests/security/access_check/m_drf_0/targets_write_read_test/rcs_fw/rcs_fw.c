...
/**
 * @file    rcs_fw.c
 * @brief   This test is to validate write/read access from DRF DMA to target registers.
 * @details There are 64 targets out of which 18 targets are available.
 */
...
int main()
{
    ...
    skip_targets1 = (1 << HUB)              |              // Not available on FRIO, so disabling it.
                    ...
                    (0 << DRF_0_ANT_1)      |
                    (0 << DRF_0_FB_ANT)     |
                    (1 << DRF_0_FB_SYS)     |              // Register space is RO at offset +0x00c, so disabling it.
                    (0 << DRF_0_MEM)        |
                    (1 << DRF_0_MEM_FB)     |              // Not accessible for DRF.
                    (0 << DRF_0_TRXANT_0)   |
                    (0 << DRF_0_TRXANT_1)   |
                    (0 << DRF_0_TRXANT_2)   |
                    (0 << DRF_0_TRXANT_3)   |
                    (1 << DRF_0_TRXSYS_0)   |              // Register space is RO at offset +0x00c, so disabling it.
                    (1 << DRF_0_TRXSYS_1)   |              // Register space is RO at offset +0x00c, so disabling it.
                    (1 << DRF_0_TRXSYS_2)   |              // Register space is RO at offset +0x00c, so disabling it.
                    (1 << DRF_0_TRXSYS_3)   |              // Register space is RO at offset +0x00c, so disabling it.
    ...
    skip_targets2 = (1 << (DRF_1_TRXSYS_0 - 32)) |         // Not available on FRIO, so disabling it.
                    ...
                    (1 << (VEX_SFC_RBM - 32))    |         // Register space not accessible after offset +0x04, so disabling it.
                    (1 << (RCS_0 - 32))          |         // Not accessible for DRF
                    (0 << (RCS_1 - 32))          |
                    ...
                    (0 << (VEX_2 - 32))          |
                    (1 << (VEX_CAR - 32))        |         // Register space not accessible after offset +0x08, so diabling it.
                    ...
                    (1 << (VHA_0 - 32))          |         // Register space not accessible after offset +0x08, so diabling it.
                    (1 << (VHA_1 - 32))          |         // Register space not accessible after offset +0x08, so diabling it.
                    (1 << (VHA_2 - 32))          |         // Register space not accessible after offset +0x08, so diabling it.
                    (1 << (VHA_3 - 32))          |         // Register space not accessible after offset +0x08, so diabling it.
    ...
    for (int i = 0; i < 32; i++)
        g_rsm_buf_temp[i] = wr_pattern;                    // Fill rsm_buf with a given pattern
    ...
    for(target_num = HUB; target_num < ARRAY_SIZE(g_noc_targets); target_num++)
    {
        ...
        if(!(skip_targets & (1 << target_num)))
        {
            ...
            for(reg_num = 0; reg_num < 1; reg_num++)       // access all target's start address
            {
                wr_rd_addr_ptr = (volatile uint32_t*)(target->regs[reg_num].base_addr + target->regs[reg_num].offset);
                // Writing by DRF DMA to target memory
                drf_dma_test_LLI(0, wr_rd_addr_ptr, (uint32_t)(&g_rsm_buf_temp));
                // Reading target memory by DRF DMA
                drf_dma_test_LLI(0, (uint32_t) (g_rsm_buf + rsm_index), wr_rd_addr_ptr);
                rsm_index++;
            }
        }
    }
