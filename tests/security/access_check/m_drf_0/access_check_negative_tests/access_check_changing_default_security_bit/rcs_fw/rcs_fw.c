 ...
 *          we change the security bit of base address of the m_drf_0 port
...
int main()
{
    ...
    uint32_t rd_addr;
    // Now modify the RBM addr mask register's security bit to deny access to targets
    for(target_num = HUB; target_num < ARRAY_SIZE(g_noc_targets); target_num++)
    {
        ...
        {
            base_addr_ptr = (volatile uint32_t*)(m_drf0_mem_fb->base_addr +
                            m_drf0_mem_fb->regs[(target_num - offset_adj) * 3].offset);
            *base_addr_ptr = ((*base_addr_ptr) | (1 << 1));
            // corrupt the base addr register's security bit so that access will be denied
