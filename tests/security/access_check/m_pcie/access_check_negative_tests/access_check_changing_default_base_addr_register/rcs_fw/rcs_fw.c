...
/**
 * @file    rcs_fw.c
 * @brief   This is a negative test to validate read access to target registers after changing
 *          the base addr rbm register for each target.
 */
...
int main()
{
    ...
    uint64_t m_pcie_unconnected_targets = (1ULL << PCIE);

    for(target_num = HUB; target_num < ARRAY_SIZE(g_noc_targets); target_num++)
    {
        if(m_pcie_unconnected_targets & (1ULL << target_num))
        {
            offset_adj++;
            continue;
        }
        ...
        {
            base_addr_ptr = (volatile uint32_t*)(m_pcie->base_addr + m_pcie->regs[(target_num - offset_adj) * 3].offset);
            *base_addr_ptr = g_rsm_pattern;                       // corrupt the base addr so that access will be denied
        }
    }
