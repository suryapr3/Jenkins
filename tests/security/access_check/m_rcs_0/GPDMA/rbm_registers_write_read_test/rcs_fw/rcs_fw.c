...
int main()
{
    ...
    if(!(skip_master_bridge & (1 << M_RCS_0)))
    {
        ...
        for(reg_num = 0; reg_num < ARRAY_SIZE(m_rcs_0->regs); reg_num++)
        {
            ...
            {
                // Writing any value other than default value to this register disable access to the RBM register space.
                //  So, skip it.
                reg_num = reg_num + 2;
                continue;
            }
            ...
            gpdma_test_LLI(0, wr_rd_addr_ptr, (uint32_t)(&g_rsm_pattern));
                                                           // gpdma write to target register
            gpdma_test_LLI(0, (uint32_t)(g_rsm_buf + index), wr_rd_addr_ptr);
                                                           // gpdma read from target register
            index++;
