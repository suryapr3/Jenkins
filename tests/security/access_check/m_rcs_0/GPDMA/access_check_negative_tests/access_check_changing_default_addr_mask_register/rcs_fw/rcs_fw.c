 ...
 *          we change the addr mask register  of the m_rcs_0 port
...
int main()
{
    ...
    for(target_num = HUB; target_num < ARRAY_SIZE(g_noc_targets); target_num++)
    {
        ...
        {
            addr_mask_ptr = (volatile uint32_t*)(m_rcs_0->base_addr + m_rcs_0->regs[((target_num - offset_adj) * 3) + 1].offset);
            *addr_mask_ptr = 0xfffff002;                   // corrupt the base addr so that access will be denied
