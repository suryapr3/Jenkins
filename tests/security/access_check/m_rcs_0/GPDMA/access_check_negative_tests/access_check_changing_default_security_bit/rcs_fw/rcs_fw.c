 ...
 *          we change the security bit of base address of the m_rcs_0 port
...
int main()
{
    ...
    skip_targets1 = g_skip_targets1;                       // Will be shared with the HOST driver
    skip_targets2 = g_skip_targets2;                       // Will be shared with the HOST driver
    ...
    uint64_t m_rcs_0_unconnected_targets = (1ULL << RCS_0) | (1ULL << RCS_1);
    uint32_t rd_addr;
    for(target_num = HUB; target_num < ARRAY_SIZE(g_noc_targets); target_num++)
    {
        if(m_rcs_0_unconnected_targets & (1ULL << target_num))
        {
            offset_adj++;
            continue;
        }
        ...
        {
            base_addr_ptr = (volatile uint32_t*)(m_rcs_0->base_addr + m_rcs_0->regs[(target_num - offset_adj) * 3].offset);
            *base_addr_ptr = ((*base_addr_ptr) | (1 << 1));               // corrupt the base addr so that access will be denied
            ...
            for(reg_num = 0; reg_num < ARRAY_SIZE(target->regs); reg_num++)
            {
               ...
               // Let GPDMA try to access the address whose access is denied, and then copy to RSM memory.
               // This call is expected to fail and hence the content of the RSM should remain the same.
               gpdma_negative_test_LLI(0, (uint32_t)(g_rsm_buf + rsm_buf_index), (uint32_t)rd_addr);
            }
            rsm_buf_index++;
        }
    }
