...
int main(int argc, const char* argv[])
{
    ...
    LOG_PRINT("Test:  Target_write_read_test\n");
    LOG_PRINT("      %s\n", g_compile_time);
                ...
                if((target_num == DRF_0_MEM_FB) || (target_num == DRF_1_MEM_FB))
                {
                    ...
                    for(j = 1; j <= 15; j++)
                    {
                        write_m_syscon_1_cs_dbg_18a((rd_addr + (j * 4)), pattern, NON_SECURE, target->name);
                                                           ...
                                                           // Error response from coresight api's for transaction satus handling
                uint32_t error_status = write_m_syscon_1_cs_dbg_18a(rd_addr, pattern, NON_SECURE, target->name);
