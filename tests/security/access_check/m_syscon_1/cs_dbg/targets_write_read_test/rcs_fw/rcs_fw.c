...
/**
 * @file    rcs_fw.c
 * @brief   This test validates m_syscon_1 port WRITE/READ access to target registers by configuring
 *          the security bit to non-secure mode.
 */
    ...
    for(target_num = HUB; target_num < ARRAY_SIZE(g_noc_targets); target_num++)
    {
      if(m_syscon_1_unconnected_targets & (1ULL << target_num))
        ...
        if(!(skip_targets & (1 << target_num)))
        {
            ...
            // Since m_syscon_1 is non-secure changing all targets to non-secure to allow access.
