...
int main(int argc, const char* argv[])
{
    ...
    skip_targets2 = (1 << (DRF_1_TRXSYS_0 - 32)) |         // Not available on FRIO, so disabling it.
                    ...
                    (1 << (SYSCON_MAIN - 32))    |         // Causing access denail to whole register space, so disabling it.
                    (0 << (V2D_0 - 32))          |
                    (0 << (V2D_1 - 32))          |
                    (0 << (V2D_2 - 32))          |
                    (0 << (V2D_3 - 32))          |
    ...
    LOG_PRINT("INFO: Waiting for the RCS CORE to finish changing security bit\n");
        ...
        if(!(skip_targets & (1ULL << target_num)))
        {
            ...
            for(reg_num = 0; reg_num < ARRAY_SIZE(target->regs); reg_num++)
            {
                ...
                if((target_num == DRF_0_MEM_FB) || (target_num == DRF_1_MEM_FB))
                {
                    ...
                    for(j = 1; j <= 15; j++)
                    {
                        write_m_syscon_1_cs_dbg_18a((rd_addr + (j * 4)),pattern, NON_SECURE, target->name);
                    }
                }
                write_m_syscon_1_cs_dbg_18a(rd_addr,pattern, NON_SECURE, target->name);
                act_val = read_m_syscon_1_cs_dbg_18a(rd_addr, NON_SECURE, target->name);
    ...
    // m_syscon_1/cs_dbg will access the target registers
    ...
    for(target_num = HUB; target_num < ARRAY_SIZE(g_noc_targets); target_num++)
    {
        if(!(skip_targets & (1ULL << target_num)))
        {
            ...
            for(reg_num = 0; reg_num < ARRAY_SIZE(target->regs); reg_num++)
            {
                rd_addr = (target->regs[reg_num].base_addr + target->regs[reg_num].offset);
                wr_mask = target->regs[reg_num].wr_mask;
                act_val = read_m_syscon_1_cs_dbg_18a(rd_addr, SECURE, target->name);
