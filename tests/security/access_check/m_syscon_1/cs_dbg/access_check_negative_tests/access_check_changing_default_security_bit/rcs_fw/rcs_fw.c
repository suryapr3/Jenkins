...
int main()
{
    ...
    uint64_t m_syscon_1_unconnected_targets = (1ULL << VEX_SFC_RBM);

    for(target_num = HUB; target_num < ARRAY_SIZE(g_noc_targets); target_num++)
    {
      if(m_syscon_1_unconnected_targets & (1ULL << target_num))
        {
            offset_adj++;
            continue;
        }
        ...
        {
            base_addr_ptr = (volatile uint32_t*)(m_syscon_1->base_addr + m_syscon_1->regs[(target_num - offset_adj) * 3].offset);
            // By default secure for all targets,
            //  Since m_syscon_1/cs_dbg is non-secure changing all targets to non-secure to allow access.
    ...
    for(target_num = HUB; target_num < ARRAY_SIZE(g_noc_targets); target_num++)
    {
        ...
        if(target_num == RCS_1)                            // As RCS_1 is by default NON-Secure changing NS valid bit to 1
        {
            addr_mask_ptr = (volatile uint32_t*)(m_syscon_1->base_addr + m_syscon_1->regs[((target_num - offset_adj) * 3) + 1].offset);
            *addr_mask_ptr = ((*addr_mask_ptr) | (1 << 1));
        }
