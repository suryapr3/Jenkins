...
/**
 * @file    rcs_fw.c
 * @brief   This test validate the target access via m_syscon_1 port
 *           after modifying the  base addr register.
 */
...
int main()
{
    ...
    for(target_num = HUB; target_num < ARRAY_SIZE(g_noc_targets); target_num++)
    {
        ...
        {
            base_addr_ptr = (volatile uint32_t*)(m_syscon_1->base_addr + m_syscon_1->regs[(target_num - offset_adj) * 3].offset);
            // corrupt the base addr so that access will be denied
            *base_addr_ptr = g_rsm_pattern;
        }
