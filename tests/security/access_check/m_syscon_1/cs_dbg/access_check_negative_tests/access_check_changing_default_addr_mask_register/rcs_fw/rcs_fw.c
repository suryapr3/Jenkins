 ...
 *          addr mask  register for each target.
 */
      ...
      if(m_syscon_1_unconnected_targets & (1ULL << target_num))
        ...
        if(!(skip_targets & (1 << target_num)))
        {
            ...
            *base_addr_ptr = ((*base_addr_ptr) | (1 << 1));
            // m_syscon_1/cs_dbg is a non-secure master & all targets are by default secure.
            //  In order to initiate access changing all targets to non-secure
        }
    }
