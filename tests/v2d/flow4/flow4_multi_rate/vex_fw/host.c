...
int hrt_main(int argc, char **argv)
{
    ...
    hrt_cell_load_program(VE00, "v2d_flow4_tx");
    ...
    hrt_scalar_store(VE00, uint, v2d_flow4_tx_g_scalar_val, add_const);
    hrt_scalar_store(VE00, uint, v2d_flow4_tx_g_b_num_vecs_in, num_vec_input);
    hrt_scalar_store(VE00, uint, v2d_flow4_tx_g_c_num_vecs_in, num_vec_input);
    hrt_scalar_store(VE00, uint, v2d_flow4_tx_g_b_dest_buffer_base, 0x8000);
    hrt_scalar_store(VE00, uint, v2d_flow4_tx_g_num_blocks, 0);
    hrt_scalar_store(VE00, uint, v2d_flow4_tx_g_initial_block_size, 1);
    hrt_scalar_store(VE00, uint, v2d_flow4_tx_g_host_cycle_count, 511);
    ...
    hrt_scalar_store(VE01, uint, v2d_flow4_rx_g_tbs_in_blk, TBS_IN_BLK);
    ...
    for(i = 0; i < num_vec_input; i++)
    {
        for(j = 0; j < NUM_OF_ELEMENTS_PER_VEC; j++)
        {
            ...
            hrt_indexed_store(VE00, int, v2d_flow4_tx_vbuffer, i * NUM_OF_ELEMENTS_PER_VEC + j,
                              (((i * NUM_OF_ELEMENTS_PER_VEC + j) << 16) |(i * NUM_OF_ELEMENTS_PER_VEC + j)));
        }
    }
    index = 0;
    ...
    // Run program
    hrt_cell_start_function(VE00, v2d_flow4_tx_v2d_flow4_tx);
