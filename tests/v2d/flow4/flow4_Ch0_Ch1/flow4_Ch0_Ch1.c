 ...
 *********************************************************************************************/
/**
 * @file    flow4_Ch0_Ch1.c
 * @details 1. This TC validates the PDPD functionality with extra terms enabled in MODE 1
               configuration and PDPD output data will be receving other VEX CORE by enabling both
...
{
    int i, rv = 0, v2d_buffer_b = 0, v2d_buffer_c= 0, pdpd_in_rate = 0, pdpd_out_rate = 0, vex_cnt = 0, apb_num = 0,
        fill_level_value = 0, tx_stream_cnt_value = 0, rx_stream_cnt_value = 0;
    ...
    printf ("Buffer b: 0x%x, Buffer C: 0x%x\n", v2d_buffer_b, v2d_buffer_c);
   ...
   for (vex_cnt = 0; vex_cnt < NUM_TX_VEX_CORES; vex_cnt++)
    {
        ...
        {
            if (rv = write_reg_18a(VE32_CELL_BASE(vex_num) + g_flow4_cio2strm_config_tbl[i].offset, g_flow4_cio2strm_config_tbl[i].value, \
        ...
        for (; i < ARRAY_SIZE(g_flow4_cio2strm_config_tbl) - 1; i++)
        {
            ...
            if (rv = write_reg_18a(VE32_CELL_BASE(vex_num) + g_flow4_cio2strm_config_tbl[i].offset, g_v2d_host_id[v2d_num][apb_num][0], \
                    g_flow4_cio2strm_config_tbl[i].mask, g_flow4_cio2strm_config_tbl[i].name))
                return rv;
        }
        for (; i < ARRAY_SIZE(g_flow4_cio2strm_config_tbl); i++)
        {
            ...
            if (rv = write_reg_18a(VE32_CELL_BASE(vex_num) + g_flow4_cio2strm_config_tbl[i].offset, g_v2d_host_id[v2d_num][apb_num][1], \
                    g_flow4_cio2strm_config_tbl[i].mask, g_flow4_cio2strm_config_tbl[i].name))
                return rv;
        }
...
int main(int argc, const char *argv[])
{
    ...
    int flags = 0;
    int lc, i, j;
    int vex_num = 0, vex_num_param[NUM_VEX_CORES] = {0}, v2d_num, apb_num = 0, vex_cnt = 0, underrun_reg_value = 0, drf_clock = 0,
        test_config = 0, post_filter_mode = 0;
    ...
    printf ("INFO: argv[0] : %d = tx_vex_ch0", vex_num_param[0]);
    printf ("INFO: argv[1] : %d = rx_vex_ch0", vex_num_param[1]);
    printf ("INFO: argv[2] : %d = tx_vex_ch1", vex_num_param[2]);
    printf ("INFO: argv[3] : %d = rx_vex_ch1", vex_num_param[3]);
    ...
    vex_tx_val_kargs[2] = g_vectors_per_block[drf_clock][pdpd_out_rate];
    ...
    for (vex_cnt = 0; vex_cnt < NUM_TX_VEX_CORES; vex_cnt++)
    {
        ...
        if ((rv = vex_loader(tx_vex_num, "vex_fw/sim_m64/v2d_flow4_tx.fw")))
            return rv;

        if ((rv = vex_loader(rx_vex_num, "vex_fw/sim_m64/v2d_flow4_rx.fw")))
            return rv;
        printf("Host: vex program load - done");

        /* Loading the host parameters to VEX  Tx core*/
        if (load_vex_param(tx_vex_num/*vex_core*/, ARRAY_SIZE(vex_tx_offset_val)/*size of array*/, vex_tx_offset_val, vex_tx_val_kargs))
        {
            LOG_ERROR("Tx vex param loading failed");
        }

        /* Loading the host parameters to VEX  Rx core*/
        if (load_vex_param(rx_vex_num/*vex_core*/, ARRAY_SIZE(vex_rx_offset_val)/*size of array*/, vex_rx_offset_val, vex_rx_val_kargs))
        {
            LOG_ERROR("Rx vex param loading failed");
        }

        /* Loading the input vector for Tx vex core*/
        write_mem_18a((VEX_VMEM(tx_vex_num) + HIVE_ADDR_v2d_flow4_tx_vbuffer),
                (VEX_VMEM_IN_VEC * NUM_OF_ELEMENTS_PER_VEC),
                val_inp,
                "TX_VEX_VMEM");
       ...
       write_mem_18a((VEX_VMEM(rx_vex_num) + HIVE_ADDR_v2d_flow4_rx_vbuffer),
    ...
    while ((apb0_b2h_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, 0) + V2D_NSIP_HBI_1_OFFSET + \
            ...
            ((g_initial_block_size * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_in_rate])) + VEC2FLIT(vex_tx_val_kargs[1] / 2)))
    {
        // do nothing wait in this loop until 1 blocks of data was received
        if (loop_wait_count >= 30)
        {
            printf("WARNING: Breaking the loop while B2H_FLIT_CNT is not the required number of flits for APB0\n");
            break;
        }
        loop_wait_count ++;
        usleep(10000);
            ...
            ((g_initial_block_size * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_in_rate])) + VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_in_rate] / 2)))
    {
        ...
        if (loop_wait_count >= 30)
        {
            printf("WARNING: Breaking the loop while B2H_FLIT_CNT is not the required number of flits for APB1\n");
            break;
        }
        loop_wait_count ++;
        usleep(10000);
    }
    printf ("apb0_b2h_flit_cnt : %d, apb1_b2h_flit_cnt: %d", apb0_b2h_flit_count, apb1_b2h_flit_count);
    host_flag = 1;
    ...
    for (vex_cnt = 0; vex_cnt < NUM_TX_VEX_CORES; vex_cnt++)
    {
        ...
        if (rv = write_reg_18a(VEX_DMEM(vex_num) + HIVE_ADDR_v2d_flow4_tx_g_host_indicator,
    ...
    {
        printf("ERROR: SHDW to LIVE transition failied for instance: %d", reg_check.apb_num);
        return 1;
    }
    else
    {
        printf("HOST: SHDW to LIVE tranition passed for instance: %d", reg_check.apb_num);
    }
            ...
            (g_max_blocks[drf_clock][pdpd_out_rate] * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_out_rate])))
    {
        ...
        {
            printf("WARNING: Breaking the loop while H2B_FLIT_CNT is not the required number of flits for APB0\n");
            break;
        }
        loop_wait_count ++;
        usleep(10000);
            ...
            (g_max_blocks[drf_clock][pdpd_out_rate] * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_out_rate])))
    {
        ...
        if (loop_wait_count >= 80)
        {
            printf("WARNING: Breaking the loop while H2B_FLIT_CNT is not the required number of flits for APB1\n");
            break;
        }
        loop_wait_count ++;
        usleep(10000);
    }
    printf ("apb0_h2b_flit_cnt is %d, apb1_h2b_flit_cnt is %d", apb0_h2b_flit_count, apb1_h2b_flit_count);
    ...
    for (vex_cnt = 0; vex_cnt < NUM_RX_VEX_CORES; vex_cnt++)
    {
        ...
        {
            host_vec_processed = read_reg_18a(VEX_DMEM(vex_num) + HIVE_ADDR_v2d_flow4_rx_g_count1, "vec_processed");
        ...
        host_result = read_reg_18a(VEX_DMEM(vex_num) + HIVE_ADDR_v2d_flow4_rx_g_compare_result, "compare_result");
        host_blocks_processed = read_reg_18a(VEX_DMEM(vex_num) + HIVE_ADDR_v2d_flow4_rx_g_add_val_out, "blocks_processed");
    ...
    if (0 == test_status)
    {
        printf("Host:---- SUCCESS ---- tx0_vex_num : %d, rx0_vex_num: %d, tx1_vex_num : %d, rx1_vex_num : %d, v2d_num : %d,drf_clock: %s, \
                ...
                pdpd_rate_str[pdpd_in_rate], pdpd_rate_str[pdpd_out_rate], g_test_config_tbl[test_config][2], post_filter_mode_str[post_filter_mode]);
    }
    else
    {
