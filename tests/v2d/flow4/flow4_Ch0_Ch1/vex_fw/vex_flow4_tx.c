...
/**
 * @file    vex_flow4_tx.c
 * @brief   Vex FW code to validate V2D Path B and V2D_Path C.
 ...
 ***********************************************/
vec32i32 AT(RX_SRC_BUFFER_BASE) vbuffer[VMEM_IN_VEC];
volatile unsigned int AT(0x4) g_b_dest_buffer_base;
volatile unsigned int AT(0x18) g_c_dest_buffer_base;
volatile int AT(0x28) g_scalar_val;
volatile int AT(0x20) g_b_num_vecs_in;
volatile int AT(0x24) g_c_num_vecs_in;
 ...
 * @return          none
 */
void v2d_flow4_tx() ENTRY
{
    int i, j = 0, k = 0, b_offset = 0, c_offset = 0;
    g_host_debug = 0;
    int b_dest_address = g_b_dest_buffer_base;
    int c_dest_address = g_c_dest_buffer_base;
    int b_iter_max = g_b_num_vecs_in;
    int c_iter_max = g_c_num_vecs_in;
    int add_offset = 0, limit = 0, rewind = 0;
    int src_addr = RX_SRC_BUFFER_BASE;
    int src_compare_offset = 0, src_compare_offset_extra_terms = 0;
    g_host_indicator = 0;
    ...
    {
        for(i = 0; i < b_iter_max; i++)
        {
            vec32i32 vin1 UNINIT;
            v_ldoi_1024_circ(Any, src_addr, src_compare_offset, limit, rewind, vin1, src_compare_offset) NO_ALIAS SYNC_WITH(buffer_read);
#if DEBUG
            ...
            v_xstomi_1024(Any, b_dest_address, b_offset, LD_OFFSET_MAX, vin1, b_offset);
        }   // for(i = 0; i < g_num_vecs_in; i++) ends

        // For writing extra terms
        for(k = 0; k < c_iter_max; k++)
        {
            vec32i32 vin1 UNINIT;

            v_ldoi_1024_circ(Any, src_addr, src_compare_offset_extra_terms, limit, rewind, vin1, src_compare_offset_extra_terms) NO_ALIAS SYNC_WITH(buffer_read);
#if DEBUG
            OP___dump(__LINE__, i);
            OP___dump(__LINE__, vin1);
#endif
            ...
            //v_sto_1024(Any,vbuffer_base,i*128,vout);
            //v_xsto_1024(Any,vbuffer_base,i,vout);
            v_xstomi_1024(Any, c_dest_address, c_offset, LD_OFFSET_MAX, vin1, c_offset);
        }   // for(i = 0; i < g_num_vecs_in; i++) ends
