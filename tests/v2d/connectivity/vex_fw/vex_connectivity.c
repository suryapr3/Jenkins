 ...
 *********************************************************************************************/
/**
 * @file    vex_connectivity.c
 * @brief   Vex FW code to validate V2D Path A.
 * @details 1. VEX will send the INITIAL_BLOCK_SIZE blocks to the HOST.
 *          2. VEX will be waiting on SDF to receive the data from HOST.
 ...
 ***********************************************/
vec32i32 AT(SRC_PATTERN_ADDR) vbuffer[VEX_IN_BUF_SIZE];
vec32i32 AT(REF_PATTERN_ADDR) vbuffer_ref[VEX_IN_BUF_SIZE];
...
#if DEBUG
volatile int AT(0x48) g_host_debug;
...
void v2d_connectivity() ENTRY
{
    ...
    while (count < (g_num_blocks))
    {
        ...
        /*ii = 3 would work for P5GSPS rate*/
...
//#pragma hive force_unroll=4
        ...
        for (int i = 0; i < iter_max; i++)
        {
          ...
          int sout0, sout1;
          (vin = vbuffer_ref[i]) NO_ALIAS SYNC_WITH(buffer_read);
