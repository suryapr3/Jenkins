...
int hrt_main(int argc, char **argv)
{
    ...
    hrt_cell_load_program(VE01, "v2d_connectivity");
    hrt_cell_load_program(VE02, "producer_prog");

    // Load the pointer copy address for the SDFGates
    hrtx_memid_set_bpca(VE01, ve32_bp_dmem, HIVE_ADDR_v2d_connectivity_consumer_sm_gate);
    hrtx_memid_set_bpca(VE02, ve32_bp_dmem, HIVE_ADDR_producer_prog_producer_sm_gate);

    hrt_scalar_store(VE01, uint, v2d_connectivity_g_scalar_val, add_const);
    hrt_scalar_store(VE01, uint, v2d_connectivity_g_num_vecs_in, num_vec_input);
    hrt_scalar_store(VE01, uint, v2d_connectivity_g_tbs_in_blk, TBS_IN_BLK);
    hrt_scalar_store(VE01, uint, v2d_connectivity_g_dest_buffer_base, 0x4000);
    hrt_scalar_store(VE01, uint, v2d_connectivity_g_dest_ptr_address,
            (STRM2CIO_0_DMEM + HIVE_ADDR_producer_prog_producer_sm_gate + (PRODUCER_SM_BUF_ID*4)));
    hrt_scalar_store(VE01, uint, v2d_connectivity_g_num_blocks, num_blocks);
    ...
    hrt_scalar_store(VE02, uint, producer_prog_g_dest_ptr_address,
            (STRM2CIO_0_DMEM + HIVE_ADDR_v2d_connectivity_consumer_sm_gate + (CONSUMER_SM_BUF_ID*4)));
    hrt_scalar_store(VE02, uint, producer_prog_g_dest_buffer_base, (STRM2CIO_0_VMEM + HIVE_ADDR_v2d_connectivity_vbuffer_out));
    ...
    for(i = 0; i < num_vec_input; i++)
    {
        for(j = 0; j < NUM_OF_ELEMENTS_PER_VEC; j++)
        {
            ...
            hrt_indexed_store(VE01, int, v2d_connectivity_vbuffer, i * NUM_OF_ELEMENTS_PER_VEC + j,
                              (((i * NUM_OF_ELEMENTS_PER_VEC + j) << 16) |(i * NUM_OF_ELEMENTS_PER_VEC + j)));
            hrt_indexed_store(VE01, int, v2d_connectivity_vbuffer_ref, i * NUM_OF_ELEMENTS_PER_VEC + j,
                              (((i * NUM_OF_ELEMENTS_PER_VEC + j) << 16) |(i * NUM_OF_ELEMENTS_PER_VEC + j)));
        }
    }
    ...
    // Run program
    hrt_cell_start_function(VE01, v2d_connectivity_v2d_connectivity);
    ...
    int vex_compare_result = hrt_scalar_load(VE01, int, v2d_connectivity_g_compare_result);
