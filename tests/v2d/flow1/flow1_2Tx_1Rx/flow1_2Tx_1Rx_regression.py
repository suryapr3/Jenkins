...
import subprocess
import time
wb = openpyxl.load_workbook("flow1_2Tx_1Rx_list.xlsx")
...
for i in range(2,row_count +1):
    ...
    split_method = wb1.cell(row=i,column=6).value
    drf_clock = wb1.cell(row=i,column=7).value
    sys_rate = wb1.cell(row=i,column=8).value
    subprocess.call(['python flow1_2Tx_1Rx.py '  f'{tx0_vex_num} '  f'{rx0_vex_num} ' f'{tx1_vex_num} ' f'{v2d_num} '  f'{apb_num} '  f'{split_method} ' f'{drf_clock} ' f'{sys_rate} '], shell=True)
