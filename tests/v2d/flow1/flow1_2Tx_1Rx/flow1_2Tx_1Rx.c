 ...
 * @return          status
 */
int configure_v2d_host(int vex_num_param[], int v2d_num, int apb_num, int split_method, int drf_clock, sys_rate_t sys_rate)
{
    int i, j, rv = 0, v2d_stream_cnt0 = 0, v2d_stream_cnt1 = 0,
    ...
    for (i = 0; i< (ARRAY_SIZE(g_v2d_config_tbl) - 6); i++)
    {
 ...
 *                  VEX cores
 * @param [in]      vex0_tx_args, vex1_tx_args, vex0_rx_args, and split_method, sys_rate
 * @return          none
 */
void vex_param_gen (unsigned int *vex0_tx_args, unsigned int *vex1_tx_args,
        unsigned int *vex0_rx_args, int split_method, sys_rate_t sys_rate)
{
    vex0_tx_args[3] = MAX_BLOCKS;
    vex1_tx_args[3] = MAX_BLOCKS;
    vex0_rx_args[2] = MAX_BLOCKS;
    vex0_rx_args[1] = g_flow1_vectors_per_block[sys_rate];
...
/**
 * @brief           Function generating the input vectors to be loaded to
 *                  VEX cores
 * @param [in]      val_inp_rx, val_inp_tx0, val_inp_tx1, num_input_vectors and sys_rate
 * @return          none
 */
void input_vec_gen (unsigned int (*val_inp_rx)[32], unsigned int (*val_inp_tx0)[32], unsigned int (*val_inp_tx1)[32],
        unsigned int vex0_num_input_vectors, sys_rate_t sys_rate)
{
    ...
    for (i = 0; i < g_flow1_vectors_per_block[sys_rate]; i++)
    {
        ...
        {
            val_inp_rx[i][j] = ((i*NUM_OF_ELEMENTS_PER_VEC + j) << 16) |(i * NUM_OF_ELEMENTS_PER_VEC + j);
...
int main(int argc, const char *argv[])
{
    ...
    unsigned int val_inp_rx[NUMBER_OF_VECTORS_INPUT][NUM_OF_ELEMENTS_PER_VEC] = {0};
    ...
    unsigned int add_val[1], vex0_tx_timer_value, vex1_tx_timer_value, vex0_tx_max_blocks, vex1_tx_max_blocks;
    ...
    printf ("INFO: argv[7] : %d = sys_rate %d\n", sys_rate);

    add_val[0] = vex0_tx_args[0];

    vex_param_gen(vex0_tx_args, vex1_tx_args, vex0_rx_args, split_method, sys_rate);
    ...
    input_vec_gen(val_inp_rx, val_inp_tx0, val_inp_tx1, vex0_tx_args[1], sys_rate);
   ...
   /*Rx Vex Core*/
    ...
    printf ("processed blocks  by vex %d 0x%x\n", (vex_num + 1), host_blocks_processed - add_val[0]);

    if (0xFFFFFFFF != host_result)
    {
        printf ("ERROR: Received output didn't match with Reference output TV for vex %d\n", (vex_num + 1));
        test_status = 1;
    }
    else
    {
        printf("Received output matches with Reference ouput\n");
    }
    ...
    if (0 == test_status)
    {
        printf("Host:---- SUCCESS ---- tx0_vex_num : %d, rx_vex_num : %d, tx1_vex_num : %d, v2d_num : %d, apb_num : %d, drf_clock: %s, \
                sys_rate: %s, split_method : %s\n",
                vex_num_param[0], vex_num_param[1], vex_num_param[2], v2d_num, apb_num, drf_clock_str[drf_clock], sys_rate_str[sys_rate],
                split_method_str[split_method]);
    }
    else
    {
        printf("Host:---- FAIL ---- tx0_vex_num : %d, rx_vex_num : %d, tx1_vex_num : %d, v2d_num : %d, apb_num : %d, drf_clock: %s, \
                sys_rate: %s, split_method : %s\n",
                vex_num_param[0], vex_num_param[1], vex_num_param[2], v2d_num, apb_num, drf_clock_str[drf_clock], sys_rate_str[sys_rate],
                split_method_str[split_method]);
        rv = 1;
