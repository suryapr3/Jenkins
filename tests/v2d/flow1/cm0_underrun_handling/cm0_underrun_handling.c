 ...
 *********************************************************************************************/
/**
 * @file    cm0_underrun_handling.c
 * @brief   HOST Configuration code for evaluating the underrun scenario for Common Moudle 0.
...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, sys_rate_t sys_rate)
{
    ...
    int fill_level_value = (g_initial_block_size) * g_flow1_vectors_per_block[sys_rate];
    printf("fill level value is 0x%x", fill_level_value);
#endif
...
int main(int argc, const char *argv[])
{
    ...
    unsigned val_kargs[] = {0, NUMBER_OF_VECTORS_INPUT, 0, 0, 3, (CYCLES_PER_BLOCK_983M - 1)};
    ...
    printf ("INFO: argv[5] : %d = sys_rate %s", sys_rate, sys_rate_str[sys_rate]);

    add_val[0] = val_kargs[0];
    val_kargs[3] = MAX_BLOCKS;
    val_kargs[1] = g_flow1_vectors_per_block[sys_rate];
    val_kargs[5] = g_initial_block_size;
    ...
    while ((b2h_flit_cnt += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_B2H_CSR_FLIT_CNT_OFFSET, "B2H_FLIT_COUNT")) <
            (g_initial_block_size * (g_flow1_vectors_per_block[sys_rate] * 2)))
    {
        // do nothing wait until 1.5 blocks are received in this loop
        if (loop_wait_count >= 30)
        {
            printf("WARNING: Breaking the loop while B2H_FLIT_CNT 0x%x is not the required number of flits", b2h_flit_cnt);
            break;
        }
    ...
    while ((underrun_reg_value = read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + (V2D_CSR_OFFSET + AV2D_CSR_INT_CSR_INT_STAT_OFFSET), \
                    "V2D_INT_STATUS")) != TX_UNDERRUN_VALUE)
    {
        if (loop_wait_count >= 80)
        {
            printf("WARNING: Breaking the loop while underrun interrupt 0x%x was not set", underrun_reg_value);
            break;
        }
        loop_wait_count ++;
        usleep(10000);
    ...
    read_mem_18a(VEX_VMEM(vex_num_param[1]) + HIVE_ADDR_v2d_flow1_rx_p5_8iq_vbuffer_out,/*local_addr*/
                        sizeof(underrun_sample), underrun_sample,
                        "RX_VEX_VMEM");
    ...
    if ((buf_status = read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_CSR_OFFSET + AV2D_CSR_TX_CSR_TX_BUF_STAT_OFFSET, "V2D_TX_BUF_STATUS")) & 0x1)
    {
        printf("ERROR: Buffer was full");
        test_status = 1;
    }

    /*Read the underrun Sample*/
   read_mem_18a(VEX_VMEM(vex_num_param[1]) + HIVE_ADDR_v2d_flow1_rx_p5_8iq_vbuffer_out,/*local_addr*/
                        sizeof(underrun_sample_new), underrun_sample_new,
                        "RX_VEX_VMEM");
