...
{
    int i,rv = 0;
    int v2d_buffer_a = 0, tx_stream_cnt_value = 0, rx_stream_cnt_value = 0;
    ...
    for (; i <  (ARRAY_SIZE(g_cm_0_config_tbl) - 3); i++)
    {
        ...
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm_0_config_tbl[i].offset), rx_stream_cnt_value, \
                g_cm_0_config_tbl[i].mask, g_cm_0_config_tbl[i].name))
            return rv;
    }
    ...
    {
       if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_config_tbl[i].offset), (AV2D_CSR_TX_CSR_TX_STRM_CNT_STRM_EN_BF_MSK | rx_stream_cnt_value), \
...
int main(int argc, const char *argv[])
{
    ...
    if (us_granularity_required)
    {
        ...
        vex_rx_val_kargs[1] = 2 * g_flow1_vectors_per_block[sys_rate];
        val_kargs[1] = 2 * g_flow1_vectors_per_block[sys_rate];

        /*max blocks has to be divide by 2*/
        vex_rx_val_kargs[2] = (MAX_BLOCKS) ;
        val_kargs[3] = vex_rx_val_kargs[2] ;
