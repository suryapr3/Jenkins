...
{
    int i, rv = 0, fill_value = 0, buffer_a_lower_bound = 0, buffer_a_upper_bound = 0, buffer_a = 0;

    buffer_a_lower_bound = V2D_BUFFER1_OFFSET - VEC2FLIT(g_flow1_vectors_per_block[sys_rate]);
    buffer_a_upper_bound = buffer_a_lower_bound + (g_num_blocks_v2d_buffer * VEC2FLIT(g_flow1_vectors_per_block[sys_rate]));
    buffer_a = ((buffer_a_upper_bound << AV2D_CSR_TX_CSR_TX_PTR_BOUND_AC_UPPER_BOUND_BF_OFF) | buffer_a_lower_bound);
    printf("Buffer A: 0x%x", buffer_a);
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_config_tbl[i].offset), buffer_a, g_v2d_config_tbl[i].mask, \
...
int main(int argc, const char *argv[])
{
    ...
    int offset_val[4] = {HIVE_ADDR_v2d_flow1_tx_p25_4iq_g_scalar_val, HIVE_ADDR_v2d_flow1_tx_p25_4iq_g_num_vecs_in,
                         HIVE_ADDR_v2d_flow1_tx_p25_4iq_g_dest_buffer_base, HIVE_ADDR_v2d_flow1_tx_p25_4iq_g_num_blocks};
    unsigned int val_kargs[4] = {0, NUMBER_OF_VECTORS_INPUT, 0, 0};
