 ...
 * @details 1. This TC validates the PDPD functionality with extra terms enabled in MODE 1
   ...
   @TC Description:
        ...
        10. The above can be used to verify whether V2D is getting underrun before the Tx VEX stop transmitting
            the data.
        11. The V2D buffers of B and C are configured such that , B buffer is alway at the higher 16K end and C Buffer is
            always at higher end of the lower 16K memory so that these two will be continuous.
...
#define TEST_VECTORS "../../flow5/test_vectors/"
/***********************************************
...
// H2B Registers
    ...
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL0_OFFSET, NSIP_HBI_H2B_ADDR_LKUP_TBL_VALID_BF_MSK, NSIP_HBI_H2B_ADDR_LKUP_TBL_WR_MASK, \
        "H2B_ADDR_LKUP_TBL0"},
...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, int test_config)
{
    ...
    buffer_low = V2D_BUFFER1_OFFSET - ((g_num_blocks_v2d_buffer) * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_out_rate]));
    buffer_high = buffer_low + ((g_num_blocks_v2d_buffer * (g_vectors_per_block[drf_clock][pdpd_out_rate])) - 1);
    v2d_buffer_c0 = ((buffer_high << AV2D_CSR_TX_CSR_TX_PD_PTR_BOUND_B_UPPER_BOUND_BF_OFF) | buffer_low);
    v2d_buffer_c1 = (((v2d_buffer_c0) >> 16) + 1 |
                    ((v2d_buffer_c0 + ((g_num_blocks_v2d_buffer) * (g_vectors_per_block[drf_clock][pdpd_out_rate] * 2)) - 1) << 16));
    ...
    printf ("Buffer b: %x, Buffer C0: %x, Buffer C1: %x", v2d_buffer_b, v2d_buffer_c0, v2d_buffer_c1);
    ...
    printf("fill_level_value : %x", fill_level_value);
#endif
    ...
    for (; i< (ARRAY_SIZE(g_cm_0_config_tbl) - 4); i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm_0_config_tbl[i].offset), \
                    (g_cm_0_config_tbl[i].value | HIVE_ADDR_v2d_flow5_rx_vbuffer_out), g_cm_0_config_tbl[i].mask, g_cm_0_config_tbl[i].name))
            return rv;
    }
...
int main(int argc, const char *argv[])
{
    ...
    unsigned int vex_tx_b_val_kargs[] = {0x0, NUMBER_OF_VECTORS_INPUT, 0, 0x10000, 0, 3, (CYCLES_PER_BLOCK_983M - 1)};
    ...
    printf ("INFO: argv[0] : %d = tx_vex_b", vex_num_param[0]);
    printf ("INFO: argv[1] : %d = tx_vex_c", vex_num_param[1]);
