...
volatile unsigned int AT(0x4) g_dest_buffer_base_c0;
volatile int AT(0x1C) g_dest_buffer_base_c1;
...
volatile int AT(0x18) g_vex_indicator;
volatile int AT(0x24) g_host_cycle_count;
 ...
 * @return          none
 */
void v2d_flow5_tx_c() ENTRY
{
    int i, j = 0, offset =0;
    g_host_debug = 0;
    int dest_address_c0 = g_dest_buffer_base_c0;
    int dest_address_c1 = g_dest_buffer_base_c1;
...
#endif
    init_timer(((g_host_cycle_count + 1) / 2) - 1);
    ...
    for (j = 0; j < g_initial_block_size; j++)
    {
        ...
        for(i = 0; i < iter_max; i++)
        {
            ...
            v_xstomi_1024(Any, dest_address_c0, offset, LD_OFFSET_MAX, vout, offset);
    ...
    for (j = 0; j < g_initial_block_size; j++)
    {
        ...
        for(i = 0; i < iter_max; i++)
        {
            ...
            //v_sto_1024(Any,vbuffer_base,i*128,vout);
            //v_xsto_1024(Any,vbuffer_base,i,vout);
            v_xstomi_1024(Any, dest_address_c1, offset, LD_OFFSET_MAX, vout, offset);
        }   // for(i = 0; i < g_num_vecs_in; i++) ends
    }   // for (j=0; j < 3; j++) ends
#endif

    while (g_vex_indicator == 0)
    {
    ...
    while(g_host_indicator != 3)
    {
        vec32i32 vin2;
        int ld_offset = 0;
        int ld_offset1 = (iter_max / 2) * BYTES_PER_VEC;
        add_offset = add_offset + g_scalar_val;
        V_CLONE_32(add_offset, vin2);
...
//#pragma hive force_unroll=4
        ...
        for(i = 0; i < (iter_max / 4); i++)
        {
            ...
            //v_sto_1024(Any,vbuffer_base,i*128,vout);
            //v_xsto_1024(Any,vbuffer_base,i,vout);
            v_xstomi_1024x2(Any, dest_address_c0, offset, LD_OFFSET_MAX, vout0, vout1, offset);
        }   // for(i = 0; i < g_num_vecs_in; i++) ends
...
//#pragma hive force_unroll=4
        ...
        for(i = 0; i < (iter_max / 4); i++)
        {
            vec32i32 vout0, vout1, vdata0, vdata1;
            //vin1= vbuffer[i];
            //v_ldoi_2048(Any, vbuffer, ld_offset1, vdata0, vdata1, ld_offset1);
            v_ldoi_2048(Any, vbuffer, ld_offset, vdata0, vdata1, ld_offset);
#if DEBUG
            OP___dump(__LINE__, i);
            OP___dump(__LINE__, vin1);
            ...
            //v_sto_1024(Any,vbuffer_base,i*128,vout);
            //v_xsto_1024(Any,vbuffer_base,i,vout);
            v_xstomi_1024x2(Any, dest_address_c1, offset, LD_OFFSET_MAX, vout0, vout1, offset);
        }   // for(i = 0; i < g_num_vecs_in; i++) ends
