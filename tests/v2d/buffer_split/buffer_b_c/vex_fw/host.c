 ...
 * @details 1. VEX00 sends the data (3 blocks) to VEX02 and sends an indication to the VE03.
...
#define VE03 abc_vex40_vex_0_3_ve32
...
//Range1
    ...
    { BASE_VE00_CIO2STRM + 0x0C0,0x000FFFF },//MEM_ADDR_HIGH[0]
...
//Range2
    { BASE_VE00_CIO2STRM + 0x084,0x00010000 },//MEM_ADDR_LOW[0] - [31-17] destination ID,[16-0] bits local address as seen from STRM2CIO
    { BASE_VE00_CIO2STRM + 0x0C4,0x0001FFFF },//MEM_ADDR_HIGH[0]
    { BASE_VE00_CIO2STRM + 0x104,0x00000000 },//MEM_OFFSET[0]- is in words
    { BASE_VE00_CIO2STRM + 0x144,0x00000003 },//STRM_DEST[0] - mapped to VE02
...
//Range1
    { BASE_VE03_CIO2STRM + 0x080,0x00000000 },//MEM_ADDR_LOW[0] - [31-17] destination ID,[16-0] bits local address as seen from STRM2CIO
    { BASE_VE03_CIO2STRM + 0x0C0,0x0001FFFF },//MEM_ADDR_HIGH[0]
    { BASE_VE03_CIO2STRM + 0x100,0x00000000 },//MEM_OFFSET[0]- is in words
    { BASE_VE03_CIO2STRM + 0x140,0x00000000 },//STRM_DEST[0] - mapped to VE01
...
int hrt_main(int argc, char **argv)
{
    ...
    hrt_cell_load_program(VE00, "v2d_flow5_tx_b");
    hrt_cell_load_program(VE01, "v2d_flow5_rx");
    hrt_cell_load_program(VE02, "producer_prog");
    hrt_cell_load_program(VE03, "v2d_flow5_tx_c");

    // Load the pointer copy address for the SDFGates
    hrtx_memid_set_bpca(VE01, ve32_bp_dmem, HIVE_ADDR_v2d_flow5_rx_consumer_sm_gate);
    hrtx_memid_set_bpca(VE02, ve32_bp_dmem, HIVE_ADDR_producer_prog_producer_sm_gate);

    hrt_scalar_store(VE00, uint, v2d_flow5_tx_b_g_scalar_val, add_const);
    hrt_scalar_store(VE00, uint, v2d_flow5_tx_b_g_num_vecs_in, num_vec_input);
    hrt_scalar_store(VE00, uint, v2d_flow5_tx_b_g_dest_buffer_base, 0x4000);
    hrt_scalar_store(VE00, uint, v2d_flow5_tx_b_g_vex_indication_address, STRM2CIO_0_DMEM + HIVE_ADDR_v2d_flow5_tx_c_g_vex_indicator);
    hrt_scalar_store(VE00, uint, v2d_flow5_tx_b_g_num_blocks, 3);
    hrt_scalar_store(VE00, uint, v2d_flow5_tx_b_g_initial_block_size, 0);
    hrt_scalar_store(VE00, uint, v2d_flow5_tx_b_g_host_indicator, 1);
    hrt_scalar_store(VE00, uint, v2d_flow5_tx_b_g_host_cycle_count, 511);

    hrt_scalar_store(VE01, uint, v2d_flow5_rx_g_scalar_val, add_const);
    hrt_scalar_store(VE01, uint, v2d_flow5_rx_g_num_vecs_in, num_vec_input);
    hrt_scalar_store(VE01, uint, v2d_flow5_rx_g_tbs_in_blk, TBS_IN_BLK);
    hrt_scalar_store(VE01, uint, v2d_flow5_rx_g_dest_ptr_address,
            (STRM2CIO_0_DMEM + HIVE_ADDR_producer_prog_producer_sm_gate + (PRODUCER_SM_BUF_ID*4)));
    hrt_scalar_store(VE01, uint, v2d_flow5_rx_g_num_blocks, num_blocks);
    ...
    hrt_scalar_store(VE02, uint, producer_prog_g_dest_ptr_address,
            (STRM2CIO_0_DMEM + HIVE_ADDR_v2d_flow5_rx_consumer_sm_gate + (CONSUMER_SM_BUF_ID*4)));
    hrt_scalar_store(VE02, uint, producer_prog_g_dest_buffer_base, (STRM2CIO_0_VMEM + HIVE_ADDR_v2d_flow5_rx_vbuffer_out));

    hrt_scalar_store(VE03, uint, v2d_flow5_tx_c_g_scalar_val, add_const);
    hrt_scalar_store(VE03, uint, v2d_flow5_tx_c_g_num_vecs_in, num_vec_input);
    hrt_scalar_store(VE03, uint, v2d_flow5_tx_c_g_dest_buffer_base_c0, 0x4000);
    hrt_scalar_store(VE03, uint, v2d_flow5_tx_c_g_dest_buffer_base_c1, 0x8000);
    hrt_scalar_store(VE03, uint, v2d_flow5_tx_c_g_num_blocks, 3);
    hrt_scalar_store(VE03, uint, v2d_flow5_tx_c_g_initial_block_size, 0);
    hrt_scalar_store(VE00, uint, v2d_flow5_tx_c_g_host_cycle_count, 511);
    ...
    for(i = 0; i < num_vec_input; i++)
    {
        for(j = 0; j < NUM_OF_ELEMENTS_PER_VEC; j++)
        {
            ...
            hrt_indexed_store(VE01, int, v2d_flow5_rx_vbuffer, i * NUM_OF_ELEMENTS_PER_VEC + j,
                              (((i * NUM_OF_ELEMENTS_PER_VEC + j) << 16) |(i * NUM_OF_ELEMENTS_PER_VEC + j)));
            hrt_indexed_store(VE00, int, v2d_flow5_tx_b_vbuffer, i * NUM_OF_ELEMENTS_PER_VEC + j,
                              (((i * NUM_OF_ELEMENTS_PER_VEC + j) << 16) |(i * NUM_OF_ELEMENTS_PER_VEC + j)));
            hrt_indexed_store(VE03, int, v2d_flow5_tx_c_vbuffer, i * NUM_OF_ELEMENTS_PER_VEC + j,
                              (((i * NUM_OF_ELEMENTS_PER_VEC + j) << 16) |(i * NUM_OF_ELEMENTS_PER_VEC + j)));
        }
    }
    ...
    // Run program
    hrt_cell_start_function(VE03, v2d_flow5_tx_c_v2d_flow5_tx_c);
    hrt_cell_start_function(VE00, v2d_flow5_tx_b_v2d_flow5_tx_b);
    printf ("abc_flow1 is started \n");
    hrt_cell_start_function(VE01, v2d_flow5_rx_v2d_flow5_rx);
    ...
    int vex_compare_result = hrt_scalar_load(VE01, int, v2d_flow5_rx_g_compare_result);
...
#if 1
    // Load output from VEX C
    int vex_indicator = hrt_scalar_load(VE03, int , v2d_flow5_tx_c_g_vex_indicator);
    if (vex_indicator != 1)
    {
        printf("vex indicator value is wrong: %d\n", vex_indicator);
        exit_status = 1;
    }
    ...
    for (j = 0; j < 8 * 3; j++)
    {
        for (i = 0; i < 32; i++)
        {
            elem = hrt_indexed_load(VE02, int, producer_prog_vbuffer_out, ((j * 32) + i));
            printf("%08x ,", elem);
        }
        printf("\n");
    }
