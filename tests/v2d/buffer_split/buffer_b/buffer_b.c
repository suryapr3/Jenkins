 ...
 * @details 1. Configuration of VEX, V2D and Common Module for sending data from VEX to V2D via B path
   ...
   @TC Description:
        ...
        8. V2D memory will be configured for 3 times of the block size of corresponding
           sample rate such that Buffer will be spread across Lower and higher halves.
...
#define TEST_VECTORS "../../flow2/test_vectors/"
#define BUFFER_SPLIT_TX_GNRL_CSR_VALUE  (1 << 23)
/***********************************************
...
{
    int i, rv = 0, v2d_buffer_b = 0, buffer_b_lower_bound = 0, buffer_b_upper_bound = 0, pdpd_in_rate = 0, pdpd_out_rate = 0;
    ...
    buffer_b_lower_bound = V2D_BUFFER1_OFFSET - VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_in_rate]);
    buffer_b_upper_bound = buffer_b_lower_bound + (g_num_blocks_v2d_buffer * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_in_rate]));
    v2d_buffer_b = ((buffer_b_upper_bound << AV2D_CSR_TX_CSR_TX_PD_PTR_BOUND_B_UPPER_BOUND_BF_OFF) | buffer_b_lower_bound);
    printf("Buffer B: 0x%x", v2d_buffer_b);
    ...
    for (; i < (ARRAY_SIZE(g_cm_0_config_tbl) - 4); i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm_0_config_tbl[i].offset), \
                (g_cm_0_config_tbl[i].value | HIVE_ADDR_v2d_flow2_rx_vbuffer_out),g_cm_0_config_tbl[i].mask, g_cm_0_config_tbl[i].name))
            return rv;
    }
    ...
    if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_CSR_OFFSET + AV2D_CSR_TX_CSR_TX_GNRL_CSR_OFFSET), BUFFER_SPLIT_TX_GNRL_CSR_VALUE, \
...
int main(int argc, const char *argv[])
{
    ...
    val_kargs[5] = CYCLES_PER_BLOCK_1P5G - 1;
     ...
     /*enabling the V2D clock for given channel*/
    ...
    while ((b2h_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_1_OFFSET + \
                    NSIP_HBI_B2H_CSR_FLIT_CNT_OFFSET, "B2H_FLIT_CNT")) <
    ...
    while ((h2b_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_0_OFFSET + \
                    NSIP_HBI_H2B_FLIT_CNT_OFFSET, "H2B_FLIT_CNT")) <
    ...
    {
        printf("Host:---- ERROR: FAIL ---- tx_vex_num : %d, rx_vex_num : %d, v2d_num : %d, apb_num : %d, drf_clock: %s, pdpd_in_rate: %s, \
                pdpd_out_rate: %s, lut_mode: %d",
                vex_num_param[0], vex_num_param[1], v2d_num, apb_num, drf_clock_str[drf_clock], pdpd_rate_str[pdpd_in_rate],
                pdpd_rate_str[pdpd_out_rate], g_test_config_tbl[test_config][2]);
        rv = 1;
