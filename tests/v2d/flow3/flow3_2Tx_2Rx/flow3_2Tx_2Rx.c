 ...
 *********************************************************************************************/
/**
 * @file    flow3_2Tx_2Rx.c
 * @brief   HOST Configuration code for evaluating the PDPD block with GMP and DDR terms enabled and
 *          data will be fed by two VEX Cores and received back by two different VEX cores.
 * @details 1. Configuration of VEX, V2D's Common Module for sending data from VEX to V2D via B path
   ...
   @TC Description:
        ...
        3. 8K samples Limited by the storage VMEM on the Rx VEX for storing output TV for given data rate will be streamed
           to V2D Tx from configured VEX cores.
        4. PDPD block was configured with GMP and DDR terms along with passthru gain.
        5. With Rx loop back enabled the data looped back at sample interface
           was expected to reach the two VEX cores in the same split ratio of Tx VEX cores
...
#define V2D_PDPD_GMP_TERMS  2
#define V2D_PDPD_DDR_TERMS  0
...
const char *g_vex_fw_binary[] = {"vex_fw/sim_m64/v2d_flow3_tx.fw", "vex_fw/sim_m64/v2d_flow3_rx.fw", "vex_fw/sim_m64/v2d_flow3_unequal_split_tx1.fw"};
...
// H2B Registers
    ...
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_BUF_SZ_HW_TBL1_OFFSET, (NUM_DATA_BLOCKS - 1), NSIP_HBI_H2B_BUF_SZ_HW_TBL_WR_MASK, \
        "H2B_BUF_SZ_HW_TBL1"},
    /* Data will be written to VEX location 0x10080 , i.e VEX DMEM location 0x80*/
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_TK_ADDR_LKUP_TBL1_OFFSET, (STRM2CIO_2_VEX_DMEM_OFFSET + HIVE_ADDR_v2d_flow3_rx_consumer_sm_gate), \
    ...
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL1_OFFSET, 0x80008000, NSIP_HBI_H2B_ADDR_LKUP_TBL_WR_MASK, "H2B_ADDR_LKUP_TBL1"},
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_MD_CONTROL_TBL0_OFFSET, 0xF, NSIP_HBI_H2B_MD_CONTROL_TBL_WR_MASK, "H2B_MD_CONTROL_TBL0"},
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_MD_CONTROL_TBL1_OFFSET, 0xF, NSIP_HBI_H2B_MD_CONTROL_TBL_WR_MASK, "H2B_MD_CONTROL_TBL1"},
...
// B2H Registers
    ...
    {(V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_B2H_CSR_B2H_ADDR_MASK_TBL1_OFFSET), ((2 * B2H_ADD_MAP_B2_OFFSET) - 1), \
...
// Tx Path Settings
    ...
    {V2D_CSR_OFFSET + AV2D_CSR_RX_CSR_RX_PACKET_SZ_OFFSET, RX_PACKET_SZ_VALUE_2STREAM, AV2D_CSR_RX_CSR_RX_PACKET_SZ_WR_MASK, "RX_PACKET_SZ"},
    /************************************************************************************************************/
 ...
 ***********************************************/
int flow3_configure_pdpd(int, int, int, int);
...
int ref_output_read (unsigned int *host_ref0_data, unsigned int *host_ref1_data, int test_config,
        unsigned int vex0_num_input_vectors, unsigned int vex1_num_input_vectors, unsigned int num_blocks);
...
int configure_v2d_host(int vex_num[], int v2d_num, int apb_num, int split_method, int drf_clock, int test_config)
{
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm_0_config_tbl[i].offset), (0x80000000 | HIVE_ADDR_v2d_flow3_rx_vbuffer_out), \
                g_cm_0_config_tbl[i].mask, g_cm_0_config_tbl[i].name))
            return rv;
    }
    ...
    for (i = 0; i < ARRAY_SIZE(g_v2d_flow3_config_tbl) - 8; i++)
    {
    ...
    for (; i < ARRAY_SIZE(g_v2d_flow3_config_tbl) - 7; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow3_config_tbl[i].offset), \
                    g_v2d_flow3_config_tbl[i].value | (drf_clock << 7), g_v2d_flow3_config_tbl[i].mask, g_v2d_flow3_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow3_config_tbl) - 6; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow3_config_tbl[i].offset), \
                    v2d_tx_ptr_bound0, g_v2d_flow3_config_tbl[i].mask, g_v2d_flow3_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow3_config_tbl) - 5; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow3_config_tbl[i].offset), \
                    v2d_tx_ptr_bound1, g_v2d_flow3_config_tbl[i].mask, g_v2d_flow3_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow3_config_tbl) - 4; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow3_config_tbl[i].offset), \
                    (g_v2d_flow3_config_tbl[i].value | (lut_mode << 12) | (pdpd_out_rate << 4) | pdpd_in_rate), g_v2d_flow3_config_tbl[i].mask, \
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow3_config_tbl[i].offset), \
                    (0x80000000 | v2d_tx_stream_cnt0),g_v2d_flow3_config_tbl[i].mask, g_v2d_flow3_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow3_config_tbl) - 2; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow3_config_tbl[i].offset), \
                    (0x80000000 | v2d_rx_stream_cnt0), g_v2d_flow3_config_tbl[i].mask, g_v2d_flow3_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow3_config_tbl) - 1; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow3_config_tbl[i].offset), \
                    (0x80000000 | v2d_tx_stream_cnt1), g_v2d_flow3_config_tbl[i].mask, g_v2d_flow3_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow3_config_tbl); i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow3_config_tbl[i].offset), \
                    (0x80000000 | v2d_rx_stream_cnt1), g_v2d_flow3_config_tbl[i].mask, g_v2d_flow3_config_tbl[i].name))
            return rv;
    }
    ...
    rv = flow3_configure_pdpd(v2d_num, apb_num, test_config, POST_C_CENTER_VALUE_UNITY_GAIN);
 ...
 *                  VEX cores based on the split method.
 * @param [in]      pointers to rx, tx0, tx1 arrays , vex0_input_vectors, vex1 input vectors and max blocks
 * @return          none
 */
int ref_output_read (unsigned int *host_ref0_data, unsigned int *host_ref1_data, int test_config,
        unsigned int vex0_num_input_vectors, unsigned int vex1_num_input_vectors, unsigned int num_blocks)
{
    ...
    int vex0_offset = 0, vex1_offset = 0, lc = 0;
    ...
    for (lc = 0; lc < num_blocks; lc++)
    {

        if (vector_read_hex_u32(fp_ref_output,
                    host_ref0_data + vex0_offset,
                    vex0_num_input_vectors * NUM_OF_ELEMENTS_PER_VEC) < \
                    (int)(vex0_num_input_vectors * NUM_OF_ELEMENTS_PER_VEC))
        {
        ...
        if (vector_read_hex_u32(fp_ref_output,
                    host_ref1_data + vex1_offset,
                    vex1_num_input_vectors * NUM_OF_ELEMENTS_PER_VEC) < \
                    (int)(vex1_num_input_vectors * NUM_OF_ELEMENTS_PER_VEC))
        {
            printf("ERROR: Insufficient Output REF data\n");
            return -1;
        }
        vex0_offset += (vex0_num_input_vectors * NUM_OF_ELEMENTS_PER_VEC);
        vex1_offset += (vex1_num_input_vectors * NUM_OF_ELEMENTS_PER_VEC);
    }
...
int main(int argc, const char *argv[])
{
    ...
    int offset_val[] = {HIVE_ADDR_v2d_flow3_tx_g_scalar_val, HIVE_ADDR_v2d_flow3_tx_g_num_vecs_in,
                         ...
                         HIVE_ADDR_v2d_flow3_tx_g_initial_block_size, HIVE_ADDR_v2d_flow3_tx_g_tx_instance,
                         HIVE_ADDR_v2d_flow3_tx_g_host_cycle_count};
    unsigned int vex0_tx_args[] = {0x00010002, NUMBER_OF_VECTORS_INPUT, 0, 0, 1, 0, (CYCLES_PER_BLOCK_983M - 1)};
    unsigned int vex1_tx_args[] = {0x00010002, NUMBER_OF_VECTORS_INPUT, 0, 0, 1, 1, (CYCLES_PER_BLOCK_983M - 1)};
    int vex_rx_offset_val[] = {HIVE_ADDR_v2d_flow3_rx_g_scalar_val, HIVE_ADDR_v2d_flow3_rx_g_num_vecs_in,
                                ...
                                HIVE_ADDR_v2d_flow3_rx_g_tokens_per_block, HIVE_ADDR_v2d_flow3_rx_g_count1};
    ...
    /* Number of blocks should be equal to max blocks*/
    if (rv = ref_output_read(host_ref0_data, host_ref1_data, test_config, vex0_rx_args[1], vex1_rx_args[1], (vex0_rx_args[2] - 1)))
    {
        return rv;
    }
    ...
    // unequal split Tx0 will be loaded with v2d_flow3_tx.fw and Tx1 will be loaded with
   ...
   /*First Rx Vex Core*/
    if ((rv = srp_vex_memid_store(vex_hdl,
                         vex_num_param[1],
                         SRP_VEX_VMEM,
                         HIVE_ADDR_v2d_flow3_rx_vbuffer,/*local_addr*/
                         host_ref0_data,
   ...
   /*Second Rx Vex Core*/
    ...
    while ((b2h_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_1_OFFSET + \
            ...
            ((g_initial_block_size * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_rate])) + VEC2FLIT(vex0_tx_args[1]/ 2)))
    {
            ...
            (g_max_blocks[drf_clock][pdpd_out_rate] * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_out_rate])))
    {
        ...
        {
            printf("WARNING: Breaking the loop while H2B_FLIT_CNT0x%x is not the required number of flits", h2b_flit_count);
            break;
        }
        loop_wait_count ++;
        usleep(10000);
    ...
    for (vex_cnt = 0; vex_cnt < NUM_RX_VEX_CORES; vex_cnt++)
    {
        ...
        while (host_vec_processed < (g_max_blocks[drf_clock][pdpd_out_rate] * num_vectors_block[vex_cnt]))
        {
            if ((rv = srp_dev_block_read32(srp,
                            SRP_BLOCK_VEX_CORE,
                            vex_num,
                            dmem_offset + HIVE_ADDR_v2d_flow3_rx_g_count1,
                            &host_vec_processed,
                            1,
                            flags)))
                return rv;
            ...
            if (loop_wait_count >= 30)
            {
                printf("ERROR: VEX core %d didn't processed enough vectors0x%x", vex_num, host_vec_processed);
                rv = 1;
                return rv;
            }
            loop_wait_count ++;
        ...
        if ((rv = srp_dev_block_read32(srp,
                        SRP_BLOCK_VEX_CORE,
                        vex_num,
                        dmem_offset + HIVE_ADDR_v2d_flow3_rx_g_compare_result,
                        &host_result,
                        1,
                        flags)))
            return rv;
        ...
        if ((rv = srp_dev_block_read32(srp,
                        SRP_BLOCK_VEX_CORE,
                        vex_num,
                        dmem_offset + HIVE_ADDR_v2d_flow3_rx_g_add_val_out,
        ...
        {
            printf("HOST: Received output matches with Reference output for vex %d", vex_num + 1);
        }
    }
    ...
    if (0 == test_status)
    {
        printf("Host:---- SUCCESS ---- tx0_vex_num : %d, rx0_vex_num: %d, tx1_vex_num : %d, rx1_vex_num : %d, v2d_num : %d, apb_num : %d, \
                    split_method: %s, drf_clock: %s, pdpd_in_rate: %s, pdpd_out_rate: %s, lut_mode: %d",
