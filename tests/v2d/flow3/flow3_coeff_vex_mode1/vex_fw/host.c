...
int hrt_main(int argc, char **argv)
{
    ...
    for (i = 0; i < 48; i++)
    {
        for (j = 0; j < 80; j++)
        {
            hrt_indexed_store(VE00, int, v2d_flow3_gbuffer_mode1_tx_vbuffer, index, (dpd_coeff_buffer[i][j]));
            index++;
    ...
    for (j = 0; j < NUM_OF_ELEMENTS_PER_VEC; j++)
    {
        hrt_indexed_store(VE00, int, v2d_flow3_gbuffer_mode1_tx_vbuffer, j, g_passthru_gain[j]);
    }
