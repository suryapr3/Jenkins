 ...
 * @details 1. Configuration of VEX, V2D's Common Module for sending data from VEX to V2D via B path
   ...
   @TC Description:
        ...
        7. G Buffer and PASSTHRU coeffecients will be written by VEX in MODE1.
...
int main(int argc, const char *argv[])
{
    ...
    unsigned int host_vec_processed = 0, host_blocks_processed = 0, host_flag = 0, host_result = 0, test_status = 0, loop_wait_count = 0;
    ...
    if ((rv = vex_loader(vex_num_param[0], "vex_fw/sim_m64/v2d_flow3_gbuffer_mode1_tx.fw")))
        return rv;
    ...
    write_mem_18a(VEX_VMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow3_gbuffer_mode1_tx_vbuffer,/*local_addr*/
                 sizeof(dpd_coeff_buffer) / sizeof(int),
                 dpd_coeff_buffer[0],
                 "VEX_VMEM");

    /*Loading the PASSTHRU Coeff buffer*/
    write_mem_18a(VEX_VMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow3_gbuffer_mode1_tx_vbuffer + (g_vectors_g_buf_lut_mode[lut_mode] * BYTES_PER_VEC),/*local_addr*/
    ...
    while (2 != host_flag)
    {
        host_flag = read_reg_18a(VEX_DMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow3_gbuffer_mode1_tx_g_host_indicator, "HOST_FLAG");
    ...
    write_mem_18a((VEX_VMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow3_gbuffer_mode1_tx_vbuffer),
    ...
    if ((rv = write_reg_18a(VEX_DMEM(vex_num_param[0])+ HIVE_ADDR_v2d_flow3_gbuffer_mode1_tx_g_host_indicator,
                    host_flag,
                    0xFFFFFFFF,
                    "HOST_INDICATION")))
        return rv;
