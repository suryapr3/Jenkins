 ...
 * @return          none
 */
void v2d_flow3_gbuffer_tx() ENTRY
{
    int i, j = 0, offset =0;
    g_host_debug = 0;
    ...
    int limit = 0, rewind = 0, src_compare_offset = 0;
    g_host_indicator = 2;
#if DEBUG
    ...
    /* for writing the 241st entry of G buffer in mode 2*/
    v_passthru = vbuffer[i];
    v_xsto_512(Any, g_passthru_coeff_base, 0, v_passthru);

    g_host_indicator = 1;

    while(g_host_indicator != 2)
    {
        // wait till the value becomes 2
    }
