...
#include <stdlib.h>
#include "utilities.h"
#include "v2d_flow3_gbuffer_tx.map.h"
...
int hrt_main(int argc, char **argv)
{
    ...
    hrt_cell_load_program(VE00, "v2d_flow3_gbuffer_tx");
    ...
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_tx_g_scalar_val, add_const);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_tx_g_num_vecs_in, num_vec_input);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_tx_g_dest_buffer_base, 0x4000);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_tx_g_num_blocks, 0);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_tx_g_initial_block_size, 1);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_tx_g_dpd_coeff_base, 0xC000);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_tx_g_passthru_coeff_base, 0xFC00);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_tx_g_host_cycle_count, 511);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_tx_g_vectors_g_buf, 120);
    ...
    for(i = 0; i < num_vec_input; i++)
    {
        for(j = 0; j < NUM_OF_ELEMENTS_PER_VEC; j++)
        {
            ...
            hrt_indexed_store(VE00, int, v2d_flow3_gbuffer_tx_vbuffer, i * NUM_OF_ELEMENTS_PER_VEC + j,
                              (((i * NUM_OF_ELEMENTS_PER_VEC + j) << 16) |(i * NUM_OF_ELEMENTS_PER_VEC + j)));
        }
    }
    ...
    for (i = 0; i < 48; i++)
    {
        for (j = 0; j < 80; j++)
        {
            hrt_indexed_store(VE00, int, v2d_flow3_gbuffer_tx_vbuffer, index, (dpd_coeff_buffer[i][j]));
            index++;
    ...
    for (j = 0; j < NUM_OF_ELEMENTS_PER_VEC; j++)
    {
        hrt_indexed_store(VE00, int, v2d_flow3_gbuffer_tx_vbuffer, j, g_passthru_gain[j]);
    }
    ...
    // Run program
    hrt_cell_start_function(VE00, v2d_flow3_gbuffer_tx_v2d_flow3_gbuffer_tx);
    ...
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_tx_g_host_indicator, 0x0);
