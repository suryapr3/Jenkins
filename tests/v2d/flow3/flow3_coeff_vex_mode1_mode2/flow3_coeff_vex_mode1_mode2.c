 ...
 *********************************************************************************************/
/**
 * @file    flow3_coeff_vex_mode1_mode2.c
 * @brief   HOST Configuration code for evaluating the PDPD block with GMP and DDR terms written from VEX core.
 * @details 1. Configuration of VEX, V2D's Common Module for sending data from VEX to V2D via B path
   ...
   @TC Description:
        ...
        2. PDPD block was configured with GMP and DDR terms along with passthru gain , which will be written by VEX core via
           MODE1 and MODE2
...
// B2H Registers
    ...
    // This was for PASSTHRU Coeff update from VEX in MODE 2
    {(V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_B2H_CSR_ADDR_TBL5_OFFSET), (0x80000000 | B2H_MAP_VALUE_PASSTHRU_GAIN), \
        NSIP_HBI_B2H_CSR_ADDR_TBL_WR_MASK, "CM_1_B2H_ADDR_MAP_TBL4"},
    // Mask was set for total 16 flits in Bytes
    {(V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_B2H_CSR_ADDR_MASK_TBL5_OFFSET), 0x3FF, NSIP_HBI_B2H_CSR_ADDR_MASK_TBL_WR, \
        "HBI_1_B2H_ADDR_MASK_TBL0"},
};
...
int ref_output_read(unsigned int *host_ref_data, int test_config)
{
    ...
    sprintf(tref_file, "%s%s", "../test_vectors_1us/", get_ref_file_flow3(PASSTHRU_MODE/*post_filter_mode*/, test_config));
    ...
    if (vector_read_hex_u32(fp_ref_output,
                ...
                (int)(VEX_VMEM_IN_VEC * NUM_OF_ELEMENTS_PER_VEC))
    {
...
int main(int argc, const char *argv[])
{
    ...
    int offset_val[] = {HIVE_ADDR_v2d_flow3_gbuffer_tx_g_scalar_val, HIVE_ADDR_v2d_flow3_gbuffer_tx_g_num_vecs_in,
                         HIVE_ADDR_v2d_flow3_gbuffer_tx_g_dest_buffer_base, HIVE_ADDR_v2d_flow3_gbuffer_tx_g_num_blocks,
                         HIVE_ADDR_v2d_flow3_gbuffer_tx_g_initial_block_size, HIVE_ADDR_v2d_flow3_gbuffer_tx_g_dpd_coeff_base,
                         HIVE_ADDR_v2d_flow3_gbuffer_tx_g_passthru_coeff_base, HIVE_ADDR_v2d_flow3_gbuffer_tx_g_host_cycle_count,
                         HIVE_ADDR_v2d_flow3_gbuffer_tx_g_vectors_g_buf};
    ...
    if ((rv = vex_loader(vex_num_param[0], "vex_fw/sim_m64/v2d_flow3_gbuffer_tx.fw")))
        return rv;
    ...
    ref_output_read(host_ref_data, test_config);
    ...
    write_mem_18a(VEX_VMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow3_gbuffer_tx_vbuffer,/*local_addr*/
                 sizeof(dpd_coeff_buffer) / sizeof(int),
                 dpd_coeff_buffer[0],
                 "VEX_VMEM");

    /*Loading the PASSTHRU Coeff buffer*/
    write_mem_18a(VEX_VMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow3_gbuffer_tx_vbuffer + (g_vectors_g_buf_lut_mode[lut_mode] * BYTES_PER_VEC),/*local_addr*/
                 (NUM_OF_ELEMENTS_PER_VEC),
                 dpd_passthru_buffer_data[0],
                 "VEX_VMEM");

    /*Loading the REF output*/
    ...
    /*Indicate the VEX core to write the PASSTHRU COEFF via MODE 2*/
    host_flag = 0;
    if ((rv = write_reg_18a(VEX_DMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow3_gbuffer_tx_g_host_indicator,
                    host_flag,
                    0xFFFFFFFF,
                    "HOST_INDICATION")))
        return rv;
    ...
    while (1 != host_flag)
    {
        host_flag = read_reg_18a(VEX_DMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow3_gbuffer_tx_g_host_indicator, "HOST_FLAG");
    ...
    if (rv = pdpd_ctrl_reg_config(v2d_num, apb_num, 1/*update_passthru*/, 1/*update_c_lut*/))
    {
    ...
    write_mem_18a((VEX_VMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow3_gbuffer_tx_vbuffer),
    ...
    while ((b2h_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_1_OFFSET + \
            ...
            (G_BUF_SIZE_FLITS + (g_initial_block_size * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_in_rate])) + \
