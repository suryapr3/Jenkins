 ...
 * @details 1. Configuration of VEX, V2D's Common Module for sending data from VEX to V2D via B path
   ...
   @TC Description:
        ...
        2. PDPD block was configured with GMP and DDR terms along with passthru gain , which will be written by RCS AXcore via
           MODE1.
        ...
        7. G Buffer and PASSTHRU coeffecients will be written by RCS in MODE1 with GPDMA transfer.
...
int ref_output_read(unsigned int *host_ref_data, int test_config)
{
    ...
    sprintf(tref_file, "%s%s", "../test_vectors_1us/",get_ref_file_flow3(0, test_config));
...
int main(int argc, const char *argv[])
{
    ...
    int rcs_host_param[7] = {0}; // corresponds to v2d_num, apb_num , numbe of vectors to write and 128 bits for mode2 update
#endif
    ...
    rcs_host_param[2] = g_vectors_g_buf_lut_mode[lut_mode];
    rcs_host_param[3] = dpd_passthru_buffer_data[0][0];
    ...
    write_mem_18a(AX_RW_OFFSET,  ((G_BUF_LUT) * (NUM_WORDS_PER_LUT)), dpd_coeff_buffer[0], "DPD_COEFF_BUFFER");
#endif
    ...
    if ((RCS_DMAC_CH1_INTSTAT_BLK_TFR_DONE_INTSTAT_BF_MSK | RCS_DMAC_CH1_INTSTAT_DMA_TFR_DONE_INTSTAT_BF_MSK) !=
    ...
    if (rv = clear_rcs_dmac_ch1_int(RCS_DMAC_CH1_INTCLRREG_CLR_BLK_TFR_DONE_INTSTAT_BF_MSK |
                RCS_DMAC_CH1_INTCLRREG_CLR_DMA_TFR_DONE_INTSTAT_BF_MSK))
    {
