...
import subprocess
import time
wb = openpyxl.load_workbook("flow5_multi_rate_list.xlsx")
...
for i in range(2,row_count +1):
    ...
    rv = subprocess.call(['python flow5_multi_rate.py '  f'{tx1_vex_num} '  f'{tx2_vex_num} ' f'{rx_vex_num} ' f'{v2d_num} '  f'{apb_num} '  f'{drf_clock} ' f'{test_config} '], shell=True)
    if (1 == rv):
        fail_cnt = fail_cnt + 1;
