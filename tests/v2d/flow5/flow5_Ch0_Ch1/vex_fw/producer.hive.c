...
vec32i32 AT(0x4000) vbuffer_out[128];
unsigned int AT(0x4) g_dest_buffer_base; //
unsigned int AT(0x8) g_dest_ptr_address;
volatile int ALIGNED(128) producer_sm_gate[16]; // read pointer @ producer core  -> consumer core will write here
unsigned int AT(0xC) g_num_vecs_in;
volatile int AT(0x10) g_tbs_in_blk;
volatile int AT(0x14) g_scalar_val;
...
void  producer_prog() ENTRY
{
    ...
    while( count < max_count )//loop untill there are blocks to send
    {
        ...
        vec32i32 vin2;
        add_val = add_val + g_scalar_val;
        V_CLONE_32(add_val, vin2);
        int ld_offset = 0;

        //process_block(idx) ;
        //for (int i =0; i < NUM_VEC_IN_BLK; i++)

        for (int i =0; i < (g_num_vecs_in / 2); i++)
        {
            vec32i32 vout0, vout1, vdata0, vdata1;
            v_ldoi_2048(Any, vbuffer_in, ld_offset, vdata0, vdata1, ld_offset);
            ...
            v_xstomi_1024x2(Any, g_dest_buffer_base, offset, (g_tbs_in_blk * g_num_vecs_in) / 2, vout0, vout1, offset);
            OP___dump(1234, offset);
        }
