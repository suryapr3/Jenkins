...
%for blc_cnt = 0:31  %max blocks this value should be 32 , 16, 8, 4, 2 for p5, 1IQ, 2IQ, 4IQ and 8IQ rates
    ...
    for i = 0:7 % vectors per block this value should be 8, 16, 32, 64, 128 for p5, 1IQ, 2IQ, 4IQ and 8IQ rates
        for j = 0:31
            ...
            xtra_in_i(extra_term_global_input_index) = data_in_i(global_input_index);
            xtra_in_q(extra_term_global_input_index) = data_in_q(global_input_index);
...
%end
    ...
    % same element at the end of first block will be repeated for C path
    for i = 0:7 % vectors per block this value should be 8, 16, 32, 64, 128 for p5, 1IQ, 2IQ, 4IQ and 8IQ rates
        for j = 0:31
            xtra_in_i(extra_term_global_input_index) = xtra_in_i(global_input_index - 1);
            xtra_in_q(extra_term_global_input_index) = xtra_in_q(global_input_index - 1);
            extra_term_global_input_index = extra_term_global_input_index + 1;
        end
    end
...
for blc_cnt = 0:1  %max blocks this value should be 32 , 16, 8, 4, 2 for p5, 1IQ, 2IQ, 4IQ and 8IQ rates
    for i = 0:7    % vectors per block this value should be 8, 16, 32, 64, 128 for p5, 1IQ, 2IQ, 4IQ and 8IQ rates
        for j = 0:31
