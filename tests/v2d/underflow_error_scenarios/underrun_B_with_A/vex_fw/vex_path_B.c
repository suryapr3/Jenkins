/**********************************************************************************************
 *
 * INTEL CONFIDENTIAL
 *
 * Copyright (C) 2022 Intel Corporation
 *
 * This software and the related documents ("Material") are Intel copyrighted materials,
 * and your use of them is governed by the express license under which they were provided
 * to you ("License"). Unless the License provides otherwise, you may not use, modify,
 * copy, publish, distribute, disclose or transmit this software or the related documents
 * without Intel's prior written permission.
 * This software and the related documents are provided as is, with no express or implied
 * warranties, other than those that are expressly stated in the License.
 * No license under any patent, copyright, trade secret or other intellectual property
 * right is granted to or conferred upon you by disclosure or delivery of the Materials,
 * either expressly, by implication, inducement, estoppel or otherwise. Any license under
 * such intellectual property rights must be expressed and approved by Intel in writing.
 *
 *********************************************************************************************/
/**
 * @file    vex_path_B.c
 * @brief   Vex FW code to validate V2D Path A.
 * @details 1. VEX will send the 1 block of data for given sample rate to HOST.
            2. VEX will be waiting on SDF to receive back the transmitted sample.
            3. VEX will do the comparison with the generated REF output and stores the result.
            4. Once all the blocks were received the result will be stored to a predefined location
               for the host to read.
            5. VEX will be waiting on indication from HOST to start continuous transmission once the
               indication was received VEX will start continuous transmission.
            6. With host indication VEX will stop the transmission and store the number of blocks
               it transferred at a pre-defined location
            4. This code is specific to sample rates 0.5IQGSPS to 4IQ GSPS
**/

/***********************************************
 * LOCAL INCLUDE STATEMENTS                    *
 ***********************************************/
#include <common_macros.h>
#include "ve32_appl_target.h"
#include <hive/support.h>
#include "cir_buf.hive.h"
#include "defines.h"

/***********************************************
 * LOCAL/PRIVATE MACROS AND DEFINES            *
 ***********************************************/

/***********************************************
 * LOCAL/PRIVATE TYPEDEFS (structs, enums)     *
 ***********************************************/

/***********************************************
 * GLOBAL DATA DEFINITIONS (consts, vars)      *
 ***********************************************/
vec32i32 AT(0) vbuffer[VEX_IN_BUF_SIZE];
vec32i32 AT(0x4000) vbuffer_out[VEX_DEST_BUF_SIZE];
volatile int ALIGNED(128) consumer_sm_gate[16]; // write pointer @ consumer core -> producer core will write here
volatile unsigned int AT(0x4) g_dest_ptr_address;
volatile int AT(0x8) g_scalar_val;
volatile int AT(0xC) g_num_vecs_in;
volatile int AT (0x10) g_num_blocks;
volatile int AT(0x14) g_tbs_in_blk;
volatile int AT(0x18) g_host_indicator;
volatile int AT(0x30) g_count1;
volatile int AT (0x34) g_compare_result;
volatile int AT(0x38) g_add_val_out;
volatile unsigned int AT(0x3C) g_dest_buffer_base;
volatile int AT(0x40) g_blocks_sent;
volatile int AT(TIMER_DEBUG_LOCATION) g_timer_debug;
#if DEBUG
volatile int AT(0x44) g_host_debug;
volatile int AT(0x48) g_host_debug1;
volatile int AT(0x4c) g_fail_cnt;
#endif

/***********************************************
 * STATIC FUNCTION PROTOTYPES                  *
 ***********************************************/

/***********************************************
 * GLOBAL FUNCTION DEFINITIONS                 *
 ***********************************************/
inline void init_timer(int cycles)
{
    // set reg_timer_tmdcfgr0, continuous watchdog
    s_xsto_32(Any, 0, TIMER_TMDCFGR0, TMD_CONTINUOUS_WATCHDOG);
    // set reg_timer_tmdinitr0, set the timeout val
    s_xsto_32(Any, 0, TIMER_TMDINITR0, cycles);
    // set reg_timer_tmdwdnotr0, enable notif
    s_xsto_32(Any, 0, TIMER_TMDWDNOTR0, 0x1);
    // set reg_timer_tmdwdaddrr0, 0x4000 is mapped to DRAM start
    s_xsto_32(Any, 0, TIMER_TMDWDADDRR0, 0x4000 + TIMER_DEBUG_LOCATION);
    // set reg_timer_tmdwddatar0, data will be written to above address
    s_xsto_32(Any, 0, TIMER_TMDWDDATAR0, 0xdeadbeef);
}


/**
 * @brief           Function comparing the blocks received from HOST
 * @param [in]      none
 * @return          none
 */
void v2d_path_b() ENTRY
{
    int sm_ch = CONSUMER_SM_BUF_ID;
    int vex_dest_buffer = 0x4000;
    int compare_offset = 0x0;
    int vec_processed = 0;
    int add_offset = 0;
    int blocks_sent = 0;
    g_compare_result = 0xFFFFFFFF;
    g_add_val_out = g_scalar_val;
    g_blocks_sent = 0;
    //g_host_indicator = 0;
#if DEBUG
    g_host_debug1 = 0;
    g_host_debug = 0;
    g_fail_cnt = 0;
#endif
    init_timer(450);
    semaphore_gate_init();
    /*-------------------------------------------------------------------------*/
    /* Initialize consumer(Buffer):                                            */
    /* g_tbs_in_blk :token buffer size for buffer                                */
    /* consumer_token_per_request : tokens processed per request and ready     */
    /* IS_CONS : the channel is configured as consumer                         */
    /* 0 intial value for local pointer @ consumer                             */
    /* sm_ch - index of Semaphore used                                         */
    /*-------------------------------------------------------------------------*/

    circ_buff_init( IS_CONS, g_tbs_in_blk, CONSUMER_TOKEN_PER_REQUEST, 0, sm_ch);
#if DEBUG
    OP___dump(__LINE__, g_scalar_val);
    OP___dump(__LINE__, g_num_vecs_in);
#endif


    int count = 0, add_temp = 0, result = 0xFFFFFFFF, j = 0, offset = 0, i = 0;
    add_temp = g_add_val_out;

    for (j = 0; j < INITIAL_BLOCK_SIZE; j++)
    {
        vec32i32 vin2 UNINIT;
        int iter_max = g_num_vecs_in;
        add_offset = add_offset + g_scalar_val;
        V_CLONE_32(add_offset, vin2);
        for(i = 0; i < iter_max; i++)
        {
            vec32i32 vin1, vout;
            vin1= vbuffer[i];
#if DEBUG
            OP___dump(__LINE__, i);
            OP___dump(__LINE__, vin1);
#endif
            V_ADD_32(vin1, vin2, vout);

            //v_sto_1024(Any,vbuffer_base,i*128,vout);
            //v_xsto_1024(Any,vbuffer_base,i,vout);
            v_xstomi_1024(Any, g_dest_buffer_base, offset, LD_OFFSET_MAX, vout, offset);
        }   // for(i = 0; i < g_num_vecs_in; i++) ends
        blocks_sent = blocks_sent + 1;
    }   // for (j=0; j < 3; j++) ends

    while (count < (g_num_blocks))
    {

        unsigned int is_data_ready, idx, ptr, iter_max;
        vec32i32 vin2 UNINIT;
        iter_max = g_num_vecs_in;

        V_CLONE_32(add_temp, vin2);
        mc_request( Any, 1, sm_ch, is_data_ready, idx)  SYNC(buffer_read);

        /* for 0.25IQ ii = 8 is needed due to prologue+epilogue is going beyond the
         * required number of iterations for all other rates ii=3 would work and it
         * will give a loop of 3 cycles with the prologue 7 and epilogue 0 cycles*/

#if !DEBUG
#ifdef VOLCANO
#pragma hive pipeline ii=3
//#pragma hive force_unroll=4
#endif
#endif
        for (int i = 0; i < iter_max; i++)
        {
          vec32i32 vin UNINIT, vout UNINIT, vadd UNINIT;

          int sout0, sout1;
          (vin = vbuffer[i]) NO_ALIAS SYNC_WITH(buffer_read);
          V_ADD_32(vin, vin2, vadd);
          v_ldoi_1024_circ(Any, vex_dest_buffer , compare_offset, LD_CTL1, LD_CTL2, vout, compare_offset) NO_ALIAS SYNC_WITH(buffer_read);
          //vout = vbuffer_out[g_count1%256];
          //Bit value of 1 in output indicates corresponding vector elements are equal
          V_EQ_32(vadd, vout, sout0, sout1) SYNC_WITH(buffer_read);
#if DEBUG
          OP___dump(__LINE__, vadd);;
          OP___dump(__LINE__, vout);;
          OP___dump(__LINE__, sout0);
          OP___dump(9008, compare_offset);
#endif
          result = result & (sout0) SYNC_WITH(buffer_read);
#if DEBUG
          OP___dump(__LINE__, result);
          if (result != 0xFFFFFFFF)
          {
              g_fail_cnt = g_fail_cnt + 1;
//              g_host_debug = sout0;
//              g_host_debug1 = i;
                V_GET_32(0,vout,g_host_debug);
                V_GET_32(0,vin,g_host_debug1);
          }
#endif
          vec_processed++;

        }   // for (i = 0; i< num_vecs; i++) ends
        add_temp ++;
        mc_complete( Any, sm_ch, ptr);
#if VEX_HOST_DEBUG
        OP___printstring("vex_flow1: at mc_complete done:\n");
        v_xsto_32(Any, g_dest_ptr_address, 0, ptr) SYNC(buffer_write); // This statement is not required for HOST to VEX communication
#endif
        count++;
#if DEBUG
        OP___dump(__LINE__,count);
#endif
    }   // while (count < g_num_blocks) ends
    g_add_val_out = add_temp;
    g_count1 = vec_processed;
    g_compare_result = result;

    OP___printstring("vex_flow1: Before while loop:\n");
    while (g_host_indicator == 0)
    {
    }
    OP___printstring("vex_flow1: after while loop:\n");

    s_xsto_32(Any, 0, TIMER_TMDCMDR0, TMD_COMMAND_START);           // start the timer
    add_offset = add_offset + g_scalar_val;

    while (1 == g_host_indicator)
    //for (j = INITIAL_BLOCK_SIZE; j < (INITIAL_BLOCK_SIZE + 1); j++)
    {
        vec32i32 vin2 UNINIT;
        int iter_max = g_num_vecs_in;
        V_CLONE_32(add_offset, vin2);

        /*  For all the rate ii=2 would work and it
         * will give a loop of 2 cycles with the prologue 6 and epilogue 1 cycles*/
#ifdef VOLCANO
#pragma hive pipeline ii=2
//#pragma hive force_unroll=4
#endif
        for(i = 0; i < iter_max; i++)
        {
            vec32i32 vin1, vout;
            vin1= vbuffer[i];
#if DEBUG
            OP___dump(__LINE__, i);
            OP___dump(__LINE__, vin1);
#endif
            V_ADD_32(vin1, vin2, vout);

            //v_sto_1024(Any,vbuffer_base,i*128,vout);
            //v_xsto_1024(Any,vbuffer_base,i,vout);
            v_xstomi_1024(Any, g_dest_buffer_base, offset, LD_OFFSET_MAX, vout, offset);
        }   // for(i = 0; i < g_num_vecs_in; i++) ends
        blocks_sent = blocks_sent + 1;
        // This flag will be set by Timer once the configured clockds were elapsed
        while (g_timer_debug != 0xdeadbeef)
        {
        }
        g_timer_debug = 0xdead;
    }   // for (j=0;

    s_xsto_32(Any, 0, TIMER_TMDCMDR0, TMD_COMMAND_STOP);           // stop the timer
    g_blocks_sent = blocks_sent;
#if DEBUG
    OP___printstring( "\n" );
    OP___printstring( "------------core:END------------\n" );
#endif
} // v2d_path_b_p5_8iq() ends
