...
python3 underrun_B_with_A.py <vex_id> <v2d_id> <apb_id> <sys_rate>
...
v2d_id  vex_id
0       0,8,16,24,32 (1,9,17,25,33 are used for Path A Tx)
1       2,10,34 (3,11,35 are used for path A Tx)
2       4,12,36 (5,13,37 are used for path A Tx)
3       6,14,38 (7,15,39 are used for path A Tx)

ex: python3 underrun_B_with_A.py 0 0 0 0
...
The above will generate underrun_B_with_A_list.xlsx file , needs to be done only if the xlsx file is not present or there were any changes done
...
The input combinations will be taken from the underrun_B_with_A_list.xlsx file.
