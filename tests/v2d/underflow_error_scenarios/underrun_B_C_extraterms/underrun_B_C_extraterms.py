...
# Created Date: July 20 2023
# =============================================================================
# test description : this test checks the V2D Buffer B & Buffer C Underflow status
# =============================================================================
# Import statements
# =============================================================================
import sys
...
if arg_len != 6:
    ...
    print("python underrun_B_C_etraterms.py <vex_id1> <vex_id2> <v2d_id> <apb_id> <pdpd_rate> \n")
...
if param[0] == param[1]:
    print("vex_id0 and vex_id1 shouldn't be same\n")
    sys.exit("proper input parameters were not provided\n")

if not (param[2] >= 0 and param[2] <=3):
    print("v2d_id should be between [0,3]\n")
    sys.exit("proper input parameters were not provided\n")

if not (param[3] >= 0 and param[3] <=1):
    print("apb_id should be between [0,1]\n")
    sys.exit("proper input parameters were not provided\n")

if not (param[4] >= 0 and param[4] < 5):
    print("provided pdpd_rate is not with in the range,pdpd_rate should be between 0 to 4\n")
    sys.exit("proper input parameters were not provided\n")

if (param[2] == 0 and not ((param[0] % 8) == 0 and ((param[1] % 8) == 0 or (param[1] % 8) == 1))):
    print("provided inputs doesn't match with the NOC connections\n")
    sys.exit("proper input parameters were not provided\n")
elif ((param[2] == 1 and not ((param[0] % 8) == 2 and ((param[1] % 8) == 2 or (param[1] % 8) == 3)))):
    print("provided inputs doesn't match with the NOC connections\n")
    sys.exit("proper input parameters were not provided\n")
elif (param[2] == 2 and not ((param[0] % 8) == 4 and ((param[1] % 8) == 4 or (param[1] % 8) == 5))):
    print("provided inputs doesn't match with the NOC connections\n")
    sys.exit("proper input parameters were not provided\n")
elif (param[2] == 3 and not ((param[0] % 8) == 6 and ((param[1] % 8) == 6 or (param[1] % 8) == 7))):
    print("provided inputs doesn't match with the NOC connections\n")
    sys.exit("proper input parameters were not provided\n")
else:
