...
/**
 * @file    underrun_B.c
 * @brief   HOST Configuration code for evaluating the underrun behaviour in B path.
...
/* Set Tx_CFG_COMPLETE (23) to 1 , TX_PD_ARB_MODE (8) to 2 and HBI_ARB_MODE (0) to 2*/
#define TX_GNRL_CSR_VALUE   ((1 << 23) | (0x2 << 8) | 0x2)
/* RX_CFG_COMPLETE (0) to 1*/
#define RX_GNRL_CSR_VALUE   (1 << 0)

#define PD_MAIN_UNDERRUN_BIT    18
#define PDPD_MAIN_UNDERRUN_VALUE    (1 << PD_MAIN_UNDERRUN_BIT)
...
// H2B Registers
    ...
    {V2D_AV2D_APB_NSIP_HBI_0_H2B_BUF_SZ_HW_TBL(0,1), 0xA, V2D_AV2D_APB_NSIP_HBI_0_H2B_BUF_SZ_HW_TBL_WR_MASK, "H2B_BUF_SZ_HW_TBL1"},
    /* Data will be written to VEX location 0x4020 * 4 = 0x10080 , i.e VEX DMEM location 0x80*/
    ...
    {V2D_AV2D_APB_NSIP_HBI_0_H2B_MD_CONTROL_TBL(0,1), 0x900, V2D_AV2D_APB_NSIP_HBI_0_H2B_MD_CONTROL_TBL_WR_MASK, "H2B_MD_CONTROL_TBL1"},
    /* Data will be written to VEX location 0x0000 * 4 = 0x0000 , i.e VEX VMEM location 0x0000*/
    {V2D_AV2D_APB_NSIP_HBI_0_H2B_ADDR_LKUP_TBL(0,1), 0x80000000, V2D_AV2D_APB_NSIP_HBI_0_H2B_ADDR_LKUP_TBL_WR_MASK, "H2B_ADDR_LKUP_TBL1"},
    ...
    {V2D_AV2D_APB_NSIP_HBI_0_H2B_MD_CONTROL_TBL(0,2), 0x900, V2D_AV2D_APB_NSIP_HBI_0_H2B_MD_CONTROL_TBL_WR_MASK, "H2B_MD_CONTROL_TBL2"},
    /* Data will be written to VEX location 0x0000 * 4 = 0x0000 , i.e VEX VMEM location 0x0000*/
    ...
    {V2D_AV2D_APB_NSIP_HBI_0_H2B_MD_CONTROL_TBL(0,3), 0x900, V2D_AV2D_APB_NSIP_HBI_0_H2B_MD_CONTROL_TBL_WR_MASK, "H2B_MD_CONTROL_TBL3"},
    /* Data will be written to VEX location 0x0000 * 4 = 0x0000 , i.e VEX VMEM location 0x0000*/
...
// All the offsets are mapped to APB 0 , while writing to the register additional offset will be added if APB1 was used..
// Last Two entries order shouldn't be modified
...
// Tx Path Settings
    ...
    /*B buffer settings.*/
    {V2D_AV2D_APB_CSR_TX_PD_PTR_BOUND(0,0), 0x7FF0000, V2D_AV2D_APB_CSR_TX_PTR_BOUND_WR_MASK, "TX_PD_PTR_BOUND0"},
...
int configure_v2d_host(int vex_num, int v2d_num, int apb_num, pdpd_rate_t pdpd_rate)
{
    ...
    if (rv = write_reg(SRP_BLOCK_VEX_CORE, vex_num, g_cio2strm_config_tbl[i].offset, g_v2d_host_id[v2d_num][apb_num][1], \
            g_cio2strm_config_tbl[i].mask, g_cio2strm_config_tbl[i].name))
        return rv;
    ...
    for (i = 0; i < ARRAY_SIZE(g_v2d_flow2_config_tbl) - 3; i++)
    {
...
int pdpd_config_reg_config(int v2d_num, int apb_num, pdpd_rate_t pdpd_rate)
{
    ...
    */
    ...
    g_v2d_pdpd_config_config_tbl[0].value =  ((pdpd_rate & 0x7) |((ext_ctrl_update_c_lut & 0x1) << 0x3) | ((ext_ctrl_update_nl_lut & 0x1) << 4) | \
                                        ...
                                        ((passthru_dly & 0x3F) << 17) | ((out_shift & 0x1) << 18)| \
...
int main(int argc, const char *argv[])
{
    ...
    int lc, i, j;
    int vex_num = 0, v2d_num, apb_num, underrun_reg_value = 0;
    pdpd_rate_t pdpd_rate;

    int dmem_offset = 0x4000, underrun_sample = 0;
    int offset_val[4] = {HIVE_ADDR_v2d_underrun_B_g_scalar_val, HIVE_ADDR_v2d_underrun_B_g_num_vecs_in ,
                         HIVE_ADDR_v2d_underrun_B_g_dest_buffer_base};
    unsigned int val_kargs[4] = {0x00110014, NUMBER_OF_VECTORS_INPUT, 0};
    ...
    unsigned int nerrors = 0, loop_wait_count = 0;
    ...
    val_kargs[1] = g_vectors_per_block[pdpd_rate];
    ...
    if ((rv = srp_vex_loader(vex_hdl, vex_num, "vex_fw/sim_m64/v2d_underrun_B.fw")))
        return rv;
    ...
    if ((rv = srp_vex_memid_store(vex_hdl,
                        vex_num,
                        SRP_VEX_VMEM,
                        HIVE_ADDR_v2d_underrun_B_vbuffer,/*local_addr*/
                        val_inp[0],
    ...
    while ((b2h_flit_count += read_reg(SRP_BLOCK_V2D, v2d_num, V2D_AV2D_APB_NSIP_HBI_1_B2H_FLIT_COUNT(apb_num), \
                    "B2H_FLIT_COUNT")) < VEC2FLIT(g_vectors_per_block[pdpd_rate])) // 1 blocks of data
    {
    ...
    while ((h2b_flit_count += read_reg(SRP_BLOCK_V2D, v2d_num, V2D_AV2D_APB_NSIP_HBI_0_H2B_FLIT_COUNT(apb_num), \
                    "H2B_FLIT_COUNT")) < VEC2FLIT(g_vectors_per_block[pdpd_rate]))
    {
        // do nothing wait in this loop
    ...
    underrun_reg_value = read_reg(SRP_BLOCK_V2D, v2d_num, V2D_AV2D_APB_CSR_INT_STATUS(apb_num), "V2D_INT_STATUS");
    printf ("underrun_reg_value : %08x\n", underrun_reg_value);

    if ((underrun_reg_value & PDPD_MAIN_UNDERRUN_VALUE))
    {
        printf(" B Buffer Underrun flag was set\n");
    }
    else
    {
    ...
    /* This is as a work around to avoid the FRIO hang issue when trying to read more than 64 vectors from HOST*/
    if (g_vectors_per_block[pdpd_rate] < 64)
    {
        /* Reading the VEX memory from the HOST and comparing with REF*/
        if ((rv = srp_vex_memid_load(vex_hdl,
                            vex_num,
                            SRP_VEX_VMEM,
                            HIVE_ADDR_v2d_underrun_B_vbuffer_out,/*local_addr*/
                            val_out[0],
                            (g_vectors_per_block[pdpd_rate] * NUM_OF_ELEMENTS_PER_VEC * sizeof(int)))))
            return rv;
    }
    ...
    {
        /* Read the first 64 vectors*/
        if ((rv = srp_vex_memid_load(vex_hdl,
                            vex_num,
                            SRP_VEX_VMEM,
                            HIVE_ADDR_v2d_underrun_B_vbuffer_out,/*local_addr*/
                            val_out[0],
                            64 * NUM_OF_ELEMENTS_PER_VEC * sizeof(int))))
            return rv;

        /* Read the remaining vectors*/
        if ((rv = srp_vex_memid_load(vex_hdl,
                            vex_num,
                            SRP_VEX_VMEM,
                            HIVE_ADDR_v2d_underrun_B_vbuffer_out + 0x2000,/*local_addr*/
                            val_out[64],
                            (g_vectors_per_block[pdpd_rate] - 64) * NUM_OF_ELEMENTS_PER_VEC * sizeof(int))))
            return rv;
    }
    ...
    if (vector_read_hex_u32(fp_ref_output,
                host_exp_data,
                g_vectors_per_block[pdpd_rate]* NUM_OF_ELEMENTS_PER_VEC) < (g_vectors_per_block[pdpd_rate]* NUM_OF_ELEMENTS_PER_VEC))
    {
    ...
    nerrors = vector_diff_u32( (uint32_t *)host_exp_data,
                        (uint32_t *)val_out,
                        0,
                        g_vectors_per_block[pdpd_rate]* NUM_OF_ELEMENTS_PER_VEC,
                        0);
    if (nerrors > 0)
    {
        nerrors = vector_diff_u32( (uint32_t *)host_exp_data,
    ...
    if (underrun_sample == host_exp_data[(g_vectors_per_block[pdpd_rate] * NUM_OF_ELEMENTS_PER_VEC) - 1])
    {
    ...
    else
    {
        ...
        printf("Expected %x, streamed %x\n", host_exp_data[(g_vectors_per_block[pdpd_rate] * NUM_OF_ELEMENTS_PER_VEC) - 1], underrun_sample);
        test_status = 1;
    }
