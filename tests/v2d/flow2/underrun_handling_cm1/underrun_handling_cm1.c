 ...
 *********************************************************************************************/
/**
 * @file    underrun_handling_cm1.c
 * @brief   HOST Configuration code for sending the data from VEX to V2D and receving it back.
 * @details 1. This TC validates the unnderrun handling of NSIP_1 instance.
 *          2. TC expects after the underrun flag was set VEX shouldn't be back pressured
 *             and VEX should transfer all the blocks of data.
   @TC Description:
        ...
        4. Initially only g_initial_block_size blocks will be transferred after that BFN will be
           triggered to start the V2D.
        5. Host will wait till the underrun flag was set and once the flag was set host will
           signal VEX to transmit remaining blocks of data.
        6. Host will wait till all the flits of data received in the B2H direction.
        7. This TC expects once the underrun occurs there won't be any recovery evn thouh VEX
           re-started data transfer.
...
int g_initial_block_size = 3;    // This corresponds to 1L and VEX will transfer 1.5L before BFN trigger.
...
/*FILL_LEVEL_TRIGGER flag will be assosiated with corresponding VEX code as well, if we want to disable this flag
    ...
    int fill_level_value = g_initial_block_size * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_in_rate]);
    ...
    for (; i <  (ARRAY_SIZE(g_cm_0_config_tbl) - 3); i++)
    {
        ...
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm_0_config_tbl[i].offset), g_md_control_tbl[drf_clock][pdpd_in_rate], \
                g_cm_0_config_tbl[i].mask, g_cm_0_config_tbl[i].name))
            return rv;
    }
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow2_config_tbl[i].offset), \
                    (g_v2d_flow2_config_tbl[i].value | (lut_mode << 12) | (pdpd_out_rate << 4) | pdpd_in_rate), g_v2d_flow2_config_tbl[i].mask, \
                    g_v2d_flow2_config_tbl[i].name))
            return rv;
    }
...
int main(int argc, const char *argv[])
{
    ...
    unsigned int host_vec_processed = 0, host_blocks_processed = 0, host_flag = 1, host_result = 0, underrun_expected_value = 0,
                 underrun_sample[8], underrun_sample_new[8], test_status = 0, loop_wait_count = 0;
    volatile int buf_status = 0, b2h_flit_count = 0, h2b_flit_count = 0;
    ...
    * whereas in 1.5G TV generation first block has to be excluded by HOST while loading the data*/
    ...
    /*FILL_LEVEL_TRIGGER flag will be assosiated with corresponding VEX code as well*/
#if !FILL_LEVEL_TRIGGER
    usleep(10000);
    ...
    while (((underrun_reg_value = read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_CSR_OFFSET + \
                            AV2D_CSR_INT_CSR_INT_STAT_OFFSET, "V2D_INT_STATUS")) & PDPD_MAIN_UNDERRUN_VALUE) == 0)
    {

        printf ("underrun reg_value : %x\n", underrun_reg_value);
        if (loop_wait_count >= 80)
        {
            printf("WARNING: Breaking the loop while underrun flag was not set\n");
            break;
        }
        loop_wait_count ++;
        usleep(10000);
    }
    printf ("underrun reg_value : 0x%x\n", underrun_reg_value);

    /*Read the underrun Sample*/
    read_mem_18a((VEX_VMEM(vex_num_param[1]) + HIVE_ADDR_v2d_flow2_rx_vbuffer_out),
                  sizeof(underrun_sample) / sizeof (uint32_t),
                  underrun_sample,
                  "underrun_sample");
    ...
    while ((b2h_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_1_OFFSET + \
                    NSIP_HBI_B2H_CSR_FLIT_CNT_OFFSET, "B2H_FLIT_COUNT")) <
            ((g_max_blocks[drf_clock][pdpd_out_rate] * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_in_rate])) + \
             VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_in_rate] / 2)))
    {
        // Wait till all the flits were transmitted
        if (loop_wait_count >= 80)
        {
    ...
    printf("b2h_flit_cnt = 0x%x", b2h_flit_count);

    if ((((buf_status = read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_CSR_OFFSET + AV2D_CSR_TX_CSR_TX_BUF_STAT_OFFSET, \
                            "V2D_TX_BUF_STATUS")) >> TX_B_BUF_FULL_BIT) & 0x1) == 1)
    {
        printf("ERROR: BUF was full");
        test_status = 1;
    }

    /*Read the underrun Sample*/
    read_mem_18a((VEX_VMEM(vex_num_param[1]) + HIVE_ADDR_v2d_flow2_rx_vbuffer_out),
                  sizeof(underrun_sample_new) / sizeof (uint32_t),
                  underrun_sample_new,
                  "underrun_sample_new");
    ...
    if (rv = interrupt_status_check(v2d_num, apb_num,V2D_INTERRUPT_VALUE, 0, 0))
    {
