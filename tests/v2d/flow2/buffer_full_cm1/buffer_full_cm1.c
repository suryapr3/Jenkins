 ...
 *********************************************************************************************/
/**
 * @file    buffer_full_cm1.c
 * @brief   HOST Configuration code for sending the data from VEX to V2D and receving it back.
 * @details 1. This TC validates the Buffer full handling of NSIP_1 instance.
 *          2. TC expects VEX should be auto recoverable once the buffer full scenario ended resulting
 *             the data integrity check at RX VEX core should pass
 * @TC Description:
        ...
        6. Host will be waiting on a memory location where VEX will update the number of vectors
           it was processed.
        7. VEX Tx core will be continuously streaming the data (No waiting for the host indication)
...
// Tx Path Settings
    ...
    //TX_RX_LOOPBACK set
    {V2D_CSR_OFFSET + AV2D_CSR_TX_CSR_TX_SIF_CTRL_OFFSET,AV2D_CSR_TX_CSR_TX_SIF_CTRL_TX_RX_LPBK_BF_MSK, AV2D_CSR_TX_CSR_TX_SIF_CTRL_WR_MASK, "TX_SIF_CTRL"},
...
int main(int argc, const char *argv[])
{
    ...
    usleep(10000);
    while ((((buf_status = read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_CSR_OFFSET + \
                            AV2D_CSR_TX_CSR_TX_BUF_STAT_OFFSET, "V2D_TX_BUF_STATUS")) >> TX_B_BUF_FULL_BIT) & 0x1) != 1)
    {
        // do nothing wait until B0 BUF was FULL
        printf("buf status :%x\n", buf_status);
    ...
    if (0 == test_status)
    {
        printf("Host:---- SUCCESS ---- tx_vex_num : %d, rx_vex_num : %d, v2d_num : %d, apb_num : %d, drf_clock: %s, pdpd_in_rate: %s, pdpd_out_rate: %s, lut_mode: %d\n",
                vex_num_param[0], vex_num_param[1], v2d_num, apb_num, drf_clock_str[drf_clock], pdpd_rate_str[pdpd_in_rate], pdpd_rate_str[pdpd_out_rate], g_test_config_tbl[test_config][2]);
    }
    else
    {
        printf("Host:---- FAIL ---- tx_vex_num : %d, rx_vex_num : %d, v2d_num : %d, apb_num : %d, drf_clock: %s, pdpd_in_rate: %s, pdpd_out_rate: %s, lut_mode: %d\n",
                vex_num_param[0], vex_num_param[1], v2d_num, apb_num, drf_clock_str[drf_clock], pdpd_rate_str[pdpd_in_rate], pdpd_rate_str[pdpd_out_rate], g_test_config_tbl[test_config][2]);
        rv = 1;
    }
