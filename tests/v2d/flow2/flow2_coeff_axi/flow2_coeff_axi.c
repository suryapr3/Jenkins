 ...
 * @details 1. Configuration of VEX, V2D and Common Module for sending data from VEX to V2D via B path
            ...
            3. This TC configures the PASSTHRU GAIN through RCS.
   @TC Description:
        1. 1000 blocks for given data rate will be streamed to V2D Tx from
           configured VEX core.
        2. PDPD block was configured with PASS THRU gain via RCS.
...
/*config 11 to 16 will be equal to config 5 to 9 as there were no LUT's present*/
...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, int test_config)
{
    ...
    if (rv = pdpd_config_reg_config(v2d_num, apb_num, FLOW2_PASSTHRU_VALUE, pdpd_out_rate))
    {
...
int main(int argc, const char *argv[])
{
    ...
    int rcs_host_param[6] = {0}; // corresponds to v2d_num, apb_num and 128 bits for mode2 update
#endif
    ...
    rcs_host_param[2] = dpd_passthru_buffer_data[0][0];
    ...
    if ((rv = srp_vex_loader(vex_hdl, vex_num_param[0], "vex_fw/sim_m64/v2d_flow2_tx.fw")))
        return rv;

    if ((rv = srp_vex_loader(vex_hdl, vex_num_param[1], "vex_fw/sim_m64/v2d_flow2_rx.fw")))
        return rv;
   ...
   if ((rv = srp_vex_memid_store(vex_hdl,
    ...
    /*This sleep needs to be kept to get the underrun on V2D*/
    usleep(1000);
