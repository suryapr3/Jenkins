...
# Created Date: Jan 16 2024
# =============================================================================
# test description : this test checks the V2D flowB with Two streams enabled for
#                    both Tx and Rx.
# =============================================================================
# Import statements
# =============================================================================
...
if arg_len != 10:
    ...
    print("python flow2_2Tx_2Rx.py <tx0_vex_id> <rx0_vex_id> <tx1_vex_id> <rx1_vex_id> <v2d_id> <apb_id> <split_method> <drf_clock> <test_config>\n")
    print("As there were no inputs provided executing with below parameters\n")
    print(" tx0_vex_id : 0\n  rx0_vex_id : 1\n tx1_vex_id : 8\n rx1_vex_id : 9\n v2d_id : 0\n apb_id : 0\n split_method : 0\n drf_clock : 0\n test_config : 4")
    param = [0, 1, 8, 9, 0, 0, 0, 0, 4]
else:
...
if (param[0] == param[1] or param[0] == param[2] or param[0] == param[3] or param[1] == param[2] or param[1] == param[3] or param[2] == param[3]):
    print("provided vex cores can't be equal\n")
    sys.exit("proper input params were not provided\n")

V2D.v2d_check(param[4])
V2D.vex_check(param[0], param[4])
V2D.vex_check(param[1], param[4])
V2D.vex_check(param[2], param[4])
V2D.vex_check(param[3], param[4])
V2D.apb_id_check(param[5])
V2D.split_method_check(param[6])
V2D.drf_clock_check(param[7])
V2D.test_config_check(param[8])
