...
const char *g_vex_fw_binary[] = {"vex_fw/sim_m64/v2d_flow2_tx.fw", "vex_fw/sim_m64/v2d_flow2_rx.fw", "vex_fw/sim_m64/v2d_flow2_unequal_split_tx1.fw"};
...
// H2B Registers
    ...
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_BUF_SZ_HW_TBL1_OFFSET, (NUM_DATA_BLOCKS - 1), NSIP_HBI_H2B_BUF_SZ_HW_TBL_WR_MASK, \
        "H2B_BUF_SZ_HW_TBL1"},
    /* Data will be written to VEX location 0x10080 , i.e VEX DMEM location 0x80*/
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_TK_ADDR_LKUP_TBL1_OFFSET, (STRM2CIO_2_VEX_DMEM_OFFSET + HIVE_ADDR_v2d_flow2_rx_consumer_sm_gate), \
    ...
    /***********************************************************************************************************************************/
    /*These last 8 entries orders shouldn't be altered as they will be modified based on the run time parameter for vex number*/
    ...
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL1_OFFSET, (NSIP_HBI_H2B_ADDR_LKUP_TBL_VALID_BF_MSK | HIVE_ADDR_v2d_flow2_rx_vbuffer_out), NSIP_HBI_H2B_ADDR_LKUP_TBL_WR_MASK, "H2B_ADDR_LKUP_TBL1"},
...
int configure_v2d_host(int vex_num[], int v2d_num, int apb_num, int split_method, int drf_clock, int test_config)
{
    ...
    for (i = 0; i < ARRAY_SIZE(g_v2d_flow2_config_tbl) - 8; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow2_config_tbl[i].offset), g_v2d_flow2_config_tbl[i].value, \
                g_v2d_flow2_config_tbl[i].mask, g_v2d_flow2_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow2_config_tbl) - 7; i++)
    {
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow2_config_tbl[i].offset), \
                    (0x80000000 | v2d_tx_stream_cnt0), g_v2d_flow2_config_tbl[i].mask, g_v2d_flow2_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow2_config_tbl) - 2; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow2_config_tbl[i].offset), \
                    (0x80000000 | v2d_rx_stream_cnt0), g_v2d_flow2_config_tbl[i].mask, g_v2d_flow2_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow2_config_tbl) - 1; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow2_config_tbl[i].offset), \
                    (0x80000000 | v2d_tx_stream_cnt1), g_v2d_flow2_config_tbl[i].mask, g_v2d_flow2_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow2_config_tbl); i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow2_config_tbl[i].offset), \
                    (0x80000000 | v2d_rx_stream_cnt1), g_v2d_flow2_config_tbl[i].mask, g_v2d_flow2_config_tbl[i].name))
            return rv;
    }
...
int ref_output_read (unsigned int *host_ref0_data, unsigned int *host_ref1_data, int test_config,
        unsigned int vex0_num_input_vectors, unsigned int vex1_num_input_vectors)
{
...
int main(int argc, const char *argv[])
{
    ...
    // unequal split Tx0 will be loaded with v2d_flow2_tx.fw and Tx1 will be loaded with
   ...
   /*Second Rx Vex Core*/
    ...
    else
    {
        ...
        {
            printf("HOST: Low watermark buffer%x value : 0x%x", lc, watermark_buf[0] & 0xFFF);
            printf("HOST: High watermark buffer%x value : 0x%x", lc, (watermark_buf[0] >> 16) & 0xFFF);
        }
    }
