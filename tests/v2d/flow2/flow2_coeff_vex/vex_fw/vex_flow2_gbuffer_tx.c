 ...
 * @details 1. VEX will send g_num_blocks of data to CM along with PASSTHRU COEFF.
**/
...
vec32i32 AT(PASSTHRU_GAIN_LOCATION) v_passthru_gain[1];
...
volatile int AT(0x1C) g_dpd_passthru_coeff_base;
 ...
 * @return          none
 */
void v2d_flow2_gbuffer_tx() ENTRY
{
    int i, j, offset =0;
    g_host_debug = 0;
    ...
    /* Mode 2 G Buffer Coeff update*/
    vec32i32 v_passthru;
    v_passthru = v_passthru_gain[0];
    //v_xsto_1024(Any, g_dpd_passthru_coeff_base, 0, v_passthru);
    v_xsto_512(Any, g_dpd_passthru_coeff_base, 0, v_passthru);

    g_host_indicator = 1;
    // wait till g_host_indicator value becomes 2
    while(2 != g_host_indicator)
    {
