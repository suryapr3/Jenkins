...
int hrt_main(int argc, char **argv)
{
    ...
    hrt_cell_load_program(VE00, "v2d_flow2_gbuffer_tx");
    hrt_cell_load_program(VE01, "v2d_flow2_rx");
    hrt_cell_load_program(VE02, "producer_prog");

    // Load the pointer copy address for the SDFGates
    hrtx_memid_set_bpca(VE01, ve32_bp_dmem, HIVE_ADDR_v2d_flow2_rx_consumer_sm_gate);
    hrtx_memid_set_bpca(VE02, ve32_bp_dmem, HIVE_ADDR_producer_prog_producer_sm_gate);

    hrt_scalar_store(VE00, uint, v2d_flow2_gbuffer_tx_g_scalar_val, add_const);
    hrt_scalar_store(VE00, uint, v2d_flow2_gbuffer_tx_g_num_vecs_in, num_vec_input);
    hrt_scalar_store(VE00, uint, v2d_flow2_gbuffer_tx_g_dest_buffer_base, 0x4000);
    hrt_scalar_store(VE00, uint, v2d_flow2_gbuffer_tx_g_num_blocks, 0);
    hrt_scalar_store(VE00, uint, v2d_flow2_gbuffer_tx_g_initial_block_size, 1);
    hrt_scalar_store(VE00, uint, v2d_flow2_gbuffer_tx_g_dpd_passthru_coeff_base, 0x6000);
    hrt_scalar_store(VE00, uint, v2d_flow2_gbuffer_tx_g_host_cycle_count, 511);

    hrt_scalar_store(VE01, uint, v2d_flow2_rx_g_scalar_val, add_const);
    hrt_scalar_store(VE01, uint, v2d_flow2_rx_g_num_vecs_in, num_vec_input);
    hrt_scalar_store(VE01, uint, v2d_flow2_rx_g_tbs_in_blk, TBS_IN_BLK);
    hrt_scalar_store(VE01, uint, v2d_flow2_rx_g_dest_ptr_address,
            (STRM2CIO_0_DMEM + HIVE_ADDR_producer_prog_producer_sm_gate + (PRODUCER_SM_BUF_ID*4)));
    hrt_scalar_store(VE01, uint, v2d_flow2_rx_g_num_blocks, num_blocks);
    ...
    hrt_scalar_store(VE02, uint, producer_prog_g_dest_ptr_address,
            (STRM2CIO_0_DMEM + HIVE_ADDR_v2d_flow2_rx_consumer_sm_gate + (CONSUMER_SM_BUF_ID*4)));
    hrt_scalar_store(VE02, uint, producer_prog_g_dest_buffer_base, (STRM2CIO_0_VMEM + HIVE_ADDR_v2d_flow2_rx_vbuffer_out));
    ...
    for(i = 0; i < num_vec_input; i++)
    {
        for(j = 0; j < NUM_OF_ELEMENTS_PER_VEC; j++)
        {
            ...
            hrt_indexed_store(VE01, int, v2d_flow2_rx_vbuffer, i * NUM_OF_ELEMENTS_PER_VEC + j,
                              (((i * NUM_OF_ELEMENTS_PER_VEC + j) << 16) |(i * NUM_OF_ELEMENTS_PER_VEC + j)));
            hrt_indexed_store(VE00, int, v2d_flow2_gbuffer_tx_vbuffer, i * NUM_OF_ELEMENTS_PER_VEC + j,
                              (((i * NUM_OF_ELEMENTS_PER_VEC + j) << 16) |(i * NUM_OF_ELEMENTS_PER_VEC + j)));
        }
    }
    ...
    for (j = 0; j < NUM_OF_ELEMENTS_PER_VEC; j++)
    {
        hrt_indexed_store(VE00, int, v2d_flow2_gbuffer_tx_v_passthru_gain, j, g_passthru_gain[j]);
    }
    ...
    // Run program
    hrt_cell_start_function(VE00, v2d_flow2_gbuffer_tx_v2d_flow2_gbuffer_tx);
    printf ("abc_flow1 is started \n");
    hrt_cell_start_function(VE01, v2d_flow2_rx_v2d_flow2_rx);
    printf ("abc_flow1_rx is started\n");

    hrt_cell_start_function(VE02, producer_prog_producer_prog);

    int host_indicator = hrt_indexed_load(VE00, int, v2d_flow2_gbuffer_tx_g_host_indicator,0);

    printf("host_indicator is %d\n", host_indicator);
    //Wait for the program finish
    hrt_cell_wait(VE01);

    //Load outputs from cell to host
    int vex_compare_result = hrt_scalar_load(VE01, int, v2d_flow2_rx_g_compare_result);
    ...
    int cell_elem = hrt_indexed_load(VE02, int, v2d_flow2_gbuffer_tx_v_passthru_gain,0);
    ...
    {
        printf("passthru gain was read properly\n");
    }
    if(exit_status ==0)
    {
        printf("host:----------------SUCCESS----------------\n");
