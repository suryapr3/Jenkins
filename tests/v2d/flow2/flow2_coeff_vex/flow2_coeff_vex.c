 ...
 * @details 1. Configuration of VEX, V2D and Common Module for sending data from VEX to V2D via B path
               with passthru gain enabled and receving it back to other VEX via F path.
            2. Compares the received output with REF output for result evaluation.
            3. This TC configures the PASSTHRU GAIN through VEX write.
   @TC Description:
        1. 1000 blocks for given data rate will be streamed to V2D Tx from
           configured VEX core.
        2. PDPD block was configured with PASS THRU gain via VEX.
        ...
        6. Host will be waiting on a memory location where VEX will update the number of vectors
           it was processed.
        7. Same Data Block will be sent max blocks number of times.
...
#include "v2d_flow2_gbuffer_tx.map.h"
#include "v2d_flow2_rx.map.h"

/***********************************************
 * LOCAL/PRIVATE MACROS AND DEFINES            *
 ***********************************************/
#define B2H_MAP_VALUE_PASSTHRU_GAIN 0x20000
...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, int test_config)
{
    ...
    for (; i <  (ARRAY_SIZE(g_cm_0_config_tbl) - 4); i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm_0_config_tbl[i].offset), \
                    (NSIP_HBI_H2B_ADDR_LKUP_TBL_VALID_BF_MSK | HIVE_ADDR_v2d_flow2_rx_vbuffer_out),\
                    g_cm_0_config_tbl[i].mask, g_cm_0_config_tbl[i].name))
            return rv;
    }
    ...
    if (rv = pdpd_cfg_reg_config(v2d_num, apb_num, FLOW2_PASSTHRU_VALUE, pdpd_out_rate))
    {
        printf ("PDPD reg config failed");
        return rv;
    }

    if (rv = pdpd_post_c_center_config(v2d_num, apb_num, POST_C_CENTER_VALUE_UNITY_GAIN))
    {
...
int main(int argc, const char *argv[])
{
    ...
    int offset_val[] = {HIVE_ADDR_v2d_flow2_gbuffer_tx_g_scalar_val, HIVE_ADDR_v2d_flow2_gbuffer_tx_g_num_vecs_in,
                          HIVE_ADDR_v2d_flow2_gbuffer_tx_g_dest_buffer_base, HIVE_ADDR_v2d_flow2_gbuffer_tx_g_num_blocks,
                         HIVE_ADDR_v2d_flow2_gbuffer_tx_g_initial_block_size, HIVE_ADDR_v2d_flow2_gbuffer_tx_g_dpd_passthru_coeff_base,
                         HIVE_ADDR_v2d_flow2_gbuffer_tx_g_host_cycle_count};
    unsigned int val_kargs[] = {0x0, NUMBER_OF_VECTORS_INPUT, 0, 0, 3, (B2H_MAP_VALUE_PASSTHRU_GAIN), (CYCLES_PER_BLOCK_983M - 1)};
    ...
    unsigned int dpd_passthru_buffer_data[1][NUM_OF_ELEMENTS_PER_VEC] = {0}, watermark_buf[MAX_NUM_STREAMS] = {0};
    ...
    {
        val_kargs[6] = CYCLES_PER_BLOCK_1P5G - 1;
    }
#endif
    ...
    if ((rv = vex_loader(vex_num_param[0], "vex_fw/sim_m64/v2d_flow2_gbuffer_tx.fw")))
        return rv;
    ...
    /*Load the input pattern to Tx VEX*/
    write_mem_18a(VEX_VMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow2_gbuffer_tx_vbuffer,/*local_addr*/
                        (g_vectors_per_block[drf_clock][pdpd_in_rate] * NUM_OF_ELEMENTS_PER_VEC), val_inp[0],
                        "VEX_VMEM");

    /*Load the REF pattern to RX VEX*/
   write_mem_18a(VEX_VMEM(vex_num_param[1]) + HIVE_ADDR_v2d_flow2_rx_vbuffer,/*local_addr*/
                        (g_vectors_per_block[drf_clock][pdpd_out_rate] * NUM_OF_ELEMENTS_PER_VEC), host_ref_data,
                         "VEX_VMEM");

   /*Write the passthru gain to the VEX memory*/
    write_mem_18a(VEX_VMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow2_gbuffer_tx_v_passthru_gain,/*local_addr*/
    ...
    int host_indicator = 0;
    loop_wait_count = 0;

    /* Waiting for Tx VEX to write the PASSTHRU coeffecients to CM*/
    while (1 == host_indicator)
    {
        if ((rv = write_reg_18a(VEX_DMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow2_gbuffer_tx_g_host_indicator,
                        host_indicator,
                        0xFFFFFFFF,
                        "HOST_INDICATION")))
            return rv;
        ...
        {
            printf("ERROR: Tx VEX didn't write the passthru gain");
            rv = 1;
            return rv;
        }
        loop_wait_count ++;
    ...
    if (rv = pdpd_ctrl_reg_config(v2d_num, apb_num, 1/*update_passthru*/, 0/*update_c_lut*/))
    {
    ...
    /*Signal VEX to start data transfer*/
    if ((rv = write_reg_18a(VEX_DMEM(vex_num_param[0])+ HIVE_ADDR_v2d_flow2_gbuffer_tx_g_host_indicator,
                    host_flag,
                    0xFFFFFFFF,
                    "HOST_INDICATION")))
