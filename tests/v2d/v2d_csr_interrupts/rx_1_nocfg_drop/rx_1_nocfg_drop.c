 ...
 *********************************************************************************************/
/**
 * @file    rx_1_nocfg_drop.c
 * @details 1. This TC validates the RX_1_NOCFG_DROP interrupt generation.
   @TC Description:
        1. Configure all the V2D registers.
        2. Program the stream en bit of RX_STREAM_CNT as zero.
        3. Trigger the V2D with fill level with loop back enabled.
...
#define V2D_RX_1_OVFL_DROP AV2D_CSR_INT_CSR_INT_STATUS_RX_1_NOCFG_DROP_BF_MSK
...
// H2B Registers
    ...
    /*Configure the CREDIT_READY value as zero*/
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_CREDIT_READY_CS_OFFSET, 0/*H2B_CREDIT_READY_CS_VALUE*/, NSIP_HBI_H2B_CREDIT_READY_CS_WR_MASK, \
        "CM_1_H2B_CREDIT_CS"},
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL0_OFFSET, (NSIP_HBI_H2B_ADDR_LKUP_TBL_VALID_BF_MSK | HIVE_ADDR_v2d_flow6_rx_vbuffer_out), \
...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, int test_config)
{
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow6_config_tbl[i].offset), \
                    (g_tx_strm_cnt0_value[drf_clock][pdpd_out_rate] ^ AV2D_CSR_RX_CSR_RX_STREAM_CNT_STREAM_EN_BF_MSK), \
                    g_v2d_flow6_config_tbl[i].mask, g_v2d_flow6_config_tbl[i].name))
            return rv;
    }
...
int main(int argc, const char *argv[])
{
   ...
   if ((rv = srp_vex_memid_store(vex_hdl,
    ...
    if (rv = interrupt_status_check(v2d_num, apb_num, V2D_RX_1_OVFL_DROP/*V2D interrupt value*/, 0/*nsip0 interrupt value*/, 0/*nsip1 interrupt value*/))
    {
    ...
    if (rv = interrupt_status_check(v2d_num, apb_num, V2D_RX_1_OVFL_DROP | V2D_INTERRUPT_VALUE, 0/*nsip0 interrupt value*/, 0/*nsip1 interrupt value*/))
    {
        printf("ERROR: interrupt check failed\n");
        test_status = 1;
    }
    ...
    if (rv = write_interrupt_clear(v2d_num, apb_num, V2D_RX_1_OVFL_DROP | V2D_INTERRUPT_VALUE, 0/*nsip0 interrupt vlaue*/, 0/*nsip1 interrupt value*/))
    {
