 ...
 *********************************************************************************************/
/**
 * @file    tx_hbi_1_nocfg_drop.c
 * @details 1. This TC validates the TX_HBI_1_NOCFG_DROP interrupt generation.
   @TC Description:
        1. Configure all the V2D registers.
        2. Program the stream en bit of TX_STREAM_CNT as zero.
        3. Trigger the data flow from VEX to V2D.
...
#define V2D_EXP_INTERRUPT_VALUE AV2D_CSR_INT_CSR_INT_STATUS_TX_HBI_1_NOCFG_DROP_BF_MSK
...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, int test_config)
{
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow6_config_tbl[i].offset), \
                    (g_tx_strm_cnt0_value[drf_clock][pdpd_in_rate] ^ AV2D_CSR_TX_CSR_TX_STREAM_CNT_STREAM_EN_BF_MSK), \
                    g_v2d_flow6_config_tbl[i].mask, g_v2d_flow6_config_tbl[i].name))
            return rv;
    }
...
int main(int argc, const char *argv[])
{
   ...
   if ((rv = srp_vex_memid_store(vex_hdl,
    ...
    /*As VEX cores stopped transmitting the tx_no_cfg interrupt should be cleared now*/
    if (rv = interrupt_status_check(v2d_num, apb_num, 0/*v2d_interrupt value*/, 0/*nsip0 interrupt value*/, 0/*nsip1 interrupt value*/))
    {
        printf("ERROR: interrupt check failed\n");
        test_status = 1;
    }
