 ...
 *********************************************************************************************/
/**
 * @file    tx_hbi_0_nocfg_drop.c
 * @details 1. This TC validates the TX_HBI_0_NO_CFG_DROP interrupt generation.
   @TC Description:
        1. Configure all the V2D registers.
        2. Program the B2H Tx stream cnt enable bit in tx_stream_cnt as zero.
        3. Initiate the data transfer from VEX to V2D.
...
#define V2D_EXP_INTERRUPT_VALUE (AV2D_CSR_INT_CSR_INT_STATUS_TX_HBI_0_NOCFG_DROP_BF_MSK)
/***********************************************
...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, sys_rate_t sys_rate)
{
    ...
    //fill_value = 3 * (VEC2FLIT(g_flow1_vectors_per_block[sys_rate]));
    ...
    /*reset the STREAM_EN bit*/
    for (; i < ARRAY_SIZE(g_v2d_config_tbl) - 1; i++)
    {
       if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_config_tbl[i].offset), \
                   g_flow1_tx_strm_cnt0_offset[sys_rate] ^ AV2D_CSR_TX_CSR_TX_STREAM_CNT_STREAM_EN_BF_MSK, \
                   g_v2d_config_tbl[i].mask, g_v2d_config_tbl[i].name))
           return rv;
    }
...
int main(int argc, const char *argv[])
{
   ...
   if ((rv = srp_vex_memid_store(vex_hdl,
    ...
    /*Check the Interrupt status again as the Tx flow to V2D was stopped the interrupt should reset*/
    if (rv = interrupt_status_check(v2d_num, apb_num, 0, 0/*nsip0 interrupt value*/, 0/*nsip1 interrupt value*/))
    {
        printf("ERROR: Interrupt check failed\n");
        test_status = 1;
    }
