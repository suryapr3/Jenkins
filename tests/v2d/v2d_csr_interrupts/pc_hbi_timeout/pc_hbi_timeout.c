 ...
 *********************************************************************************************/
/*
 * @file    pc_hbi_timeout.c
 * @details 1. This TC validates the PC_HBI_TIMEOUT interrupt generation.
   @TC Description:
        1. Configure all the V2D registers.
        2. Trigger the MODE1 transfer from VEX and transfer only 240 flits and won't transfer
           the 241st entry.
        3. POLL on the HOST until the PC_HBI_TIMEOUT interrupt was set and later trigger the
           SHDW to LIVE transfer.
        4. The SHDW to LIVE transfer is happening.
        5. Check whether in addition to the PC_HBI_TIMEOUT interrupt PC_UPDATE_ERR interrupt was
           set or not, expectation is both interrupts should be set.
 */
...
#define V2D_EXP_INTERRUPT_VALUE (AV2D_CSR_INT_CSR_INT_STATUS_PC_UPDATE_ERR_BF_MSK | AV2D_CSR_INT_CSR_INT_STATUS_PC_HBI_TIMEOUT_BF_MSK)
/***********************************************
...
int main(int argc, const char *argv[])
{
    ...
    int offset_val[] = {HIVE_ADDR_v2d_pc_hbi_timeout_g_scalar_val, HIVE_ADDR_v2d_pc_hbi_timeout_g_num_vecs_in,
                         HIVE_ADDR_v2d_pc_hbi_timeout_g_dest_buffer_base, HIVE_ADDR_v2d_pc_hbi_timeout_g_num_blocks,
                         HIVE_ADDR_v2d_pc_hbi_timeout_g_initial_block_size, HIVE_ADDR_v2d_pc_hbi_timeout_g_dpd_coeff_base,
                         HIVE_ADDR_v2d_pc_hbi_timeout_g_passthru_coeff_base, HIVE_ADDR_v2d_pc_hbi_timeout_g_host_cycle_count,
                         HIVE_ADDR_v2d_pc_hbi_timeout_g_vectors_g_buf};
    ...
    uint32_t bfn_lo_int_value = 0, bfn_hi_int_value = 0, test_status = 0, loop_wait_count = 0, host_flag = 0;
    ...
    if ((rv = srp_vex_loader(vex_hdl, vex_num_param[0], "vex_fw/sim_m64/v2d_pc_hbi_timeout.fw")))
        return rv;
    ...
    if ((rv = srp_vex_memid_store(vex_hdl,
                        vex_num_param[0],
                        SRP_VEX_VMEM,
                        HIVE_ADDR_v2d_pc_hbi_timeout_v_gbuffer,/*local_addr*/
                        dpd_coeff_buffer[0],
    ...
    if ((rv = srp_vex_memid_store(vex_hdl,
                        vex_num_param[0],
                        SRP_VEX_VMEM,
                        HIVE_ADDR_v2d_pc_hbi_timeout_v_passthru_gain,/*local_addr*/
    ...
    /* Waiting for Tx VEX to write the coeffecients till passthru coeffecient*/
    while (2 == host_flag)
    {
        if ((rv = srp_dev_block_read32(srp,
                        SRP_BLOCK_VEX_CORE,
                        vex_num_param[0],
                        dmem_offset + HIVE_ADDR_v2d_pc_hbi_timeout_g_host_indicator,
                        &host_flag,
                        1,
                        flags)))
            return rv;
    ...
    while (!(get_v2d_interrupt_status(v2d_num, apb_num) >> AV2D_CSR_INT_CSR_INT_STATUS_PC_HBI_TIMEOUT_BF_OFF))
    {
        if (loop_wait_count >= 30)
        {
            printf("ERROR: HBI_TIMEOUT was not set");
            break;
            rv = 1;
        }
        loop_wait_count ++;
    ...
    if (rv = pdpd_control_reg_config(v2d_num, apb_num, 1/*update_passthru*/, 1/*update_c_lut*/))
    {
    ...
    reg_check.num_gmp_terms = V2D_PDPD_GMP_TERMS;
    reg_check.num_ddr_terms = V2D_PDPD_DDR_TERMS;
    reg_check.gmp_c_i_value = 0;
    reg_check.gmp_c_q_value = 0;
    reg_check.ddr_c_i_value = 0;
    reg_check.ddr_c_q_value = 0;
    reg_check.gmp_shift_value = 0;
    reg_check.ddr_shift_value = 0;

    /*To Check whether SHADOW to LIVE transition happened or not*/
    if (rv = gmp_ddr_shdw_live_reg_check(&reg_check))
    {
        printf("WARNING: SHDW to LIVE transition failied");
    }
    else
    {
    ...
    if (rv = passthru_reg_check(v2d_num, apb_num, 0))
    {
        printf("WARNING: PASSTHRU REG SHDW TO LIVE Transition failed");
    }
    else
    {
    ...
    /*As the interrupt was cleared BFN interrupt also should be in reset*/
    if (DRF_CHANNEL_0 == apb_num)
    {
        bfn_lo_int_value = BFN_CSR_SUBB_LO_INTR_DEFAULT;
        bfn_hi_int_value = BFN_CSR_SUBB_HI_INTR_DEFAULT;
    }
    else
    {
        bfn_lo_int_value = BFN_CSR_SUBB_LO_INTR_DEFAULT;
