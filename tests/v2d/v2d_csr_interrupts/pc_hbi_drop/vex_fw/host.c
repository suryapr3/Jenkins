...
#define NUMBER_OF_VECTORS_OUT 256 // Corresponds to 2 blocks of data for 8GSPS
...
#define GMP_C_SHDW_I_VALUE  (1 << 11)
#define GMP_C_SHDW_Q_VALUE  0
...
void dpd_coeff_buffer_fill(unsigned int (*dpd_coeff_buffer)[80])
{
    int i, j;
    unsigned int value = ((GMP_C_SHDW_I_VALUE & 0x1FFF) << 16) | (GMP_C_SHDW_Q_VALUE & 0x1FFF);

    for (i = 0; i < 2; i++)
    {
        for (j = 0; j < V2D_PDPD_MAX_C_LUT_SEGMENTS; j++)
        {
            dpd_coeff_buffer[i][j] = value;
        }

        dpd_coeff_buffer[i][j] = SHIFT_VALUE;
    }
...
int hrt_main(int argc, char **argv)
{
    ...
    unsigned int dpd_coeff_buffer[48][80] = {0};

    // Load program in cell program memory
    hrt_cell_load_program(VE00, "v2d_flow3_gbuffer_mode1_tx");
    hrt_cell_load_program(VE01, "v2d_flow3_rx");
    hrt_cell_load_program(VE02, "producer_prog");

    // Load the pointer copy address for the SDFGates
    hrtx_memid_set_bpca(VE01, ve32_bp_dmem, HIVE_ADDR_v2d_flow3_rx_consumer_sm_gate);
    hrtx_memid_set_bpca(VE02, ve32_bp_dmem, HIVE_ADDR_producer_prog_producer_sm_gate);

    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_mode1_tx_g_scalar_val, add_const);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_mode1_tx_g_num_vecs_in, num_vec_input);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_mode1_tx_g_dest_buffer_base, 0x4000);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_mode1_tx_g_num_blocks, 0);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_mode1_tx_g_initial_block_size, 1);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_mode1_tx_g_dpd_coeff_base, 0xC000);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_mode1_tx_g_passthru_coeff_base, 0xFC00);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_mode1_tx_g_host_cycle_count, 511);
    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_mode1_tx_g_vectors_g_buf, 120);

    hrt_scalar_store(VE01, uint, v2d_flow3_rx_g_scalar_val, add_const);
    hrt_scalar_store(VE01, uint, v2d_flow3_rx_g_num_vecs_in, num_vec_input);
    hrt_scalar_store(VE01, uint, v2d_flow3_rx_g_tbs_in_blk, TBS_IN_BLK);
    hrt_scalar_store(VE01, uint, v2d_flow3_rx_g_dest_ptr_address,
            (STRM2CIO_0_DMEM + HIVE_ADDR_producer_prog_producer_sm_gate + (PRODUCER_SM_BUF_ID*4)));
    hrt_scalar_store(VE01, uint, v2d_flow3_rx_g_num_blocks, num_blocks);
    ...
    hrt_scalar_store(VE02, uint, producer_prog_g_dest_ptr_address,
            (STRM2CIO_0_DMEM + HIVE_ADDR_v2d_flow3_rx_consumer_sm_gate + (CONSUMER_SM_BUF_ID*4)));
    hrt_scalar_store(VE02, uint, producer_prog_g_dest_buffer_base, (STRM2CIO_0_VMEM + HIVE_ADDR_v2d_flow3_rx_vbuffer_out));

    dpd_coeff_buffer_fill(dpd_coeff_buffer);
    for(i = 0; i < num_vec_input; i++)
    {
        for(j = 0; j < NUM_OF_ELEMENTS_PER_VEC; j++)
        {
            ...
            hrt_indexed_store(VE00, int, v2d_flow3_gbuffer_mode1_tx_vbuffer, i * NUM_OF_ELEMENTS_PER_VEC + j,
                              (((i * NUM_OF_ELEMENTS_PER_VEC + j) << 16) |(i * NUM_OF_ELEMENTS_PER_VEC + j)));
        }
    }
    ...
    index = 0;
    offset += add_const;
    for (k = 0; k < num_vec_output / num_vec_input; k++)
    {
        offset += add_const;
        for (i = 0; i < num_vec_input; i++)
        {
            for(j = 0; j < NUM_OF_ELEMENTS_PER_VEC; j++)
            {
                int temp = (((i * NUM_OF_ELEMENTS_PER_VEC + j) << 16) |(i * NUM_OF_ELEMENTS_PER_VEC + j));
                hrt_indexed_store(VE01, int, v2d_flow3_rx_vbuffer, index, (temp + offset));
                index++;
            }
        }
    }
    ...
    for (i = 0; i < 48; i++)
    {
        for (j = 0; j < 80; j++)
        {
            hrt_indexed_store(VE00, int, v2d_flow3_gbuffer_mode1_tx_v_gbuffer, index, (dpd_coeff_buffer[i][j]));
            index++;
            //printf("HOST: dpd_coeff_buffer: %x, i: %d, j :%d , index : %d\n", dpd_coeff_buffer[i][j],i,j, index);
        }
    }
    // Copy the passthru gain
    g_passthru_gain[0] = ((1 << 13) << 16); // I value is 2^13 and Q value 0;

    for (j = 0; j < NUM_OF_ELEMENTS_PER_VEC; j++)
    {
        hrt_indexed_store(VE00, int, v2d_flow3_gbuffer_mode1_tx_v_passthru_gain, j, g_passthru_gain[j]);
    }
    ...
    // Run program
    hrt_cell_start_function(VE00, v2d_flow3_gbuffer_mode1_tx_v2d_flow3_gbuffer_mode1_tx);
    printf ("abc_flow1 is started \n");
    hrt_cell_start_function(VE01, v2d_flow3_rx_v2d_flow3_rx);
    printf ("abc_flow1_rx is started\n");

    hrt_cell_start_function(VE02, producer_prog_producer_prog);

    hrt_scalar_store(VE00, uint, v2d_flow3_gbuffer_mode1_tx_g_host_indicator, 0x0);
    //Wait for the program finish
    hrt_cell_wait(VE01);

    //Load outputs from cell to host
    int vex_compare_result = hrt_scalar_load(VE01, int, v2d_flow3_rx_g_compare_result);
    ...
    index = 0;
    // Compare the G Buffer
    for (i = 0; i < 48; i++)
    {
        for (j = 0; j < 80; j++)
        {
            int cell_elem = hrt_indexed_load(VE02, int, producer_prog_gbuffer, index);
            if (cell_elem != dpd_coeff_buffer[i][j])
            {
                printf("host: error in G Buffer , ref: %x, cell_elem: %x , i: %d , j: %d\n", dpd_coeff_buffer[i][j], cell_elem, i, j);
            }
            index++;
        }
    }
...
#if 1
    // Compare passthru gain
    int cell_elem = hrt_indexed_load(VE02, int, producer_prog_gbuffer, index);
    //int cell_elem = hrt_indexed_load(VE02, int, producer_prog_g_passthru_gain, 0);
    if (cell_elem != g_passthru_gain[0])
    {
        printf("host: passthru gain ref = %x, cell_elem = %x\n", g_passthru_gain[0], cell_elem);
        exit_status = 1;
    }
    else
    {
        printf("passthru gain read : %x\n", cell_elem);
    }
#endif
