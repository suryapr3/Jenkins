 ...
 *********************************************************************************************/
/**
 * @file    rx_0_nocfg_drop.c
 * @details 1. This TC validates the RX_0_NO_CFG_DROP interrupt generation.
   @TC Description:
        1. Configure all the V2D registers.
        2. Program the H2B Rx stream cnt enable bit in rx_stream_cnt as zero.
...
#define V2D_EXP_INTERRUPT_VALUE (AV2D_CSR_INT_CSR_INT_STATUS_RX_0_NOCFG_DROP_BF_MSK | AV2D_CSR_INT_CSR_INT_STATUS_TX_UNDERRUN_BF_MSK)
/***********************************************
...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, sys_rate_t sys_rate)
{
    ...
    {
       if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_config_tbl[i].offset), \
                   g_flow1_tx_strm_cnt0_offset[sys_rate] ^ AV2D_CSR_RX_CSR_RX_STREAM_CNT_STREAM_EN_BF_MSK, \
                   g_v2d_config_tbl[i].mask, g_v2d_config_tbl[i].name))
           return rv;
    }
