...
int main(int argc, const char *argv[])
{
    ...
    // trigger next test bfn_test
    rd_wr_value[0] = 0xC000D006;
    write_mem_18a(g_rsm_buffer[4], 1, &rd_wr_value[0], "rsm_com_shared_flag");

    // wait for handshake from ax - bfn_sys_ref() test
    do
    {
        rd_wr_value[0] = read_reg_18a(g_rsm_buffer[0], "SRAM_AX_FLAG");
        ...
        if (loop_wait_count >= 100)
        {
            printf("WARNING: breaking the sys_ref_load test loop before RCS handshake");
            break;
        }
    ...
    } while(rd_wr_value[0] != 0xC000C007);

    read_mem_18a(g_dcode_buffer[0], 8, rd_wr_value, "DCODE_BUFFER");

    printf("BFN sys_ref load");
    printf("Before sys_ref_load RCS:%d, V2D: %d", rd_wr_value[0], rd_wr_value[1]);
    printf("After sys_ref_load RCS:%d, V2D: %d", rd_wr_value[2], rd_wr_value[3]);
    printf("Before second sys_ref_load RCS:%d, V2D: %d", rd_wr_value[4], rd_wr_value[5]);
    printf("After second sys_ref_load RCS:%d, V2D: %d", rd_wr_value[6], rd_wr_value[7]);

    if ((rd_wr_value[2] < 0x50500 && rd_wr_value[2] > 0x50000) && (rd_wr_value[3] < 0x50500 && rd_wr_value[3] > 0x50000) &&
        (rd_wr_value[6] < 0x50500 && rd_wr_value[6] > 0x50000) && (rd_wr_value[7] < 0x50500 && rd_wr_value[7] > 0x50000))
    {
        printf("SYS_REF_LOAD test pass");
    }
    else
    {
        printf("ERROR: SYS_REF_LOAD fail");
    }
    ...
    // trigger next test bfn_test
    rd_wr_value[0] = 0xC000D007;
    write_mem_18a(g_rsm_buffer[4], 1, &rd_wr_value[0], "rsm_com_shared_flag");

    // wait for handshake from ax - bfn_follower_reset() test
    do
    {
        rd_wr_value[0] = read_reg_18a(g_rsm_buffer[0], "SRAM_AX_FLAG");
    ...
    } while(rd_wr_value[0] != 0xC000C008);
    ...
    if ((rd_wr_value[1]) != 0 && (rd_wr_value[3] == 0))
    {
        printf("FOLLOWER Reset pass");
    }
    else
    {
        printf("ERROR:FOLLOWER RESET FAIL");
    }
