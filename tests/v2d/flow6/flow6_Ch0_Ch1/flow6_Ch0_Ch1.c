 ...
 * @details 1. Configuration of VEX, V2D and Common Module for sending data from VEX to V2D via B path
               ...
               other VEX for both the channels via D path.
   ...
   @TC Description:
        ...
        2. PDPD block was configured with PASS THRU gain and POST processing block was configured
           in PASSTHRU mode.
        ...
        4. VEX core on which comparison algo was running will update the comparison result for
           every vector comparison and once the total 1000 block comparison was done HOST will
           read the result.
...
int g_initial_block_size = 0;    // VEX will transfer initial_block_size + 0.5L before BFN trigger.
#endif
...
// H2B Registers
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL0_OFFSET, (NSIP_HBI_H2B_ADDR_LKUP_TBL_VALID_BF_MSK | HIVE_ADDR_v2d_flow6_rx_vbuffer_out), NSIP_HBI_H2B_ADDR_LKUP_TBL_WR_MASK, "H2B_ADDR_LKUP_TBL0"},
    ...
    if (rv = write_reg_18a(VE32_CELL_BASE(vex_num) + VE32_CELL_MEM_LM_SDFGATE_BPCA_BP_DMEM_OFFSET, \
        ...
        for (; i < (ARRAY_SIZE(g_cm_1_config_tbl) - 4); i++)
        {
            if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm_1_config_tbl[i].offset), \
                        (0x80000000 | HIVE_ADDR_v2d_flow6_rx_vbuffer_out), g_cm_1_config_tbl[i].mask, g_cm_1_config_tbl[i].name))
                return rv;
        }
...
int main(int argc, const char *argv[])
{
    ...
    for (vex_cnt = 0; vex_cnt < NUM_TX_VEX_CORES; vex_cnt++)
    {
        ...
        if ((rv = srp_vex_loader(vex_hdl, tx_vex_num, "vex_fw/sim_m64/v2d_flow6_tx.fw")))
            return rv;

        if ((rv = srp_vex_loader(vex_hdl, rx_vex_num, "vex_fw/sim_m64/v2d_flow6_rx.fw")))
            return rv;
 ...
 * corresponding flag has to be disabled in the VEX code as well and VEX FW has to be re-compiled*/
    ...
    while ((apb0_b2h_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, 0) + V2D_NSIP_HBI_1_OFFSET + \
                    NSIP_HBI_B2H_CSR_B2H_FLIT_COUNT_OFFSET, "APB0_B2H_FLIT_COUNT")) <
    ...
    while ((apb0_h2b_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, 0) + V2D_NSIP_HBI_1_OFFSET + \
    ...
    while ((apb1_h2b_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, 1) + V2D_NSIP_HBI_1_OFFSET + \
    ...
    for (vex_cnt = 0; vex_cnt < NUM_RX_VEX_CORES; vex_cnt++)
    {
        ...
        if (rv = interrupt_status_check(v2d_num, vex_cnt, V2D_INTERRUPT_VALUE, 0, 0))
        {
            printf("ERROR:Interrupt check failed\n");
            test_status |= 1;
        }
