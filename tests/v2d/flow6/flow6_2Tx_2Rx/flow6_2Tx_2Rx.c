...
const char *g_vex_fw_binary[] = {"vex_fw/sim_m64/v2d_flow6_tx.fw", "vex_fw/sim_m64/v2d_flow6_rx.fw", "vex_fw/sim_m64/v2d_flow6_unequal_split_tx1.fw"};
...
// H2B Registers
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_TK_ADDR_LKUP_TBL0_OFFSET, (STRM2CIO_2_VEX_DMEM_OFFSET + HIVE_ADDR_v2d_flow6_rx_consumer_sm_gate), NSIP_HBI_H2B_TK_ADDR_LKUP_TBL_WR_MASK, \
    ...
    /* Data will be written to VEX location 0x10080 , i.e VEX DMEM location 0x80*/
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_TK_ADDR_LKUP_TBL1_OFFSET, (STRM2CIO_2_VEX_DMEM_OFFSET + HIVE_ADDR_v2d_flow6_rx_consumer_sm_gate), NSIP_HBI_H2B_TK_ADDR_LKUP_TBL_WR_MASK, \
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL0_OFFSET, (NSIP_HBI_H2B_ADDR_LKUP_TBL_VALID_BF_MSK | HIVE_ADDR_v2d_flow6_rx_vbuffer_out), NSIP_HBI_H2B_ADDR_LKUP_TBL_WR_MASK, \
        "HBI_1_H2B_ADDR_LKUP_TBL0"},
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL1_OFFSET, (NSIP_HBI_H2B_ADDR_LKUP_TBL_VALID_BF_MSK | HIVE_ADDR_v2d_flow6_rx_vbuffer_out), NSIP_HBI_H2B_ADDR_LKUP_TBL_WR_MASK, \
...
int configure_v2d_host(int vex_num[], int v2d_num, int apb_num, int split_method, int drf_clock, int test_config)
{
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm_1_config_tbl[i].offset), (0x80000000 | HIVE_ADDR_v2d_flow6_rx_vbuffer_out), \
                g_cm_1_config_tbl[i].mask, g_cm_1_config_tbl[i].name))
            return rv;
    }
    ...
    for (i = 0; i < ARRAY_SIZE(g_v2d_flow6_config_tbl) - 8; i++)
    {
    ...
    for (; i < ARRAY_SIZE(g_v2d_flow6_config_tbl) - 7; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow6_config_tbl[i].offset), \
                    g_v2d_flow6_config_tbl[i].value | (drf_clock << 7), g_v2d_flow6_config_tbl[i].mask, g_v2d_flow6_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow6_config_tbl) - 6; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow6_config_tbl[i].offset), \
                    v2d_tx_ptr_bound0, g_v2d_flow6_config_tbl[i].mask, g_v2d_flow6_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow6_config_tbl) - 5; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow6_config_tbl[i].offset), \
                    v2d_tx_ptr_bound1, g_v2d_flow6_config_tbl[i].mask, g_v2d_flow6_config_tbl[i].name))
            return rv;
    }
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow6_config_tbl[i].offset), \
                    (0x80000000 | v2d_tx_stream_cnt0), g_v2d_flow6_config_tbl[i].mask, g_v2d_flow6_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow6_config_tbl) - 2; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow6_config_tbl[i].offset), \
                    (0x80000000 | v2d_rx_stream_cnt0), g_v2d_flow6_config_tbl[i].mask, g_v2d_flow6_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow6_config_tbl) - 1; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow6_config_tbl[i].offset), \
                    (0x80000000 | v2d_tx_stream_cnt1), g_v2d_flow6_config_tbl[i].mask, g_v2d_flow6_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow6_config_tbl); i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow6_config_tbl[i].offset), \
                    (0x80000000 | v2d_rx_stream_cnt1), g_v2d_flow6_config_tbl[i].mask, g_v2d_flow6_config_tbl[i].name))
            return rv;
    }
...
int main(int argc, const char *argv[])
{
    ...
    int offset_val[] = {HIVE_ADDR_v2d_flow6_tx_g_scalar_val, HIVE_ADDR_v2d_flow6_tx_g_num_vecs_in,
                         ...
                         HIVE_ADDR_v2d_flow6_tx_g_initial_block_size, HIVE_ADDR_v2d_flow6_tx_g_tx_instance,
                         HIVE_ADDR_v2d_flow6_tx_g_host_cycle_count};
    ...
    int vex_rx_offset_val[] = {HIVE_ADDR_v2d_flow6_rx_g_scalar_val, HIVE_ADDR_v2d_flow6_rx_g_num_vecs_in,
                                ...
                                HIVE_ADDR_v2d_flow6_rx_g_tokens_per_block, HIVE_ADDR_v2d_flow6_rx_g_count1};
    unsigned int vex0_rx_args[] = {0x0, NUMBER_OF_VECTORS_INPUT, 1, 64, 1, 0};
    unsigned int vex1_rx_args[] = {0x0, NUMBER_OF_VECTORS_INPUT, 1, 64, 1, 0};
    ...
    ref_output_read(host_ref0_data, host_ref1_data, test_config, vex0_rx_args[1], vex1_rx_args[1]);
    {
        return 1;
    }
    ...
    // unequal split Tx0 will be loaded with v2d_flow6_tx.fw and Tx1 will be loaded with
   ...
   /*Second Rx Vex Core*/
    ...
    for (vex_cnt = 0; vex_cnt < NUM_RX_VEX_CORES; vex_cnt++)
    {
        ...
        int vex_num = vex_num_param[vex_index];
        while (host_vec_processed < (g_max_blocks[drf_clock][pdpd_out_rate] * num_vectors_block[vex_cnt]))
        {
