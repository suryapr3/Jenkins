 ...
 * @details 1. Configuration of VEX, V2D and Common Module for sending data from VEX to V2D via B path
               with GMP, DDR coeff along with passthru gain enabled and receving it back to
               other VEX via D path.
   ...
   @TC Description:
        1. 1000 blocks for given data rate will be streamed to V2D Tx from
           configured VEX core.
        2. PDPD block was configured with GMP and DDR terms along with  PASS THRU gain, post filter
           was configured in PASSTHRU Mode.
        ...
        4. VEX core on which comparison algo was running will update the comparison result for
           every vector comparison and once the total sample comparison was done HOST will
           read the result.
...
#include "v2d_flow6_mode1_tx.map.h"
#include "v2d_flow6_mode1_rx.map.h"
...
#define FLOW6_RX_GNRL_CSR_VALUE  AV2D_CSR_RX_CSR_RX_GNRL_CSR_RX_PD_EN_BF_MSK | AV2D_CSR_RX_CSR_RX_GNRL_CSR_RX_PD_CFG_COMPLETE_BF_MSK
#define TEST_VECTOR_PATH_PREFIX  "test_vectors/"
/***********************************************
...
/*REF output files which are generated in MATLAB*/
...
// H2B Registers
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_CREDIT_READY_CS_OFFSET, H2B_CREDIT_READY_CS_VALUE, NSIP_HBI_H2B_CREDIT_READY_CS_WR_MASK, \
        "CM_1_H2B_CREDIT_CS"},
    /*For Any Data rate two data blocks will be the buffer size*/
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_TK_ADDR_LKUP_TBL0_OFFSET, (STRM2CIO_2_VEX_DMEM_OFFSET + HIVE_ADDR_v2d_flow6_mode1_rx_consumer_sm_gate), NSIP_HBI_H2B_TK_ADDR_LKUP_TBL_WR_MASK, \
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL0_OFFSET, NSIP_HBI_H2B_ADDR_LKUP_TBL_VALID_BF_MSK | HIVE_ADDR_v2d_flow6_mode1_rx_vbuffer_out, \
        NSIP_HBI_H2B_ADDR_LKUP_TBL_WR_MASK, "HBI_1_H2B_ADDR_LKUP_TBL"},
    /***********************************************************************************************************************************/
...
// Interrupt Enable
    ...
    /************************************************************************************************************/
    /* The last Fve entries order shouldn't change as these values will be modified based on the sample rate which
...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, int test_config)
{
    ...
    if (rv = write_reg_18a(VE32_CELL_BASE(vex_num[1]) + VE32_CELL_MEM_LM_SDFGATE_BPCA_BP_DMEM_OFFSET, \
                HIVE_ADDR_v2d_flow6_mode1_rx_consumer_sm_gate, VE32_CELL_MEM_LM_SDFGATE_BPCA_BP_DMEM_WR_MASK, "BPCA_BP_DMEM"))
        return rv;
    ...
    for (i = 0; i < ARRAY_SIZE(g_v2d_flow6_config_tbl) - 5; i++)
    {
    ...
    if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_CSR_OFFSET + AV2D_CSR_RX_CSR_RX_GNRL_CSR_OFFSET), \
                FLOW6_RX_GNRL_CSR_VALUE, AV2D_CSR_RX_CSR_RX_GNRL_CSR_WR_MASK, "RX_GNRL_CSR"))
        return rv;
    /************************************************************************************************************/
...
int main(int argc, const char *argv[])
{
    ...
    int offset_val[] = {HIVE_ADDR_v2d_flow6_mode1_tx_g_scalar_val, HIVE_ADDR_v2d_flow6_mode1_tx_g_num_vecs_in,
                         HIVE_ADDR_v2d_flow6_mode1_tx_g_dest_buffer_base, HIVE_ADDR_v2d_flow6_mode1_tx_g_num_blocks,
                         HIVE_ADDR_v2d_flow6_mode1_tx_g_initial_block_size, HIVE_ADDR_v2d_flow6_mode1_tx_g_host_cycle_count};
    unsigned int val_kargs[] = {0x0, NUMBER_OF_VECTORS_INPUT, 0, 0, 3, (CYCLES_PER_BLOCK_983M - 1)};
    int vex_rx_offset_val[] = {HIVE_ADDR_v2d_flow6_mode1_rx_g_scalar_val, HIVE_ADDR_v2d_flow6_mode1_rx_g_num_vecs_in,
                                HIVE_ADDR_v2d_flow6_mode1_rx_g_num_blocks, HIVE_ADDR_v2d_flow6_mode1_rx_g_tbs_in_blk,
                                HIVE_ADDR_v2d_flow6_mode1_rx_g_count1};
    ...
    unsigned int host_vec_processed = 0, host_blocks_processed = 0, host_flag = 1, host_result = 0, loop_wait_count = 0, test_status = 0;
    ...
    if ((rv = srp_vex_loader(vex_hdl, vex_num_param[0], "vex_fw/sim_m64/v2d_flow6_mode1_tx.fw")))
        return rv;

    if ((rv = srp_vex_loader(vex_hdl, vex_num_param[1], "vex_fw/sim_m64/v2d_flow6_mode1_rx.fw")))
        return rv;
    ...
    if ((rv = srp_vex_memid_store(vex_hdl,
                        vex_num_param[0],
                        SRP_VEX_VMEM,
                        HIVE_ADDR_v2d_flow6_mode1_tx_vbuffer,/*local_addr*/
                        val_inp[0],
                        (g_vectors_per_block[drf_clock][pdpd_in_rate] * NUM_OF_ELEMENTS_PER_VEC * sizeof(int)))))
        return rv;

    /* Loading the output test pattern to Rx*/
   if ((rv = srp_vex_memid_store(vex_hdl,
                        vex_num_param[1],
                        SRP_VEX_VMEM,
                        HIVE_ADDR_v2d_flow6_mode1_rx_vbuffer,/*local_addr*/
                        host_ref_data,
    ...
    if ((rv = srp_dev_block_write32(srp,
                    SRP_BLOCK_VEX_CORE,
                    vex_num_param[0],
                    dmem_offset + HIVE_ADDR_v2d_flow6_mode1_tx_g_host_indicator,
                    &host_flag,
                    1,
                    flags)))
        return rv;
    ...
    printf ("h2b_flit cnt is %d", h2b_flit_count);
    ...
    while (host_vec_processed < (g_max_blocks[drf_clock][pdpd_out_rate] * g_vectors_per_block[drf_clock][pdpd_out_rate]))
    {
        if ((rv = srp_dev_block_read32(srp,
                        SRP_BLOCK_VEX_CORE,
                        vex_num_param[1],
                        dmem_offset + HIVE_ADDR_v2d_flow6_mode1_rx_g_count1,
                        &host_vec_processed,
                        1,
                        flags)))
            return rv;
    ...
    if ((rv = srp_dev_block_read32(srp,
                    SRP_BLOCK_VEX_CORE,
                    vex_num_param[1],
                    dmem_offset + HIVE_ADDR_v2d_flow6_mode1_rx_g_compare_result,
                    &host_result,
                    1,
                    flags)))
        return rv;
    ...
    if ((rv = srp_dev_block_read32(srp,
                    SRP_BLOCK_VEX_CORE,
                    vex_num_param[1],
                    dmem_offset + HIVE_ADDR_v2d_flow6_mode1_rx_g_add_val_out,
