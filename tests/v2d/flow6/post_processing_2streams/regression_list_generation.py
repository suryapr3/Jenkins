...
wa = wb.active
header = ["vex_tx_b", "vex_rx_d0", "vex_rx_d1", "vex_rx_f", "v2d_id", "apb_id", "drf_clock","test_config"]
row = 1
...
for vex_tx_b in v2d_0_valid_vex_list:
    for vex_rx_d0 in v2d_0_valid_vex_list:
        if(vex_tx_b == vex_rx_d0):
            continue
        for vex_rx_d1 in v2d_0_valid_vex_list:
            for vex_rx_f in v2d_0_valid_vex_list:
                if(vex_tx_b == vex_rx_f) or (vex_rx_d1 == vex_rx_f):
                    continue
                if (vex_tx_b == vex_rx_d1) or (vex_rx_d1 == vex_rx_d0) or (vex_rx_d0 == vex_rx_f) or (vex_tx_b == vex_rx_f):
                    continue
                for apb_num in range (2):
                    for drf_clock in range(1):
                        for test_config in test_config_list:
                            wa.cell(row,1).value = vex_tx_b
                            wa.cell(row,2).value = vex_rx_d0
                            wa.cell(row,3).value = vex_rx_d1
                            wa.cell(row,4).value = vex_rx_f
                            wa.cell(row,5).value = 0
                            wa.cell(row,6).value = apb_num
                            wa.cell(row,7).value = drf_clock
                            wa.cell(row,8).value = test_config
                            row+=1

for vex_tx_b in v2d_1_valid_vex_list:
    for vex_rx_d0 in v2d_1_valid_vex_list:
        if(vex_tx_b == vex_rx_d0):
            continue
        for vex_rx_d1 in v2d_1_valid_vex_list:
...
for vex_tx_b in v2d_2_valid_vex_list:
    for vex_rx_d0 in v2d_2_valid_vex_list:
        if(vex_tx_b == vex_rx_d0):
            continue
        for vex_rx_d1 in v2d_2_valid_vex_list:
...
for vex_tx_b in v2d_3_valid_vex_list:
    for vex_rx_d0 in v2d_3_valid_vex_list:
        if(vex_tx_b == vex_rx_d0):
            continue
        for vex_rx_d1 in v2d_3_valid_vex_list:
...
sheet_name= wb['Sheet']
sheet_name.title ='post_processing_2streams'

wb.save("post_processing_2streams_list.xlsx")
