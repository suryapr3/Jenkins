...
// B2H Registers
    ...
    {(V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_B2H_CSR_B2H_ADDR_MASK_TBL1_OFFSET), (B2H_ADD_MAP_B2_OFFSET - 1), NSIP_HBI_B2H_CSR_B2H_ADDR_MASK_TBL_WR_MASK, \
...
// H2B Registers
    ...
    /* Data will be written to VEX location 0x4000 , i.e VEX VMEM location 0x6000*/
...
{
    int i, j = 0, rv = 0, v2d_tx_stream_cnt0 = 0, v2d_tx_stream_cnt1 = 0,v2d_rx_stream_cnt0 = 0,
        md_control0 = 0, md_control1 = 0, v2d_tx_ptr_bound0 = 0, v2d_tx_ptr_bound1 = 0, vex_num = 0;
   ...
   /*FILL_LEVEL_TRIGGER falg will be assosiated with corresponding VEX code as well, if we want to disable this flag
    ...
    for (; i <  (ARRAY_SIZE(g_cm_1_config_tbl)); i++)
    {
        ...
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm_1_config_tbl[i].offset), vex_num_param[1], \
                g_cm_1_config_tbl[i].mask, g_cm_1_config_tbl[i].name))
            return rv;
    }
    ...
    for (i = 0; i < ARRAY_SIZE(g_v2d_flow6_config_tbl) - 7; i++)
    {
...
int main(int argc, const char *argv[])
{
    ...
    printf ("INFO: argv[7] : %d = test_config\n", test_config);
