...
if arg_len != 9:
    ...
    print("python flow6_2Tx_1Rx.py <tx0_vex_id> <rx_vex_id> <tx1_vex_id> <v2d_id> <apb_id> <split_method> <drf_clock> <test_config>\n")
    print("As there were no inputs provided executing with below parameters\n")
    print(" tx0_vex_id : 0\n  rx_vex_id : 1\n tx1_vex_id : 8\n v2d_id : 0\n apb_id : 0\n split_method : 0\n drf_clock : 0\n test_config: 4")
    param = [0, 1, 8, 0, 0, 0, 0, 4]
else:
...
V2D.v2d_check(param[3])
V2D.vex_check(param[0], param[3])
V2D.vex_check(param[1], param[3])
V2D.vex_check(param[2], param[3])
V2D.apb_id_check(param[4])
V2D.split_method_check(param[5])
