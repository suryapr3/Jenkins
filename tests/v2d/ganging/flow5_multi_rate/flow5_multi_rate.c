...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, int test_config)
{
    ...
    for (; i < ARRAY_SIZE(g_v2d_flow5_config_tbl) - 4; i++)
    {
        ...
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, PDPD_SECONDARY_INSTANCE) + g_v2d_flow5_config_tbl[i].offset), \
                    (g_v2d_flow5_config_tbl[i].value | (lut_mode << 12) | (pdpd_out_rate << 4) | pdpd_in_rate), \
                    g_v2d_flow5_config_tbl[i].mask, g_v2d_flow5_config_tbl[i].name))
            return rv;
    }
...
int main(int argc, const char *argv[])
{
    ...
    /*enabling the V2D clock for both the channels for the given V2D*/
    ...
    if (0 == test_status)
    {
        printf("Host:---- SUCCESS ---- vex_tx_b : %d, vex_tx_c : %d, vex_rx : %d, v2d_num : %d, drf_clock: %s, \
                pdpd_in_rate : %s, pdpd_out_rate : %s, lut_mode : %d",
                vex_num_param[0], vex_num_param[1], vex_num_param[2], v2d_num, drf_clock_str[drf_clock],
                pdpd_rate_str[pdpd_in_rate], pdpd_rate_str[pdpd_out_rate], g_test_config_tbl[test_config][2]);
    }
    else
    {
        printf("Host:---- FAIL ---- vex_tx_b : %d, vex_tx_c : %d, vex_rx : %d, v2d_num : %d, drf_clock: %s, \
                pdpd_in_rate : %s, pdpd_out_rate : %s, lut_mode : %d",
                vex_num_param[0], vex_num_param[1], vex_num_param[2], v2d_num, drf_clock_str[drf_clock],
                pdpd_rate_str[pdpd_in_rate], pdpd_rate_str[pdpd_out_rate], g_test_config_tbl[test_config][2]);
        rv = 1;
