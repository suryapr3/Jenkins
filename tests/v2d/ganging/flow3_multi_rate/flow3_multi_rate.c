 ...
 * @param [in]      Vex num, V2d num , apb_num, drf_clock , test_config and post_filter_mode
 * @return          status value
 */
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, int test_config, int post_filter_mode)
{
    ...
    for (; i < ARRAY_SIZE(g_v2d_flow3_config_tbl) - 2; i++)
    {
        ...
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, PDPD_SECONDARY_INSTANCE) + g_v2d_flow3_config_tbl[i].offset), \
                    (g_v2d_flow3_config_tbl[i].value | (lut_mode << 12) | (pdpd_out_rate << 4) | pdpd_in_rate), \
                    g_v2d_flow3_config_tbl[i].mask, g_v2d_flow3_config_tbl[i].name))
            return rv;
    }
    ...
    /*For PDPD Main instance*/
    rv = configure_pdpd(v2d_num, PDPD_MAIN_INSTANCE/*apb_num*/, test_config);
    /*For PDPD Secondary instance*/
    rv = configure_pdpd(v2d_num, PDPD_SECONDARY_INSTANCE/*apb_num*/, test_config);
...
int main(int argc, const char *argv[])
{
    ...
    if (rv = input_pattern_read(val_inp, "./test_vectors/input_iq.txt"))
    {
    ...
    if (rv = ref_output_read(host_ref_data, (VEX_VMEM_IN_VEC)/*total number of vectors*/,
    ...
    if (rv = get_tx_pdpd_wm_hilo(v2d_num, PDPD_MAIN_INSTANCE/*apb_num*/, 1/*Number of streams*/, watermark_buf))
    {
