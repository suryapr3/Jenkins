...
V2D = fw.ABC_V2D()
wa = wb.active
header = ["tx_vex_id" ,"rx_vex_id" ,"v2d_id" ,"drf_clock" ,"test_config" ,"post_fliter_mode"]
row = 1
    ...
    for rx_vex_num in v2d_0_valid_vex_list:
        ...
        for test_config in test_config_list:
            for post_filter_mode in range(2):
                ...
                wa.cell(row,4).value = drf_clock
                wa.cell(row,5).value = test_config
                wa.cell(row,6).value = post_filter_mode
                row+=1
...
sheet_name= wb['Sheet']
sheet_name.title ='flow3_multi_rate_list'

wb.save("flow3_multi_rate_list.xlsx")
