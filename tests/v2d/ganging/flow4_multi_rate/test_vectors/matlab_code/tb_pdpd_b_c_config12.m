...
fileId1 = fopen("ref_output_config12.txt","w");
...
for blc_cnt = 0:8  %max blocks this value should be 32 , 16, 8, 4, 2 for p5, 1IQ, 2IQ, 4IQ and 8IQ rates
    i_add = i_add + 0x0001;
    j_add = j_add + 0x0002;
    for i = 0:63 % vectors per block this value should be 7, 15, 31, 63, 127 for p5, 1IQ, 2IQ, 4IQ and 8IQ rates
        for j = 0:31
...
for blc_cnt = 0:8  %max blocks this value should be 32 , 16, 8, 4, 2 for p5, 1IQ, 2IQ, 4IQ and 8IQ rates
    i_add = i_add + 0x0001;
    j_add = j_add + 0x0002;
    for i = 0:31 % vectors per block this value should be 7, 15, 31, 63, 127 for p5, 1IQ, 2IQ, 4IQ and 8IQ rates
        for j = 0:31
...
for i = 0:4     %append as many zeros of passthru delay
    fprintf(fileId, "%08x\n",0);
...
for blc_cnt = 0:8  %max blocks this value should be 32 , 16, 8, 4, 2 for p5, 1IQ, 2IQ, 4IQ and 8IQ rates
    for i = 0:31    % vectors per block this value should be 7, 15, 31, 63, 127 for p5, 1IQ, 2IQ, 4IQ and 8IQ rates
        for j = 0:31
            ...
            fprintf(fileId, "%08x\n", temp_var);
            if output_file_index > 1024  % value should be 256, 512, 1024, 2048 and 4096 for P5, 1IQ, 2IQ , 4IQ and 8IQ rates
                fprintf(fileId1, "%08x\n", temp_var);
            end
