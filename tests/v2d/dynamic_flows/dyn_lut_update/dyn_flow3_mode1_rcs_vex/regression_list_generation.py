...
V2D = fw.ABC_V2D()
wa = wb.active
header = ["tx_vex_id" ,"adapt_vex_id" ,"v2d_id" ,"apb_id" ,"drf_clock" ,"test_config" ,"post_fliter_mode", "delay_value", "capture_delay_value"]
row = 1
...
for tx_vex_num in v2d_1_valid_vex_list:
    for adapt_vex_num in v2d_1_valid_vex_list:
        if (adapt_vex_num != 34):
            continue
        if(tx_vex_num == adapt_vex_num):
            continue
...
for tx_vex_num in v2d_2_valid_vex_list:
    for adapt_vex_num in v2d_2_valid_vex_list:
        if (adapt_vex_num != 36):
            continue
        if(tx_vex_num == adapt_vex_num):
            continue
...
for tx_vex_num in v2d_3_valid_vex_list:
    for adapt_vex_num in v2d_3_valid_vex_list:
        if (adapt_vex_num != 38):
            continue
        if(tx_vex_num == adapt_vex_num):
            continue
...
sheet_name= wb['Sheet']
sheet_name.title ='dyn_flow3_mode1_rcs_vex'

wb.save("dyn_flow3_mode1_rcs_vex_list.xlsx")
