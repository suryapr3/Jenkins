...
/**
 * @file    vex_flow3_tx.c
 * @brief   Vex FW code to validate Dynamic LUT update.
 * @details 1. VEX will send the data continuously to the HOST until there was an indication from
 *             HOST to stop the transmission.
 *          2. After every data block that was sent a block number will be sent to GBUFFER_TX VEX.
**/
...
volatile int AT (0x1c) g_num_vecs_in;
...
volatile int AT(0x24) g_vex_indication_address;
...
void v2d_flow3_tx() ENTRY
{
    ...
    int add_offset = 0;
    int src_addr = 0x0;
    int src_compare_offset = 0;
    int limit = 0, rewind = 0, cnt = 0;
    g_host_indicator = 0;
#if DEBUG
...
//#pragma hive force_unroll=4
...
#endif
            v_xstomi_1024(Any, dest_address, offset, LD_OFFSET_MAX, vin1, offset);
        ...
        v_xsto_32(Any, g_vex_indication_address, 0, cnt);
