 ...
 *********************************************************************************************/
/**
 * @file    dyn_flow3_mode1_vex_vex.c
 ...
 * @details 1. Configuration of VEX, V2D's Common Module for sending data from VEX to V2D via B path
   ...
   @TC Description:
        ...
        3. The GMP and DDR terms will be updated by ADAPT VEX core at the boot time and at regular intervals and the corresponding
           LUT coeffecients will be updated in the VEX VMEM by the HOST.
        4. Host will be waiting on a memory location where ADAPT VEX will update an indication to indicate it has transferred the
           first set of LUT's towards PDPD.
        5. The Initial LUT will be transferred from SHADOW to LIVE via immediate transfer mode from VEX and this will be verified by HOST.
...
void lut_mem_copy(unsigned int (*dpd_coeff_buffer)[NUM_WORDS_PER_LUT],
                   ...
                   unsigned int (*dpd_coeff_buffer3)[NUM_WORDS_PER_LUT],
                   int vex_num,
                   lut_mode_t lut_mode);
...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, int test_config, int post_filter_mode)
{
    ...
    rv = dynamic_gbuffer_update_configure_pdpd(v2d_num, apb_num, test_config, gmp_terms, V2D_PDPD_DDR_TERMS);
                   ...
                   lut_mode_t lut_mode)
{
    ...
    unsigned int *buffer_address[] = {dpd_coeff_buffer[0], dpd_coeff_buffer1[0], dpd_coeff_buffer2[0], dpd_coeff_buffer3[0]};
...
int main(int argc, const char *argv[])
{
    ...
    int gbuffer_tx_val_args[] = {120, B2H_MAP_VALUE_G_COEFF, (CYCLES_PER_BLOCK_983M - 1), 0, MAX_LUT_UPDATES};
    ...
    /*loading the 4 LUTs to VEX_ADAPT*/
    lut_mem_copy(dpd_coeff_buffer, dpd_coeff_buffer_1, dpd_coeff_buffer_2, dpd_coeff_buffer_3, vex_num_param[1], lut_mode);

    /*Start the VEX ADAPT core*/
    vex_start(vex_num_param[1], 0);

    loop_wait_count = 0;
    usleep(10000);

    /*waiting for VEX_ADAPT to complete transfer of LUT0(boot time LUT)*/
    while (read_reg_18a(VEX_DMEM(vex_num_param[1]) + HIVE_ADDR_v2d_adapt_mode1_g_host_indicator, "GBUF_HOST_INDICATION") != 1)
    {
        // do nothing wait in this loop until VEX indication
        if (loop_wait_count >= 30)
        {
            printf("WARNING: Breaking the loop while VEX didn't write the coeff");
            break;
        }
        loop_wait_count ++;
        usleep(10000);
