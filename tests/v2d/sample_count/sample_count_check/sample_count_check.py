...
if arg_len != 6:
    ...
    print("python sample_count_check.py <vex_id> <v2d_id> <apb_id> <drf_clock> <test_config>\n")
...
if not (param[4] >= 0 and param[4] <= 16):
    print("test_config should be between 0 to 16\n")
...
if not (param[3] >= 0 and param[3] <= 1):
    print("drf_clock should be either 0 (983MHz clock) or 1 (1.4MHz clock)")
    sys.exit("proper input parameters were not provided\n")
...
if not (param[1] >= 0 and param[1] <= 3):
    print("provided v2d_num is not with in the range,v2d_num should be between [0,3]\n")
    sys.exit("proper input parameters were not provided\n")
...
if (param[1] == 0 and not ((param[0] % 8) == 0)):
    ...
    print(f"vex_id: {param[0]}, v2d_id: {param[1]}, apb_id: {param[2]}, drf_clock: {param[3]}, test_config: {param[4]}\n")
