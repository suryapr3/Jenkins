 ...
 *********************************************************************************************/
/**
 * @file    quad_flow3_pdpd_ingress_repeat.c
...
int main(int argc, const char *argv[])
{
    ...
    int vex_num = 0, vex_num_param[NUM_VEX_CORES] = {0}, v2d_num, apb_num, underrun_reg_value = 0, drf_clock = 0, test_config = 0,
        ...
        ant3_cycles = 0, exp_b2h_value = 0, num_captures = 0;
    ...
    ant0_cycles = (int) argv[4];
    ant1_cycles = (int) argv[5];
    ant2_cycles = (int) argv[6];
    ant3_cycles = (int) argv[7];
    ...
    LOG_INFO("argv[4] : %d = ant0_cycles", ant0_cycles);
    LOG_INFO("argv[5] : %d = ant1_cycles", ant1_cycles);
    LOG_INFO("argv[6] : %d = ant2_cycles", ant2_cycles);
    LOG_INFO("argv[7] : %d = ant3_cycles", ant3_cycles);
    ...
    v2d_rcs_host_params.num_antennas = 4;
    ...
    write_mem_18a(RSM_MEM_OFFSET, (sizeof(v2d_rcs_host_params) / sizeof(int)), (void *)&v2d_rcs_host_params, "RCS_HOST_PARAM");
#endif
    ...
    if (quad_mem_fb_x_capture_repeat_mode_configuration(quad_instance, 4/*num_antennas*/, size_per_antenna/*size_per_antenna*/))
    {
    ...
    exp_b2h_value = ((ant0_cycles / cycles_per_block) + (2 * capture_blocks_per_antenna)) * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_out_rate]);
    ...
    if ((4 * capture_blocks_per_antenna * num_captures == rd_wr_value[4]) && (0xFFFF == rd_wr_value[12]))
    {
    ...
    if (0 == test_status)
    {
        printf("Host:---- SUCCESS ---- quad_instance: %d, drf_clock : %s, pdpd_in_rate: %s, \
                pdpd_out_rate: %s, lut_mode: %d, delay_value: %d, ant0_cycles: %d, ant1_cycles: %d, ant2_cycles: %d, ant3_cycles: %d\
                capture_blocks_per_antenna: %d, skip_cycles: %d, num_captures: %d\n",
                ...
                ant3_cycles, capture_blocks_per_antenna, skip_cycles, num_captures);
    }
    else
    {
