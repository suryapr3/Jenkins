 ...
 *********************************************************************************************/
/**
 * @file    flow3_pdpd_ingress_repeat_2_ch.c
 * @brief   HOST Configuration code for evaluating the PDPD block with GMP and DDR terms enabled
 *          with QUAD delayed start for any two channels of the QUAD will be fed with data along
...
int main(int argc, const char *argv[])
{
    ...
    int vex_num = 0, vex_num_param[NUM_VEX_CORES] = {0}, v2d_num, apb_num, underrun_reg_value = 0, drf_clock = 0, test_config = 0,
        ...
        ant3_cycles = 0, exp_b2h_value = 0, num_captures = 0, configured_channel[2] = {0};
    ...
    test_config = argv[4];
    delay_value = (int) argv[5];
    ant0_cycles = (int) argv[6];
    ant1_cycles = (int) argv[7];
    capture_blocks_per_antenna = (int)  argv[8];
    skip_cycles = (int) argv[9];
    num_captures = (int) argv[10];
    pdpd_in_rate = g_test_config_tbl[test_config][0];
    pdpd_out_rate = g_test_config_tbl[test_config][1];
    ...
    LOG_INFO("argv[3] : %d = drf_clock %s", drf_clock, drf_clock_str[drf_clock]);
    LOG_INFO("argv[4] : %d = test_config", test_config);
    LOG_INFO("argv[5] : %d = delay_value", delay_value);
    LOG_INFO("argv[6] : %d = ant0_cycles", ant0_cycles);
    LOG_INFO("argv[7] : %d = ant1_cycles", ant1_cycles);
    LOG_INFO("argv[8] : %d = capture_blocks_per_antenna", capture_blocks_per_antenna);
    LOG_INFO("argv[9] : %d = skip_cycles", skip_cycles);
    LOG_INFO("argv[10] : %d = num_captures", num_captures);
    ...
    if ((2 * capture_blocks_per_antenna * num_captures == rd_wr_value[4]) && (0xFFFF == rd_wr_value[12]))
    {
    ...
    if (0 == test_status)
    {
        printf("Host:---- SUCCESS ---- quad_instance: %d, drf_clock : %s, pdpd_in_rate: %s, \
                pdpd_out_rate: %s, lut_mode: %d, delay_value: %d, ant0_cycles: %d, ant1_cycles: %d,\
                capture_blocks_per_antenna: %d, skip_cycles: %d, num_captures: %d, Ant0: %d, Ant1: %d\n",
                quad_instance, drf_clock_str[drf_clock], pdpd_rate_str[pdpd_in_rate],
                pdpd_rate_str[pdpd_out_rate], g_test_config_tbl[test_config][2], delay_value, ant0_cycles, ant1_cycles,
                capture_blocks_per_antenna, skip_cycles, num_captures, configured_channel[0], configured_channel[1]);
    }
    else
    {
