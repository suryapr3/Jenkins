 ...
 *********************************************************************************************/
/**
 * @file    quad_flow3_pdpd_ingress.c
 * @brief   HOST Configuration code for evaluating the PDPD block with GMP and DDR terms enabled
 *          with QUAD delayed start and all four channels of the QUAD will be fed with data along
 *          with enabling the X-Capture at PDPD ingress
 * @details 1. Configuration of VEX, V2D's Common Module for sending data from VEX to V2D via B path
   ...
   @TC Description:
        ...
        7. All the path B of QUAD channels needs to be configured and all channels will be
           fed with the data.
        8. All the PDPD channels of the QUAD will be triggered with a timed value and the delay value can be passed as
           run time parameter through python script.
        9. X-Data capture at PDPD Ingress was enabled in single shot mode for 256K samples and corresponding MEM_FB configuration
           was done from the HOST.
        10. X-Data capture will be triggered from BFN timed trigger which will be programmed from RCS.
        11. Host will read and compare the 256K samples from Capture memory to evaluate PASS FAIL criteria.
        12. This TC supports below rates
            0.5IQ 1IQ 2IQ 4IQ and 8IQ
        13. VEX CORES triggering along with the BFN programming will be triggered from RCS.
 */
...
#define SAMPLES_PER_ANTENNA 65536
/***********************************************
...
#if QUAD_DELAYED_CAL    //index 3  
    /* fill level setting, enable fill level */
    {V2D_CSR_OFFSET + AV2D_CSR_TX_BUF_WM_TX_START_WM_OFFSET, AV2D_CSR_TX_BUF_WM_TX_START_WM_START_B_WM_EN_BF_MSK, \
      AV2D_CSR_TX_BUF_WM_TX_START_WM_WR_MASK, "WM_START_B_WM_EN"},
        
    //dsiable fill level trigger
    //{V2D_CSR_OFFSET + AV2D_CSR_TX_BUF_WM_TX_START_WM_OFFSET, 0, \
...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, int test_config, int post_filter_mode)
{
    ...
    for (i = 0; i < ARRAY_SIZE(g_v2d_flow3_config_tbl) - 6; i++)
    {
    ...
    // for B flow, fill level set as 1.5L 
  
    fill_value = (VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_in_rate]))*3/2;
    //experiment with a other fill value
    //fill_value = fill_value -2; //0.5 and 8gsps worked ok
   // fill_value = fill_value -8 ;//under run opbserved for 0.5 gsps;
   // fill_value = fill_value +2; // no issues found
   // fill_value = fill_value +8; 
   //fill_value = fill_value +9; 
   // fill_value = fill_value +10; 
    //fill_value = fill_value +55; // 8gsps passed, but lower rates 0.5 and 1failed, saying VEX did not process enough vectors
    //LOG_INFO("QUAD_DELAYED_CAL: ;g_vectors_per_block[drf_clock][pdpd_in_rate]=%d\n",g_vectors_per_block[drf_clock][pdpd_in_rate]);
    LOG_INFO("QUAD_DELAYED_CAL: ;fill_value B_path=%d, L value = %d \n",fill_value,(g_vectors_per_block[drf_clock][pdpd_in_rate])*2);
    // adjusting the bitfiled for B path
    fill_value = fill_value <<16;

    for (; i< (ARRAY_SIZE(g_v2d_flow3_config_tbl) - 5); i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow3_config_tbl[i].offset), (g_v2d_flow3_config_tbl[i].value | fill_value), g_v2d_flow3_config_tbl[i].mask, g_v2d_flow3_config_tbl[i].name))
                return rv;
    }  
#else //QUAD_DELAYED_CAL 
...
int main(int argc, const char *argv[])
{
    ...
    int vex_num = 0, vex_num_param[NUM_VEX_CORES] = {0}, v2d_num, apb_num, underrun_reg_value = 0, drf_clock = 0, test_config = 0,
        post_filter_mode = 0, quad_instance = 0, v2d_index = 0, delay_value = 0, capture_delay_value = 0, exp_b2h_value = 0;
    /*Value indicates the delay between consecutive antenna captures*/
    int delay_antenna_capture = 0;
    ...
    uint32_t size_per_antenna = 0;
    v2d_shdw_live_reg_check_t reg_check;
    quad_mem_compare_t quad_mem_compare;

#if RCS_TRIGGER
    srp_rcs_config_t rcs_params;
    srp_rcs_rcp_t rcs_core_id = SRP_RCS_RCP_CTRL_AX;
    // corresponds to quad_instance, delay value, capture_delay value, delay_antenna_capture, pdpd_in_rate, drf clock
    int rcs_host_param[7] = {0};
#endif
    ...
    test_config = argv[2];
    delay_value = (int) argv[3];
    capture_delay_value = (int) argv[4];
    delay_antenna_capture = (int) argv[5];
    pdpd_in_rate = g_test_config_tbl[test_config][0];
    pdpd_out_rate = g_test_config_tbl[test_config][1];

    LOG_INFO("argv[0] : %d = quad_instance", quad_instance);
    LOG_INFO("argv[1] : %d = drf_clock %s", drf_clock, drf_clock_str[drf_clock]);
    LOG_INFO("argv[2] : %d = test_config", test_config);
    LOG_INFO("argv[3] : %d = delay_value", delay_value);
    LOG_INFO("argv[4] : %d = start_capture_delay_value", capture_delay_value);
    LOG_INFO("argv[5] : %d = delay_antenna_capture", delay_antenna_capture);
    ...
    {
        LOG_ERROR("v2d_quad_clk_enable failed");
        return rv;
    }
    ...
    rcs_host_param[2] = capture_delay_value + CAPTURE_DELAY_DELTA_VALUE;
    rcs_host_param[3] = delay_antenna_capture;
    rcs_host_param[4] = pdpd_in_rate;
    rcs_host_param[5] = drf_clock;
    rcs_host_param[6] = SAMPLES_PER_ANTENNA - (SAMPLES_PER_ANTENNA % ((g_vectors_per_block[drf_clock][pdpd_in_rate]) * SAMPLES_PER_VEC));

    //clear the dram
    memset(rd_wr_value, 0, sizeof(rd_wr_value));
    write_mem_18a(g_dcode_buffer[0], 32, rd_wr_value, "DCODE_BUFFER");
    write_mem_18a(g_rsm_buffer[0], 1, rd_wr_value, "HANDSHAKE_FLAG");
    ...
    size_per_antenna = SAMPLES_TO_FB_MEM_SIZE(SAMPLES_PER_ANTENNA);

    /*Perform the FB_MEM configuration for X Capture for a QUAD*/
    if (quad_mem_fb_x_capture_configuration(SINGLE_CAPTURE_MODE, quad_instance, 4/*num_antennas*/, size_per_antenna/*size_per_antenna*/))
    {
        LOG_ERROR("FB_MEM configuration failed");
    }

    /*Prefill the MEM_FB with known pattern*/
#if !REGRESSION
    fill_mem_fb(g_v2d_instance_in_quad[quad_instance][0], MEM_FB_INIT_VALUE);
#endif
	...
	//read the START_LM_COUNT for both the V2Ds in the quad 
	//for B path, read the PDPD0 and 1 values for both the V2Ds
	
	uint pdpd0_start_l_cnt_read_value =0, pdpd1_start_l_cnt_read_value=0, fill_trigger_value=0;
	v2d_index = 0;

	v2d_num = g_v2d_instance_in_quad[quad_instance][v2d_index];

	if (quad_instance==0){
		pdpd0_start_l_cnt_read_value = read_reg_18a(V2D0_BFN_CSR_BASE+BFN_CSR_START_L_CNT2_OFFSET, "Even V2D BFN_CSR_START_L_CNT2_OFFSET");
		pdpd1_start_l_cnt_read_value = read_reg_18a(V2D0_BFN_CSR_BASE+BFN_CSR_START_L_CNT3_OFFSET, "Even V2D BFN_CSR_START_L_CNT3_OFFSET");	
		printf("********************QUAD_DELAYED_CAL:QUAD=%d V2Did = %d, IN_RATE (%s),START_L_CNT2 =%d , START_L_CNT3 = %d\n",quad_instance,v2d_num,pdpd_rate_str[g_test_config_tbl[test_config][0]],pdpd0_start_l_cnt_read_value,pdpd1_start_l_cnt_read_value);
		...
		//for odd V2D in the Quad    
		v2d_num = v2d_num+1;
		pdpd0_start_l_cnt_read_value = read_reg_18a(V2D1_BFN_CSR_BASE+BFN_CSR_START_L_CNT2_OFFSET, "Odd V2D BFN_CSR_START_L_CNT2_OFFSET");
		pdpd1_start_l_cnt_read_value = read_reg_18a(V2D1_BFN_CSR_BASE+BFN_CSR_START_L_CNT3_OFFSET, "Odd V2D BFN_CSR_START_L_CNT3_OFFSET");
		printf("********************QUAD_DELAYED_CAL: QUAD=%d V2Did = %d, IN_RATE (%s),START_L_CNT2 =%d , START_L_CNT3 = %d\n",quad_instance,v2d_num,pdpd_rate_str[g_test_config_tbl[test_config][0]],pdpd0_start_l_cnt_read_value,pdpd1_start_l_cnt_read_value);
	} else if(quad_instance==1) {
		pdpd0_start_l_cnt_read_value = read_reg_18a(V2D2_BFN_CSR_BASE+BFN_CSR_START_L_CNT2_OFFSET, "Even V2D BFN_CSR_START_L_CNT2_OFFSET");
		pdpd1_start_l_cnt_read_value = read_reg_18a(V2D2_BFN_CSR_BASE+BFN_CSR_START_L_CNT3_OFFSET, "Even V2D BFN_CSR_START_L_CNT3_OFFSET");	
		printf("********************QUAD_DELAYED_CAL: QUAD=%d V2Did = %d, IN_RATE (%s),START_L_CNT2 =%d , START_L_CNT3 = %d\n",quad_instance,v2d_num,pdpd_rate_str[g_test_config_tbl[test_config][0]],pdpd0_start_l_cnt_read_value,pdpd1_start_l_cnt_read_value);
		//for odd V2D in the Quad    
		v2d_num = v2d_num+1;
		pdpd0_start_l_cnt_read_value = read_reg_18a(V2D3_BFN_CSR_BASE+BFN_CSR_START_L_CNT2_OFFSET, "Odd V2D BFN_CSR_START_L_CNT2_OFFSET");
		pdpd1_start_l_cnt_read_value = read_reg_18a(V2D3_BFN_CSR_BASE+BFN_CSR_START_L_CNT3_OFFSET, "Odd V2D BFN_CSR_START_L_CNT3_OFFSET");
		printf("********************QUAD_DELAYED_CAL: QUAD=%d V2Did = %d, IN_RATE (%s),START_L_CNT2 =%d , START_L_CNT3 = %d\n",quad_instance,v2d_num,pdpd_rate_str[g_test_config_tbl[test_config][0]],pdpd0_start_l_cnt_read_value,pdpd1_start_l_cnt_read_value);		
	}
    ...
    /*B2H VALUE will be in FLITS*/
    exp_b2h_value = ((SAMPLES_PER_ANTENNA / 16));
    ...
    for (lc = 0; lc < V2D_PER_QUAD; lc++)
    {
        for (apb_num = PDPD_MAIN_INSTANCE; apb_num < PDPD_MAX_INSTANCE; apb_num++)
        {
            ...
            /*There are no REF vectors for 1.5G clock for this specific input pattern hence excluding the comparison status check*/
...
#if QUAD_DELAYED_CAL
            printf("QUAD_DELAYED_CAL:V2d%d Ch%d low watermark value(Flits): %d, high watermark value: %d",
            ...
            while ((h2b_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_0_OFFSET + \
                            NSIP_HBI_H2B_FLIT_CNT_OFFSET, "H2B_FLIT_COUNT")) < exp_b2h_value)
            {
                // do nothing wait in this loop
                if (loop_wait_count >= 80)
                {
    ...
    {
#if QUAD_DELAYED_CAL
    printf("INFO: QUAD_DELAYED_CAL BFN count for Tx transmission to PDPD is not same across the QUAD channels,\
	as Expected  for WM fill level triggering");
#else	
        printf("ERROR: BFN count for Tx transmission to PDPD is not same across the QUAD channels");
#endif //QUAD_DELAYED_CAL		
    }

    memset(&quad_mem_compare, 0, sizeof(quad_mem_compare));

    quad_mem_compare.quad_instance = quad_instance;
    quad_mem_compare.additional_factor = val_kargs[0];
    quad_mem_compare.num_vectors_per_block = val_kargs[1];
    quad_mem_compare.capture_delay_value = capture_delay_value;
    quad_mem_compare.num_antenna = 4;
    quad_mem_compare.delay_value_between_antenna = delay_antenna_capture;
    quad_mem_compare.samples_per_antenna = SAMPLES_PER_ANTENNA;
    quad_mem_compare.drf_clk = drf_clock;
    quad_mem_compare.num_iter = 1;
	
#if !QUAD_DELAYED_CAL //     integrity check not required for calibration. This could fail as well,\
                             due to the fact that it is fill lelvel trigger in this mode and not a BFN timed trigger

    /*Capture Memory comparison*/
    if (rv = quad_pdpd_ingress_cap_mem_compare(&quad_mem_compare))
    {
        LOG_ERROR("FB_MEM comparison failed");
    }
#endif //!#if QUAD_DELAYED_CAL
    if (0 == test_status)
    {
        printf("Host:---- SUCCESS ---- quad_instance: %d, drf_clock : %s, pdpd_in_rate: %s, \
                pdpd_out_rate: %s, lut_mode: %d, delay_value: %d, start_capture_delay_value: %d, delay_antenna_capture: %d",
                quad_instance, drf_clock_str[drf_clock], pdpd_rate_str[pdpd_in_rate],
                pdpd_rate_str[pdpd_out_rate], g_test_config_tbl[test_config][2], delay_value, capture_delay_value, delay_antenna_capture);
    }
    else
    {
