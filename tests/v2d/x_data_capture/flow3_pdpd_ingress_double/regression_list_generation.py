...
V2D = fw.ABC_V2D()
wa = wb.active
header = ["tx_vex_id" ,"rx_vex_id" ,"v2d_id" ,"apb_id" ,"drf_clock" ,"test_config" ,"delay_value", "capture_delay_value", "capture_blocks", "skip_cycles"]
row = 1
...
if (0 == drf_clock):
    ...
    capture_blocks_list = [30, 50]
    skip_cycles_list = [1024, 2048]
else:
    capture_delay_value_list = [0, 768, 1536]
    capture_blocks_list = [10, 20]
    skip_cycles_list = [1536, 2304]
        ...
        for apb_num in range (2):
            for test_config in test_config_list:
                for capture_delay_value in capture_delay_value_list:
                    for capture_blocks in capture_blocks_list:
                        for skip_cycles in skip_cycles_list:
                            ...
                            wa.cell(row,7).value = delay_value_list[drf_clock]
                            wa.cell(row,8).value = capture_delay_value
                            wa.cell(row,9).value = capture_blocks
                            wa.cell(row,10).value = skip_cycles
                            row+=1
...
sheet_name= wb['Sheet']
sheet_name.title ='flow3_pdpd_ingress_double'

wb.save("flow3_pdpd_ingress_double_list.xlsx")
