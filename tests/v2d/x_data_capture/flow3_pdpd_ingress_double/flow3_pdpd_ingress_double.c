 ...
 * @details 1. Configuration of VEX, V2D's Common Module for sending data from VEX to V2D via B path
            ...
            3. Configure the MEM_FB to receive the data from PDPD Ingress where capture is configured in double
               mode and do the comparison from HOST.
            4. The limitation was the total capture size should be with in 1MB.
   @TC Description:
        ...
        8. X-Data capture will be triggered from BFN timed trigger in a DOUBLE mode which will be programmed from RCS.
        9. Host will read and compare received data from Capture memory to evaluate PASS FAIL criteria.
...
int main(int argc, const char *argv[])
{
    ...
    int vex_num = 0, vex_num_param[NUM_VEX_CORES] = {0}, v2d_num, apb_num, underrun_reg_value = 0, drf_clock = 0, test_config = 0,
        post_filter_mode = 0, delay_value = 0, capture_delay_value = 0, capture_samples = 0;
    uint32_t capture_blocks = 0, skip_cycles = 0, blocks_per_1mb = 0;
    ...
    repeat_mem_compare_t repeat_mem_compare;

#if RCS_TRIGGER
    srp_rcs_config_t rcs_params;
    srp_rcs_rcp_t rcs_core_id = SRP_RCS_RCP_CTRL_AX;
    /*corresponds to v2d_num, apb_num, tx_vex_num, rx_vex_num, drf_clock, delay value, capture delay value, capture samples
     * delay_between_consecutive_captures and capture_blocks*/
    int rcs_host_param[10] = {0};
#endif
    ...
    capture_delay_value = (int) argv[7];
    ...
    LOG_INFO("argv[7] : %d = capture_delay_value", capture_delay_value);
    ...
    if ((2 * capture_blocks) > blocks_per_1mb)
    {
        capture_blocks = (blocks_per_1mb / 2);
        LOG_WARNING("provided block number exceedes the blocks_per_1mb for the given data rate \
                     new value for capture_blocks: %d", capture_blocks);
    }

    capture_samples = capture_blocks * val_kargs[1] * SAMPLES_PER_VEC;

    LOG_INFO("capture_samples: 0x%x", capture_samples);

    /*Perform the FB_MEM configuration for X Capture*/
    if (mem_fb_x_capture_configuration(DOUBLE_CAPTURE_MODE, (SAMPLES_TO_FB_MEM_SIZE(capture_samples) - 1), v2d_num, apb_num))
    {
    ...
    /*There are no REF vectors for 1.5G clock for this specific input pattern hence excluding the Rx VEX comparison check.*/
    ...
    while ((h2b_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_0_OFFSET + \
            ...
            ((capture_delay_value / CYCLES_PER_BLOCK_983M) + (2 * capture_blocks)) * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_out_rate]))
    {
        // do nothing wait in this loop
        if (loop_wait_count >= 80)
        {
    ...
    repeat_mem_compare.v2d_num = v2d_num;
    repeat_mem_compare.additional_factor = val_kargs[0];
    repeat_mem_compare.num_vectors_per_block = val_kargs[1];
    repeat_mem_compare.capture_delay_value = capture_delay_value;
    repeat_mem_compare.drf_clk = drf_clock;
    repeat_mem_compare.num_repeat = 2;
    repeat_mem_compare.num_blocks = capture_blocks;
    repeat_mem_compare.skip_cycles = skip_cycles;

    if (rv = pdpd_ingress_cap_mem_compare_repeat(&repeat_mem_compare))
    {
    ...
    if (0 == test_status)
    {
        printf("Host:---- SUCCESS ---- tx_vex : %d, rx_vex : %d, v2d_num : %d, apb_num : %d, drf_clock : %s, pdpd_in_rate: %s, \
                pdpd_out_rate: %s, lut_mode: %d, delay_value: %d, capture_delay_value: %d, capture_blocks: %d, \
                ...
                pdpd_rate_str[pdpd_out_rate], g_test_config_tbl[test_config][2], delay_value, capture_delay_value,
                capture_blocks, skip_cycles);
    }
    else
    {
