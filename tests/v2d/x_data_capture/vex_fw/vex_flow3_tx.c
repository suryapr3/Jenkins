 ...
 * @details 1. VEX will send the required number of vectors for every 512 clock cycles until there
 *             is a host indication to stop the transfer.
**/
...
//vec32i32 AT(RX_SRC_BUFFER_BASE) vbuffer[VEX_MEM_SIZE];
vec32i32 AT(TX_SRC_BUFFER_BASE) vbuffer[VEX_MEM_SIZE];
#else
...
volatile int AT(0x20) g_scalar_val;
volatile int AT(0x24) g_num_vecs_in;
...
void v2d_flow3_tx() ENTRY
{
    ...
    g_host_indicator = 0;
#if LIVEIQ_SAMPLES
    //int src_address = RX_SRC_BUFFER_BASE, compare_offset = 0;
    int src_address = TX_SRC_BUFFER_BASE, compare_offset = 0;

    //int limit =  ((BYTES_PER_VEC) << 20) | ((g_num_blocks * g_num_vecs_in * BYTES_PER_VEC) - (BYTES_PER_VEC));
    int limit =  ((BYTES_PER_VEC) << 20) | ((g_num_blocks * g_num_vecs_in * BYTES_PER_VEC) - (BYTES_PER_VEC))
    int rewind = (g_num_blocks * g_num_vecs_in * BYTES_PER_VEC) - (BYTES_PER_VEC); // for 1gsps (18*16*32 - 32 )= 9184
...
//#pragma hive force_unroll=4
        ...
        for(i = 0; i < iter_max; i++)
        {
            vec32i32 vin1, vout;
#if LIVEIQ_SAMPLES
          v_ldoi_1024_circ(Any, src_address, compare_offset, limit, rewind, vin1, compare_offset) NO_ALIAS SYNC_WITH(buffer_read);
#else
            vin1= vbuffer[i];
#endif //LIVEIQ_SAMPLES
#if DEBUG
...
#if LIVEIQ_SAMPLES
            v_xstomi_1024(Any, dest_address, offset, LD_OFFSET_MAX, vin1, offset); //jelli
#else
