 ...
 *********************************************************************************************/
/**
 * @file    flow3_pdpd_ingress_continuous.c
 ...
 * @details 1. Configuration of VEX, V2D's Common Module for sending data from VEX to V2D via B path
            ...
            3. Configure the MEM_FB to receive the data from PDPD Ingress and do the comparison from HOST
               for 1MB captured data which was captured in continuous mode.
   @TC Description:
        ...
        8. X-Data capture start and end will be triggered from BFN timed trigger which will be programmed from RCS.
...
int main(int argc, const char *argv[])
{
    ...
    if (mem_fb_x_capture_configuration(CONTINUOUS_CAPTURE_MODE, 0x3FFF, v2d_num, apb_num))
    {
    ...
    rcs_host_param[7] = capture_blocks * cycles_per_block;  // equal to capture duration
    ...
    write_mem_18a(g_rsm_buffer[0], 1, rd_wr_value, "RSM_MEMORY");
