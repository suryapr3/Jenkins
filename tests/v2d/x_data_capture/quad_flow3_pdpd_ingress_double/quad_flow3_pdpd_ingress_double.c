 ...
 *********************************************************************************************/
/**
 * @file    quad_flow3_pdpd_ingress_double.c
 ...
 *          with enabling the X-Capture at PDPD ingress in double configuration.
 * @details 1. Configuration of VEX, V2D's Common Module for sending data from VEX to V2D via B path
            ...
            3. Configure the MEM_FB to receive the data from PDPD Ingress and do the comparison from HOST in
               double capture mode at max for 1MB.
   @TC Description:
...
int main(int argc, const char *argv[])
{
    ...
    uint32_t size_per_antenna = 0, blocks_per_1mb = 0;
    ...
    // capture blocks per antenna and consecutive capture delay
    ...
    capture_blocks_per_antenna = (int)  argv[6];
    consecutive_capture_delay = (int) argv[7];
    pdpd_in_rate = g_test_config_tbl[test_config][0];
    pdpd_out_rate = g_test_config_tbl[test_config][1];
    ...
    LOG_INFO("argv[6] : %d = capture_blocks_per_antenna", capture_blocks_per_antenna);
    LOG_INFO("argv[7] : %d = consecutive_capture_delay", consecutive_capture_delay);
    ...
    if ((4 * 2 * capture_blocks_per_antenna) > blocks_per_1mb)
    {
        capture_blocks_per_antenna = (blocks_per_1mb / 8);
        LOG_WARNING("provided block number exceedes the blocks_per_1mb for the given data rate \
                     new value for capture_blocks: %d", capture_blocks_per_antenna);
    }
    ...
    rcs_host_param[6] = capture_blocks_per_antenna;
    rcs_host_param[7] = consecutive_capture_delay;
    ...
    size_per_antenna = SAMPLES_TO_FB_MEM_SIZE(capture_blocks_per_antenna * g_vectors_per_block[drf_clock][pdpd_in_rate] * SAMPLES_PER_VEC);

    /*Perform the FB_MEM configuration for X Capture for a QUAD*/
    if (quad_mem_fb_x_capture_configuration(DOUBLE_CAPTURE_MODE, quad_instance, 4/*num_antennas*/, size_per_antenna/*size_per_antenna*/))
    {
    ...
    exp_b2h_value = ((capture_delay_value / cycles_per_block) + (2 * capture_blocks_per_antenna)) * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_out_rate]);
    ...
    quad_mem_compare.samples_per_antenna = (capture_blocks_per_antenna * g_vectors_per_block[drf_clock][pdpd_in_rate] * SAMPLES_PER_VEC);
    quad_mem_compare.drf_clk = drf_clock;
    quad_mem_compare.num_iter = 2;
    quad_mem_compare.consecutive_capture_delay = consecutive_capture_delay;
