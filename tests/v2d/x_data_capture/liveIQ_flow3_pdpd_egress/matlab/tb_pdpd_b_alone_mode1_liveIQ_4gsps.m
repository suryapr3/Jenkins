...
temp1 = dlmread( 'liveIQ_ref_LUTall_64_i_4gsps.txt');
temp2 = dlmread( 'liveIQ_ref_LUTall_64_q_4gsps.txt');
temp3 = dlmread( 'liveIQ_ref_LUTall_64_i.txt');
temp4 = dlmread( 'liveIQ_ref_LUTall_64_q.txt');
temp5 = dlmread( 'liveIQ_FP_LUTall_64_i_4gsps.txt');
temp6 = dlmread( 'liveIQ_FP_LUTall_64_q_4gsps.txt');
...
% selecting first 16GMP terms and first 8 DDR terms for 4GSPS and  then
% applying duplication  logic on the coefts.

%gmp terms
gmp_index = 1;
i = 1 ;
term_types_4gsps = zeros(48,1);
q_dlys_4gsps = zeros(48,1);
m_dlys_4gsps = zeros(48,1);
lut_c_i_4gsps = zeros(48,65);
lut_c_q_4gsps = zeros(48,65);
lut_s_4gsps = zeros(48,1);
%M = zeros(array_size,8);
while  ( (gmp_index<=16) & (i<=48) )
    if  ( (term_types(i)==0) | (term_types(i)==1) |(term_types(i)==2) )
        term_types_4gsps(gmp_index) = term_types(i);
        q_dlys_4gsps(gmp_index) = q_dly(i);
        m_dlys_4gsps(gmp_index) = m_dly(i);
        lut_c_i_4gsps (gmp_index,1:65) = lut_c_i(i,1:65);
        lut_c_q_4gsps (gmp_index,1:65) = lut_c_q(i,1:65);
        lut_s_4gsps (gmp_index) = lut_s(i);
...
ddr_index = 1;
i = 1;
while ( (ddr_index<=8)& (i<=48))
    if  ( (term_types(i)==3) | (term_types(i)==4) |(term_types(i)==5) )
        term_types_4gsps(ddr_index+16) = term_types(i);
        q_dlys_4gsps(ddr_index+16) = q_dly(i);
        m_dlys_4gsps(ddr_index+16) = m_dly(i);
        lut_c_i_4gsps ((ddr_index+16),1:65) = lut_c_i(i,1:65);
        lut_c_q_4gsps ((ddr_index+16), 1:65) = lut_c_q(i,1:65);
        lut_s_4gsps (ddr_index+16) = lut_s(i);
...
term_types = term_types_4gsps;
q_dly = q_dlys_4gsps;
m_dly = m_dlys_4gsps;
lut_c_i = lut_c_i_4gsps ;
lut_c_q = lut_c_q_4gsps;
lut_s = lut_s_4gsps;


fileId = fopen("liveIQ_lut_c_i_4gsps.txt","w");
fileId1 = fopen("liveIQ_lut_c_q_4gsps.txt","w");
...
%
fileId = fopen("liveIQ_ref_LUTall_64_i_4gsps.txt","w");
fileId1 = fopen("liveIQ_ref_LUTall_64_q_4gsps.txt","w");
