 ...
 *********************************************************************************************/
/**
 * @file    flow3_pdpd_ingress_repeat_3_ch.c
 * @brief   HOST Configuration code for evaluating the PDPD block with GMP and DDR terms enabled
 *          with QUAD delayed start for any three channels of the QUAD will be fed with data along
 *          with enabling the X-Capture at PDPD ingress in repeat configuration.
 * @details 1. Configuration of VEX, V2D's Common Module for sending data from VEX to V2D via B path
            ...
            3. Configure the MEM_FB to receive the data from PDPD Ingress and do the comparison from HOST in
               repeat capture mode.
   @TC Description:
        ...
        7. All the path B of QUAD channels needs to be configured and only two channels will be
           fed with the data.
        8. The two PDPD channels of the QUAD will be triggered with a timed value and the delay value can be passed as
           run time parameter through python script.
        9. X-Data capture at PDPD Ingress was enabled in repeat mode and corresponding MEM_FB configuration
           was done from the HOST.
        10. X-Data capture will be triggered from BFN timed trigger which will be programmed from RCS.
        11. Host will read and compare the programmed number of samples from Capture memory to evaluate PASS FAIL criteria.
...
int main(int argc, const char *argv[])
{
    ...
    int vex_num = 0, vex_num_param[NUM_VEX_CORES] = {0}, v2d_num, apb_num, underrun_reg_value = 0, drf_clock = 0, test_config = 0,
        post_filter_mode = 0, quad_instance = 0, v2d_index = 0, delay_value = 0, ant0_cycles = 0, ant1_cycles = 0, ant2_cycles = 0,
        ant3_cycles = 0, exp_b2h_value = 0, num_captures = 0, configured_channel[3] = {0};
    uint32_t capture_blocks_per_antenna = 0, max_blocks_per_antenna = 0, cycles_per_block = 0, capture_cycles = 0,
             skip_cycles = 0;
    ...
    configured_channel[0] = (int) argv[1];
    configured_channel[1] = (int) argv[2];
    configured_channel[2] = (int) argv[3];
    drf_clock = (int) argv[4];
    test_config = argv[5];
    delay_value = (int) argv[6];
    ant0_cycles = (int) argv[7];
    ant1_cycles = (int) argv[8];
    ant2_cycles = (int) argv[9];
    capture_blocks_per_antenna = (int)  argv[10];
    skip_cycles = (int) argv[11];
    num_captures = (int) argv[12];
    pdpd_in_rate = g_test_config_tbl[test_config][0];
    pdpd_out_rate = g_test_config_tbl[test_config][1];

    LOG_INFO("argv[0] : %d = quad_instance", quad_instance);
    LOG_INFO("argv[1] : %d = configured_channel[0]", configured_channel[0]);
    LOG_INFO("argv[2] : %d = configured_channel[1]", configured_channel[1]);
    LOG_INFO("argv[3] : %d = configured_channel[2]", configured_channel[1]);
    ...
    LOG_INFO("argv[7] : %d = ant0_cycles", ant0_cycles);
    LOG_INFO("argv[8] : %d = ant1_cycles", ant1_cycles);
    LOG_INFO("argv[9] : %d = ant2_cycles", ant1_cycles);
    LOG_INFO("argv[10] : %d = capture_blocks_per_antenna", capture_blocks_per_antenna);
    LOG_INFO("argv[11] : %d = skip_cycles", skip_cycles);
    LOG_INFO("argv[12] : %d = num_captures", num_captures);
    ...
    memset(&v2d_rcs_host_params, 0, sizeof(v2d_rcs_host_params));
    ...
    v2d_rcs_host_params.quad_instance = quad_instance;
    v2d_rcs_host_params.drf_clock = drf_clock;
    v2d_rcs_host_params.pdpd_rate = pdpd_in_rate;
    v2d_rcs_host_params.match_value = delay_value;
    /*Internal PDPD delay of CAPTURE_DELAY_DELTA_VALUE added*/
    v2d_rcs_host_params.ant0_cycles = ant0_cycles + CAPTURE_DELAY_DELTA_VALUE;
    v2d_rcs_host_params.ant1_cycles = ant1_cycles;
    v2d_rcs_host_params.ant2_cycles = ant2_cycles;
    v2d_rcs_host_params.ant3_cycles = ant3_cycles;
    v2d_rcs_host_params.skip_cycles = skip_cycles;
    v2d_rcs_host_params.capture_blocks = capture_blocks_per_antenna;
    v2d_rcs_host_params.num_captures = num_captures;
    v2d_rcs_host_params.add_factor = val_kargs[0];
    v2d_rcs_host_params.num_antennas = NUM_ANTENNA_CAPTURES;

    for (i = 0; i < v2d_rcs_host_params.num_antennas; i++)
    {
        v2d_rcs_host_params.configured_channel[i] = configured_channel[i];
    }

    LOG_INFO("loaded the elf:%s", rcs_params.rcs_cfg[SRP_RCS_RCP_CTRL_AX].img_file);

    //clear the dram where RCS results stored
    write_mem_18a(g_dcode_buffer[0], 32, rd_wr_value, "DCODE_BUFFER");

    //clear the RSM memory where RCS parameters were used*/
    ...
    write_mem_18a(RSM_MEM_OFFSET, (sizeof(v2d_rcs_host_params) / sizeof(int)), (uint32_t *)&v2d_rcs_host_params, "RCS_HOST_PARAMS");
#endif
    ...
    size_per_antenna = SAMPLES_TO_FB_MEM_SIZE(capture_blocks_per_antenna * val_kargs[1] * SAMPLES_PER_VEC);

    /*Perform the FB_MEM configuration for X Capture for a QUAD*/
    if (quad_mem_fb_x_capture_repeat_mode_configuration(quad_instance, NUM_ANTENNA_CAPTURES/*num_antennas*/, size_per_antenna/*size_per_antenna*/))
    {
    ...
    exp_b2h_value = ((ant0_cycles / cycles_per_block) +
                 (NUM_ANTENNA_CAPTURES * capture_blocks_per_antenna)) * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_out_rate]);

    /*Per configured antenna*/
    for (lc = 0 ; lc < (NUM_ANTENNA_CAPTURES); lc++)
    {
        v2d_num = g_v2d_instance_in_quad[quad_instance][configured_channel[lc] / 2];
        apb_num = (configured_channel[lc] % 2);
        vex_num_param[0] = g_vex_instance_in_v2d[v2d_num][2 * (configured_channel[lc] % 2)];    // Tx VEX
        vex_num_param[1] = g_vex_instance_in_v2d[v2d_num][(2 * (configured_channel[lc] % 2)) + 1];    // Rx VEX
        g_exp_interrupt_value[(configured_channel[lc] / 2)][apb_num] = V2D_INTERRUPT_VALUE;

        printf("INFO: v2d_num: %d, tx_vex: %d, rx_vex: %d\n", v2d_num, vex_num_param[0], vex_num_param[1]);
        loop_wait_count = 0;
        ...
        while ((h2b_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_0_OFFSET + \
                        NSIP_HBI_H2B_FLIT_CNT_OFFSET, "B2H_FLIT_COUNT")) < exp_b2h_value)
        {
            // do nothing wait in this loop
            if (loop_wait_count >= 80)
            {
    ...
    /*Checking the MEM_FB compare result*/
    read_mem_18a(g_dcode_buffer[0], 32, rd_wr_value, "RCS_DMEM");

    if ((NUM_ANTENNA_CAPTURES * capture_blocks_per_antenna * num_captures == rd_wr_value[4]) && (0xFFFF == rd_wr_value[12]))
    {
        LOG_INFO("RCS comparision of MEM_FB passed");
    }
    else
    {
        LOG_ERROR("RCS comparision of MEM_FB was failed for captured block 0x%x, 0x%x", rd_wr_value[4], rd_wr_value[12]);
        LOG_INFO("expected_value: 0x%x, actual_value: 0x%x, add_factor: 0x%x", rd_wr_value[7], rd_wr_value[8], rd_wr_value[9]);
        test_status |= 1;
    }
#endif
    ...
    if (0 == test_status)
    {
        printf("Host:---- SUCCESS ---- quad_instance: %d, drf_clock : %s, pdpd_in_rate: %s, \
                pdpd_out_rate: %s, lut_mode: %d, delay_value: %d, ant0_cycles: %d, ant1_cycles: %d, ant2_cycles: %d\
                capture_blocks_per_antenna: %d, skip_cycles: %d, num_captures: %d, Ant0: %d, Ant1: %d Ant2: %d\n",
                quad_instance, drf_clock_str[drf_clock], pdpd_rate_str[pdpd_in_rate],
                pdpd_rate_str[pdpd_out_rate], g_test_config_tbl[test_config][2], delay_value, ant0_cycles, ant1_cycles, ant2_cycles,
                capture_blocks_per_antenna, skip_cycles, num_captures, configured_channel[0], configured_channel[1], configured_channel[2]);
    }
    else
    {
