...
if arg_len != 7:
    ...
    print("python flow3_pdpd_ingress_repeat_3_ch.py <quad_instance> <channel 0> <channel 1> <channel 2> <drf_clock> <test_config>\n")
...
print("delay_value: %s" %param[6])
print("ant0_cycles: %s" %param[7])
print("ant1_cycles: %s" %param[8])
print("ant2_cycles: %s" %param[9])
param.append(2)
print("num_blocks_per_capture: %s" %param[10])
...
skip_cycles = 6 * V2D.get_skip_cycles(param[4], param[5])
param.append(int(skip_cycles))
print("skip_cycles: %s" %param[11])
num_captures = (int)(V2D.get_num_captures(param[4], param[5]) / 6)
#The minimum number of captures should not be less than 3 for repeat mode
if (num_captures <= 2):
    num_captures = 3
param.append(int(num_captures))
print("num_captures: %s" %param[12])
print (param)


V2D.capture_delay_value_check(param[4], param[7])
V2D.capture_delay_value_check(param[4], param[8])
V2D.capture_delay_value_check(param[4], param[9])
V2D.capture_delay_value_check(param[4], param[11])

if (param[1] == param[2]) or (param[1] == param[3]) or (param[2] == param[3]):
