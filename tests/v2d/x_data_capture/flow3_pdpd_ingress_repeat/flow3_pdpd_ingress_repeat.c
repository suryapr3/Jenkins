 ...
 * @details 1. Configuration of VEX, V2D's Common Module for sending data from VEX to V2D via B path
            ...
            3. Configure the MEM_FB to receive the data from PDPD Ingress where capture is configured in repeat
               mode and do the comparison in RCS.
            4. The limitation was the total TC duration will be within 10ms.
   @TC Description:
        ...
        8. X-Data capture will be triggered from BFN timed trigger in a repeat mode which will be programmed from RCS and
           capture will be halted based on the number of captures.
        9. The MEM_FB comparison will be performed in RCS and corresponding result will be read by HOST to evaluat the TC.
...
int main(int argc, const char *argv[])
{
    ...
    int vex_num = 0, vex_num_param[NUM_VEX_CORES] = {0}, v2d_num, apb_num, underrun_reg_value = 0, drf_clock = 0, test_config = 0,
        post_filter_mode = 0, delay_value = 0, capture_start_delay_value = 0, capture_samples = 0;
    uint32_t capture_blocks = 0, skip_cycles = 0, blocks_per_1mb = 0, num_captures = 0, exp_h2b_flit_count_value = 0;
    pdpd_rate_t pdpd_in_rate, pdpd_out_rate;
    int cycles_per_blk = 0;
    ...
    capture_start_delay_value = (int) argv[7];
    capture_blocks = (int) argv[8];
    ...
    LOG_INFO("argv[7] : %d = capture_start_delay_value", capture_start_delay_value);
    LOG_INFO("argv[8] : %d = capture_blocks", capture_blocks);
    LOG_INFO("argv[9] : %d = skip cycles", skip_cycles);
    ...
    capture_samples = capture_blocks * val_kargs[1] * SAMPLES_PER_VEC;

    /*Perform the FB_MEM configuration for X Capture*/
    if (mem_fb_x_capture_repeat_mode_configuration(SAMPLES_TO_FB_MEM_SIZE(capture_samples), v2d_num, apb_num))
    {
    ...
    rcs_host_param[6] = capture_start_delay_value + CAPTURE_DELAY_DELTA_VALUE;
    rcs_host_param[7] = capture_samples;
    rcs_host_param[8] = skip_cycles;
    rcs_host_param[9] = capture_blocks;
    rcs_host_param[10] = num_captures;
    rcs_host_param[11] = val_kargs[0];
    rcs_host_param[12] = pdpd_in_rate;
            ...
            (VEC2FLIT(VEX_MAX_BUF_SIZE)))
    {
        ...
        {
            LOG_WARNING("Breaking the loop while H2B_FLIT_CNT0 x%x is not the required number of flits\n", h2b_flit_count);
            break;
        }
        loop_wait_count ++;
        usleep(10000);
    ...
    /*There are no REF vectors for 1.5G clock for this specific input pattern hence excluding the REF vector comparison in VEX*/
    ...
    cycles_per_blk = (drf_clock) ? CYCLES_PER_BLOCK_1P5G : CYCLES_PER_BLOCK_983M;

    exp_h2b_flit_count_value = ((capture_start_delay_value / cycles_per_blk) + (num_captures * capture_blocks + (skip_cycles / cycles_per_blk))) *
                                   VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_out_rate]);

    /*Wait till required number of flits reached the H2B interface
     * Checking the H2B registers because the B2H count might not include the delay*/
    while ((h2b_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_0_OFFSET + \
                    NSIP_HBI_H2B_FLIT_CNT_OFFSET, "B2H_FLIT_COUNT")) < exp_h2b_flit_count_value )
    {
        // do nothing wait in this loop
        if (loop_wait_count >= 80)
        {
    ...
    if (rd_wr_value[4])
    {
        LOG_ERROR("RCS comparision of MEM_FB was failed for captured block 0x%x", rd_wr_value[4]);
    ...
    if (0 == test_status)
    {
        printf("Host:---- SUCCESS ---- tx_vex : %d, rx_vex : %d, v2d_num : %d, apb_num : %d, drf_clock : %s, pdpd_in_rate: %s, \
                pdpd_out_rate: %s, lut_mode: %d, delay_value: %d, capture_start_delay_value: %d, capture_blocks: %d, \
                skip delay: %d, num_of_captures: %d\n",
                vex_num_param[0], vex_num_param[1], v2d_num, apb_num, drf_clock_str[drf_clock], pdpd_rate_str[pdpd_in_rate],
                pdpd_rate_str[pdpd_out_rate], g_test_config_tbl[test_config][2], delay_value, capture_start_delay_value,
                capture_blocks, skip_cycles, num_captures);
    }
    else
    {
