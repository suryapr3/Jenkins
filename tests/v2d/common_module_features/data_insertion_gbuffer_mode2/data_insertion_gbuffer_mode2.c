 ...
 *********************************************************************************************/
/**
 * @file    data_insertion_gbuffer_mode2.c
 ...
 * @details 1. Configuration of VEX, V2D and Common Module for sending data from VEX to V2D via B path
   ...
   @TC Description:
        ...
        12. When V2D is in insertion period another Tx VEX will send the G buffer data in MODE2 configuration i.e 1 filt.
        13. Rx VEX will do the integrity check for the initial 100 blocks and later 500 insertion blocks
        14. Later Host will check whether pragrammed data from VEX reached the PASSTHRU SHDW register.
...
#define NUM_RX_VEX_CORES    2   /*G Buffer Tx will receive indication from first Tx VEX and Rx VEX which
                                  receives the data from V2D*/
...
// B2H Registers
    ...
    // This was for GMP DDR Coeff update from VEX in MODE 2 for stream 5
    {(V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_B2H_CSR_ADDR_TBL5_OFFSET), (0x80000000 | B2H_MAP_VALUE_PASSTHRU_GAIN), \
        NSIP_HBI_B2H_CSR_ADDR_TBL_WR_MASK, "CM_1_B2H_ADDR_MAP_TBL4"},
    // Mask was set for total 1 flit in Bytes
    {(V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_B2H_CSR_ADDR_MASK_TBL5_OFFSET), 0x3F, NSIP_HBI_B2H_CSR_ADDR_TBL_WR_MASK, \
...
// H2B Registers
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_TK_ADDR_LKUP_TBL0_OFFSET, (STRM2CIO_2_VEX_DMEM_OFFSET + HIVE_ADDR_cm1_data_insertion_rx_p5_4iq_consumer_sm_gate), NSIP_HBI_H2B_TK_ADDR_LKUP_TBL_WR_MASK, \
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL0_OFFSET, 0x80006000, NSIP_HBI_H2B_ADDR_LKUP_TBL_WR_MASK, "H2B_ADDR_LKUP_TBL0"},
...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, int test_config)
{
    ...
    for (lc = 0; lc < NUM_RX_VEX_CORES; lc++)
    {
        for (i = 0; i < ARRAY_SIZE(g_strm2cio_config_tbl); i++)
        {
            if (rv = write_reg_18a(VE32_CELL_BASE(vex_num[lc + 1]) + g_strm2cio_config_tbl[i].offset, g_strm2cio_config_tbl[i].value, \
...
int main(int argc, const char *argv[])
{
    ...
    int gbuffer_tx_offset_val[] = {HIVE_ADDR_cm1_gbuffer_tx_g_passthru_coeff_base};
    unsigned int gbuffer_tx_val_kargs[] = {B2H_MAP_VALUE_PASSTHRU_GAIN};
    ...
    dpd_passthru_buffer_data[0][0] = (PASSTHRU_I_COEFF_UNITY_GAIN + 1) << 16 | (PASSTHRU_Q_COEFF_UNITY_GAIN + 1);
    ...
    if (rv = passthru_shdw_reg_check(v2d_num, apb_num, dpd_passthru_buffer_data[0][0]/*passthru value*/))
    {
        printf("ERROR: SHDW reg check failed");
        test_status |= 1;
    }
