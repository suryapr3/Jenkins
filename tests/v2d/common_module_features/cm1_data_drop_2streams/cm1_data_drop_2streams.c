 ...
 * @details 1. Configuration of VEX, V2D and Common Module for sending data from VEX to V2D via B path
   ...
   @TC Description:
        ...
        8. The above can be used to verify whether V2D is getting underrun before the Tx VEX stop transmitting
            the data.
        9. The DROP feature will be enabled after the initial 100 blocks and again disabled at 500 block
           boundries.
        10. As the DROP feature was enabled RX VEX will process only 600 blocks of given data rate for
...
#include "cm1_data_drop_2streams_tx.map.h"
#include "cm1_data_drop_2streams_rx.map.h"
...
#define DROP_BLOCKS_START    0x64
#define DROP_BLOCKS_END 0x1F4
#define DROP_BLOCKS (DROP_BLOCKS_END - DROP_BLOCKS_START)
...
const char *g_vex_fw_binary[] = {"vex_fw/sim_m64/cm1_data_drop_2streams_tx.fw", "vex_fw/sim_m64/cm1_data_drop_2streams_rx.fw",
    "vex_fw/sim_m64/cm1_data_drop_2streams_unequal_split_tx1.fw"};
...
// H2B Registers
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_TK_ADDR_LKUP_TBL0_OFFSET, (STRM2CIO_2_VEX_DMEM_OFFSET + HIVE_ADDR_cm1_data_drop_2streams_rx_consumer_sm_gate), NSIP_HBI_H2B_TK_ADDR_LKUP_TBL_WR_MASK, \
    ...
    /* Data will be written to VEX location 0x10080 , i.e VEX DMEM location 0x80*/
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_TK_ADDR_LKUP_TBL1_OFFSET, (STRM2CIO_2_VEX_DMEM_OFFSET + HIVE_ADDR_cm1_data_drop_2streams_rx_consumer_sm_gate), NSIP_HBI_H2B_TK_ADDR_LKUP_TBL_WR_MASK, \
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_DROP_CMD0_CMD_DROP_CONTROL_OFFSET, 0x80000000 | (DROP_BLOCKS_START), NSIP_HBI_DROP_CMD_DROP_CONTROL_WR_MASK, \
        "H2B_CMD_DROP_CONTROL"},
    /*Disabling the Drop for Stream 0*/
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_DROP_CMD0_CMD_DROP_CONTROL_OFFSET, (DROP_BLOCKS_END), NSIP_HBI_DROP_CMD_DROP_CONTROL_WR_MASK, \
        "H2B_CMD_DROP_CONTROL"},
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_DROP_CMD1_CMD_DROP_STREAM_EN_OFFSET, 0x2, NSIP_HBI_DROP_CMD_DROP_STREAM_EN_WR_MASK, \
        "H2B_CMD_DTROP_STREAM_EN"},
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_DROP_CMD1_CMD_DROP_CONTROL_OFFSET, 0x80000000 | (DROP_BLOCKS_START), NSIP_HBI_DROP_CMD_DROP_CONTROL_WR_MASK, \
        "H2B_CMD_DROP_CONTROL"},
    /*Disabling the Drop for Stream 1*/
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_DROP_CMD1_CMD_DROP_CONTROL_OFFSET, (DROP_BLOCKS_END), NSIP_HBI_DROP_CMD_DROP_CONTROL_WR_MASK, \
        "H2B_CMD_DROP_CONTROL"},
    /***********************************************************************************************************************************/
    /*These last ten entries orders shouldn't be altered as they will be modified based on the run time parameter for vex number*/
    /* Data will be written to VEX location 0x6000 , i.e VEX VMEM location 0x6000*/
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL0_OFFSET, (NSIP_HBI_H2B_ADDR_LKUP_TBL_VALID_BF_MSK | HIVE_ADDR_cm1_data_drop_2streams_rx_vbuffer_out), NSIP_HBI_H2B_ADDR_LKUP_TBL_WR_MASK, \
        "HBI_1_H2B_ADDR_LKUP_TBL0"},
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL1_OFFSET, (NSIP_HBI_H2B_ADDR_LKUP_TBL_VALID_BF_MSK | HIVE_ADDR_cm1_data_drop_2streams_rx_vbuffer_out), NSIP_HBI_H2B_ADDR_LKUP_TBL_WR_MASK, \
...
static v2d_flow_tbl_t g_cm1_data_drop_2streams_config_tbl[] = {
// Interrupt Enable
...
// Tx Path Settings
    ...
    {V2D_CSR_OFFSET + AV2D_CSR_RX_CSR_RX_PACKET_SZ_OFFSET, RX_PD_PACKET_SZ_VALUE_2STREAM, AV2D_CSR_RX_CSR_RX_PACKET_SZ_WR_MASK, "RX_PACKET_SZ"},
    /************************************************************************************************************/
...
int configure_v2d_host(int vex_num[], int v2d_num, int apb_num, int split_method, int drf_clock, int test_config)
{
    int i, j = 0, rv = 0, v2d_tx_stream_cnt0 = 0, v2d_tx_stream_cnt1 = 0, v2d_rx_stream_cnt0 = 0, v2d_rx_stream_cnt1 = 0,
        md_control0 = 0, md_control1 = 0, v2d_tx_ptr_bound0 = 0, v2d_tx_ptr_bound1 = 0, vex_index = 0;
    ...
    for (j = 0; j < NUM_TX_VEX_CORES; j++)
    {
        ...
        if (rv = write_reg_18a(VE32_CELL_BASE(vex_num[vex_index]) + VE32_CELL_MEM_LM_SDFGATE_BPCA_BP_DMEM_OFFSET, \
                    HIVE_ADDR_cm1_data_drop_2streams_rx_consumer_sm_gate, VE32_CELL_MEM_LM_SDFGATE_BPCA_BP_DMEM_WR_MASK, "BPCA_BP_DMEM"))
            return rv;
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm_1_config_tbl[i].offset), (0x80000000 | HIVE_ADDR_cm1_data_drop_2streams_rx_vbuffer_out), \
                g_cm_1_config_tbl[i].mask, g_cm_1_config_tbl[i].name))
            return rv;
    }
    ...
    for (i = 0; i < ARRAY_SIZE(g_cm1_data_drop_2streams_config_tbl) - 8; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_2streams_config_tbl[i].offset), \
                    g_cm1_data_drop_2streams_config_tbl[i].value, g_cm1_data_drop_2streams_config_tbl[i].mask, g_cm1_data_drop_2streams_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_cm1_data_drop_2streams_config_tbl) - 7; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_2streams_config_tbl[i].offset), \
                    g_cm1_data_drop_2streams_config_tbl[i].value | (drf_clock << 7), g_cm1_data_drop_2streams_config_tbl[i].mask, g_cm1_data_drop_2streams_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_cm1_data_drop_2streams_config_tbl) - 6; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_2streams_config_tbl[i].offset), \
                    v2d_tx_ptr_bound0, g_cm1_data_drop_2streams_config_tbl[i].mask, g_cm1_data_drop_2streams_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_cm1_data_drop_2streams_config_tbl) - 5; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_2streams_config_tbl[i].offset), \
                    v2d_tx_ptr_bound1, g_cm1_data_drop_2streams_config_tbl[i].mask, g_cm1_data_drop_2streams_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_cm1_data_drop_2streams_config_tbl) - 4; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_2streams_config_tbl[i].offset), \
                    (g_cm1_data_drop_2streams_config_tbl[i].value | (lut_mode << 12) | (pdpd_out_rate << 4) | pdpd_in_rate), \
                    g_cm1_data_drop_2streams_config_tbl[i].mask, g_cm1_data_drop_2streams_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_cm1_data_drop_2streams_config_tbl) - 3; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_2streams_config_tbl[i].offset), \
                    (0x80000000 | v2d_tx_stream_cnt0), g_cm1_data_drop_2streams_config_tbl[i].mask, g_cm1_data_drop_2streams_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_cm1_data_drop_2streams_config_tbl) - 2; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_2streams_config_tbl[i].offset), \
                    (0x80000000 | v2d_rx_stream_cnt0), g_cm1_data_drop_2streams_config_tbl[i].mask, g_cm1_data_drop_2streams_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_cm1_data_drop_2streams_config_tbl) - 1; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_2streams_config_tbl[i].offset), \
                    (0x80000000 | v2d_tx_stream_cnt1), g_cm1_data_drop_2streams_config_tbl[i].mask, g_cm1_data_drop_2streams_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_cm1_data_drop_2streams_config_tbl); i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_2streams_config_tbl[i].offset), \
                    (0x80000000 | v2d_rx_stream_cnt1), g_cm1_data_drop_2streams_config_tbl[i].mask, g_cm1_data_drop_2streams_config_tbl[i].name))
            return rv;
    }
 ...
 * @param [in]      pointers to rx, tx0, tx1 arrays , vex0_input_vectors and pdpd_rate
 * @return          none
 */
...
int main(int argc, const char *argv[])
{
    ...
    int offset_val[] = {HIVE_ADDR_cm1_data_drop_2streams_tx_g_scalar_val, HIVE_ADDR_cm1_data_drop_2streams_tx_g_num_vecs_in,
                         HIVE_ADDR_cm1_data_drop_2streams_tx_g_dest_buffer_base, HIVE_ADDR_cm1_data_drop_2streams_tx_g_num_blocks,
                         HIVE_ADDR_cm1_data_drop_2streams_tx_g_initial_block_size, HIVE_ADDR_cm1_data_drop_2streams_tx_g_tx_instance,
                         HIVE_ADDR_cm1_data_drop_2streams_tx_g_host_cycle_count};
    ...
    int vex_rx_offset_val[] = {HIVE_ADDR_cm1_data_drop_2streams_rx_g_scalar_val, HIVE_ADDR_cm1_data_drop_2streams_rx_g_num_vecs_in,
                                HIVE_ADDR_cm1_data_drop_2streams_rx_g_num_blocks, HIVE_ADDR_cm1_data_drop_2streams_rx_g_tbs_in_blk,
                                HIVE_ADDR_cm1_data_drop_2streams_rx_g_tokens_per_block, HIVE_ADDR_cm1_data_drop_2streams_rx_g_count1,
                                HIVE_ADDR_cm1_data_drop_2streams_rx_g_data_drop_block_start, HIVE_ADDR_cm1_data_drop_2streams_rx_g_data_drop_block_end};
    unsigned int vex0_rx_args[] = {0x0, NUMBER_OF_VECTORS_INPUT, 1, 64, 1, 0, DROP_BLOCKS_START, DROP_BLOCKS_END};
    unsigned int vex1_rx_args[] = {0x0, NUMBER_OF_VECTORS_INPUT, 1, 64, 1, 0, DROP_BLOCKS_START, DROP_BLOCKS_END};
    ...
    // unequal split Tx0 will be loaded with cm1_data_drop_2streams_tx.fw and Tx1 will be loaded with
    ...
    if ((rv = srp_vex_memid_store(vex_hdl,
                        vex_num_param[0],
                        SRP_VEX_VMEM,
                        HIVE_ADDR_cm1_data_drop_2streams_tx_vbuffer,/*local_addr*/
                        val_inp_tx0[0],
   ...
   /*First Rx Vex Core*/
    if ((rv = srp_vex_memid_store(vex_hdl,
                         vex_num_param[1],
                         SRP_VEX_VMEM,
                         HIVE_ADDR_cm1_data_drop_2streams_rx_vbuffer,/*local_addr*/
                         host_ref0_data,
   ...
   /*Second Rx Vex Core*/
    ...
    for (vex_cnt = 0; vex_cnt < NUM_TX_VEX_CORES; vex_cnt++)
    {
        ...
        if ((rv = srp_dev_block_write32(srp,
                        SRP_BLOCK_VEX_CORE,
                        vex_num,
                        dmem_offset + HIVE_ADDR_cm1_data_drop_2streams_tx_g_host_indicator,
                        &host_flag,
                        1,
                        flags)))
            return rv;
    ...
    /*wait till the flits before dropping starts reached*/
    while ((h2b_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_1_OFFSET + \
            ...
            (DROP_BLOCKS_START * VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_out_rate])))
    {
    ...
    /*Read the Drop state, it should be set for the corresponding stream*/
    int drop_state;
    drop_state = read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_DROP_STATE_OFFSET, "DROP_STATE");

    if (3 != drop_state)
    {
        printf("WARNING: Drop state 0x%x is not set", drop_state);
    }
    ...
    for (vex_cnt = 0; vex_cnt < NUM_RX_VEX_CORES; vex_cnt++)
    {
        ...
        int vex_num = vex_num_param[vex_index];
        while (host_vec_processed < ((g_max_blocks[drf_clock][pdpd_out_rate] - DROP_BLOCKS) * num_vectors_block[vex_cnt]))
        {
            if ((rv = srp_dev_block_read32(srp,
                            SRP_BLOCK_VEX_CORE,
                            vex_num,
                            dmem_offset + HIVE_ADDR_cm1_data_drop_2streams_rx_g_count1,
                            &host_vec_processed,
                            1,
                            flags)))
                return rv;
        ...
        if ((rv = srp_dev_block_read32(srp,
                        SRP_BLOCK_VEX_CORE,
                        vex_num,
                        dmem_offset + HIVE_ADDR_cm1_data_drop_2streams_rx_g_compare_result,
                        &host_result,
                        1,
                        flags)))
            return rv;
        ...
        if ((rv = srp_dev_block_read32(srp,
                        SRP_BLOCK_VEX_CORE,
                        vex_num,
                        dmem_offset + HIVE_ADDR_cm1_data_drop_2streams_rx_g_add_val_out,
