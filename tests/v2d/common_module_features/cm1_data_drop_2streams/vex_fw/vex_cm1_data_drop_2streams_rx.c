 ...
 *********************************************************************************************/
/**
 * @file    vex_cm1_data_drop_2streams_rx.c
 * @brief   Vex FW code to validate V2D Path D.
...
volatile int AT(0x48) g_data_drop_block_start;
volatile int AT(0x4c) g_data_drop_block_end;
#if DEBUG
 ...
 * @return          none
 */
void cm1_data_drop_2streams_rx() ENTRY
{
...
//    circ_buff_init( IS_CONS, g_tbs_in_blk, CONSUMER_TOKEN_PER_REQUEST, 0, sm_ch);
    ...
    while (count < (g_data_drop_block_start))
    {
...
//#pragma hive force_unroll=4
        ...
        mc_complete( Any, sm_ch, ptr);
#if VEX_HOST_DEBUG
        OP___printstring("vex_cm1_data_drop_2streams: at mc_complete done:\n");
        v_xsto_32(Any, g_dest_ptr_address, 0, ptr) SYNC(buffer_write); // This statement is not required for HOST to VEX communication
#endif
        count++;
#if DEBUG
    ...
    count = g_data_drop_block_end;
    add_temp = (g_data_drop_block_end);

    /*For the blocks after the drop disabled*/
...
#endif
} // cm1_data_drop_2streams_rx() ends
