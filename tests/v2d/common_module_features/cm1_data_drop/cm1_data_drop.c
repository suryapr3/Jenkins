 ...
 * @details 1. Configuration of VEX, V2D and Common Module for sending data from VEX to V2D via B path
   ...
   @TC Description:
        ...
        9. The above can be used to verify whether V2D is getting underrun before the Tx VEX stop transmitting
            the data.
        10. The DROP feature will be enabled after the initial 100 blocks and again disabled at 500 block
           boundries.
        11. As the DROP feature was enabled RX VEX will process only 600 blocks of given data rate for
...
#include "cm1_data_drop_tx.map.h"
#include "cm1_data_drop_rx.map.h"
...
// H2B Registers
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_TK_ADDR_LKUP_TBL0_OFFSET, (STRM2CIO_2_VEX_DMEM_OFFSET + HIVE_ADDR_cm1_data_drop_rx_consumer_sm_gate), NSIP_HBI_H2B_TK_ADDR_LKUP_TBL_WR_MASK, \
    ...
    /*Enabling the Drop*/
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_DROP_CMD0_CMD_DROP_CONTROL_OFFSET, 0x80000000 | (DROP_BLOCKS_START), \
    ...
    {V2D_NSIP_HBI_1_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL0_OFFSET, (NSIP_HBI_H2B_ADDR_LKUP_TBL_VALID_BF_MSK | HIVE_ADDR_cm1_data_drop_rx_vbuffer_out ), NSIP_HBI_H2B_ADDR_LKUP_TBL_WR_MASK, "H2B_ADDR_LKUP_TBL0"},
...
static v2d_flow_tbl_t g_cm1_data_drop_config_tbl[] = {
// Interrupt Enable
...
int configure_v2d_host(int *vex_num, int v2d_num, int apb_num, int drf_clock, int test_config)
{
    ...
    if (rv = write_reg_18a(VE32_CELL_BASE(vex_num[1]) + VE32_CELL_MEM_LM_SDFGATE_BPCA_BP_DMEM_OFFSET, \
                HIVE_ADDR_cm1_data_drop_rx_consumer_sm_gate, VE32_CELL_MEM_LM_SDFGATE_BPCA_BP_DMEM_WR_MASK, "BPCA_BP_DMEM"))
        return rv;
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm_1_config_tbl[i].offset), (0x80000000 | HIVE_ADDR_cm1_data_drop_rx_vbuffer_out), \
                g_cm_1_config_tbl[i].mask, g_cm_1_config_tbl[i].name))
            return rv;
    }
    ...
    for (i = 0; i < ARRAY_SIZE(g_cm1_data_drop_config_tbl) - 5; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_config_tbl[i].offset), \
                    g_cm1_data_drop_config_tbl[i].value, g_cm1_data_drop_config_tbl[i].mask, g_cm1_data_drop_config_tbl[i].name))
            return rv;
    }
    for (;i < ARRAY_SIZE(g_cm1_data_drop_config_tbl) - 4; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_config_tbl[i].offset), \
                    g_cm1_data_drop_config_tbl[i].value | (drf_clock << 7), g_cm1_data_drop_config_tbl[i].mask, g_cm1_data_drop_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_cm1_data_drop_config_tbl) - 3; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_config_tbl[i].offset), \
                    v2d_buffer_b, g_cm1_data_drop_config_tbl[i].mask, g_cm1_data_drop_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_cm1_data_drop_config_tbl) - 2; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_config_tbl[i].offset), \
                    (g_cm1_data_drop_config_tbl[i].value | (lut_mode << 12) | (pdpd_out_rate << 4) | pdpd_in_rate), \
                    g_cm1_data_drop_config_tbl[i].mask, g_cm1_data_drop_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_cm1_data_drop_config_tbl) - 1; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_config_tbl[i].offset), \
                    g_tx_strm_cnt0_value[drf_clock][pdpd_in_rate], g_cm1_data_drop_config_tbl[i].mask, g_cm1_data_drop_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_cm1_data_drop_config_tbl); i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm1_data_drop_config_tbl[i].offset), \
                    g_tx_strm_cnt0_value[drf_clock][pdpd_out_rate], g_cm1_data_drop_config_tbl[i].mask, g_cm1_data_drop_config_tbl[i].name))
            return rv;
    }
...
int main(int argc, const char *argv[])
{
    ...
    int offset_val[] = {HIVE_ADDR_cm1_data_drop_tx_g_scalar_val, HIVE_ADDR_cm1_data_drop_tx_g_num_vecs_in,
                         HIVE_ADDR_cm1_data_drop_tx_g_dest_buffer_base, HIVE_ADDR_cm1_data_drop_tx_g_num_blocks,
                         HIVE_ADDR_cm1_data_drop_tx_g_initial_block_size, HIVE_ADDR_cm1_data_drop_tx_g_host_cycle_count};
    unsigned int val_kargs[] = {0x0, NUMBER_OF_VECTORS_INPUT, 0, 0, 3, (CYCLES_PER_BLOCK_983M - 1)};
    int vex_rx_offset_val[] = {HIVE_ADDR_cm1_data_drop_rx_g_scalar_val, HIVE_ADDR_cm1_data_drop_rx_g_num_vecs_in,
                                HIVE_ADDR_cm1_data_drop_rx_g_num_blocks, HIVE_ADDR_cm1_data_drop_rx_g_tbs_in_blk,
                                HIVE_ADDR_cm1_data_drop_rx_g_count1, HIVE_ADDR_cm1_data_drop_rx_g_data_drop_block_start,
                                HIVE_ADDR_cm1_data_drop_rx_g_data_drop_block_end};
    unsigned int vex_rx_val_kargs[] = {0x0, NUMBER_OF_VECTORS_INPUT, 1, 64, 0, DROP_BLOCKS_START, DROP_BLOCKS_END};
    ...
    if ((rv = srp_vex_loader(vex_hdl, vex_num_param[0], "vex_fw/sim_m64/cm1_data_drop_tx.fw")))
        return rv;

    if ((rv = srp_vex_loader(vex_hdl, vex_num_param[1], "vex_fw/sim_m64/cm1_data_drop_rx.fw")))
        return rv;
    ...
    if (rv = ref_output_read(host_ref_data, vex_rx_val_kargs[1]/*number of vectors*/, test_config))
    {
        return rv;
    }
    ...
    if ((rv = srp_vex_memid_store(vex_hdl,
                        vex_num_param[0],
                        SRP_VEX_VMEM,
                        HIVE_ADDR_cm1_data_drop_tx_vbuffer,/*local_addr*/
                        val_inp[0],
   ...
   if ((rv = srp_vex_memid_store(vex_hdl,
                        vex_num_param[1],
                        SRP_VEX_VMEM,
                        HIVE_ADDR_cm1_data_drop_rx_vbuffer,/*local_addr*/
                        host_ref_data,
    ...
    if ((rv = srp_dev_block_write32(srp,
                    SRP_BLOCK_VEX_CORE,
                    vex_num_param[0],
                    dmem_offset + HIVE_ADDR_cm1_data_drop_tx_g_host_indicator,
                    &host_flag,
                    1,
                    flags)))
        return rv;
    ...
    if (1 != drop_state)
    {
        printf("WARNING: Drop state: 0x%x is not set\n", drop_state);
    }
    ...
    while (host_vec_processed < ((g_max_blocks[drf_clock][pdpd_out_rate] - DROP_BLOCKS)* g_vectors_per_block[drf_clock][pdpd_out_rate]))
    {
        if ((rv = srp_dev_block_read32(srp,
                        SRP_BLOCK_VEX_CORE,
                        vex_num_param[1],
                        dmem_offset + HIVE_ADDR_cm1_data_drop_rx_g_count1,
                        &host_vec_processed,
                        1,
                        flags)))
            return rv;
    ...
    if ((rv = srp_dev_block_read32(srp,
                    SRP_BLOCK_VEX_CORE,
                    vex_num_param[1],
                    dmem_offset + HIVE_ADDR_cm1_data_drop_rx_g_compare_result,
                    &host_result,
                    1,
                    flags)))
        return rv;
    ...
    if ((rv = srp_dev_block_read32(srp,
                    SRP_BLOCK_VEX_CORE,
                    vex_num_param[1],
                    dmem_offset + HIVE_ADDR_cm1_data_drop_rx_g_add_val_out,
