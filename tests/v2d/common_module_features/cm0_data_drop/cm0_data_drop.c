 ...
 *********************************************************************************************/
/**
 * @file    cm0_data_drop.c
 * @brief   HOST Configuration code for validating the data drop feature in CM0
 * @TC Description:
        ...
        5. Host will be waiting on a memory location where VEX will update the number of vectors
           it was processed.
        6. The DROP feature will be enabled after the initial 100 blocks and again disabled at 500 block
           boundries.
        7. As the DROP feature was enabled RX VEX will process only 600 blocks of given data rate for
           the data integrity check.
        6. The supported Sample rates are[0.25IQ , 16R]
 */
...
// H2B Registers
    ...
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_TK_ADDR_LKUP_TBL0_OFFSET, \
        (STRM2CIO_2_VEX_DMEM_OFFSET + HIVE_ADDR_v2d_flow1_rx_p5_8iq_consumer_sm_gate), \
    ...
    /*Enabling the Drop*/
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_DROP_CMD0_CMD_STRM_EN_OFFSET, 0x1, NSIP_HBI_DROP_CMD_STRM_EN_WR_MASK, \
        "H2B_CMD_DTROP_STREAM_EN"},
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_DROP_CMD0_CMD_CTRL_OFFSET, 0x80000000 | (DROP_BLOCKS_START), NSIP_HBI_DROP_CMD_CTRL_WR_MASK, \
        "H2B_CMD_DROP_CONTROL"},
    /*Disabling the Drop*/
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_DROP_CMD0_CMD_CTRL_OFFSET, (DROP_BLOCKS_END), NSIP_HBI_DROP_CMD_CTRL_WR_MASK, \
        "H2B_CMD_DROP_CONTROL"},
    /***********************************************************************************************************************************/
    ...
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL0_OFFSET, \
...
{
    int i, rv = 0, tx_stream_cnt_value = 0, v2d_buffer_a = 0;
    bool us_granularity_required = FALSE;

#if VEX_1US_WINDOW
    if (sys_rate <= SYS_RATE_IQ_2)
    {
        us_granularity_required = TRUE;
    }

    if (us_granularity_required)
    {
        tx_stream_cnt_value = (2 * VEC2FLIT(g_flow1_vectors_per_block[sys_rate])) - 1;
        v2d_buffer_a = ((((g_num_blocks_v2d_buffer) * (2 * (g_flow1_vectors_per_block[sys_rate] * 2))) - 1) << 16);
    }
    else
#endif
    {
        tx_stream_cnt_value = (VEC2FLIT(g_flow1_vectors_per_block[sys_rate])) - 1;
        v2d_buffer_a = ((((g_num_blocks_v2d_buffer) * (g_flow1_vectors_per_block[sys_rate] * 2)) - 1) << 16);
    }
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm_0_config_tbl[i].offset), tx_stream_cnt_value, \
                g_cm_0_config_tbl[i].mask, g_cm_0_config_tbl[i].name))
            return rv;
    }
    ...
    {
       if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_config_tbl[i].offset), \
                   (AV2D_CSR_TX_CSR_TX_STRM_CNT_STRM_EN_BF_MSK | tx_stream_cnt_value), g_v2d_config_tbl[i].mask, g_v2d_config_tbl[i].name))
           return rv;
    }
    /************************************************************************************************************/
...
int main(int argc, const char *argv[])
{
    ...
    int offset_val[] = {HIVE_ADDR_v2d_flow1_tx_p25_4iq_g_scalar_val, HIVE_ADDR_v2d_flow1_tx_p25_4iq_g_num_vecs_in,
                         ...
                         HIVE_ADDR_v2d_flow1_tx_p25_4iq_g_initial_block_size, HIVE_ADDR_v2d_flow1_tx_p25_4iq_g_host_cycle_count};
    ...
    int vex_rx_offset_val[] = {HIVE_ADDR_v2d_flow1_rx_p5_8iq_g_scalar_val, HIVE_ADDR_v2d_flow1_rx_p5_8iq_g_num_vecs_in,
                               ...
                               HIVE_ADDR_v2d_flow1_rx_p5_8iq_g_count1, HIVE_ADDR_v2d_flow1_rx_p5_8iq_g_data_drop_block_start,
                               HIVE_ADDR_v2d_flow1_rx_p5_8iq_g_data_drop_block_end};
    unsigned int vex_rx_val_kargs[] = {0, NUMBER_OF_VECTORS_INPUT, 1, 64, 0, DROP_BLOCKS_START, DROP_BLOCKS_END};
    ...
    printf ("INFO: argv[5] : %d = sys_rate %d\n", sys_rate, sys_rate);
    ...
    if (us_granularity_required)
    {
        ...
        vex_rx_val_kargs[1] = 2 * vex_rx_val_kargs[1];
        val_kargs[1] = 2 * val_kargs[1];
    ...
    write_mem_18a(VEX_VMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow1_tx_p25_4iq_vbuffer,/*local_addr*/
                        (val_kargs[1] * NUM_OF_ELEMENTS_PER_VEC),
                        val_inp[0],
                        "TX_VEX_VMEM");
    ...
    if (rv = write_reg_18a(VEX_DMEM(vex_num_param[0]) + HIVE_ADDR_v2d_flow1_tx_p25_4iq_g_host_indicator,
    ...
    /*Wait till 50 blocks before the actual drop starts before reading the DROP state register*/
    while ((h2b_flit_cnt += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_FLIT_CNT_OFFSET, "H2B_FLIT_COUNT"))  <
            (((DROP_BLOCKS_START - 50)* (g_flow1_vectors_per_block[sys_rate] * 2))))
    {
    ...
    int drop_state;
    drop_state = read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_DROP_STATE_OFFSET, "DROP_STATE");

    while (1 != drop_state)
    {
        drop_state = read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_DROP_STATE_OFFSET, "DROP_STATE");
        if (loop_wait_count >= 180)
        {
            printf("WARNING: Drop state is not set\n");
            break;
        }
        loop_wait_count ++;
        usleep(100);
    ...
    while (host_vec_processed < ((MAX_BLOCKS - DROP_BLOCKS)* val_kargs[1]))
    {
    ...
    printf ("processed blocks 0x%x\n", host_vec_processed / val_kargs[1]);
    ...
    if (rv = interrupt_status_check(v2d_num, apb_num, TX_UNDERRUN_VALUE, 0/*NSIP0 exp value*/, 0/*NSIP1 exp value*/))
    {
