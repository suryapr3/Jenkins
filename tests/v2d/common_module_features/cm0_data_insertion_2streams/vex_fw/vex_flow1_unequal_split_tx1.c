 ...
 *********************************************************************************************/
/**
 * @file    vex_flow1_unequal_split_tx1.c
...
volatile int AT(0x14) g_tx_instance;
volatile int AT(0x18) g_initial_block_size;
volatile int AT(0x1c) g_host_cycle_count;
volatile int AT(0x20) g_host_insert_start_block;
volatile int AT(0x24) g_host_insert_stop_block;
...
/**
 * @brief           Function sending pre defined blocks of data L/4 th of Block data to HOST
 * @param [in]      none
 * @return          none
 */
void v2d_flow1_unequal_split_tx1() ENTRY
{
    int i, j = 0, offset =0;
    g_host_debug = 0;
    ...
    for (; j < g_host_insert_start_block; j++)
    {
        vec32i32 vin2;
        V_CLONE_32(g_scalar_val+j, vin2);
...
//#pragma hive force_unroll=4
    ...
    for ( ; j < g_host_insert_stop_block; j++)
    {
        while (g_timer_debug != 0xdeadbeef)
        {
        }
    ...
    while (3 != g_host_indicator)
    {
        ...
        /* for unequal split case sending 1/4th of 8GSPS block ii = 14 is needed to provide
         * enough time for V2D to read from the second buffer , for other cases ii=2 would work and it
         * will give a loop of 2 cycles with the prologue 6 and epilogue 1 cycles*/
#ifdef VOLCANO
#pragma hive pipeline ii=14
//#pragma hive force_unroll=4
#endif
        for(i = 0; i < iter_max; i++)
        {
