...
python3 cm0_data_insertion_2streams.py <tx0_vex_id> <rx0_vex_id> <tx1_vex_id> <rx1_vex_id> <v2d_id> <apb_id> <split_method> <drf_clock> <sys_rate>
...
sys_rate should be [4, 5, 6, 7, 8, 9, 10, 21, 22, 23, 24, 25, 26]
...
ex: python3 cm0_data_insertion_2streams.py 0 1 8 9 0 0 0 0 10

Note: This information is for the FPGA model ABC_18A_0p6_R1.0
      None of the VEX ID's should be equal

Steps to create regression excel list (one time):
-------------------------------------------------------------------------------------------------------------------------------------------------------
python3 regression_list_generation.py

The above will generate cm0_data_insertion_2streams_list.xlsx file , needs to be done only if the xlsx file is not present or there were any changes done
in the regression_list_generation.py file..


Steps to run regression:
-------------------------------------------------------------------------------------------------------------------------------------------------------
python3 cm0_data_insertion_2streams_regression.py

The above will run all the possible combination which was mentioned at the first step.
The input combinations will be taken from the cm0_data_insertion_2streams_list.xlsx file.
