 ...
 *********************************************************************************************/
/**
 * @file    vex_cm1_data_insertion_2streams_rx.c
 ...
 * @details 1. VEX will be waiting on SDF to receive the data from HOST.
            ...
            4. This code is specific to sample rates 0.5IQGSPS to 8IQ GSPS
**/
...
vec32i32 AT(OUTPUT_VMEM_LOCATION) vinsert_pattern_buffer[INSERT_PATTERN_SIZE];
vec32i32 AT(RX_OUTPUT_VMEM_LOCATION) vbuffer_out[RX_VEX_DEST_BUF_SIZE];
...
volatile int AT(0x48) g_data_insertion_block_start;
volatile int AT(0x4c) g_data_insertion_block_end;
volatile int AT(0x50) g_tokens_per_block;
#if DEBUG
 ...
 * @return          none
 */
void cm1_data_insertion_2streams_rx() ENTRY
{
    int sm_ch = CONSUMER_SM_BUF_ID;
    int vex_dest_buffer = RX_OUTPUT_VMEM_LOCATION;
    ...
    /*compare till insertion block start*/
    while (count < (g_data_insertion_block_start))
    {
...
#pragma hive pipeline merge_epilog=off
//#pragma hive force_unroll=4
    ...
    count = g_data_insertion_block_start;
    add_temp = (g_data_insertion_block_start);

    /*one Block has to be ignored from comparision as there is no space for storing the REF data during
     * transition*/
    {
        unsigned int is_data_ready, idx, ptr;
        mc_request( Any, 1, sm_ch, is_data_ready, idx);
    ...
    /*Compare During Insertion period*/
    while(count < (g_data_insertion_block_end))
    {
        unsigned int is_data_ready, idx, ptr, iter_max;
        vec32i32 vin2 UNINIT;
        iter_max = g_num_vecs_in;
...
#ifdef VOLCANO
#pragma hive pipeline ii=3
#pragma hive pipeline merge_prolog=off
//#pragma hive force_unroll=4
#endif
#endif
        for (int i = 0; i < iter_max; i++)
        {
          ...
          int sout0, sout1;
          (vin = vinsert_pattern_buffer[0]) NO_ALIAS SYNC_WITH(buffer_read);
          V_ADD_32(vin, vin2, vadd);
          v_ldoi_1024_circ(Any, vex_dest_buffer ,compare_offset, limit, rewind, vout, compare_offset) NO_ALIAS SYNC_WITH(buffer_read);
          //vout = vbuffer_out[g_count1%256];
          //Bit value of 1 in output indicates corresponding vector elements are equal
    ...
    /*One Block has to be ignored from comparision after the insertioin ends*/
    {
        unsigned int is_data_ready, idx, ptr;
        mc_request( Any, 1, sm_ch, is_data_ready, idx);
        //v_ldoi_1024_circ(Any, vex_dest_buffer ,compare_offset, limit, rewind, vtemp, compare_offset);
    ...
    /*do the comparison after insertion ends*/
    {
        unsigned int is_data_ready, idx, ptr, iter_max;
        vec32i32 vin2 UNINIT;
        iter_max = g_num_vecs_in;
...
#endif
} // cm1_data_insertion_2streams_rx() ends
