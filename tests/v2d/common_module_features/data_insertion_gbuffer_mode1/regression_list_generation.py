...
V2D = fw.ABC_V2D()
wa = wb.active
header = ["tx0_vex_id", "tx_gbuffer_vex_id" "rx0_vex_id",  "v2d_id", "apb_id", "drf_clock","test_config"]
row = 1
...
for tx0_vex_num in v2d_0_valid_vex_list:
    for tx_gbuffer_vex_num in v2d_0_valid_vex_list:
        if(tx0_vex_num == tx_gbuffer_vex_num):
            continue
        for rx0_vex_num in v2d_0_valid_vex_list:
            if (tx0_vex_num == rx0_vex_num) or (rx0_vex_num == tx_gbuffer_vex_num):
                continue
            for apb_num in range (2):
                for test_config in test_config_list:
                    wa.cell(row,1).value = tx0_vex_num
                    wa.cell(row,2).value = tx_gbuffer_vex_num
                    wa.cell(row,3).value = rx0_vex_num
                    wa.cell(row,4).value = 0
                    wa.cell(row,5).value = apb_num
                    wa.cell(row,6).value = drf_clock
                    wa.cell(row,7).value = test_config
                    row+=1

for tx0_vex_num in v2d_1_valid_vex_list:
    for tx_gbuffer_vex_num in v2d_1_valid_vex_list:
        if(tx0_vex_num == tx_gbuffer_vex_num):
            continue
...
for tx0_vex_num in v2d_2_valid_vex_list:
    for tx_gbuffer_vex_num in v2d_2_valid_vex_list:
        if(tx0_vex_num == tx_gbuffer_vex_num):
            continue
...
for tx0_vex_num in v2d_3_valid_vex_list:
    for tx_gbuffer_vex_num in v2d_3_valid_vex_list:
        if(tx0_vex_num == tx_gbuffer_vex_num):
            continue
...
sheet_name= wb['Sheet']
sheet_name.title ='data_insertion_gbuffer_mode1'

wb.save("data_insertion_gbuffer_mode1_list.xlsx")
