...
// H2B Registers
    ...
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_DROP_CMD0_CMD_DROP_CONTROL_OFFSET, 0x80000000 | (DROP_BLOCKS_START), NSIP_HBI_DROP_CMD_DROP_CONTROL_WR_MASK, \
        "H2B_CMD_DROP_CONTROL"},
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_DROP_CMD0_CMD_DROP_CONTROL_OFFSET, (DROP_BLOCKS_END), NSIP_HBI_DROP_CMD_DROP_CONTROL_WR_MASK, \
        "H2B_CMD_DROP_CONTROL"},
    ...
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_DROP_CMD1_CMD_DROP_STREAM_EN_OFFSET, 0x2, NSIP_HBI_DROP_CMD_DROP_STREAM_EN_WR_MASK, \
        "H2B_CMD_DTROP_STREAM_EN"},
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_DROP_CMD1_CMD_DROP_CONTROL_OFFSET, 0x80000000 | (DROP_BLOCKS_START), NSIP_HBI_DROP_CMD_DROP_CONTROL_WR_MASK, \
        "H2B_CMD_DROP_CONTROL"},
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_DROP_CMD1_CMD_DROP_CONTROL_OFFSET, (DROP_BLOCKS_END), NSIP_HBI_DROP_CMD_DROP_CONTROL_WR_MASK, \
        "H2B_CMD_DROP_CONTROL"},
    /***********************************************************************************************************************************/
...
int main(int argc, const char *argv[])
{
    ...
    unsigned int vex0_tx_args[] = {0, NUMBER_OF_VECTORS_INPUT, 0, 0, 0, 0};
    unsigned int vex1_tx_args[] = {0, NUMBER_OF_VECTORS_INPUT, 0, 0, 1, 0};
    int vex_rx_offset_val[] = {HIVE_ADDR_v2d_flow1_rx_g_scalar_val, HIVE_ADDR_v2d_flow1_rx_g_num_vecs_in,
                                ...
                                HIVE_ADDR_v2d_flow1_rx_g_data_drop_block_start, HIVE_ADDR_v2d_flow1_rx_g_data_drop_block_end};
    unsigned int vex0_rx_args[] = {0, NUMBER_OF_VECTORS_INPUT, 1, 64, 1, 0, DROP_BLOCKS_START, DROP_BLOCKS_END};
    unsigned int vex1_rx_args[] = {0, NUMBER_OF_VECTORS_INPUT, 1, 64, 1, 0, DROP_BLOCKS_START, DROP_BLOCKS_END};
   ...
   host_flag = 1;
    ...
    while ((h2b_flit_cnt += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_FLIT_COUNT_OFFSET, "H2B_FLIT_COUNT"))  <
            ((DROP_BLOCKS_START * (g_flow1_vectors_per_block[sys_rate] * 2))))
    {
    ...
    for (vex_cnt = 0; vex_cnt < NUM_RX_VEX_CORES; vex_cnt++)
    {
        ...
        while (host_vec_processed < (((MAX_BLOCKS - DROP_BLOCKS) * num_vectors_block[vex_cnt])))
        {
        ...
        printf ("processed blocks  by vex %d 0x%x\n", (vex_num), host_vec_processed / g_flow1_vectors_per_block[sys_rate]);

        if (0xFFFFFFFF != host_result)
        {
            printf ("ERROR: Received output didn't match with Reference output TV for Rx VEX%d\n", (vex_num));
            test_status |= 1;
        }
        else
        {
