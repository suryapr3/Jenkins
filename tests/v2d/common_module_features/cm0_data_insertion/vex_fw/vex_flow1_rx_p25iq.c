 ...
 *********************************************************************************************/
/**
 * @file    vex_flow1_rx_p25iq.c
 * @brief   Vex FW code to validate V2D Path A.
 * @details 1. VEX will be waiting on SDF to receive the data from HOST V2D.
...
volatile int AT(0x48) g_data_insert_block_start;
volatile int AT(0x4c) g_data_insert_block_end;
#if DEBUG
...
void v2d_flow1_rx_p25iq() ENTRY
{
    ...
    while (count < (g_data_insert_block_start))
    {
        ...
        /* for 0.25IQ ii = 6 is needed due to prologue+epilogue is going beyond the
...
//#pragma hive force_unroll=4
    ...
    count = g_data_insert_block_start;
    add_temp = g_data_insert_block_start;

    /*For the blocks during insertion*/
    while (count < (g_data_insert_block_end))
    {
...
//#pragma hive force_unroll=4
    ...
    /*for the block after insertion disabled*/
    {
        unsigned int is_data_ready, idx, ptr, iter_max;
        vec32i32 vin2 UNINIT;
        iter_max = g_num_vecs_in;
