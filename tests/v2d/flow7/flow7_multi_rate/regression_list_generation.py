...
wa = wb.active
header = ["tx_vex", "rx_vex", "v2d_num", "apb_num", "drf_clock", "test_config", "filter_mode"]
row = 1
            ...
            for drf_clock in range(1):
                for test_config in test_config_list:
                    for filter_mode in range (2):
                        ...
                        wa.cell(row,7).value = filter_mode
                        row+=1
...
sheet_name= wb['Sheet']
sheet_name.title ='flow7_multi_rate'

wb.save("flow7_multi_rate_list.xlsx")
