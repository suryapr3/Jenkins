 ...
 *********************************************************************************************/
/**
 * @file    vex_flow7_filter_mode_rx.c
 ...
 ***********************************************/
vec32i32 AT(INPUT_VMEM_LOCATION) vbuffer[VEX_VMEM_SIZE / 2];
vec32i32 AT(RX_DEST_BUFFER_BASE) vbuffer_out[VEX_VMEM_SIZE / 2];
 ...
 * @return          none
 */
void v2d_flow7_filter_mode_rx() ENTRY
{
    int sm_ch = CONSUMER_SM_BUF_ID;
    int vex_dest_buffer = RX_DEST_BUFFER_BASE;
    int vex_src_buffer = INPUT_VMEM_LOCATION;
...
//#pragma hive force_unroll=4
        ...
        for (int i = 0; i < iter_max; i++)
        {
          ...
          int sout0, sout1;
          v_ldoi_1024_circ(Any, vex_src_buffer , src_compare_offset, LD_CTL1, LD_CTL2, vin, src_compare_offset) NO_ALIAS SYNC_WITH(buffer_read);
          //vin = vbuffer[vec_processed];
...
//              g_host_debug = sout0;
//              g_host_debug1 = i;
                V_GET_32(31,vout,g_host_debug);
                V_GET_32(31,vin,g_host_debug1);
          }
#endif
          vec_processed++;
