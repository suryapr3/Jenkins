...
import subprocess
import time
wb = openpyxl.load_workbook("flow7_multi_rate_list.xlsx")
...
for i in range(2,row_count +1):
    ...
    filter_mode = wb1.cell(row=i,column=7).value
    rv = subprocess.call(['python flow7_multi_rate.py '  f'{tx_vex_num} '  f'{rx_vex_num} '  f'{v2d_num} '  f'{apb_num} '  f'{drf_clock} '  f'{test_config} ' f'{filter_mode} '], shell=True)
