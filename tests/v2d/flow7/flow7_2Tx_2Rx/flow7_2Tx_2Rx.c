 ...
 * @details 1. Configuration of VEX, V2D and Common Module for sending data from VEX to V2D via C path
               with post filter enabled and receving it back to other VEX.
 ...
 ***********************************************/
#define B2H_ADD_MAP_C2_OFFSET  0x10000
...
const char *g_vex_fw_binary[] = {"vex_fw/sim_m64/v2d_flow7_tx.fw", "vex_fw/sim_m64/v2d_flow7_rx.fw", "vex_fw/sim_m64/v2d_flow7_unequal_split_tx1.fw"};

int g_initial_block_size = 1;    // This corresponds to 2L and VEX will transfer 2.5L before BFN trigger.
...
/* The value should be number of blocks defined as per order in pdpd_rate_t enum */
...
// B2H Registers
    ...
    {(V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_B2H_CSR_B2H_ADDR_MASK_TBL0_OFFSET), (B2H_ADD_MAP_C2_OFFSET - 1), NSIP_HBI_B2H_CSR_B2H_ADDR_MASK_TBL_WR_MASK, \
        "B2H_ADDR_MASK_TBL0"},
    // These are for Buffer C1
    {(V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_B2H_CSR_B2H_ADDR_TBL1_OFFSET), (0x80000000 | B2H_ADD_MAP_C2_OFFSET), \
        NSIP_HBI_B2H_CSR_B2H_ADDR_TBL_WR_MASK, "HBI_1_B2H_ADDR_MAP_TBL1"},
    // Mask was set for total 2K flits in Bytes
    {(V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_B2H_CSR_B2H_ADDR_MASK_TBL1_OFFSET), 0x1FFFF, NSIP_HBI_B2H_CSR_B2H_ADDR_MASK_TBL_WR_MASK, \
...
// H2B Registers
    ...
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_BUF_SZ_HW_TBL1_OFFSET, (NUM_DATA_BLOCKS - 1), NSIP_HBI_H2B_BUF_SZ_HW_TBL_WR_MASK, \
        "H2B_BUF_SZ_HW_TBL1"},
    /* Data will be written to VEX location 0x10080 , i.e VEX DMEM location 0x80*/
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_TK_ADDR_LKUP_TBL1_OFFSET, (STRM2CIO_2_VEX_DMEM_OFFSET + HIVE_ADDR_v2d_flow7_rx_consumer_sm_gate), NSIP_HBI_H2B_TK_ADDR_LKUP_TBL_WR_MASK, \
    ...
    {V2D_NSIP_HBI_0_OFFSET + NSIP_HBI_H2B_ADDR_LKUP_TBL1_OFFSET, (NSIP_HBI_H2B_ADDR_LKUP_TBL_VALID_BF_MSK | HIVE_ADDR_v2d_flow7_rx_vbuffer_out), NSIP_HBI_H2B_ADDR_LKUP_TBL_WR_MASK, "H2B_ADDR_LKUP_TBL1"},
...
// Tx Path Settings
    ...
    {V2D_CSR_OFFSET + AV2D_CSR_TX_CSR_TX_PTR_BOUND0_OFFSET, 0x3FF0000, AV2D_CSR_TX_CSR_TX_PD_PTR_BOUND_WR_MASK, "TX_PD_PTR_BOUND0"},
    /*Upper Bound [26:16]and Lower Bound [10:0]in flits for Buffer B1*/
    {V2D_CSR_OFFSET + AV2D_CSR_TX_CSR_TX_PTR_BOUND1_OFFSET, 0x7FF0400, AV2D_CSR_TX_CSR_TX_PD_PTR_BOUND_WR_MASK, "TX_PD_PTR_BOUND1"},
...
{
    int i, j = 0, rv = 0, v2d_stream_cnt0 = 0, v2d_stream_cnt1 = 0, md_control0 = 0, md_control1 = 0,
    ...
    /*pdpd out rate was used as extra terms needs to be fed with the PDPD out put rate*/
    if (SPLIT_METHOD_EQUAL == split_method)  // equal split
    {
        v2d_stream_cnt0 = (g_vectors_per_block[drf_clock][pdpd_out_rate] - 1);
        v2d_stream_cnt1 = (g_vectors_per_block[drf_clock][pdpd_out_rate] - 1);
    }
    else if (SPLIT_METHOD_UNEQUAL == split_method)   // split in 3:1 ratio i.e 3/4 th by first VEX and 1/4th by second VEX cores
    {
        v2d_stream_cnt0 = (((VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_out_rate]) / 4) * 3) - 1);
        v2d_stream_cnt1 = ((VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_out_rate]) / 4) - 1);
    }
    ...
    v2d_tx_ptr_bound1 = (((g_num_blocks_v2d_buffer * g_vectors_per_block[drf_clock][pdpd_out_rate] * 2) - 1) << 16) |
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_cm_0_config_tbl[i].offset), (0x80000000 | HIVE_ADDR_v2d_flow7_rx_vbuffer_out), \
                g_cm_0_config_tbl[i].mask, g_cm_0_config_tbl[i].name))
            return rv;
    }
    ...
    for (i = 0; i < ARRAY_SIZE(g_v2d_flow7_config_tbl) - 8; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow7_config_tbl[i].offset), g_v2d_flow7_config_tbl[i].value, \
                g_v2d_flow7_config_tbl[i].mask, g_v2d_flow7_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow7_config_tbl) - 7; i++)
    {
    ...
    for (; i < ARRAY_SIZE(g_v2d_flow7_config_tbl) - 6; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow7_config_tbl[i].offset), \
                    v2d_tx_ptr_bound0, g_v2d_flow7_config_tbl[i].mask, g_v2d_flow7_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow7_config_tbl) - 5; i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow7_config_tbl[i].offset), \
                    v2d_tx_ptr_bound1, g_v2d_flow7_config_tbl[i].mask, g_v2d_flow7_config_tbl[i].name))
            return rv;
    }
    ...
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow7_config_tbl[i].offset), \
                    (0x80000000 | v2d_stream_cnt0), g_v2d_flow7_config_tbl[i].mask, g_v2d_flow7_config_tbl[i].name))
            return rv;
    }
    for (; i < ARRAY_SIZE(g_v2d_flow7_config_tbl); i++)
    {
        if (rv = write_reg_18a((PDPD_BASE_ADDR(v2d_num, apb_num) + g_v2d_flow7_config_tbl[i].offset), \
                    (0x80000000 | v2d_stream_cnt1), g_v2d_flow7_config_tbl[i].mask, g_v2d_flow7_config_tbl[i].name))
            return rv;
    }
 ...
 *                  VEX cores
 * @param [in]      vex0_tx_args, vex1_tx_args, vex0_rx_args, split_method, drf_clok and pdpd_rate
 * @return          none
 */
void vex_param_gen (unsigned int *vex0_tx_args, unsigned int *vex1_tx_args,
        unsigned int *vex0_rx_args, unsigned int *vex1_rx_args, int split_method, int drf_clock, pdpd_rate_t pdpd_out_rate)
{
    vex0_rx_args[2] = g_max_blocks[drf_clock][pdpd_out_rate];
    vex1_rx_args[2] = g_max_blocks[drf_clock][pdpd_out_rate];
    ...
    vex1_tx_args[2] = (B2H_ADD_MAP_C2_OFFSET); // in bytes
    vex0_rx_args[4] = 1;
    vex1_rx_args[4] = 1;

    if (SPLIT_METHOD_EQUAL == split_method)  // equal split
    {
        vex0_tx_args[1] = g_vectors_per_block[drf_clock][pdpd_out_rate] / 2;
        vex1_tx_args[1] = g_vectors_per_block[drf_clock][pdpd_out_rate] / 2;
    ...
    {
        vex0_tx_args[1] = (g_vectors_per_block[drf_clock][pdpd_out_rate] / 4) * 3;
        vex1_tx_args[1] = g_vectors_per_block[drf_clock][pdpd_out_rate] / 4;
        vex0_rx_args[1] = vex0_tx_args[1];
        vex1_rx_args[1] = vex1_tx_args[1];
        vex0_rx_args[3] = NUM_DATA_BLOCKS;
        vex1_rx_args[3] = NUM_DATA_BLOCKS;
 ...
 *                  VEX cores
 * @param [in]      pointers to rx, tx0, tx1 arrays , vex0_input_vectors , drf_clock and pdpd_rate
 * @return          none
 */
void input_vec_gen (unsigned int (*val_inp_tx0)[32], unsigned int (*val_inp_tx1)[32],
        unsigned int vex0_num_input_vectors, int drf_clock, pdpd_rate_t pdpd_rate)
{
    int i, j;
    ...
    for (i = vex0_num_input_vectors; i < g_vectors_per_block[drf_clock][pdpd_rate]; i++)
    {
        for (j = 0; j < NUM_OF_ELEMENTS_PER_VEC; j++)
        {
...
int main(int argc, const char *argv[])
{
    ...
    int offset_val[] = {HIVE_ADDR_v2d_flow7_tx_g_scalar_val, HIVE_ADDR_v2d_flow7_tx_g_num_vecs_in,
                         ...
                         HIVE_ADDR_v2d_flow7_tx_g_initial_block_size, HIVE_ADDR_v2d_flow7_tx_g_tx_instance,
                         HIVE_ADDR_v2d_flow7_tx_g_host_cycle_count};
    ...
    int vex_rx_offset_val[] = {HIVE_ADDR_v2d_flow7_rx_g_scalar_val, HIVE_ADDR_v2d_flow7_rx_g_num_vecs_in,
                                ...
                                HIVE_ADDR_v2d_flow7_rx_g_tokens_per_block, HIVE_ADDR_v2d_flow7_rx_g_count1};
    ...
    unsigned int host_ref1_data[NUMBER_OF_VECTORS_OUTPUT * NUM_OF_ELEMENTS_PER_VEC] = {0};
    ...
    /*pdpd out rate was used as extra terms needs to be fed with the PDPD out put rate*/
    vex_param_gen(vex0_tx_args, vex1_tx_args, vex0_rx_args, vex1_rx_args, split_method, drf_clock, pdpd_out_rate);
    ...
    /*pdpd out rate was used as extra terms needs to be fed with the PDPD out put rate*/
    input_vec_gen(val_inp_tx0, val_inp_tx1, vex0_tx_args[1], drf_clock, pdpd_out_rate);
    ...
    if (vector_read_hex_u32(fp_ref_output,
                host_ref0_data,
                vex0_tx_args[1] * NUM_OF_ELEMENTS_PER_VEC) < \
                vex0_tx_args[1] * NUM_OF_ELEMENTS_PER_VEC)
    {
    ...
    if (vector_read_hex_u32(fp_ref_output,
                host_ref1_data,
                vex1_tx_args[1] * NUM_OF_ELEMENTS_PER_VEC) < \
                vex1_tx_args[1] * NUM_OF_ELEMENTS_PER_VEC)
    {
        printf("ERROR: Insufficient Output REF data\n");
        return -1;
    ...
    // unequal split Tx0 will be loaded with v2d_flow7_tx.fw and Tx1 will be loaded with
   ...
   /*Second Rx Vex Core*/
    ...
    while ((b2h_flit_count += read_reg_18a(PDPD_BASE_ADDR(v2d_num, apb_num) + V2D_NSIP_HBI_1_OFFSET + \
            ...
            (VEC2FLIT(g_vectors_per_block[drf_clock][pdpd_out_rate])) + VEC2FLIT(vex0_tx_args[1] / 2))
    {
    ...
    for (vex_cnt = 0; vex_cnt < NUM_RX_VEX_CORES; vex_cnt++)
    {
        ...
        int vex_num = vex_num_param[vex_index];
        while (host_vec_processed < ((vex0_rx_args[2] - 1) * num_vectors_block[vex_cnt]))
        {
        ...
        printf ("processed blocks by vex %d 0x%x\n", (vex_num), host_blocks_processed);
    ...
    if (0 == test_status)
    {
        printf("Host:---- SUCCESS ---- tx0_vex_num : %d, rx0_vex_num: %d, tx1_vex_num : %d, rx1_vex_num : %d, v2d_num : %d, apb_num : %d, split_method: %s, drf_clock: %s, pdpd_in_rate: %s, pdpd_out_rate: %s\n",
                vex_num_param[0], vex_num_param[1], vex_num_param[2], vex_num_param[3], v2d_num, apb_num, split_method_str[split_method], drf_clock_str[drf_clock], pdpd_rate_str[pdpd_in_rate], pdpd_rate_str[pdpd_out_rate]);
    }
    else
    {
        printf("Host:---- FAIL ---- tx0_vex_num : %d, rx0_vex_num: %d, tx1_vex_num : %d, rx1_vex_num : %d, v2d_num : %d, apb_num : %d, split_method: %s, drf_clock: %s, pdpd_in_rate: %s, pdpd_out_rate: %s\n",
                vex_num_param[0], vex_num_param[1], vex_num_param[2], vex_num_param[3], v2d_num, apb_num, split_method_str[split_method], drf_clock_str[drf_clock], pdpd_rate_str[pdpd_in_rate], pdpd_rate_str[pdpd_out_rate]);
        rv = 1;
    }
