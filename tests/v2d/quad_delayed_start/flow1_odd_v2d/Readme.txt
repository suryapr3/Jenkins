...
python3 flow1_odd_v2d.py <quad_instance> <apb_id> <drf_clock> <sys_rate> <delay_value>

quad instance can be within the range [0,1]
apb_id can be within the range [0,1]
drf_clock should be 0
sys_rate can be with in the range [0,11] and [21,26]

ex: python3 flow1_odd_v2d.py 0 0 0 0 768
...
The above will generate flow1_odd_v2d_list.xlsx file , needs to be done only if the xlsx file is not present or there were any changes done in the regression_list_generation.py file.


Steps to run regression:
-------------------------------------------------------------------------------------------------------------------------------------------------------
python3 flow1_odd_v2d_regression.py

The above will run all the possible combination which was mentioned at the first step.
The input combinations will be taken from the flow1_odd_v2d_list.xlsx file.
