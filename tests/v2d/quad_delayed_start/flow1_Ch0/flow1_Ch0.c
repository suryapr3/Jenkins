 ...
 *          and receving it back via path F by using QUAD delayed start and only Ch0
 *          of even V2D is FED with data.
 * @TC Description:
        ...
        5. Host will be waiting on a memory location where VEX will update the number of vectors
           it was processed.
        6. All the path A of QUAD channels needs to be configured and only Ch0 of even V2D will be
           fed with the data.
        7. All the DRF channels of the QUAD will be triggered and the delay value can be passed as
...
int g_exp_interrupt_value[][PDPD_MAX_INSTANCE] = {
    {TX_UNDERRUN_VALUE, AV2D_CSR_INT_CSR_INT_STATUS_TX_NODATA_START_BF_MSK},
    {AV2D_CSR_INT_CSR_INT_STATUS_TX_NODATA_START_BF_MSK, AV2D_CSR_INT_CSR_INT_STATUS_TX_NODATA_START_BF_MSK}
};
...
int main(int argc, const char *argv[])
{
    ...
    int fill_level_value[V2D_PER_QUAD][PDPD_MAX_INSTANCE][MAX_NUM_STREAMS] = {0};
    int watermark_buf[V2D_PER_QUAD][PDPD_MAX_INSTANCE][MAX_NUM_STREAMS] = {0};
    ...
    int rcs_host_param[2] = {0}; // corresponds to quad_instance and delay value
#endif
    ...
    printf ("INFO: argv[2] : %d = sys_rate %s", sys_rate, sys_rate_str[sys_rate]);
    printf ("INFO: argv[3] : %d = delay_value", delay_value);
    ...
    apb_num = DRF_CHANNEL_0;    // Only Channel 0
    ...
    /*resetting the values of even V2D Ch0 of given quad*/
    apb_num = DRF_CHANNEL_0;
    v2d_num = g_v2d_instance_in_quad[quad_instance][0];
    vex_num_param[0] = g_vex_instance_in_v2d[v2d_num][0];    // Tx VEX
    vex_num_param[1] = g_vex_instance_in_v2d[v2d_num][1];    // Rx VEX
    ...
    loop_wait_count = 0;
    /*Wait till required number of flits reached the H2B buffer*/
    ...
    /*As Tx VEX is continuously transmitting the data for V2D0 Ch0 no underrun interrupt should be set*/
    ...
    /*Read the watermark level value when Tx on DRF channels in the QUAD started*/
    get_quad_tx_trigger_fill_level(quad_instance, 1/*number of streams*/, fill_level_value);

    for (quad_v2d_index = 0; quad_v2d_index < V2D_PER_QUAD; quad_v2d_index++)
    {
        for (lc = PDPD_MAIN_INSTANCE; lc < PDPD_MAX_INSTANCE; lc++)
        {
            printf("BFN triggered when buffer is @0x%x", fill_level_value[quad_v2d_index][lc][0]);
        }
    }

    /*Read the water mark register for DRF channels in the QUAD*/
    get_quad_tx_wm_hilo(quad_instance, 1/*number of streams*/, watermark_buf);

    for (quad_v2d_index = 0; quad_v2d_index < V2D_PER_QUAD; quad_v2d_index++)
    {
        for (lc = PDPD_MAIN_INSTANCE; lc < PDPD_MAX_INSTANCE; lc++)
        {
            printf("low watermark value 0x%x", (watermark_buf[quad_v2d_index][lc][0] & 0xFFF));
            printf("high watermark value 0x%x", ((watermark_buf[quad_v2d_index][lc][0] >> 16) & 0xFFF));
        }
    }
    ...
    /*Signal from host to stop continuous flow on B Tx Vex Core , which in turn will signal the Tx Vex core to stop continuous transmission*/
    if ((rv = srp_dev_block_write32(srp,
                    SRP_BLOCK_VEX_CORE,
                    vex_num_param[0],
