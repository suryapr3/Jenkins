 ...
 *********************************************************************************************/
/**
 * @file    post_processing_2_channel.c
 * @details 1. This TC validates the Post Processing path with QUAD deayed start for enabling two channels
 *              in the given QUAD, one channel per V2D.
 ...
 * @TC Description:
        ...
        11. The PDPD channel and the post processing channel will be triggered by using QUAD delayed start.
        12. VEX cores triggering along with BFN programming will be done from RCS.
 */
...
#define RX_GNRL_CSR_VALUE_POST_PROCESSING   (AV2D_CSR_RX_CSR_RX_GNRL_CSR_RX_PD_EN_BF_MSK | \
        AV2D_CSR_RX_CSR_RX_GNRL_CSR_RX_PD_CFG_COMPLETE_BF_MSK | AV2D_CSR_RX_CSR_RX_GNRL_CSR_RX_CFG_COMPLETE_BF_MSK)
...
int ref_output_read(unsigned int *host_ref_data, int num_vectors, int test_config)
{
    ...
    sprintf(tref_file, "%s%s", TEST_VECTORS,ref_file[test_config]);
...
int main(int argc, const char *argv[])
{
    ...
    int lc, i, j;
    int vex_num_param[NUM_VEX_CORES] = {0}, v2d_num, apb_num, underrun_reg_value = 0, drf_clock = 0, test_config = 0, quad_instance = 0,
        delay_value = 0, v2d_index = 0;
    ...
    unsigned int host_ref_data[NUMBER_OF_VECTORS_OUTPUT * NUM_OF_ELEMENTS_PER_VEC] = {0}, watermark_buf[MAX_NUM_STREAMS] = {0},
                 fill_level_value[MAX_NUM_STREAMS] = {0};
    ...
    int rcs_host_param[4] = {0}; // corresponds to quad_instance, apb_num ,delay value and drf_clock
#endif
    ...
    printf ("INFO: argv[3] : %d = test_config", test_config);
    ...
    if (rv = ref_output_read(host_ref_data, g_vectors_per_block[drf_clock][pdpd_out_rate], test_config))
    {
        return rv;
    }
    ...
    for (v2d_index = 0; v2d_index < V2D_PER_QUAD; v2d_index++)
    {
        ...
        vex_num_param[2] = g_vex_instance_in_v2d[v2d_num][2];    // Rx VEX
    ...
    for (v2d_index = 0; v2d_index < V2D_PER_QUAD; v2d_index++)
    {
                ...
                (g_max_blocks[drf_clock][pdpd_out_rate] * VEC2FLIT(g_flow1_vectors_per_block[sys_rate])))
        {
            ...
            {
                printf("WARNING: Breaking the loop while H2B_FLIT_CNT 0x%x is not the required number of flits for V2D%d, Ch_no:%d",
                        h2b_flit_count, v2d_num, apb_num);
                break;
            }
            loop_wait_count ++;
            usleep(10000);
        ...
        while (host_vec_processed < (vex_path_f_rx_args_val[2] * g_flow1_vectors_per_block[sys_rate]))
        {
            ...
            {
                printf("ERROR: VEX core %x didn't processed enough vectors, host_vec_processed : 0x%x", vex_num_param[2], host_vec_processed);
                rv = 1;
                return rv;
            }
        ...
        {
            printf("Received output matches with Reference output for V2D%x Ch_no:%d\n", v2d_num, apb_num);
        }
        ...
        {
            printf("ERROR: interrupt check failed for V2D%x Ch_no:%d\n", v2d_num, apb_num);
            test_status |= 1;
        }
        ...
        printf("BFN triggered when PDPD Buffer is @0x%x for V2D%x Ch_no:%d", fill_level_value[0], v2d_num, apb_num);

        /*Read the watermark level value when Tx on for DRF channel*/
        get_tx_trigger_fill_level(v2d_num, apb_num, 1/*Number of streams*/, fill_level_value);
        printf("BFN triggered when Tx Buffer is @0x%x for V2D%x Ch_no:%d", fill_level_value[0], v2d_num, apb_num);

        /* Get the watermark high , low values for V2D PDPD Buffers*/
        ...
        {
            printf("HOST: Low watermark value for B buffer0 : 0x%x for V2D%x Ch_no:%d", watermark_buf[0] & 0xFFF, v2d_num, apb_num);
            printf("HOST: High watermark value for B buffer0: 0x%x for V2D%x Ch_no:%d", (watermark_buf[0] >> 16) & 0xFFF, v2d_num, apb_num);
        }

        /* Get the watermark high , low values for V2D DRF Buffers*/
        ...
        else
        {
            for (lc = 0; lc < 1; lc++)
            {
                printf("HOST: Low watermark value for C buffer%d: 0x%x for V2D%x Ch_no:%d", lc, watermark_buf[lc] & 0xFFF, v2d_num, apb_num);
                printf("HOST: High watermark value for C buffer%d: 0x%x for V2D%x Ch_no:%d",
                        lc, (watermark_buf[lc] >> 16) & 0xFFF, v2d_num, apb_num);
            }
        }
        ...
        {
            printf("HOST: Interrupt check success for V2D%x Ch_no:%d", v2d_num, apb_num);
        }
    }

    if (rv = quad_2_channel_tx_pdpd_check(quad_instance, apb_num))
    {
        printf("ERROR: BFN count for Tx transmission to PDPD is not same across the QUAD channels");
    }

    if (rv = quad_2_channel_tx_drf_check(quad_instance, apb_num))
    {
        printf("ERROR: BFN count for Tx transmission to DRF is not same across the QUAD channels");
    }

    if (0 == test_status)
    {
        printf("Host:---- SUCCESS ---- quad_instance: %d, apb_num: %d, drf_clock: %s, \
                pdpd_in_rate : %s, pdpd_out_rate : %s, lut_mode : %d",
                quad_instance, apb_num, drf_clock_str[drf_clock],
                pdpd_rate_str[pdpd_in_rate], pdpd_rate_str[pdpd_out_rate], g_test_config_tbl[test_config][2]);
    }
    else
    {
        printf("Host:---- FAIL ---- quad_instance: %d, apb_num: %d, drf_clock: %s, \
                pdpd_in_rate : %s, pdpd_out_rate : %s, lut_mode : %d",
                quad_instance, apb_num, drf_clock_str[drf_clock],
                pdpd_rate_str[pdpd_in_rate], pdpd_rate_str[pdpd_out_rate], g_test_config_tbl[test_config][2]);
