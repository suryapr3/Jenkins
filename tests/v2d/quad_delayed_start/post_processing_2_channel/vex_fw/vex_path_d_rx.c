 ...
 * @details 1. VEX will be waiting on SDF to receive the data from HOST.
            ...
            3. Once the comparison is done it will tranmsit the received vectors towards HOST.
            4. Once all the blocks were received the result will be stored to a predefined location
               for the host to read..
            5. This code is specific to sample rates 0.5IQGSPS to 8IQ GSPS and 3R GSPS to 16R GSPS
**/
...
volatile int AT(0x48) g_dest_buffer_base;
volatile int AT(0x50) g_host_indicator;
#if DEBUG
volatile int AT(0x3C) g_host_debug;
volatile int AT(0x40) g_host_debug1;
volatile int AT(0x4c) g_host_debug2;
 ...
 * @return          none
 */
void v2d_path_d_rx() ENTRY
{
    int sm_ch = CONSUMER_SM_BUF_ID;
    int vex_dest_buffer = RX_DEST_BUFFER_BASE;
    int vex_src_buffer = RX_SRC_BUFFER_BASE;
    ...
    g_fail_cnt = 0;
#endif
    limit =  ((2 * BYTES_PER_VEC) << 20) | ((tbs_in_block * g_num_vecs_in * BYTES_PER_VEC) - (2 * BYTES_PER_VEC));
    rewind = (tbs_in_block * g_num_vecs_in * BYTES_PER_VEC) - (2 * BYTES_PER_VEC);
    ...
    {
        unsigned int is_data_ready, idx, ptr, iter_max, dest_address;
        vec32i32 vin2 UNINIT;
        int src_compare_offset = 0x0;
        iter_max = g_num_vecs_in;
        dest_address = g_dest_buffer_base;
...
//#pragma hive force_unroll=2
#endif
#endif
        for (int i = 0; i < (iter_max / 2); i++)
        {
          vec32i32 vdata0 UNINIT, vdata1 UNINIT, vout0 UNINIT, vout1 UNINIT, vadd0 UNINIT, vadd1 UNINIT;

          int sout0, sout1, sout2, sout3;
          v_ldoi_2048(Any, vex_src_buffer, src_compare_offset, vdata0, vdata1, src_compare_offset) NO_ALIAS SYNC(buffer_read);
          //V_ADD_32(vdata0, vin2, vadd0);
          //V_ADD_32(vdata1, vin2, vadd1);
          v_ldoi_2048_circ(Any, vex_dest_buffer, compare_offset, limit, rewind, vout0, vout1, compare_offset) NO_ALIAS SYNC_WITH(buffer_read);
          //Bit value of 1 in output indicates corresponding vector elements are equal
          V_EQ_32(vdata0, vout0, sout0, sout1) SYNC_WITH(buffer_read);
          V_EQ_32(vdata1, vout1, sout2, sout3) SYNC_WITH(buffer_read);
#if DEBUG
          ...
          result = result & (sout2) SYNC_WITH(buffer_read);
          v_xstomi_1024x2(Any, dest_address, offset, LD_OFFSET_MAX, vout0, vout1, offset);
#if DEBUG
          OP___dump(__LINE__, result);
          if (result != 0xFFFFFFFF && (g_fail_cnt == 0))
          {
              g_fail_cnt = g_fail_cnt + count + 1;
//              g_host_debug = sout0;
//              g_host_debug1 = i;
                V_GET_32(0,vout,g_host_debug);
                V_GET_32(15,vout,g_host_debug2);
                V_GET_32(0,vadd,g_host_debug1);
          }
#endif
          vec_processed = vec_processed + 2;
