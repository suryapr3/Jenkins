 ...
 *          with QUAD delayed start and only Ch0 of even V2D will be fed with data.
 * @details 1. Configuration of VEX, V2D's Common Module for sending data from VEX to V2D via B path
   ...
   @TC Description:
        ...
        7. All the path B of QUAD channels needs to be configured and only Ch0 of even V2D will be
           fed with the data.
...
int g_exp_interrupt_value[][PDPD_MAX_INSTANCE] = {
    {V2D_INTERRUPT_VALUE, AV2D_CSR_INT_CSR_INT_STAT_PD_MAIN_NODATA_START_BF_MSK },
    {AV2D_CSR_INT_CSR_INT_STAT_PD_MAIN_NODATA_START_BF_MSK , AV2D_CSR_INT_CSR_INT_STAT_PD_MAIN_NODATA_START_BF_MSK }
};
...
int main(int argc, const char *argv[])
{
    ...
    int vex_num = 0, vex_num_param[NUM_VEX_CORES] = {0}, v2d_num, apb_num, underrun_reg_value = 0, drf_clock = 0, test_config = 0,
        post_filter_mode = 0, quad_instance = 0, quad_v2d_index = 0, delay_value = 0;
    ...
    /*Read the watermark level value when Tx on PDPD channels in the QUAD started*/
    get_quad_pdpd_tx_trigger_fill_level(quad_instance, 1/*number of streams*/, fill_level_value);
    ...
    /*Read the water mark register for PDPD channels in the QUAD*/
    get_quad_pdpd_tx_wm_hilo(quad_instance, 1/*number of streams*/, watermark_buf);
