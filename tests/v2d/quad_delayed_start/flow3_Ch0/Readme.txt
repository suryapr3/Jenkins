...
ex: gmake clean; gmake VEX_1P5G=1 VEX_1US_WINDOW=1

Steps to run individual TC:
-------------------------------------------------------------------------------------------------------------------------------------------------------
This script will take 5 arguments and script should be run in below format

python3 flow3_Ch0.py <quad_instance> <drf_clock> <test_config> <filter_mode> <delay_value>

quad_instance can be with in the range [0,1]
drf_clock can be with in the range [0,1]
test_config can be with in the range [0,16]
filter_mode can be with in the range [0,1]
delay value can be [768]

ex: python3 flow3_Ch0.py 0 1 0 0 768
...
The above will generate flow3_Ch0_list.xlsx file , needs to be done only if the xlsx file is not present or there were any changes done in the regression_list_generation.py file.

Steps to run regression:
-------------------------------------------------------------------------------------------------------------------------------------------------------
python3 flow3_Ch0_regression.py

The above will run all the possible combination which was mentioned at the first step.
The input combinations will be taken from the flow3_Ch0_list.xlsx file.
