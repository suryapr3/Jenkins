 ...
 * @TC Description:
        ...
        5. Host will be waiting on a memory location where VEX will update the number of vectors
           it was processed.
        6. All the path A of QUAD channels needs to be configured and all channels of V2D will be
           fed with the data.
...
int g_exp_interrupt_value[][PDPD_MAX_INSTANCE] = {
    {TX_UNDERRUN_VALUE, TX_UNDERRUN_VALUE},
    {TX_UNDERRUN_VALUE, TX_UNDERRUN_VALUE}
};
...
int main(int argc, const char *argv[])
{
    ...
    int vex_num_param[2] = {0}, v2d_num, apb_num, host_flag = 1, test_status = 0, loop_wait_count = 0, drf_clock=0, host_result = 0,
        host_vec_processed = 0, host_blocks_processed = 0, quad_instance = 0, quad_v2d_index = 0, delay_value = 0, v2d_index = 0;
    sys_rate_t sys_rate;
    ...
    for (lc = 0; lc < V2D_PER_QUAD; lc++)
    {
        for (apb_num = PDPD_MAIN_INSTANCE; apb_num < PDPD_MAX_INSTANCE; apb_num++)
        {
            ...
            {
                printf("HOST: Received output matches with Reference output for V2D%d Ch%d vex%d", v2d_num, apb_num, vex_num_param[1]);
            }
    ...
    if (rv = quad_tx_drf_check(quad_instance))
    {
