...
static void test_interrupt_with_mask(int vexId, int bMaskEnabled)
{
    ...
    int expected_bit_pos = CAR_HIGH_INT_STAT_VEX_HIGH0_INT_STAT_BF_OFF + 8*(vexId%4);
    
    unsigned int vex_high_int_status;
    unsigned int vex_high_int_masked_stat;
    unsigned int vex_high_int_enable;
    unsigned int vex_high_int_force;
    unsigned int vex_high_int_clear;
    unsigned int vex_high_int_bitmask_pos;

    if (vexId < 32)
    {
        vex_high_int_status = CAR_VEX_HIGH_INT_STAT0_ADR;
        vex_high_int_masked_stat = CAR_VEX_HIGH_INT_MASKED_STAT0_ADR;
        vex_high_int_enable = CAR_VEX_HIGH_INT_EN0_ADR;
        vex_high_int_force  = CAR_VEX_HIGH_INT_FORCE0_ADR;
        vex_high_int_clear = CAR_VEX_HIGH_INT_CLR0_ADR;
        vex_high_int_bitmask_pos = vexId;
    }
    else
    {
        vex_high_int_status = CAR_VEX_HIGH_INT_STAT1_ADR;
        vex_high_int_masked_stat = CAR_VEX_HIGH_INT_MASKED_STAT1_ADR;
        vex_high_int_enable = CAR_VEX_HIGH_INT_EN1_ADR;
        vex_high_int_force  = CAR_VEX_HIGH_INT_FORCE1_ADR;
        vex_high_int_clear = CAR_VEX_HIGH_INT_CLR1_ADR;
        vex_high_int_bitmask_pos = vexId - 32;
    }


    // pre check status, masked status, car_high_int_status
    read_reg_expect_18a(vex_high_int_status, 0, 0xFFFFFFFF, "vex_high_int_status");
    read_reg_expect_18a(vex_high_int_masked_stat, 0, 0xFFFFFFFF, "vex_high_int_masked_stat");
    read_reg_expect_18a(vex_car_high_int_status_addr, 0, 0xFFFFFFFF, "car_high_int_status");

    //enable interrupt
    write_reg_18a(vex_high_int_enable, (1 & bMaskEnabled) << vex_high_int_bitmask_pos, 0xFFFFFFFF, "vex_high_int_enable");

    //force interrupt
    write_reg_18a(vex_high_int_force, 1 << vex_high_int_bitmask_pos, 0xFFFFFFFF, "vex_high_int_force");

    // check status, masked status
    read_reg_expect_18a(vex_high_int_status, 1 << vex_high_int_bitmask_pos, 0xFFFFFFFF, "vex_high_int_status");
    read_reg_expect_18a(vex_high_int_masked_stat, (1 & bMaskEnabled) << vex_high_int_bitmask_pos, 0xFFFFFFFF, "vex_high_int_masked_stat");
    ...
    write_reg_18a(vex_high_int_clear, 1 << vex_high_int_bitmask_pos, 0xFFFFFFFF, "vex_high_int_clear");
...
main(int vexCoreId)
{
    ...
    print("Host: Testing vex_high interrupt on vex %d!\n", vexCoreId);
