...
/**
 * @file    vex_timer.c
 * @brief   Vex FW code to validate vex timer modification at runtime.
 * @details VEX will program the timer in continous watchdog mode and will update the notification data at runtime.
...
inline void init_timer(int cycles , int notif_data, int timer_id)
{
    ...
    s_xsto_32(Any, 0, TIMER_TMDCFGR(timer_id), TMD_CONTINUOUS_WATCHDOG);
...
void vex_timer() ENTRY
{
    ...
    do
    {
        ...
        // Update the timer notification address for the next cycle
        s_xsto_32(Any, 0, TIMER_TMDWDADDRR(g_timer_id), 0x4000 + TIMER_NOTIF_LOCATION + (g_timeout_count * 4));
        s_xsto_32(Any, 0, TIMER_TMDWDDATAR(g_timer_id), ++g_notif_data);
