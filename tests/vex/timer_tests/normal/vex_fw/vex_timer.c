...
/**
 * @file    vex_timer.c
 * @brief   Vex FW code to validate vex timer in Normal count up mode
 * @details Vex will program the timer in Normal mode
**/
...
#define TIMER_NOTIF_LOCATION (0x10)
...
volatile int AT(0x20) g_timer_tmdvalr;
...
void vex_timer() ENTRY
{
    ...
    init_timer(g_timer_dut_cycles, 0, g_timer_dut_id);
    ...
    s_xsto_32(Any, 0, TIMER_TMDCFGR(g_timer_perf_start_id),TMD_ONE_SHOT_WATCHDOG);
    s_xsto_32(Any, 0, TIMER_TMDCFGR(g_timer_perf_stop_id),TMD_ONE_SHOT_WATCHDOG);
    s_xsto_32(Any, 0, TIMER_TMDCFGR(g_timer_perf_snap_id),TMD_ONE_SHOT_WATCHDOG);
   
    
    s_xsto_32(Any, 0, TIMER_TMDCMDR(g_timer_dut_id), TMD_COMMAND_START); 
    s_xldo_32(Any, 0, TIMER_TMDVALR(g_timer_dut_id), g_timer_tmdvalr);
    ...
    } while (timer_tmdvalr > g_timer_dut_cycles);/* while (timer_tmdvalr < 0xFF);*/
    ...
    do {
        ...
        s_xldo_32(Any, 0, TIMER_TMDVALR(g_timer_perf_stop_id), timer_tmdvalr);
