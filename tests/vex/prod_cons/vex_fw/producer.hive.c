...
unsigned int buffer_va; //base address of desination vector buffer in consumer core
unsigned int buffer_wa; //address in consumer core to update write pointer
...
void process_block(unsigned int buffer_va, int token_per_request, int blk_idx)
{
  ...
  for(j=0;j<(token_per_request*NUM_VEC_IN_BLK);j++)
  {
    ...
    vec32i32 v1;
    static const int add_scalar_ctrl =  SCALAR_AS_VIP1 | ALU1_CT_OPER_SIZE32 | SRS_OUT_SIZE32 | ALU1_CT_ADD | SRS_RND_DIS | SRS_SAT_DIS;
    V_ALU_B_SCALAR(add_scalar_ctrl, 1, vstate, v1);
    vstate = v1;
