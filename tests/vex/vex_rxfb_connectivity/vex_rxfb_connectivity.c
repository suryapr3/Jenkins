...
static void configure_dma(int rxfbId, int dmaId, int chId, int nsipId)
{
	...
	if (chId == 0)
    {
		...
		write_reg64_18a(MEM_FB_DMAC_CHENREG_ADR(mem_fb_dmac_base), chen_reg.value, MEM_FB_DMAC_CHENREG_WR_MASK, "MEM_FB_DMAC_CHENREG");
...
#if 1
    vex_start(vexIdx, 0); // vijay

    vex_wait(vexIdx);
    
    for (i = 0; i < NUMBER_OF_VECTORS_INPUT; i++)
    {
		read_mem_18a(mem_fb_sram_base[rxfb_num] + (i*NUM_OF_ELEMENTS_PER_VEC*sizeof(int)), (NUM_OF_ELEMENTS_PER_VEC), &val_op[i], "val_op");
        for (j = 0; j < NUM_OF_ELEMENTS_PER_VEC; j++)
        {
            if (val_op[i][j] != val_inp[i][j])
            {
                print("ERROR: data mismatch, vec %d, index %d expected[0x%8x], received[0x%8x]\n", i, j, val_inp[i][j], val_op[i][j]);
            }
        }
    }
#endif
...
static void check_transfer_data(int vexId, int rxfbId)
{	
	unsigned int vex_vmem_offset = VEXABC1_VMEM_VE32_SIMD0_BASE - VEXABC0_VMEM_VE32_SIMD0_BASE;
...
int main (int vexId, int rxfbId, int dmaId, int chId, int nsipId)
{
    ...
    check_transfer_data(vexId, rxfbId);
