...
//main(int argc, const char * argv[])
{
    ...
    int offsetVal[1] = {HIVE_ADDR_dmem_read_prog_pattern
    };
    ...
    vex_loader(vexCoreId, "vex_fw/sim_m64/dmem_read_prog.fw");
    ...
    for (lc = 0; lc < VEX_DMEM_TEST_SIZE; lc++)
    {
        write_mem_18a( vex_dmem_base_addr + HIVE_ADDR_dmem_read_prog_gscalar + (lc * 4), 1, &test_pattern, "test_pattern");
    }
    ...
    for (lc = 0; lc < VEX_DMEM_TEST_SIZE; lc++)
    {
        ...
        read_mem_18a( vex_dmem_base_addr + HIVE_ADDR_dmem_read_prog_gscalar + (lc * 4), 1, &data, "gscalar");
