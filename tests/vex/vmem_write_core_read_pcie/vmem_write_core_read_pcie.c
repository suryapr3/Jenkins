...
//main(int argc, const char * argv[])
{
    ...
    int offsetVal[1] = {HIVE_ADDR_vmem_write_prog_pattern
    };
    ...
    vex_loader(vexCoreId, "vex_fw/sim_m64/vmem_write_prog.fw");
    ...
    for (lc = 0; lc < VEX_NUM_VEC; lc++)
    {
        
        for(i=0;i<32;i++)
        {
            read_mem_18a(vex_vmem_base_addr + HIVE_ADDR_vmem_write_prog_gvector + (lc * NUM_OF_ELEMENTS_PER_VEC * sizeof(unsigned))+(i * sizeof(int)), 1, &Val_out[i] , "Val_out");
