...
static void test_interrupt_with_mask(int vexId, int bMaskEnabled, srp_rcs_rcp_t rcsCoreId, int tx_map_bmask)
{
    ...
    int expected_bit_pos = CAR_HIGH_INT_STAT_S2C_ERR0_INT_STAT_BF_OFF + 8*(vexId%4);
    ...
    //interrupt select
    write_reg_18a(vex_car_int_sel_addr, CAR_VEX_INT_SEL_STRM2CIO_INT_SEL_BF_MSK, 0xFFFFFFFF, "int_select");
    // enable tx alarm
    ...
    {    
        ...
        read_reg_expect_18a(STRM2CIO_INT_STAT_ADR(strm2cio_base), 0, 0xFFFFFFFF, "strm2cio_int_status");
        read_reg_expect_18a(STRM2CIO_INT_MASKED_STAT_ADR(strm2cio_base), 0, 0xFFFFFFFF, "strm2cio_int_masked_stat");
        read_reg_expect_18a(vex_car_high_int_status_addr, 0, 0xFFFFFFFF, "car_high_int_status");
    
        //enable interrupt
        write_reg_18a(STRM2CIO_INT_EN_ADR(strm2cio_base), (1 & bMaskEnabled) << i, 0xFFFFFFFF, "strm2cio_int_enable");
        
        //force interrupt
        write_reg_18a(STRM2CIO_INT_FORCE_ADR(strm2cio_base), 1 << i, 0xFFFFFFFF, "strm2cio_int_force");
        
        // check status, masked status
        read_reg_expect_18a(STRM2CIO_INT_STAT_ADR(strm2cio_base), 1 << i, 0xFFFFFFFF, "strm2cio_int_status");
        read_reg_expect_18a(STRM2CIO_INT_MASKED_STAT_ADR(strm2cio_base), (1 & bMaskEnabled) << i, 0xFFFFFFFF, "strm2cio_int_masked_stat");
        ...
        if(bMaskEnabled)
        {
            ...
            write_reg_18a(STRM2CIO_INT_CLR_ADR(strm2cio_base), 1 << i, 0xFFFFFFFF, "strm2cio_int_clear");
            // check sticky register
            read_reg_expect_18a(trx_sys_csr_base + TRX_SYS_CSR_TX_ALARM0_STICKY_OFFSET,0x43,0xFFFFFFFF ,"Digrf trx_sys sticky");
            // check status register
...
main(int vexCoreId)
{
    ...
    for (rcsCoreId = SRP_RCS_RCP_CTRL_AX; rcsCoreId < SRP_RCS_RCP_MAX; rcsCoreId++)
    {
        ...
        test_interrupt_with_mask(vexCoreId, 0, rcsCoreId, 0x0);
        test_interrupt_with_mask(vexCoreId, 0, rcsCoreId, 0x1);
        test_interrupt_with_mask(vexCoreId, 1, rcsCoreId, 0x0);
        test_interrupt_with_mask(vexCoreId, 1, rcsCoreId, 0x1);
