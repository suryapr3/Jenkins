...
stem(n1, input_iq); title("Input IQ samples @p5");

% write this into a file which can be sourced while running on FRIO
fileId = fopen("input_samples/upconversion/single_band_upconversion_p5.txt", "w");
fprintf(fileId, "%08x\n", input_iq);
fclose(fileId);
...
% no filters enabled
config.vha_hbf_stages = [0 0 0 0 0];
...
stem(n2, output_iq_p5);  title ("Output - No filters enabled");

% write this output to another file
fileId = fopen("output_samples/upconversion/p5/single_band_upconversion_p5.txt", "w");
fprintf(fileId, "%08x\n", output_iq_p5);
fclose(fileId);

% enable w1
config.vha_hbf_stages = [1 0 0 0 0];
...
fileId = fopen("output_samples/upconversion/p5/single_band_upconversion_1.txt", "w");
fprintf(fileId, "%08x\n", output_iq_1);
fclose(fileId);

% enable w1, w2
config.vha_hbf_stages = [1 1 0 0 0];
...
stem(n2, output_iq_2);title ("Output - W1, W2 enabled");
fileId = fopen("output_samples/upconversion/p5/single_band_upconversion_2.txt", "w");
fprintf(fileId, "%08x\n", output_iq_2);
fclose(fileId);

% enable w1, w2, w3
config.vha_hbf_stages = [1 1 1 0 0];
...
stem(n2, output_iq_4); title ("Output - W1, W2, W3 enabled");
fileId = fopen("output_samples/upconversion/p5/single_band_upconversion_4.txt", "w");
fprintf(fileId, "%08x\n", output_iq_4);
fclose(fileId);

% enable w1, w2, w3, w4
config.vha_hbf_stages = [1 1 1 1 0];

% now do an interpolation
output_iq_from_model_8 = vha_interpolate(input_iq_for_model, config);

% now scale the samples up
output_iq_8 = (output_iq_from_model_8 * scale_factor);

n2 = 1:1:length(output_iq_8);
subplot(6, 1, 6); grid on;
stem(n2, output_iq_8);title ("Output - W1, W2, W3, W4 enabled");
fileId = fopen("output_samples/upconversion/p5/single_band_upconversion_8.txt", "w");
fprintf(fileId, "%08x\n", output_iq_8);
fclose(fileId);
