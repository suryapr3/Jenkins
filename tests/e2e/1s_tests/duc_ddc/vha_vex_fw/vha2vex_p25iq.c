...
vec32i32 AT(0xd000) vbuffer_copy[4];
...
#endif
volatile int AT(0x50) sout0_copy[4];
//volatile int AT(0x60) g_dbg[8];

volatile int AT(0x48) g_debug;

volatile int AT(0x60) g_vec_processed;
...
#pragma hive pipeline ii=6
        ...
        for (int i = 0; i < iter_max; i++)
        {
          ...
          //v_xstomi_1024(Any, 0, offset, 1023, vout, offset);
          v_xsto_1024_i(Any, g_dest_ptr_address, 0x0, vout);

#if DEBUG
          OP___dump(__LINE__, result);
          if(0 == count)
          {
              (vbuffer_copy[i] = vout) SYNC_WITH(buffer_read); //vbuffer_out[count];
              (vbuffer_out_copy[i] = vadd) SYNC_WITH(buffer_read); //vbuffer_out[count];
          }
#endif
          g_vec_processed++;
