    ...
    def read(self, file_path, h_filename):
        try:
            ...
            with open(file_path, 'r', encoding='latin-1') as f:
                ...
                for line in content:
                    if '0x' in line:
                        match = re.findall(r'#define\s+(\w+_OFFSET)\s*\(\s*0x[0-9a-fA-F]+U?\s*\)', line)
                        match1 = re.findall(r'#define\s+(\w+_DEFAULT)', line)
                        match2 = re.findall(r'#define\s+(\w+_RD_MASK)\s+\(0x(?!0+U?\b)(?!0+UL?\b)(?!0+ULL?\b)[a-fA-F0-9]+(?:U|UL|ULL)?\)', line)

                        match3 = re.findall(r'#define\s+(\w+_WR_MASK)\s+\(0x(?!0+U?\b)(?!0+UL?\b)(?!0+ULL?\b)[a-fA-F0-9]+(?:U|UL|ULL)?\)', line)
                        match4 = re.findall(r'#define\s+(\w+_BASE)\s*\(\s*0x[0-9a-fA-F]+U?\s*\)', line)


                        self.offset.extend(match)

                        self.default.extend(match1)

                        self.read_mask.extend(match2)

                        self.wr_mask.extend(match3)
...
if __name__ == '__main__':
    ...
    c_file_path = PROJECT_HOME + "/tests/reg_access/smoke_test/"
