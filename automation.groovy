    ...
    stage('Regression') {
        try {
            ...
            script {
                ...
                bat """
                set PYTHONPATH=C:\\Program Files\\Python311\\Scripts\
                C:\\Program Files\\Python311\\python.exe --version
                C:\\Program Files\\Python311\\python.exe -u jenkins_runner.py --clone_ip "${CLONE_IP}" --clone_username "${CLONE_USERNAME}" --clone_password "${CLONE_PASSWORD}" --run_ip "${RUN_IP}" --run_username "${RUN_USERNAME}" --run_password "${RUN_PASSWORD}" --dir "${EXECUTION_DIR}" --env "${SETUP_ENV_CMD}" --db_hw "${DESIGN_LOADING_HW_CMD}"
                """
            }
